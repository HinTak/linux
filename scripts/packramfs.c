...
int main( int argc, char *argv[] )
{
...
#else
	printf("BBBBBBBB\n");
	stHead.initramfs_start = 0x0a000000 + sizeof( ST_INITRAMFS_HEAD );
#endif
	...
	//printf("start: 0x%08x, end: 0x%08x\n", stHead.initramfs_start, stHead.initramfs_end);
	rewind( fp_in );
	buf = malloc( 0x400000 );
	if ( buf == NULL ) {
		perror( "malloc fail" );
	...
	//system( "cp arch/arm/boot/vmlinux.bin vmlinux.bin" );
	system( "cp arch/arm/boot/Image vmlinux.bin" );
	//system( "gzip -f vmlinux.bin > vmlinux.bin.gz" );
	memset( cmd_str, 0, 512 );
	sprintf( cmd_str, "./arch/arm/boot/mkimage -n 'linux-3.8.2' -A arm -O linux -T kernel -C none -a 0x%08X -e 0x%08X -d vmlinux.bin vmlinux.img", (unsigned int)ker_dst_base, (unsigned int)ker_dst_entry );
	system( cmd_str );
	...
	sprintf( cmd_str, "./arch/arm/boot/mkimage -n 'initramfs' -A arm -O linux -T ramdisk -C none -a 0x%08X -d initramfs.bin initramfs.img", stHead.initramfs_start - sizeof( ST_INITRAMFS_HEAD ) );
	system( cmd_str );
	printf( "\r\n");
#endif
	return 0;
