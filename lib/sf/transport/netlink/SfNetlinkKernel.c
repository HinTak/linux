...
#define SF_MNT_INFO_DELI_SIZE   1
#define SF_MNT_INFO_DELI         ":"
...
/*
****************************************************************************************************
* @brief get the size of data that will be loaded to netlink packet.
****************************************************************************************************
*/
int CalculateDataSize(const SfPacket* const pPacket)
{
    int total_len = 0;
    const int pre_for_netlink_msg = 64;

    if (pPacket && pPacket->op && pPacket->env)
    {
        switch (pPacket->op->type)
        {
            case SF_OPERATION_TYPE_OPEN:
                {
                    SfFileEnvironment* pEnv = (SfFileEnvironment*)pPacket->env;
                    total_len += sizeof(SfProtocolHeader);
                    total_len += sizeof(SfOperationFileOpen);
                    total_len += sizeof(SfFileEnvironment);
                    total_len += ((pEnv->pFileName) ? strlen(pEnv->pFileName) + 1:0);
                    total_len += ((pEnv->processContext.pProcessName) ? strlen(pEnv->processContext.pProcessName) + 1:0);
                }
                break;
            case SF_OPERATION_TYPE_EXEC:
                {
                    ...
                    total_len += sizeof(SfProtocolHeader);
                    total_len += sizeof(SfOperationBprmCheckSecurity);
                    total_len += sizeof(SfProcessEnvironment);
                    total_len += ((pEnv->pProcessName) ? strlen(pEnv->pProcessName) + 1:0);
                    total_len += ((pEnv->processContext.pProcessName) ? strlen(pEnv->processContext.pProcessName) + 1:0);
                }
                break;
            case SF_OPERATION_TYPE_MMAP:
                {
                    SfMmapEnvironment* pEnv = (SfMmapEnvironment*)pPacket->env;
                    total_len += sizeof(SfProtocolHeader);
                    total_len += sizeof(SfOperationFileMmap);
                    total_len += sizeof(SfMmapEnvironment);
                    total_len += ((pEnv->pLibraryName) ? strlen(pEnv->pLibraryName) + 1:0);
                    total_len += ((pEnv->processContext.pProcessName) ? strlen(pEnv->processContext.pProcessName) + 1:0);
                }
                break;
            case SF_OPERATION_TYPE_CONNECT:
                {
                    SfNetworkEnvironment* pEnv = (SfNetworkEnvironment*)pPacket->env;
                    total_len += sizeof(SfProtocolHeader);
                    total_len += sizeof(SfOperationSocketConnect);
                    total_len += sizeof(SfNetworkEnvironment);
                    total_len += ((pEnv->processContext.pProcessName) ? strlen(pEnv->processContext.pProcessName) + 1:0);
                }
                break;
            case SF_OPERATION_TYPE_SENDMSG: 
                {
                    SfSndRcvEnvironment* pEnv = (SfSndRcvEnvironment*)pPacket->env;
                    total_len += sizeof(SfProtocolHeader);
                    total_len += sizeof(SfOperationSocketSendmsg);
                    total_len += sizeof(SfSndRcvEnvironment);
                    total_len += ((pEnv->processContext.pProcessName) ? strlen(pEnv->processContext.pProcessName) + 1:0);
                }
                break;
            case SF_OPERATION_TYPE_RECVMSG:
                {
                    SfSndRcvEnvironment* pEnv = (SfSndRcvEnvironment*)pPacket->env;
                    total_len += sizeof(SfProtocolHeader);
                    total_len += sizeof(SfOperationSocketRecvmsg);
                    total_len += sizeof(SfSndRcvEnvironment);
                    total_len += ((pEnv->processContext.pProcessName) ? strlen(pEnv->processContext.pProcessName) + 1:0);
                }
                break;
    ...
    else if (pPacket && pPacket->op)
    {
        SfOperationSecurityReport* pOp = (SfOperationSecurityReport*)pPacket->op;
        if ( (pPacket->op->type == SF_OPERATION_TYPE_REPORT_NET) ||
             (pPacket->op->type == SF_OPERATION_TYPE_REPORT_PROCESS) ||
             (pPacket->op->type == SF_OPERATION_TYPE_REPORT_FILE) )
        {
            total_len += sizeof(SfProtocolHeader);
            total_len += ((pOp->caller) ? strlen(pOp->caller) + 1:0);
            total_len += ((pOp->filetype) ? strlen(pOp->filetype) + 1:0);
            total_len += ((pOp->filepath) ? strlen(pOp->filepath) + 1:0);
            total_len += ((pOp->description) ? strlen(pOp->description) + 1:0);
            total_len += ((pOp->desc_time) ? strlen(pOp->description) + 1:0);
        }
    }
        
    return ( (total_len > 0) ? total_len + pre_for_netlink_msg:0 );
}

/*
****************************************************************************************************
* @brief Adding mount information by using struct file if uep failed.
****************************************************************************************************
*/
SF_STATUS AddMountInfo(const SfProtocolHeader* const pPacketInterface)
{
    SF_ENVIRONMENT_TYPE envType = SF_ENVIRONMENT_TYPE_MAX;
    ...
    if ( (pPacket->env == NULL) || (pPacket->op == NULL) )
    {
        return SF_STATUS_BAD_ARG;
    }
    
    envType = pPacket->env->type;

    switch(envType)
    {
        case SF_ENVIRONMENT_TYPE_PROCESS:
            pProcessEnv = (SfProcessEnvironment*)pPacket->env;
            pProcessOp = (SfOperationBprmCheckSecurity*)pPacket->op;
            pTemp = pProcessEnv->pProcessName;
            break;
        case SF_ENVIRONMENT_TYPE_MMAP:
            pMmapEnv = (SfMmapEnvironment*)pPacket->env;
            pMmapOp = (SfOperationFileMmap*)pPacket->op;
            pTemp = pMmapEnv->pLibraryName;
            break;
        default:
            return SF_STATUS_BAD_ARG;
    }
    ...
    // merge text
    total_size = strlen(pTemp);
    total_size += SF_MNT_INFO_SEP_SIZE;
    total_size += SF_MNT_INFO_DELI_SIZE;
    switch(envType)
    {
        case SF_ENVIRONMENT_TYPE_PROCESS:
            total_size += strlen(pProcessOp->szMntInfo);
            total_size += strlen(pProcessOp->szMntFlag);
            total_size += SF_MNT_INFO_DELI_SIZE;
            total_size += strlen(pProcessOp->szRetInfo);
            pBuff = (Char*)sf_malloc(total_size + 1);
            if (pBuff == NULL)
            {
                return SF_STATUS_NOT_ENOUGH_RESOURCE;
            }
            ret = snprintf(pBuff, total_size + 1, "%s%s%s%s%s%s%s", 
                pTemp, SF_MNT_INFO_SEP, pProcessOp->szMntInfo, SF_MNT_INFO_DELI, pProcessOp->szMntFlag, SF_MNT_INFO_DELI, pProcessOp->szRetInfo);
            
            if (ret != total_size)
            {
                SF_LOG_E("snprintf was failed %d;%d;", total_size, ret);
                sf_free(pBuff);
                result = SF_STATUS_FAIL;
            }
            else
            {
                sf_free(pProcessEnv->pBuffer);
                pProcessEnv->pBuffer = pBuff;
                pProcessEnv->pProcessName = pBuff;
            }
            break;
        case SF_ENVIRONMENT_TYPE_MMAP:
            total_size += strlen(pMmapOp->szMntInfo);
            total_size += strlen(pMmapOp->szMntFlag);
            total_size += SF_MNT_INFO_DELI_SIZE;
            total_size += strlen(pMmapOp->szRetInfo);
            pBuff = (Char*)sf_malloc(total_size + 1);
            if (pBuff == NULL)
            {
                return SF_STATUS_NOT_ENOUGH_RESOURCE;
            ...
            ret = snprintf(pBuff, total_size + 1, "%s%s%s%s%s%s%s", 
                pTemp, SF_MNT_INFO_SEP, pMmapOp->szMntInfo, SF_MNT_INFO_DELI, pMmapOp->szMntFlag, SF_MNT_INFO_DELI, pMmapOp->szRetInfo);
            ...
            {
                sf_free(pMmapEnv->pBuffer);
                pMmapEnv->pBuffer = pBuff;
                pMmapEnv->pLibraryName = pBuff;
            }
            break;
        default:
            return SF_STATUS_BAD_ARG;
...
SF_STATUS SFAPI SfSendPacket(SfNode* const pNode, const SfPacket* const pPacket)
{
    ...
    if (pPacket->op->type == SF_OPERATION_TYPE_EXEC)
    {
        ...
        if ((pBprmCheckOperation->result == SF_STATUS_UEP_SIGNATURE_INCORRECT) ||
            ...
            (pBprmCheckOperation->result == SF_STATUS_SYSTEM_ERROR_IN_READ) ||
            (pBprmCheckOperation->result == SF_STATUS_NOT_ENOUGH_BUFFER))
        {
            if (AddMountInfo(&pPacket->header) != SF_STATUS_OK)
            {
                SF_LOG_E("Adding mount information was failed[%d];", result);
            }
            bUepBlocked = TRUE;
        }
    }
    ...
    else if (pPacket->op->type == SF_OPERATION_TYPE_MMAP)
    {
        ...
        if ((pFileMmapOperation->result == SF_STATUS_UEP_SIGNATURE_INCORRECT) ||
            ...
            (pFileMmapOperation->result == SF_STATUS_SYSTEM_ERROR_IN_READ) ||
            (pFileMmapOperation->result == SF_STATUS_NOT_ENOUGH_BUFFER))
        {
            if (AddMountInfo(&pPacket->header) != SF_STATUS_OK)
            {
                SF_LOG_E("Adding mount information was failed[%d];", result);
        ...
        )
    {
        do
        {
            ...
            nDataSize = CalculateDataSize(pPacket);
            if (nDataSize == 0)
            {
                SF_LOG_E( "%s(): failed to get data size", __FUNCTION__);
                result = SF_STATUS_FAIL;
                break;
            }
            pNetlinkPacket = SfSerializePacket(pPacket, nDataSize);
            if ( !pNetlinkPacket )
            {
            ...
            // broadcast message
            nlmsg_multicast(pCurrentNode->pHandle, pNetlinkPacket->pBuffer, 0, pPacket->op->type, GFP_ATOMIC);
    ...
    SfReceiveMessagePtr pHandler)
{
    ...
    do
    {
        ...
        *ppNode = sf_malloc(sizeof(SfNode));
        ...
        sf_memset(pNode->id.name, 0x00, sizeof(pNode->id.name));
        name_size = strlen(name);
        if (name_size < sizeof(pNode->id.name))
        {
            copy_size = name_size;
        }
        else
        {
            copy_size = (sizeof(pNode->id.name)) - 1;
        }
        sf_strncpy(pNode->id.name, name, copy_size);
        pNode->id.magic = id;
