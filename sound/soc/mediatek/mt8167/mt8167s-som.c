...
struct mt8167s_som_priv {
	...
	uint32_t au_spk_amp_warmup_time_us;
	uint32_t au_spk_amp_shutdown_time_us;
	uint32_t au_amp_mute_time_us;
};
...
static int soc_mute_state_get(struct snd_kcontrol *kctl,
	struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = res_mgr->mute_state;
	return 0;
}
static int soc_mute_state_put(struct snd_kcontrol *kctl,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kctl);
	struct mt8167s_som_priv *card_data = snd_soc_card_get_drvdata(card);
	...
	res_mgr->mute_state = value;
	switch(value){
		case 1:
			if (IS_ERR(card_data->pin_states[PIN_STATE_MUTE]))
				return -1;
			ret = pinctrl_select_state(card_data->pinctrl,
				card_data->pin_states[PIN_STATE_MUTE]);
			if (ret)
				dev_err(card->dev, "%s failed to select state %d\n",
				__func__, ret);
			break;
		case 0:
			if (IS_ERR(card_data->pin_states[PIN_STATE_UNMUTE]))
				return -1;
			ret = pinctrl_select_state(card_data->pinctrl,
				card_data->pin_states[PIN_STATE_UNMUTE]);
			if (ret)
				dev_err(card->dev, "%s failed to select state %d\n",
				__func__, ret);
			break;
	...
	if (card_data->au_amp_mute_time_us > 0)
		usleep_range(card_data->au_amp_mute_time_us,
		card_data->au_amp_mute_time_us + 1);
	return 0;
}
...
static void mt8516_codec_ext_au_amp_turn_on(struct snd_soc_card *card)
{
	...
	if (ret)
		dev_err(card->dev, "%s failed to select state %d\n",
		__func__, ret);
	if (card_data->au_spk_amp_warmup_time_us > 0)
		usleep_range(card_data->au_spk_amp_warmup_time_us,
		card_data->au_spk_amp_warmup_time_us + 1);
	
}
static void mt8516_codec_ext_au_amp_turn_off(struct snd_soc_card *card)
{
	...
	if (ret)
		dev_err(card->dev, "%s failed to select state %d\n",
		__func__, ret);
	if (card_data->au_spk_amp_shutdown_time_us > 0)
		usleep_range(card_data->au_spk_amp_shutdown_time_us,
		card_data->au_spk_amp_shutdown_time_us + 1);
		
}
static int mt8516_codec_au_spk_amp_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_dapm_context *dapm = w->dapm;
	struct snd_soc_card *card = dapm->card;
	dev_err(card->dev, "%s, event %d\n", __func__, event);
	printk("Call the value now");
	switch (event) {
		case SND_SOC_DAPM_POST_PMU:
		printk("call turn on\n");
			mt8516_codec_ext_au_amp_turn_on(card);
			break;
		case SND_SOC_DAPM_PRE_PMD:
		printk("call turn off\n");
			mt8516_codec_ext_au_amp_turn_off(card);
			break;
			default:
			break;
	}
...
static const struct snd_kcontrol_new mt8516_codec_au_ext_amp_switch_ctrl =
...
static const struct snd_kcontrol_new mt8167s_som_soc_controls[] = {
	...
	SOC_SINGLE_BOOL_EXT("Master Switch X",
			    ...
			    soc_ctlx_get,
			    soc_ctlx_put),
	SOC_SINGLE_BOOL_EXT("AMP Mute Switch",
			    0,
			    soc_mute_state_get,
			    soc_mute_state_put),
...
static const struct snd_soc_dapm_widget mt8167s_som_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SWITCH("AU Ext Amp",SND_SOC_NOPM, 0, 0, &mt8516_codec_au_ext_amp_switch_ctrl),
	SND_SOC_DAPM_SPK("AU Spk Amp", mt8516_codec_au_spk_amp_event),
};
...
static const struct snd_soc_dapm_route mt8167s_som_audio_map[] = {
	...
	{"AU Ext Amp","Switch","AU_LOL"},
	{"AU Spk Amp",NULL,"AU Ext Amp"},
};
...
*/
	...
	of_property_read_u32(pdev->dev.of_node,
		"mediatek,au-spk-amp-warmup-time-us",
		&card_data->au_spk_amp_warmup_time_us);
	of_property_read_u32(pdev->dev.of_node,
		"mediatek,au-spk-amp-shutdown-time-us",
		&card_data->au_spk_amp_shutdown_time_us);
	of_property_read_u32(pdev->dev.of_node,
		"mediatek,au-amp-mute-time-us",
		&card_data->au_amp_mute_time_us);
