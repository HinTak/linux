...
*/	
	for (i = tlv320->adc_pos; i < tlv320->adc_num; i++) {
		...
		snd_soc_write(codec, ADC_GPIO2_CTRL(i),0x28);//reg 51
		snd_soc_write(codec, ADC_GPIO1_CTRL(i),0x04);//reg 52
		snd_soc_write(codec, ADC_MIC_POLARITY_CTRL(i),0x01);//reg 80
		snd_soc_write(codec, ADC_ADC_DIGITAL(i),0x0E);//reg 81
		snd_soc_write(codec, ADC_ADC_DIGITAL(i),0xCE);//reg 81
		snd_soc_write(codec, ADC_LADC_VOL(i),0x10);//reg 83
		snd_soc_write(codec, ADC_RADC_VOL(i),0x10);//reg 84
		snd_soc_write(codec, ADC_ADC_FGA(i),0x00);//reg 82
	}
			     ...
			     const struct i2c_device_id *id)
{
	...
	if(0 == i) {
		...
		dev_info(&pdev->dev, "TLV320AIC3101 Audio Codec %s (%d), adc_pos:%d,ref_ch:%d, dmic_cfg:%d\n",
				TLV320_VERSION, i, adc_pos, ref_ch, mic_cfg);
	}
