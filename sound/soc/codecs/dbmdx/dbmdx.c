...
static int dbmdx_is_hw_vad_enabled(struct dbmdx_private *p, bool *is_enabled);
...
static int allocate_kfifo_mem(void **buf, size_t size, gfp_t flags)
{
	int ret = 0;

#ifdef USE_VMALLOC_ALLOCATOR_FOR_KFIFO
	*buf = vmalloc(size);
	if (*buf == NULL)
		ret = -ENOMEM;
#else
...
static void free_kfifo_mem(const void *buf)
{
#ifdef USE_VMALLOC_ALLOCATOR_FOR_KFIFO
	vfree(buf);
#else
...
*/
	void __iomem * v_addr;
	v_addr = ioremap(0x9c0648, 0x4);	
	dev_err(p->dev, "[JUR]%s 0x9c0648 : 0x%x\n", __func__, readl(v_addr));
	writel((readl(v_addr) & ~(1<<3)), v_addr);
	dev_err(p->dev, "[JUR]%s 0x9c0648 : 0x%x\n", __func__, readl(v_addr));
...
static int dbmdx_set_mode(struct dbmdx_private *p, int mode)
{
	...
	if (mode == DBMDX_IDLE)
		...
		bool hw_vad_enabled = false;

		send_set_mode_cmd = 0;
		required_mode = DBMDX_DETECTION;

		ret = dbmdx_is_hw_vad_enabled(p, &hw_vad_enabled);
		if (ret < 0) {
			dev_err(p->dev, "%s: failed to read hw vad\n",
				__func__);
			p->va_flags.irq_inuse = 0;
			goto out;
		}
	...
	if (send_set_mode_cmd) {
		unsigned short cur_set_mode;

		ret = dbmdx_send_cmd(p, DBMDX_VA_OPR_MODE, &cur_set_mode);
		if (ret < 0) {
			dev_err(p->dev,
		...
		if (ret < 0) {
			...
			dev_dbg(p->dev,	"%s: No need to update mode value\n",
					__func__);
			send_set_mode_cmd = false;
		}
	}
	...
	if (required_mode == DBMDX_BUFFERING) {
		...
		if ((p->buffering_options &
			DBMDX_BUFFERING_OPT_BUFFER_WITH_BACKLOG) &&
			dbmdx_amodel_loaded(p)) {
			if (p->va_flags.a_model_downloaded_to_fw)
...
#endif
			dev_dbg(p->dev,
				"%s: Backlog for buffering was set to 0x%x\n",
				__func__, p->va_cur_backlog_length);
		}
...
static int dbmdx_is_hw_vad_enabled(struct dbmdx_private *p, bool *is_enabled)
{
	...
	*is_enabled = false;
	...
	if (cur_config & DBMDX_HW_VAD_MASK) {
		dev_dbg(p->dev, "%s: HW VAD is enabled\n", __func__);
		*is_enabled = true;
	} else {
		dev_dbg(p->dev, "%s: HW VAD is disabled\n", __func__);
	}
...
/*********************** DBMDX IOCTL Interface ************************/
#ifdef	DBMDX_IOCTL_SUPPORT

#define DBMDX_IOC_MAGIC			'D'
#define DBMDX_IOCTL_AMODEL_LOAD		_IOW(DBMDX_IOC_MAGIC, 0, int32_t)
#define DBMDX_IOCTL_SET_OPERATION_MODE	_IOW(DBMDX_IOC_MAGIC, 1, int32_t)
#define DBMDX_IOCTL_GET_OPERATION_MODE	_IOR(DBMDX_IOC_MAGIC, 2, int32_t*)
#define DBMDX_IOCTL_SET_MIC_MODE	_IOW(DBMDX_IOC_MAGIC, 3, int32_t)
#define DBMDX_IOCTL_GET_MIC_MODE	_IOR(DBMDX_IOC_MAGIC, 4, int32_t*)
#define DBMDX_IOCTL_SET_HISTORY		_IOW(DBMDX_IOC_MAGIC, 5, int32_t)
#define DBMDX_IOCTL_GET_HISTORY		_IOR(DBMDX_IOC_MAGIC, 6, int32_t*)
#define DBMDX_IOCTL_SET_VA_REGISTER	_IOW(DBMDX_IOC_MAGIC, 7, int32_t)
#define DBMDX_IOCTL_GET_VA_REGISTER	_IOWR(DBMDX_IOC_MAGIC, 8, int32_t*)
#define DBMDX_IOCTL_TRIGGER_ISR		_IO(DBMDX_IOC_MAGIC, 9)
#define DBMDX_IOCTL_NR			10

static int dbmdx_va_acoustic_model_ioctl_set(struct dbmdx_private *p, int value)
{
	int ret;
...
static int dbmdx_mic_mode_ioctl_set(struct dbmdx_private *p, int value)
{
	int ret = 0;
...
#endif
	case DBMDX_MIC_MODE_DISABLE:
		ret = dbmdx_reconfigure_microphones(p, value);
		break;
	default:
		dev_err(p->dev, "%s: unsupported microphone mode %d\n",
			__func__, value);
...
static int dbmdx_mic_mode_ioctl_get(struct dbmdx_private *p, int *value)
{
	int ret = 0;
	...
	/* VA controls */
	if (p->active_fw != DBMDX_FW_VA) {
		*value = DBMDX_MIC_MODE_DISABLE;
		dev_dbg(p->dev, "%s: VA firmware not active\n", __func__);
		goto out;
	}

	*value = p->va_active_mic_config;
out:
	return ret;
}


static int dbmdx_va_register_ioctl_set(struct dbmdx_private *p,
				u32 reg, unsigned short val)
{
	int ret = 0;
...
static int dbmdx_va_register_ioctl_get(struct dbmdx_private *p,
				u32 reg, unsigned short *reg_val)
{
	int ret = 0;
	unsigned short val;
...
static int dbmdx_opr_mode_ioctl_get(struct dbmdx_private *p, int *opmode)
{
	unsigned short val;
	int ret = 0;
...
static int dbmdx_opr_mode_ioctl_set(struct dbmdx_private *p, int value)
{
	int ret = 0;
	...
	dev_dbg(p->dev, "%s, mode=%d", __func__, value);
	if (p->active_fw != DBMDX_FW_VA) {
		dev_dbg(p->dev, "%s: VA firmware not active\n", __func__);
		goto out;
	}
	...
	if (value == DBMDX_IDLE || value == DBMDX_SLEEP_PLL_ON ||
		value == DBMDX_SLEEP_PLL_OFF || value == DBMDX_HIBERNATE) {
		ret = dbmdx_set_mode(p, value);
	} else if (value == DBMDX_DETECTION) {
		/* default detection mode - VT, i.e. PHRASE */
		p->va_detection_mode = DETECTION_MODE_PHRASE;
		to_suspend_pcm_streaming = false;
		ret = dbmdx_trigger_detection(p);
	} else if (value == DBMDX_BUFFERING) {
		ret = dbmdx_set_mode(p, DBMDX_BUFFERING);
		to_suspend_pcm_streaming = false;
	} else {
		dev_err(p->dev, "%s, Unsupported mode: %d", __func__, value);
		ret = -EINVAL;
		p->unlock(p);
		return ret;
	}
	...
	if (ret < 0 && !p->pdata->va_recovery_disabled) {
		...
		if (value == DBMDX_IDLE || value == DBMDX_SLEEP_PLL_ON ||
			value == DBMDX_SLEEP_PLL_OFF ||
			value == DBMDX_HIBERNATE) {
			ret = dbmdx_set_mode(p, value);
		} else if (value == DBMDX_DETECTION) {
			/* default detection mode - VT, i.e. PHRASE */
			p->va_detection_mode = DETECTION_MODE_PHRASE;
...
static int dbmdx_sv_interrupt_soft_ioctl(struct dbmdx_private *p)
{
	dev_dbg(p->dev, "%s\n", __func__);
...
long dbmdx_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (_IOC_TYPE(cmd) != DBMDX_IOC_MAGIC)
		return -ENOIOCTLCMD;
	if (_IOC_NR(cmd) >= DBMDX_IOCTL_NR)
		return -ENOIOCTLCMD;

	dev_info(p->dev, "%s: Processing IOCTL cmd 0x%x (0x%x), size=%d\n",
		__func__, cmd, _IOC_NR(cmd), _IOC_SIZE(cmd));


	switch (cmd) {
	case DBMDX_IOCTL_AMODEL_LOAD:
		ret = copy_from_user(&value, (const void *)arg, sizeof(value));
		if (ret) {
			dev_err(p->dev,
				"%s: Error during copying ioctl data from user\n",
				__func__);
			ret = -EFAULT;
			goto out_err;
		}
		ret = dbmdx_va_acoustic_model_ioctl_set(p, value);
		if (ret < 0) {
			dev_err(p->dev,
	...
	case DBMDX_IOCTL_SET_OPERATION_MODE:
		ret = copy_from_user(&value, (const void *)arg, sizeof(value));
		if (ret) {
			dev_err(p->dev,
				"%s: Error during copying ioctl data from user\n",
		...
		ret = dbmdx_opr_mode_ioctl_set(p, value);
		if (ret < 0) {
			dev_err(p->dev,
				"%s: Error setting opmode\n", __func__);
			ret = -EFAULT;
			goto out_err;
		}
		break;

	case DBMDX_IOCTL_GET_OPERATION_MODE:
		ret = dbmdx_opr_mode_ioctl_get(p, &value);
		if (ret < 0) {
			dev_err(p->dev,
				"%s: Error reading opmode\n", __func__);
			ret = -EFAULT;
			goto out_err;
		}
		...
		if (ret) {
			dev_err(p->dev,
				"%s: Error during copying ioctl data to user\n",
				__func__);
			ret = -EFAULT;
			goto out_err;
		}
		break;
	case DBMDX_IOCTL_SET_MIC_MODE:
		ret = copy_from_user(&value, (const void *)arg, sizeof(value));
		if (ret) {
			dev_err(p->dev,
				"%s: Error during copying ioctl data from user\n",
		...
		ret = dbmdx_mic_mode_ioctl_set(p, value);
		if (ret < 0) {
			dev_err(p->dev,
				"%s: Error setting opmode\n", __func__);
			ret = -EFAULT;
	...
	case DBMDX_IOCTL_GET_MIC_MODE:
		ret = dbmdx_mic_mode_ioctl_get(p, &value);
		if (ret < 0) {
			dev_err(p->dev,
				"%s: Error reading mic mode\n", __func__);
			ret = -EFAULT;
			goto out_err;
		}
		ret = copy_to_user((void *)arg,
	...
	case DBMDX_IOCTL_SET_HISTORY:
		...
		ret = dbmdx_va_register_ioctl_set(p,
					DBMDX_VA_AUDIO_HISTORY, (u16)value);
		if (ret < 0) {
			dev_err(p->dev,
				"%s: Error setting History\n", __func__);
			ret = -EFAULT;
			goto out_err;
		}
		break;

	case DBMDX_IOCTL_GET_HISTORY:
		ret = dbmdx_va_register_ioctl_get(p,
				DBMDX_VA_AUDIO_HISTORY, &reg_val);
		if (ret < 0) {
			dev_err(p->dev,
				"%s: Error reading History\n", __func__);
			ret = -EFAULT;
			goto out_err;
		}

		value = (int32_t)reg_val;
		...
		break;
	case DBMDX_IOCTL_SET_VA_REGISTER:
		ret = copy_from_user(&value, (const void *)arg, sizeof(value));
		if (ret) {
			dev_err(p->dev,
				"%s: Error during copying ioctl data from user\n",
		...
		command = value & 0xFFFF0000;
		reg_val = value & 0xFFFF;
		ret = dbmdx_va_register_ioctl_set(p, command, reg_val);
		if (ret < 0) {
			dev_err(p->dev,
				"%s: Error setting VA register\n", __func__);
			ret = -EFAULT;
			goto out_err;
		}
		break;

	case DBMDX_IOCTL_GET_VA_REGISTER:
		ret = copy_from_user(&value, (const void *)arg, sizeof(value));
		if (ret) {
			dev_err(p->dev,
				"%s: Error during copying ioctl data from user\n",
		...
		ret = dbmdx_va_register_ioctl_get(p, command, &reg_val);
		if (ret < 0) {
			dev_err(p->dev,
				"%s: Error reading register\n", __func__);
			ret = -EFAULT;
			goto out_err;
		}
	...
	case DBMDX_IOCTL_TRIGGER_ISR:
		ret = dbmdx_sv_interrupt_soft_ioctl(p);
		if (ret < 0) {
			dev_err(p->dev,
				"%s: Error generation SV IRQ\n", __func__);
			ret = -EFAULT;
			goto out_err;
		}
		break;
	...
	default:
		dev_err(p->dev, "%s: UNKNOWN IOCTL CMD\n", __func__);
		ret = -EFAULT;
		goto out_err;
	}

	dev_info(p->dev,
		"%s: IOCTL cmd: 0x%x (0x%x), size=%d was processed successfully\n",
		__func__, cmd, _IOC_NR(cmd), _IOC_SIZE(cmd));
...
static void dbmdx_sv_work(struct work_struct *work)
{
	...
	/* read configuration */
	ret = dbmdx_send_cmd(p, DBMDX_VA_GENERAL_CONFIGURATION_1, &cur_config);
	if (ret < 0) {
		dev_err(p->dev,
	...
	/* Reset position to backlog start */
		ret = dbmdx_send_cmd(p, DBMDX_VA_AUDIO_HISTORY |
			...
			if (ret < 0) {
				...
				if (split_passphrase_and_audio &&
					...
					(total + bytes_to_read >=
						mp_buf_end_point_in_bytes)) {
					dev_dbg(p->dev,
						"%s Send Detection Event:\tMP.Start: %zu\tMp.End: %zu,\tTotal before %u \tTotal after %u bytes\n",
						__func__,
						mp_buf_start_point_in_bytes,
						mp_buf_end_point_in_bytes,
						total,
...
static void dbmdx_pcm_streaming_work_mod_0(struct work_struct *work)
{
	...
	if ((p->buffering_options & DBMDX_BUFFERING_OPT_STREAM_WITH_BACKLOG) &&
			dbmdx_amodel_loaded(p)) {
		if (p->va_flags.a_model_downloaded_to_fw)
			cur_backlog = p->pdata->va_backlog_length;
#ifdef DMBDX_OKG_AMODEL_SUPPORT
		if (p->va_flags.okg_a_model_downloaded_to_fw &&
			(p->pdata->va_backlog_length_okg > cur_backlog))
			cur_backlog = p->pdata->va_backlog_length_okg;
#endif
		dev_dbg(p->dev, "%s: Backlog for streaming was set to 0x%x\n",
			__func__, cur_backlog);
	}

	/* Start PCM streaming, FW should be in detection mode */
	ret = dbmdx_send_cmd(p,
			(DBMDX_VA_AUDIO_HISTORY | cur_backlog | 0x1000),
			NULL);
...
static int dbmdx_get_va_resources(struct dbmdx_private *p)
{
...
#if defined(CONFIG_SND_SOC_DBMDX_SND_CAPTURE)
	ret = allocate_kfifo_mem(&p->pcm_kfifo_buf,
			MAX_KFIFO_BUFFER_SIZE_MONO, GFP_KERNEL);
	if (ret)  {
	...
	kfifo_init(&p->pcm_kfifo,
		&p->pcm_kfifo_buf, MAX_KFIFO_BUFFER_SIZE_MONO);
#endif
	p->detection_samples_kfifo_buf_size = MAX_KFIFO_BUFFER_SIZE;

	ret = allocate_kfifo_mem(&p->detection_samples_kfifo_buf,
	...
	if (p->multipurpose_kfifo_in_use) {
		...
		ret = allocate_kfifo_mem(&p->multipurpose_samples_kfifo_buf,
			MAX_KFIFO_BUFFER_SIZE, GFP_KERNEL);
...
err_kfifo_mp_free:
	if (p->multipurpose_kfifo_in_use)
		free_kfifo_mem(p->multipurpose_samples_kfifo_buf);
err_kfifo_free:
	free_kfifo_mem(p->detection_samples_kfifo_buf);
err_kfifo_pcm_free:
#if defined(CONFIG_SND_SOC_DBMDX_SND_CAPTURE)
	free_kfifo_mem(p->pcm_kfifo_buf);
err:
#endif
	return ret;
}
...
static int dbmdx_parse_va_advanced_options(struct dbmdx_private *p)
{
	...
	if (p->pdata->va_advanced_options &
				DBMDX_AVD_OPT_BUFFERING_WITH_BACKLOG) {
		p->buffering_options |= DBMDX_BUFFERING_OPT_BUFFER_WITH_BACKLOG;
		p->buffering_options |= DBMDX_BUFFERING_OPT_STREAM_WITH_BACKLOG;
	}
...
static int dbmdx_common_probe(struct dbmdx_private *p)
{
	...
	{		
		void __iomem * v_addr;
		v_addr = ioremap(0x9c0644, 0x4);
		writel((readl(v_addr)|0x00003000), v_addr);
		iounmap(v_addr);

		v_addr = ioremap(0x9c0648, 0x4);	

		dev_err(p->dev, "[JUR]11 0x9c0648 : 0x%x\n", readl(v_addr));
		writel((readl(v_addr) | (1<<3)), v_addr);
		dev_err(p->dev, "[JUR]22 0x9c0648 : 0x%x\n", readl(v_addr));
		//writel((readl(v_addr) & ~(1<<3)), v_addr);
		iounmap(v_addr);
