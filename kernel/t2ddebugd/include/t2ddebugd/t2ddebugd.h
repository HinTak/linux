...
typedef struct sec_tdbg_input_event tdebugd_event_t;
struct tdebugd_queue {
    unsigned int            event_head;
    unsigned int            event_tail;
    tdebugd_event_t      events[T2DDEBUGD_MAX_EVENTS];
};

extern char t2d_sched_serial[10];
extern unsigned int t2ddebugd_running;
extern struct tdebugd_queue t2ddebugd_queue;
extern int t2ddbgd_outside_print;

int t2ddebugd_start(void);

void t2d_queue_add_event(struct tdebugd_queue *q, tdebugd_event_t *event);
long t2d_dbg_get_event_as_numeric(tdebugd_event_t *event, int *is_number);
long t2d_dbg_get_event_as_hex(tdebugd_event_t *event, int *is_number);
void t2d_dbg_get_event(tdebugd_event_t *event);
...
#define  PRINT_T2D(fmt , args ...)  	 printk(KERN_WARNING fmt, ##args)
