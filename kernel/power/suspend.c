...
#define GPIO_US_MCOM            0xfc040210
#define GPIO_US_MCOM_SIZE       0x4
#elif defined(CONFIG_ARCH_KANTL)
#define GPIO_US_MCOM            0xfc040210
#define GPIO_US_MCOM_SIZE       0x4
#endif
#endif
...
static int suspend_enter_pwsv(suspend_state_t state, bool *wakeup)
{
	...
	/*
	## It is already disabled by pwsv mode ##
...
// Enable_cpus:
	//## It is not disabled in this function ##
	//enable_nonboot_cpus();
...
#define PRGCODE_SUSPEND_BEGIN   0xCA
#define PRGCODE_SUSPEND_END   0xCB
#define PRGCODE_RESUME_BEGIN   0xCC
#define PRGCODE_RESUME_END   0xCD

#if (defined(CONFIG_ARCH_SDP) && !(defined(CONFIG_ARCH_SDP1412) || defined(CONFIG_ARCH_SDP1501) || defined(CONFIG_ARCH_SDP1601))) || \
		(defined(CONFIG_ARCH_NVT72671D) || defined(CONFIG_ARCH_NVT72172) || defined(CONFIG_ARCH_NVT72673))
extern int mark_boot_progress(u32 value);
#else
#define mark_boot_progress(v)
#endif
...
static int suspend_enter(suspend_state_t state, bool *wakeup)
{
	...
	if (!error) {
		*wakeup = pm_wakeup_pending();
		if (!(suspend_test(TEST_CORE) || *wakeup)) {
			vdfs_trace_insert_pivot(vdfs_trace_pivot_suspend);
			mark_boot_progress(PRGCODE_SUSPEND_END);
			trace_suspend_resume(TPS("machine_suspend"),
				state, true);
#ifdef CONFIG_PM_CRC_CHECK
			save_suspend_crc();
			...
			trace_suspend_resume(TPS("machine_suspend"),
				state, false);
			vdfs_trace_insert_pivot(vdfs_trace_pivot_resume);
			mark_boot_progress(PRGCODE_RESUME_BEGIN);
			events_check_enabled = false;
		}
		syscore_resume();
	}
...
int suspend_devices_and_enter(suspend_state_t state)
{
	...
	mark_boot_progress(PRGCODE_SUSPEND_BEGIN);
	error = platform_suspend_begin(state);
	if (error)
		goto Close;
	suspend_console();
 ...
 Close:
	platform_resume_end(state);
	mark_boot_progress(PRGCODE_RESUME_END);
	return error;
...
static int enter_state(suspend_state_t state)
{
	...
	if (state == PM_SUSPEND_FREEZE) {
		...
		if (pm_test_level != TEST_NONE && pm_test_level <= TEST_CPUS) {
			start_suspend = false;
			pr_err("PM[%s] Unsupported test mode : pm_test_level [%d] for freeze state,"
	...
	if(pwsv_mode_flags) {
		...
		while(true){
			...
			error = pwsv_check_wdt(wdt_count++);
			if(error > 0)
				pwsv_reboot(error);
			else if(error < 0)
				wdt_count = 0;
			...
			if((pwsv_mode_flags & PWSV_MODE_SUSPEND) && 
				...
				//pwsv_clear_suspend_mode() // clear every time?
			}

			if(pwsv_check_wakeup_req()) {
				pr_debug("PWSV] wakeup Current MODE : %d Requested MODE : %d\n",
						pwsv_current_mode, pwsv_get_wakeup_state());
