...
//	irq_desc_t *idp = irq_desc + ip->irq; 
	irq_desc_t *idp = irq_to_desc((unsigned int)(ip->irq));
	int i;
	int err;
	...
	if ((i = atomic_read(&ip->count)) == 0) {
		if (idp && (idp->irq_data.state_use_accessors & IRQD_IRQ_DISABLED))
			enable_irq((unsigned int)(ip->irq));
...
ssize_t irq_proc_write(struct file *fp, const char *bufp, size_t len, loff_t *where)
{
	...
	if (enable)
	{
		...
		if (irq_to_desc((unsigned int)(ip->irq)) && (irq_to_desc((unsigned int)(ip->irq))->irq_data.state_use_accessors & IRQD_IRQ_DISABLED))
			enable_irq((unsigned int)(ip->irq));
	}
	else
	{
	    disable_irq_nosync((unsigned int)(ip->irq));
	}
...
int irq_proc_open(struct inode *inop, struct file *fp)
{
	...
	strlcpy(ip->devname, current->comm, sizeof(ip->devname));
	...
	else if(ip->irq == E_FIQ_SE_DSP2UP)
	{
		irqflags = IRQF_SHARED;
	}
	else
	...
	if ((error = request_irq((unsigned int)(ip->irq),
...
int irq_proc_release(struct inode *inop, struct file *fp)
{
	...
	{
	    struct irq_proc *ip = (struct irq_proc *)fp->private_data;
	    free_irq((unsigned int)(ip->irq), ip);
	    kfree(ip);
...
#ifdef CONFIG_MSTAR_CHIP
	entry = create_proc_entry("irq", 0666, desc->dir);
	if (entry) {
...
out:
	...
	printk(KERN_ERR"PREEMPT COUNT : 0x%x\n", preempt_count());
	printk( KERN_ALERT "=============================================================\n");
#endif
