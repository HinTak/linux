 ...
 * Created by choi young-ho, kim gun-ho, mun kye-ryeon, maithani himanshu
 *
...
#endif
static int dtvlogd_reader_waiting;
static int dtvlogd_printk_time = 1;
...
/*
 * macros for kuid/kgid for proc interface supported by dtvlogd
 *  - /proc/dtvlogd
 *  - /proc/dtvlogd_all
 *  - proc/dtvlogd_info/
 */
...
int get_dtvlogd_max_length(void)
{
	return dlog_buf_len;
}
...
static int configure_dtvlogd_addr(void)
{
	uint32_t data[3];
	int ret;
	phys_addr_t address;
	...
	if (np_mem == NULL) {
		ddebug_err("Could Not find sdp_mem_map entry\n");
		return -EINVAL;

	}
	ret = of_property_read_u32_array(np_mem, "samsung,emergency_log", data, 3);
	if (ret) {
		ddebug_err("failed to parse samsung,emergency_log memory\n");
		return ret;
	}
	...
	emeg_log_addr = address;
	emeg_log_len = size;
	ddebug_info("emeg_log_addr %pa[p], emeg_log_len %d\n",
			&emeg_log_addr, emeg_log_len);
	return 0;
}
...
static int rambuf_init(void)
{
	...
	ret = configure_dtvlogd_addr();
	if (ret) {
		ddebug_err("Emergency log address not configured...\n");
		return -1;
	}
...
static void ram_flush(void)
{
	...
	} else if (fp->f_op->write_iter) {
		new_sync_write(fp, flush_buf, len, &fp->f_pos);
		ddebug_info("file written to flash\n");
	} else {
...
static void print_option(void)
{
	...
	ddebug_print("   ram_address: %pa[p] (len: %d, total: %d)\n",
			&emeg_log_addr, dram_buf_len,
			emeg_log_len);
#endif
	ddebug_print("\n");
...
static void usblog_flush(void)
{
	...
	dtvlogd_write_stop();
	ret = acquire_dtvlogd_buffer(&dtvbuf1, &validlen1,
			&dtvbuf2, &validlen2);
	if (!ret) {
		ssize_t wret = 0;
		loff_t opos = dtvfile->f_pos;
		pos = dtvfile->f_pos;

		if (validlen1 > 0) {
			wret = __kernel_write(dtvfile, dtvbuf1, validlen1, &pos);
			if (wret != validlen1) {
				dlogbuf_writefail++;
				goto done;
			}
		...
		if (validlen2 > 0) {
			wret = __kernel_write(dtvfile, dtvbuf2, validlen2, &pos);
			if (wret != validlen2) {
				dlogbuf_writefail++;
				dtvfile->f_pos = opos;
				goto done;
...
static void print_flush_time(void)
{
	u64 ts_nsec;
	char prefix[64] = {0};
	...
	this_cpu = (int)smp_processor_id();
	ts_nsec = local_clock();
	rem_nsec = do_div(ts_nsec, 1000000000);

	prefix_cnt = snprintf(prefix, 64, "[%d-%lu.%04lu] ", this_cpu,
			(unsigned long)ts_nsec, ((rem_nsec / 1000)/100));
	tmp_len = dtvlogd_print(prefix, prefix_cnt);
	if (tmp_len < prefix_cnt)
...
static int dtvlogd_write(const char __user *str, unsigned int count, int type, int print_time)
{
	static char f_buf[LINE_BUFF_SIZE];
	static char k_buf[LINE_BUFF_SIZE];
	static unsigned int f_len;
	static unsigned int k_len;
	static pid_t p_pid;
	pid_t c_pid;
	int tmp_len;
	const char *buf = NULL;
	...
	if (count > LINE_BUFF_SIZE - 1) {
		count = LINE_BUFF_SIZE - 1;
		curr_nl = 1;
	}

	if (type == 1) {
		/* printf */
		if (f_len + count > LINE_BUFF_SIZE-1) {

			if (f_len > LINE_BUFF_SIZE-1)
				f_len = LINE_BUFF_SIZE-1;

			f_buf[f_len++] = '\n';

			tmp_len = dtvlogd_print((const char *)f_buf, (int)f_len);

			if (tmp_len < (int)f_len)
				ddebug_info("Written %d of %d\n", tmp_len, f_len);

			f_len = 0;
		}

		memcpy(f_buf+f_len, str, (unsigned long)count);
		f_len += count;
		if (curr_nl)
			f_buf[f_len-1] = '\n';
		...
		c_pid = current->pid;
		if (k_len > 0) {
			if ((k_len + count > LINE_BUFF_SIZE-1) | (p_pid != c_pid)) {

				if (k_len > LINE_BUFF_SIZE-1)
					k_len = LINE_BUFF_SIZE-1;

				k_buf[k_len++] = '\n';

				print_flush_time();
				(void)dtvlogd_print((const char *)k_buf, (int)k_len);

				k_len = 0;
			}
		}

		memcpy(k_buf+k_len, str, count);
		k_len += count;
		if (curr_nl)
			k_buf[k_len-1] = '\n';
		...
		/* this may add overhead, so we allow disable too */
		if (print_time && dtvlogd_printk_time && k_buf[k_len-1] == '\n') {
			/* we assume same timestamp and cpu across the function call.
			 * it shouldn't change as this is direct function call
			 */

			count = k_len;
			k_len = 0;
			print_flush_time();
			buf = (const char *)k_buf;
		} else {
			goto ret;
		}
	} else {
	...
	tmp_len = dtvlogd_print((const char *)buf, (int)count);
	if (tmp_len < (int)count)
		ddebug_info("Written %d of %d\n", tmp_len, count);

	/* check if we need to trigger any reader currently waiting */
	if (dtvlogd_reader_waiting && (dlogged_chars > (int)PAGE_SIZE))
...
int do_dtvlog(int type, const char __user *buf, int len)
{
	...
	case 3:
		...
		dtvlogd_write(buf, (unsigned int)len, 2, 1);     /* from printk */
		break;
	case 4:
#ifdef CONFIG_DTVLOGD_USB_LOG
	...
	case 11:
		/* Write some data to ring buffer. */
		dtvlogd_write(buf, (unsigned int)len, 1, 0);     /* from printf */
		break;
...
int acquire_dtvlogd_buffer(char **buf1, int *len1, char **buf2, int *len2)
{
	...
	spin_lock_irqsave(&dlogbuf_lock, flags);
	...
	*buf1 = &DLOG_BUF(dlogbuf_start);
	...
	spin_unlock_irqrestore(&dlogbuf_lock, flags);
...
#endif
	ddebug_print("  echo 6 > /proc/dtvlogd             # enable printk time\n");
	ddebug_print("  echo 7 > /proc/dtvlogd             # disable printk time\n");
}
		...
		size_t count, loff_t *ppos)
{
	...
	case 6:
		dtvlogd_printk_time = 1;
		ddebug_print("DTVLOGD: Enable printk time logging\n");
		break;

	case 7:
		dtvlogd_printk_time = 0;
		ddebug_print("DTVLOGD: Disable printk time logging\n");
		break;
...
/* this function reads current info and copy to user
 * It also updates the buffer for next time
 * @return: in case of success, the no. of chars copied, else -1
 */
static ssize_t __dtvlogd_read(struct proc_read_info *proc_info,
		char __user *buf, size_t count)
{
	...
	while (!ret && (start != end) && (size_t)read_len < count) {
		char ch = DLOG_BUF(start);
		start++;
		len--;
		spin_unlock_irq(&dlogbuf_lock);
		ret = __put_user(ch, (buf + read_len));
		read_len++;
		cond_resched();
		spin_lock_irq(&dlogbuf_lock);
	}
		...
		size_t count, loff_t *ppos)
{
	...
	do {
		...
		ret = __dtvlogd_read(proc_info, buf, count);
		/* update the buffers before returning to user */
		if (ret > 0) {
			proc_info->start += ret;
			proc_info->len -= ret;
		...
		if (ret == 0) {
			...
			proc_info->is_init = 0;
			/* in case of cat /proc/dtvlogd_all, we don't want to wait
			 * for next buffer
			 */
			if (!proc_info->is_clear_mode)
				break;

			dtvlogd_reader_waiting = 1;
			ret = wait_event_interruptible_timeout(dlogbuf_wq, 0, HZ);
			/* > 0 (condition true)
			   = 0 - timeout
			   < 0 - error
			 */
			dtvlogd_reader_waiting = 0;
			if (ret < 0) {
				ddebug_err("error in waiting...\n");
				break;
			}
			/* it should be same for dtvlogd(_all) */
			proc_info->end = dlogbuf_end;
			proc_info->len = dlogged_chars;
			proc_info->start = dlogbuf_start;
		}
	} while (should_read);
 ...
 */
static int  init_procfs_dtvlogd_info(kuid_t kuid, kgid_t kgid)
{
	...
	proc_set_user(info_dir, kuid, kgid);
...
static int __init init_procfs_msg(void)
{
	...
	kuid_t dtvlogd_user = {DTVLOGD_USER_ID};
	kgid_t dtvlogd_group = {DTVLOGD_GROUP_ID};
