...
static int dtvlogd_print(char *str, int len)
{
	int printed_len = 0;
	char *p;
	...
	/* we can also use the buffer itself, why this copy? */
	printed_len = snprintf(print_buf, len + 1, "%s", str);

	/* Copy the output into log buf
	 * snprintf adds NULL to end of string */
	for (p = print_buf; *p ; p++)
		dlogbuf_write(*p);
		...
		size_t count, loff_t *ppos)
{
	 ...
	 */
	while (!ret && (start != end) && read_len < count) {
		ch = DLOG_BUF(start);
		start++;
		len--;
		spin_unlock_irq(&dlogbuf_lock);
			  ...
			  size_t count, loff_t *ppos)
{
	...
	info_buff = PDE(file->f_dentry->d_inode)->data;
...
static int __init init_procfs_msg(void)
{
	...
	if (!dtvlogd_entry)
		goto err;
	dtvlogd_entry->uid = dtvlogd_user;
	dtvlogd_entry->gid = dtvlogd_group;

	/* read/write permission for user/group (0660) */
	dtvlogd_all_entry = proc_create("dtvlogd_all", 0660, NULL,
			&proc_dtvlogd_all_ops);
	if (!dtvlogd_all_entry)
		goto err;
	dtvlogd_all_entry->uid = dtvlogd_user;
	dtvlogd_all_entry->gid = dtvlogd_group;
