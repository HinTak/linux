...
void write_dlog_info(int buf_type, const char *fmt, ...);

#define LINE_BUFF_SIZE 1024

/* Control Version */
#define DTVLOGD_VERSION	"6.1.0"
...
static int configure_dtvlogd_addr(void)
{
	...
	emeg_log_len = size;
	ddebug_info("emeg_log_addr 0x%llx, emeg_log_len %d\n",
...
static void print_option(void)
{
	...
	ddebug_print("   ram_address: 0x%llx (len: %d, total: %d)\n",
...
int do_dtvlog(int type, const char __user *buf, int len)
{
	...
	case 11:
		/* Write some data to ring buffer. */
		/* printf data is similar to printk i.e. in kernel string
		 * we still keep it seperate, in case later we want to
		 * save buffer seperately or treat in different manner
		 */
		dtvlogd_write(buf, (unsigned int)len, 1, 0);
		break;
