...
#ifdef CONFIG_VD_RELEASE
	/* SAVE ERROR LOG */
	set_kpi_fault(0, 0, "IOT-HUB", process_name, "IOT-HUB",-1/* tgid is unnessesary */);
...
static ssize_t iotnode_read(struct file *filp, char __user *user_buf,
...
static ssize_t iotnode_write(struct file *filp, const char __user *user_buf,
                size_t size, loff_t *ppos)
{
        struct task_struct *p, *g;
        int ret;
        ...
        if(strstr(set_data,TV_MODE_COMMAND)!=0){
                ...
                while(temp--)
                {
                        if(iot_thread_list[temp] == tgid) // to skip duplicated tgid
                                goto out;
                }
...
static int iotnode_open(struct inode *inode, struct file *file)
{
	return 0;
}
