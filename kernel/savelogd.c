...
static struct mutex savelogd_write_lock;

char * save_thread_name;
int savelogbuf_pid = -1;
int savelogbuf_status = 0;

static ssize_t save_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
...
ssize_t save_write2(struct file *filp, const char __user *buf, size_t count, loff_t *ppos)
{
	char set_data[SEND_BUFSIZE];
	char magic, status=0;
	int i,fd, value = 1, file_save_result = 0;
	struct stat64 statbuf;
	mm_segment_t old_fs;

	mutex_lock(&savelogd_write_lock);
	memcpy(set_data,buf,256);
	if(set_data[0] != 'Z')
	{
		mutex_unlock(&savelogd_write_lock);
		return 1;
	}
...
static ssize_t save_write(struct file *filp, const char __user *buf, size_t count, loff_t *ppos)
{
	char set_data[SEND_BUFSIZE];
	char magic, status=0;
	int i,fd, value = 1, file_save_result = 0;
...
static int __init save_log_init(void)
{
	...
	mutex_init(&savelogd_write_lock);
