...
/* Flag is used to turn the sysrq for PSS mem usage on or off */
static int sec_pss_memusage_status;
...
{
	PRINT_KD
	    ("time    [ pid ]   name     ProcessCur    Code       Data     LibCode     LibData    Heap-BRK      Stack      Other      Swap\n");
	PRINT_KD
	    ("==== ========== ========== ========== ========== ========== ========== "
	    "========== ========== ========== ========== =========\n");
}

/* This function is to show PSS header */
static void pss_memusage_show_header(void)
{
	PRINT_KD
	    ("time    [ pid ]   name     ProcessCur    Code       Data     LibCode     LibData    Heap-BRK      Stack      Other      Swap\n");
	PRINT_KD
...
static void sec_show_phy_memusage_info(pid_t pid)
{
	...
	swap_usage = get_mm_counter(mm, MM_SWAPENTS);

	PRINT_KD("%04ld  [%7d] %-10.10s  %7lu K  %7lu K  %7lu K  %7lu K  %7lu K  %7lu K  %7lu K  %7lu K %7lu K\n",
		 kdbg_get_uptime(), task_pid_nr(p), p->comm,
		 P2K(tot_cur_cnt),
		 P2K(cur_cnt[0]), P2K(cur_cnt[1]),
		 P2K(cur_cnt[2]), P2K(cur_cnt[3]),
		 P2K(cur_cnt[4]), P2K(cur_cnt[5]),
		 P2K(cur_cnt[6]), P2K(swap_usage));
#else
...
static int kdbg_show_phy_mem_stat(void)
{
	...
	static int rss_delay_print = MEMPS_DELAY_PRINT;

	if (!sec_phy_memusage_status)
		return -1;

	if(rss_delay_print++ < MEMPS_DELAY_PRINT){
		return 0;
	}
...
static int kdbg_get_user_pss(pid_t pid, struct smap_mem *pss)
{
	int idx = 0;
	unsigned long swap_usage;
	struct task_struct *p;
	...
	PRINT_KD("%04ld  [%7d] %-10.10s  %7lu K  %7lu K  %7lu K  %7lu K  %7lu K  %7lu K  %7lu K  %7lu K %7lu K\n",
		 kdbg_get_uptime(), task_pid_nr(p), p->comm,
		 (pss->total),
		 (pss->vmag[0]), (pss->vmag[1]),
		 (pss->vmag[2]), (pss->vmag[3]),
		 (pss->vmag[4]), (pss->vmag[5]),
		 (pss->vmag[6]), P2K(swap_usage));
	return 1;
}

/* show physical memory stats */
static int kdbg_show_pss_mem_stat(void)
{
	const int MAX_EXTRA_PROCESSES = 50;	/* allow for variation of 50 threads */
	pid_t *pid_arr = NULL;
	struct task_struct *p;
	...
	int i, count = 0;
	struct smap_mem pss;
	static int pss_delay_print = MEMPS_DELAY_PRINT;

	if (!sec_pss_memusage_status)
		return -1;

	if(pss_delay_print++ < MEMPS_DELAY_PRINT){
		return 0;
	}

	pss_delay_print = 1;

	pss_memusage_show_header();
	...
	for (i = 0; i < count; i++) {
		...
		if (kdbg_get_user_pss(pid_arr[i], &pss) == 0)
			continue;
	}
...
/* Global variable to store agent PSS mem usage status */
static int agent_pss_memusage_status;
		...
		struct sec_process_memory *kmem)
{
	...
	kmem->swap_usage = P2K(swap_usage);
...
#define AGENT_PHY_MEM_FIXED_DATA (2)
	...
	pr_debug("[Agent PhyMem Info] Data Written in RB %d\n", total_write_size);
...
static int agent_kdbg_get_user_pss(struct task_struct *p, struct smap_mem *pss, unsigned long *swap_usage)
{
	int idx = 0;
	struct mm_struct *mm = NULL;
	struct vm_area_struct *vma = NULL;
...
static int agent_show_pss_memusage_info(pid_t pid,
		struct sec_process_memory *kmem)
{
	struct task_struct *p;
	struct smap_mem pss;
	...
	if (IS_ERR(p)) {
		/* Note: Comment below code to minimize prints */
		PRINT_KD("[Agent PSS Mem Error] No Thread\n");
		return -1;
	}
	...
	memset(&pss, 0, sizeof(struct smap_mem));
	if (agent_kdbg_get_user_pss(p, &pss, &swap_usage) == 0) {
		put_task_struct(p);
		return -1;
	}
	...
	/* Fill individual memory segments */
	kmem->code = (int)(pss.vmag[0]);
	kmem->data = (int)(pss.vmag[1]);
	kmem->libCode = (int)(pss.vmag[2]);
	kmem->libData = (int)(pss.vmag[3]);
	kmem->heap_brk = (int)(pss.vmag[4]);
	kmem->stack = (int)(pss.vmag[5]);
	kmem->other = (int)(pss.vmag[6]);
	kmem->swap_usage = P2K(swap_usage);

	return 0;
}

/* @func: agent_write_pss_mem()
 * @desc: Writes PhyMem data on kdebugd RB.
 *	  Uses advance writer APIs as PhyMem data is very large.
 * @args: count- total number of processes running in system
 *	  pid_arr- array of pids of all the processes
 * @ret : 0 for success, -1 for failure.
 */
static int agent_write_pss_mem(size_t count, pid_t *pid_arr)
{
	struct tool_header t_hdr;
	unsigned long sec = 0;
	size_t pcount = 0, err_count = 0;
	...
	/* Fized size data Sec & No of process */
#define AGENT_PSS_MEM_FIXED_DATA (2)
	...
	if (!pid_arr) {
		PRINT_KD("[Agent PSS Mem Error] pid_arr list is NULL\n");
		return -1;
	}

	/* This will be the maximum PhyMem data size to be written on RB */
	data_len = (AGENT_PSS_MEM_FIXED_DATA  * sizeof(int)) +
		(count * sizeof(struct sec_process_memory));
	...
	while (available < (sizeof(struct tool_header) + data_len)) {
		...
		if (err_count == AGENT_ERROR_PRINT_FREQ) {
			/* Print every 30 sec */
			if (print_count == AGENT_ERROR_PRINT_FREQ) {
				PRINT_KD("[Agent PSS Mem Write Error] ");
				PRINT_KD("Available: %d, need: %d\n", available,
						sizeof(struct tool_header)
						+ data_len);
				print_count = 0;
	...
	/* 'tool_header', 'sec' and 'num_of_process' will be written later on */
	later_write_size = sizeof(struct tool_header) + (AGENT_PSS_MEM_FIXED_DATA * sizeof(int));
	...
	for (pcount = 0; pcount < count; pcount++) {
		/* Update 'kmem' structure */
		ret = agent_show_pss_memusage_info(pid_arr[pcount], &kmem);
		if (!ret) {
			/* 1) Advance Write PhyMem structure on RB. */
			written = phy_mem_adv_write((void *)&kmem,
					sizeof(struct sec_process_memory));
			if (!written) {
				PRINT_KD("[Agent PSS Mem Write Error] kmem\n");
				kdbg_ringbuffer_reset_adv_writer();
				mutex_unlock(&g_agent_rb_lock);
				return -1;
			}
	...
	/* Fill in tool_header structure appropriately */
	memset(&t_hdr, 0, sizeof(struct tool_header));
	t_hdr.cmd = KDBG_CMD_CM_PSS_MEM_USAGE;
	t_hdr.data_len = (int32_t)((AGENT_PSS_MEM_FIXED_DATA * sizeof(int)) +
				total_write_size);
	...
	written = phy_mem_adv_write((void *)&t_hdr,
			sizeof(struct tool_header));
	if (!written) {
		PRINT_KD("[Agent PSS Mem Write Error] tool header\n");
		kdbg_ringbuffer_reset_adv_writer();
		mutex_unlock(&g_agent_rb_lock);
		return -1;
	}
	...
	written = phy_mem_adv_write((void *)&sec, sizeof(int));
	if (!written) {
		PRINT_KD("[Agent PSS Mem Write Error] time\n");
		kdbg_ringbuffer_reset_adv_writer();
		mutex_unlock(&g_agent_rb_lock);
		return -1;
	}
	...
	written = phy_mem_adv_write((void *)&pcount, sizeof(size_t));
	if (!written) {
		PRINT_KD("[Agent PSS Mem Write Error] number of process\n");
		kdbg_ringbuffer_reset_adv_writer();
		mutex_unlock(&g_agent_rb_lock);
		return -1;
	}
...
int agent_show_pss_mem_stat(void)
{
	const int MAX_EXTRA_PROCESSES = 50;	/* allow for variation of 50 threads */
	pid_t *pid_arr = NULL;
	struct task_struct *p;
	...
	/* Write PhyMem data on RB */
	written = agent_write_pss_mem(count, pid_arr);
	if (written == -1)
		PRINT_KD("[Agent PSS Mem Error] Error writing data to RB.\n");
...
/* Toggle Agen PSS memory usage status */
void agent_pss_memusage_on_off(void)
{
	if (!sec_memusage_init_flag) {
		/*BUG: if initialization failed */
		sec_memusage_init();
	...
	agent_pss_memusage_status = (agent_pss_memusage_status) ? 0 : 1;

	if (agent_pss_memusage_status)
		PRINT_KD("Agent PSS Memory USAGE Dump ON\n");
	else
		PRINT_KD("Agent PSS Memory USAGE Dump OFF\n");
}
#endif /* CONFIG_KDEBUGD_AGENT_SUPPORT */
...
static int sec_memusage_init(void)
{
	...
	if (register_counter_monitor_func(kdbg_show_pss_mem_stat) < 0) {
		PRINT_KD("WARN: Fail to Register PSS Counter Monitor function\n");
		if (unregister_counter_monitor_func (kdbg_show_phy_mem_stat) < 0) {
			PRINT_KD("WARN: Fail to Unregister phy_mem_stat Counter Monitor function\n");
		}
		if (unregister_counter_monitor_func (mem_show_func) < 0) {
			PRINT_KD("WARN: Fail to mem_show_func Unregister Counter Monitor function\n");
		}
		return -1;
	}
...
void sec_memusage_destroy(void)
{
	...
	if (unregister_counter_monitor_func(kdbg_show_pss_mem_stat) < 0) {
		PRINT_KD("WARN: Fail to Unregister PSS Counter Monitor function\n");
	}
...
static void turnoff_memusage(void)
{
	...
	if (sec_pss_memusage_status) {
		sec_pss_memusage_status = 0;
		PRINT_KD("\n");
		PRINT_KD("PSS Memory USAGE Dump OFF\n");
	}
}
 ...
 *or off depending on the previous status.
 */
void sec_pss_memusage_prints_on_off(void)
{
	sec_pss_memusage_status = (sec_pss_memusage_status) ? 0 : 1;

	if (sec_pss_memusage_status)
		PRINT_KD("PSS Memory USAGE Dump ON\n");
	else
		PRINT_KD("PSS Memory USAGE Dump OFF\n");
}
...
static int sec_memusage_control(void)
{
	...
	PRINT_KD("6. PSS Info(Per Process)\n");
	...
	case 5:
		...
		break;
	case 6:
		sec_pss_memusage_prints_on_off();
		ret = 0;
		break;
	default:
		break;
