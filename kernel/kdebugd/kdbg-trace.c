 ...
 * */
	 ...
	 */
	if (pid == 1) {
		SMART_DEADLOCK_PRINT(1, "Skipping PID: %d\n", pid);
		return;
	}
#endif
	...
	if (IS_ERR(task)) {
		SMART_DEADLOCK_PRINT(1, "[ERROR] No Thread\n");
	...
	mm = get_task_mm(task);
	if (!mm) {
		SMART_DEADLOCK_PRINT(1, "[ERROR] %d doesn't have mm, (Kernel Thread ?)\n",
			 pid);
	...
	if (current_mm) {
		SMART_DEADLOCK_PRINT(1, "backtrace called outside kernel thread\n");
	...
	if (use_ptrace) {
		...
		if (ret != 0) {
			SMART_DEADLOCK_PRINT
			    (1, "[ERROR] %s: Error in attaching pid: %d, ret: %ld\n",
...
print_backtrace:
	...
	if (use_ptrace) {
		...
		do {
			...
			if (ret != 0) {
				SMART_DEADLOCK_PRINT
					(1, "[ERROR] %s: Error in detaching pid: %d, ret: %ld (state: 0x%lx, exit: 0x%x)\n",
					 __FUNCTION__, pid, ret,
					task->state,
					task->exit_state);
				...
				if (ret == -ESRCH) {
					...
					if (WIFSIGNALED(status)) {
						SMART_DEADLOCK_PRINT(1, "#### %d terminated by signal: %d ####\n", pid,
								WTERMSIG(status));
						break;
					}
				}
