...
static int kdml_register_tracer(void)
{
	...
	/* vmalloc/vfree traces */
	if (filter & KDML_TRACE_VMALLOC) {
		ret = register_trace_vmalloc_kdml(probe_vmalloc, NULL);
		if (ret == -EEXIST) {
			ddebug_err("Tracer already registered for vmalloc\n");
		} else if (ret) {
			ddebug_err("Tracer failed\n");
			goto err_trace_vmalloc;
		} else {
			ddebug_info("Tracer registered for vmalloc\n");
		}

		ret = register_trace_vfree_kdml(probe_vfree, NULL);
		if (ret == -EEXIST) {
			ddebug_err("Tracer already registered for vfree\n");
		} else if (ret) {
			ddebug_err("Tracer failed\n");
			goto err_trace_vfree;
		} else {
			ddebug_info("Tracer registered for vfree\n");
		}
	}
...
err_trace_vfree:
	unregister_trace_vmalloc_kdml(probe_vmalloc, NULL);
err_trace_vmalloc:
...
static void kdml_unregister_tracer(void)
{
	...
	if (filter & KDML_TRACE_VMALLOC) {
		unregister_trace_vmalloc_kdml(probe_vmalloc, NULL);
		unregister_trace_vfree_kdml(probe_vfree, NULL);
	}
}
...
int kdbg_kdml_init(void)
{
	...
	ret = kdml_alloc_probe_memory();
	if (ret) {
		ddebug_err("Error in allocation..\n");
		goto err_destroy_module_cache;
	}

	conf->entries_per_caller = MAX_ENTRIES_PER_CALLER;
	conf->kdml_filter = KDML_TRACE_KMALLOC | KDML_TRACE_KMEMCACHE
		| KDML_TRACE_PAGE_ALLOC | KDML_TRACE_VMALLOC;
