...
static void kdml_main_menu(void)
{
	...
	PRINT_KD("%2d) KDML Counter: Vmalloc Summary\n", KDML_MENU_VMALLOC_SUMMARY);
	...
	PRINT_KD("%2d) KDML: Stop Backtrace Filter\n", KDML_MENU_FILTER_FUNC_STOP);
...
static unsigned long get_addr_from_buffer(void)
{
	...
	func_offset = debugd_get_event_as_numeric(&event, NULL);
	PRINT_KD("\n");
...
int kdml_menu(void)
{
	...
	do {
		if (!((operation >= KDML_MENU_TRACE_SUMMARY)
					&& (operation <= KDML_MENU_VMALLOC_SUMMARY)))
			kdml_main_menu();
		...
		case KDML_MENU_VMALLOC_SUMMARY:
			if (kdml_ctr_meminfo_vmalloc_on_off())
				operation = 0; /* in case of failure, show menu */
			break;
		...
		case KDML_MENU_FILTER_FUNC_STOP:
			kdml_backtrace_addr = 0;
			PRINT_KD("Reset backtrace\n");
			break;
