...
#define MAX_CALLER_ENTRY_COUNT 1600
...
static void handle_alloc(struct kdml_alloc_info *entry, gfp_t gfp_flags)
{
	...
	/* ratelimit would remove certain prints,
	   but it could reduce the count...
	 */
	if (printk_ratelimit())
	if (call_site == kdml_backtrace_addr) {
		...
		nr_entries = __save_stack_trace(trace);
		ddebug_print("---------------------\n");
...
/* common for vmalloc/ioremap/vmap */
void
probe_vmalloc(void *ignore, void *ptr, unsigned long real_size,
		...
		enum kdml_vmalloc_trace_type vmalloc_type)
{
	...
	struct kdml_alloc_info entry = {.ptr = (uint32_t)ptr,
					.call_site = (uint32_t)call_site,
					.bytes_req = (uint32_t)real_size,
					.timestamp = (uint64_t)0
					};

	if (vmalloc_type == KDML_VMALLOC_TYPE_VMALLOC)
		entry.allocator_type = (uint32_t)KDML_ALLOC_VMALLOC;
	else
		entry.allocator_type = (uint32_t)KDML_ALLOC_IOREMAP_VMAP;

	/* initialize other fields */
	do_posix_clock_monotonic_gettime(&ts);
	entry.timestamp = timespec_to_ns(&ts);

	ddebug_info("vmalloc/vmap, call_site %pS (%p), ptr %p, size %lu, gfp_flags %x\n",
			call_site, call_site, ptr, real_size, gfp_flags);
...
/*
 * probe_vfree
 * tracepoint handler vfree/vunmap family.
 */
void
probe_vfree(void *ignore, unsigned long call_site, const void *ptr)
{
	/* kfree(NULL) is allowed and called from kernel */
	if (!ptr)
		return;

	ddebug_info("vfree/vunmap, call_site %pS (%p), ptr %p\n",
			(void *)call_site, (void *)call_site, ptr);
...
static int kdml_get_summary_snapshot(unsigned long *caller_list, int *count)
{
	...
	struct rb_node *node;
	int i;
	struct kdml_summary_node *snapshot_buffer;

	snapshot_buffer = conf->kdml_summary_snapshot_buffer;

	WARN_ON(!snapshot_buffer);
	if (!snapshot_buffer) {
	...
	for (node = rb_first(root), i = 0; node; node = rb_next(node), i++) {
		...
		memcpy(snapshot_buffer + i, summary, sizeof(struct kdml_summary_node));
		/* print_summary_line(summary); */
	}
	*count = i;
	*caller_list = (unsigned long)snapshot_buffer;
	spin_unlock_irqrestore(&kdml_summary_lock, flags);
...
int kdml_alloc_probe_memory(void)
{
	struct kdml_conf *conf = &kdml_global_conf;
	size_t caller_summary_mem = MAX_CALLER_ENTRY_COUNT * sizeof(struct kdml_summary_node);
	struct kdml_summary_node *buf;
	buf = vmalloc(caller_summary_mem);
	if (!buf) {
		ddebug_err("Error in allocating snapshot memory...\n");
		return -1;
	}

	conf->kdml_summary_snapshot_buffer = buf;
	return 0;
...
{
	struct kdml_conf *conf = &kdml_global_conf;
	vfree(conf->kdml_summary_snapshot_buffer);
	conf->kdml_summary_snapshot_buffer = NULL;
}

#ifdef CONFIG_KDEBUGD_AGENT_SUPPORT
void kdml_handle_start_stop(int stop)
{
	if (stop)
...
void kdml_send_alloc_summary(void)
{
	...
	if (!summary_count || summary_count < 0) {
	...
	/* Start Sending Data */
	/* Start */
	agent_write(KDBG_CMD_KDML_ALLOCS_SUMMARY_START, NULL, 0);
	/* Cont */
	remain = (size_t)summary_count;
	sent = 0;
	while (remain != 0) {
		size_t send_size;
		size_t ret1;
		const size_t max_send = 100;
		send_size = remain < max_send ? remain : max_send;
		ret1 = agent_write(KDBG_CMD_KDML_ALLOCS_SUMMARY_CONT, &summary_arr[sent], send_size * sizeof(struct kdml_summary_node));
		if (!ret1) {
			ddebug_err("Error in writing to RB.\n");
			break;
		}
		sent += send_size;
		remain -= send_size;
	...
	/* End */
	agent_write(KDBG_CMD_KDML_ALLOCS_SUMMARY_END, NULL, 0);
	ddebug_info("$$$$$$$$$$$$ %s sent: %d (from: %d) $$$$$$$$$$$$$$$\n", __func__, sent, summary_count);
}

#define MAX_CACHE_COUNT 200
void kdml_send_slabinfo(void)
{
	...
	WARN_ON(!cache_count);
	ddebug_info("$$$$$$$$$$$$ %s count: %d $$$$$$$$$$$$$$$\n", __func__, cache_count);
	/* Start Sending Data */
	/* Start */
	agent_write(KDBG_CMD_KDML_SLABINFO_START, NULL, 0);
	/* Cont */
	remain = (size_t)cache_count;
	sent = 0;
	while (remain) {
		size_t send_size;
		size_t ret1;
		const size_t max_send = 100;
		send_size = remain < max_send ? remain : max_send;
		ret1 = agent_write(KDBG_CMD_KDML_SLABINFO_CONT, &slabinfo_arr[sent], send_size * sizeof(struct kdml_slab_info_line));
		if (!ret1) {
			ddebug_err("Error in writing to RB.\n");
			break;
		}
	...
	/* End */
	agent_write(KDBG_CMD_KDML_SLABINFO_END, NULL, 0);
	ddebug_info("$$$$$$$$$$$$ %s sent: %d (from: %d) $$$$$$$$$$$$$$$\n", __func__, sent, cache_count);
...
#define MAX_MODULE_ARRAY	(2)
#define MAX_MODULE_ENTRIES	(128)
#define MAGIC_MOD_START 	(0xaaaabbbb)
#define MAGIC_MOD_END		(0xdeaddead)
...
/* TODO: This is needed as we can't take mutex while original module is added
   Instead, we can update module_info to have end_timestamp
 */
static struct kdml_module_info *mod_ent_list[MAX_MODULE_ARRAY]; /* thread entry list */
static int mod_ent_ctr[MAX_MODULE_ARRAY]; /* counter for thread entries */

/*
 * add_module_start_entry
 * adds a thread entry to the thread entry list.
 */
static inline void
add_module_start_entry(struct kdml_module_info *mod_info)
{
	/* TODO: Lock can be moved to caller */
	spin_lock(&kdml_module_lock);
	/* thread list as circular array, discard the old entry when array is full */
	if (mod_ent_list[MOD_ARRAY_WRITE]) {
		/* wrap around */
		if (mod_ent_ctr[MOD_ARRAY_WRITE] >= MAX_MODULE_ENTRIES) {
			if (printk_ratelimit())
				ddebug_err("add thread entry failure, idx %d.\n", mod_ent_ctr[MOD_ARRAY_WRITE]);
			mod_ent_ctr[MOD_ARRAY_WRITE] = 0;
		}
		/* fill the next entry */
		memcpy(&mod_ent_list[MOD_ARRAY_WRITE][mod_ent_ctr[MOD_ARRAY_WRITE]++], mod_info, sizeof(struct kdml_module_info));

		ddebug_info("entry success, idx %d for %s, operation: %s(%x).\n", mod_ent_ctr[MOD_ARRAY_WRITE] - 1, mod_info->name,
				mod_info->unused == MAGIC_MOD_START ? "ADD" : "REMOVE",
				mod_info->unused);
	}
	spin_unlock(&kdml_module_lock);
}

void kdbg_module_info_write(void)
{
	int ctr = 0;

	if (!mod_ent_list[MOD_ARRAY_READ]) {
		ddebug_info("No new module entry....\n");
		return;
	}

	if (!mod_ent_ctr[MOD_ARRAY_READ]) {
		/* scan the list and copy to read array */
		spin_lock(&kdml_module_lock);
		if (mod_ent_ctr[MOD_ARRAY_WRITE] > 0) {
			memcpy(mod_ent_list[MOD_ARRAY_READ], mod_ent_list[MOD_ARRAY_WRITE],
					sizeof(struct kdml_module_info) * (size_t)mod_ent_ctr[MOD_ARRAY_WRITE]);
			/* maintain counter */
			mod_ent_ctr[MOD_ARRAY_READ] = mod_ent_ctr[MOD_ARRAY_WRITE];
			/* reset the counter */
			mod_ent_ctr[MOD_ARRAY_WRITE] = 0;
		}
		spin_unlock(&kdml_module_lock);
		...
		/* write in ringbuffer */
		for (ctr = 0; ctr < mod_ent_ctr[MOD_ARRAY_READ]; ctr++) {
			struct kdml_module_info *mod_info;
			struct kdml_module_info_end module_end = {0,};

			mod_info = &mod_ent_list[MOD_ARRAY_READ][ctr];
			if (mod_info->unused == MAGIC_MOD_START) {
				if (kdml_mod_start_write(mod_info, sizeof(struct kdml_module_info)) == 0) {
					if (printk_ratelimit())
						ddebug_err("ringbuffer write failure, idx %d.\n", ctr);
					break;
				} else {
					ddebug_info("KDML: sent module start for %s (ptr: %x, start: %x)\n",
							mod_info->name,
							mod_info->mod_ptr,
							mod_info->module_core);
				}
			} else if (mod_info->unused == MAGIC_MOD_END) {
				/* this is for module end */
				module_end.mod_ptr = mod_info->mod_ptr;
				module_end.timestamp = mod_info->timestamp;
				kdml_mod_end_write(&module_end, sizeof(struct kdml_module_info_end));
				ddebug_info("KDML: sent module end for %s (ptr: %x, start: %x)\n",
						mod_info->name,
						mod_info->mod_ptr,
						mod_info->module_core);
			} else {
				/* it is an error.. */
				ddebug_err("Error !!!! Invalid Packet ....\n");
			}
		}
		/* reset the counter */
		mod_ent_ctr[MOD_ARRAY_READ] = 0;
	}
}

void kdml_send_module_start(struct kdml_module_info *mod_info)
{
	ddebug_info("sending module start for %s (ptr: %x, start: %x)\n",
			mod_info->name,
			mod_info->mod_ptr,
			mod_info->module_core);
	mod_info->unused = MAGIC_MOD_START;
	add_module_start_entry(mod_info);
}

void kdml_send_module_end(struct kdml_module_info *mod_info)
{
	int64_t timestamp;
	struct timespec ts;
	ddebug_info("sending module end for %s (ptr: %x, start: %x)\n",
			mod_info->name,
			mod_info->mod_ptr,
			mod_info->module_core);
	do_posix_clock_monotonic_gettime(&ts);
	timestamp = mod_info->timestamp;
	mod_info->timestamp = timespec_to_ns(&ts);
	/* utilize unused field to differentiate b/w start/end module */
	mod_info->unused = MAGIC_MOD_END;
	add_module_start_entry(mod_info);
	mod_info->timestamp = timestamp;
}
...
EXPORT_SYMBOL(kdml_print_caller_summary_info);
...
static void probe_module_load(void *ignore, struct module *mod)
{
	...
	spin_unlock(&kdml_module_lock);
#ifdef CONFIG_KDEBUGD_AGENT_SUPPORT
	kdml_send_module_start(mod_info);
#endif
}
...
static void probe_module_free(void *ignore, struct module *mod)
{
	...
	struct kdml_conf *conf = &kdml_global_conf;
	int found = 0;
	struct kdml_module_info *mod_info = NULL;
...
int kdml_print_module_summary_info(void)
{
	...
	caller_idx = 0;
	for (i = 0; i < module_count; i++) {
		unsigned long max_core, max_init, max_module_addr;
		max_core = module_arr[i].module_core + module_arr[i].core_text_size;
		max_init = module_arr[i].module_init + module_arr[i].init_text_size;
		max_module_addr = max_init > max_core ? max_init : max_core;
		...
		for (caller_idx = 0; caller_idx < summary_count; caller_idx++) {
			...
			if (within(addr, module_arr[i].module_core, module_arr[i].core_text_size)
				...
				/*
				 * either core or init addr can be larger
				 *    core: be400000-be400094, init: be402000-be402000
				 *    core: be46a000-be46c0b8, init: be410000-be41001c
				 */
			} else if (addr > max_module_addr) {
				ddebug_info("break at: %pS (%p)\n", addr, addr);
				break;
			}
		}
	}
...
EXPORT_SYMBOL(kdml_print_module_summary_info);
...
#ifdef CONFIG_KDEBUGD_AGENT_SUPPORT
	/* allocate memory fot module list */
	for (ctr = 0; ctr < MAX_MODULE_ARRAY; ctr++) {
		mod_ent_list[ctr] = kzalloc(sizeof(struct kdml_module_info) * MAX_MODULE_ENTRIES, GFP_KERNEL);
		if (!mod_ent_list[ctr]) {
			ddebug_err("memory allocation failure\n");
			goto err_kzalloc;
		}
...
#ifdef CONFIG_KDEBUGD_AGENT_SUPPORT
err_kzalloc:
	/* deallocate memory of module list */
	for (ctr = 0; ctr < MAX_MODULE_ARRAY; ctr++) {
		kfree(mod_ent_list[ctr]);
		mod_ent_list[ctr] = NULL;
	}
	return -1;
#endif
}
