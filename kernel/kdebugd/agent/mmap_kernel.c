...
int kdbg_agent_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	ret = remap_pfn_range(vma, vma->vm_start,
				virt_to_phys((void *)info->rb) >> PAGE_SHIFT,
...
int kdbg_agent_open(struct inode *inode, struct file *filp)
{
	...
	if (info) {
		rb_err("Device Open Fails!!! Instance is already Open\n");
		return -EACCES;
	}
...
/* Considering Size will be TOOL_HDR_SIZE + data lenght */
size_t kdebugd_ringbuffer_writer(void *hdr, void *data, size_t size)
{
	size_t available;
	...
	ret = kern_ringbuffer_write(g_rbuf, hdr, TOOL_HDR_SIZE);
	if (data)
		ret += kern_ringbuffer_write(g_rbuf, data, size-TOOL_HDR_SIZE);
	return ret;
}
