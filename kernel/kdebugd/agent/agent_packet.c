...
size_t agent_write(int cmd, void *data, size_t data_len)
{
	...
	/* only command might be sent with 0 data_len */
	if (data_len && !data) {
	...
	bytes_written = kdebugd_ringbuffer_writer((char *)(&t_hdr),
			(char *)(data),	TOOL_HDR_SIZE + data_len);
	if (bytes_written < TOOL_HDR_SIZE + data_len) {
