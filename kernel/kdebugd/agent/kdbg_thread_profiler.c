 ...
 */
static void
probe_task_rename(void *ignore, struct task_struct *task, const char *comm)
{
	struct thread_entry th_ent;
	struct timespec ts;
...
void kdbg_thread_profiler_write(void)
{
	...
	if (th_ent_list[THREAD_ARRAY_READ] && th_ent_list[THREAD_ARRAY_WRITE]) {
...
int kdbg_thread_profiler_start(void)
{
	...
	if (kdbg_agent_get_mode() == KDEBUGD_MODE) {
		th_error("Start kdebugd in Agent mode\n");
		return -1;
	}
