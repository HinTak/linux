...
/* version information */
#define KDEBUGD_AGENT_VERSION_STRING		"v2.2"
		...
		if (IS_AGENT_RUNNING()) {
			...
			if (!(i % AGENT_SAMPLING_TIME_1_SEC)) {
				...
				if (CM_START ==  agent_meminfo_get_status())
					agent_get_meminfo();
#ifdef CONFIG_KDEBUGD_FD_DEBUG
				/*Open fd count for each process*/
				if (CM_START ==  agent_open_fd_get_status()) {
					static int fd_count;

					if (!(fd_count++%2)) /*Send data in every 2 second*/
						kdbg_get_fd_info();
				}
#endif
				/* Write PSS info data */
				if (CM_START ==  agent_pss_mem_get_status())
					agent_show_pss_mem_stat();

				/* Write Stack info info data */
				if (CM_START ==  agent_stackinfo_get_status())
					kdbg_agent_get_stack_info();
#ifdef CONFIG_KDML
				/* Write thread profiler data in each 20 ms */
				kdbg_module_info_write();
#endif
				i = 0;
			}
			i++;
