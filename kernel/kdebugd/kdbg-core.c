...
#define SFD_UEP_LEVEL_CONTAINER_PATH  '7'
	current->uepLevel = SFD_UEP_LEVEL_CONTAINER_PATH;
#endif
...
int kdebugd_status(void)
{
	...
	while (1) {
		...
		case 2:
			{
			/* TODO: Remove the menu. No longer in use */
...
static void kdbg_cmd_handler(void *data, int len)
{
	...
	case KDBG_CMD_CM_PHY_MEM_USAGE:
		...
		break;
	case KDBG_CMD_CM_PSS_MEM_USAGE:
		{
			if (tvis_event != agent_pss_mem_get_status()) {
				agent_pss_mem_set_status(tvis_event);
				agent_pss_memusage_on_off();
			}
		}
		break;
	...
	case KDBG_CMD_START_THREAD_PROFILE_INFO:
		{
			...
			if (ret < 0)
				...
				agent_thread_profiler_set_status(CM_START);
	...
	case KDBG_CMD_STOP_THREAD_PROFILE_INFO:
		...
		agent_thread_profiler_set_status(CM_STOP);
	...
	case KDBG_CMD_CM_FD_INFO:
		PRINT_KD("Open fd packet received.\n");
		if (tvis_event != agent_open_fd_get_status())
			agent_open_fd_set_status(tvis_event);
		break;

	case KDBG_CMD_CM_FD_GET_DETAIL:
		if (t_hdr->data_len)
			memcpy(&tvis_event, (char *) data + TOOL_HDR_SIZE, sizeof(enum agent_cm));

		PRINT_KD("Get fd detail packet received tgid = %d.\n", (int32_t)tvis_event);
		kdebugd_send_fd_detail((int32_t)tvis_event);
		break;

	case KDBG_CMD_PROC_FD_BT:
		{
			struct kdbg_pid_fd_bt bt_data;

			if (t_hdr->data_len)
				memcpy(&bt_data, (char *)data + TOOL_HDR_SIZE,
						sizeof(struct kdbg_pid_fd_bt));
			PRINT_KD("Get process BT packet received for Process(%d)\n",
					(int32_t)bt_data.pid);
			kdbg_set_fd_bt(&bt_data);
		}
		break;
#endif
	case KDBG_CMD_CM_STACK_SIZE_TRACER:
		PRINT_KD("Stack size  packet received.\n");
		if (tvis_event != agent_stackinfo_get_status())
			agent_stackinfo_set_status(tvis_event);
		break;

#ifdef CONFIG_KDML
	case KDBG_CMD_KDML_START_STOP:
		PRINT_KD("KDML start/stop packet received\n");
		/* 0 for start, 1 for stop */
		if (tvis_event < CM_START || tvis_event > CM_STOP) {
			PRINT_KD("Error in packet KDBG_CMD_KDML_START_STOP\n");
			break;
		}
		kdml_handle_start_stop(tvis_event);
		break;

	case KDBG_CMD_KDML_GET_ALLOCS_SUMMARY:
		PRINT_KD("KDML command received for Allocation Summary\n");
		kdml_work.data = (void *)NULL;
		kdml_work.pwork = (void *)kdml_send_alloc_summary;
		kdbg_workq_add_event(kdml_work, NULL);
		/* if kdml not started, start */
		break;

	case KDBG_CMD_KDML_GET_SLABINFO:
		PRINT_KD("KDML command received for Slabinfo\n");
		kdml_work.data = (void *)NULL;
		kdml_work.pwork = (void *)kdml_send_slabinfo;
		kdbg_workq_add_event(kdml_work, NULL);
		break;
#endif

	default:
		PRINT_KD("Kdebugd/Agent: Unknown command 0x%x\n", t_hdr->cmd);
		break;
	}
