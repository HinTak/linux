...
DEFINE_MUTEX(change_state_lock);
...
static void topthread_get_header(int header_row)
{
	...
	case TOPTHREAD_TITLE_ROW_1:
		PRINT_KD("\n\n");
		len += snprintf(buf, sizeof(buf) - len, "time   total  ");

		for (loop = 0; loop < TOPTHREAD_MAX_ITEM-1; loop++)
			len += snprintf(buf+len, sizeof(buf) - len, "top#%d                ", loop + 1);

		len += snprintf(buf+len, sizeof(buf) - len, "top#%d        Etc", loop + 1);
		PRINT_KD("%s", buf);
		PRINT_KD("\n");
		break;
	...
	case TOPTHREAD_TITLE_ROW_2:
		...
		if (sec_get_top_context() == TOP_THREAD_CONTEXT) {
			for (loop = 0; loop < TOPTHREAD_MAX_ITEM; loop++)
				len += snprintf(buf+len, sizeof(buf) - len, "thread               ");
		} else {
			for (loop = 0; loop < TOPTHREAD_MAX_ITEM; loop++)
				len += snprintf(buf+len, sizeof(buf) - len, "process              ");
		}
...
static void topthread_show_given_entry(int idx, int disp_option)
{
	...
	{
		int total_cpu_per = 0;
		len += snprintf(buf+len, sizeof(buf) - len, "%5ld  ", ptop_proc_buff_copy[idx].sec);
		...
		len += snprintf(buf+len, sizeof(buf) - len, "%3d ", total_cpu_per);
		...
		while (loop < TOPTHREAD_MAX_ITEM) {
			...
			if (ptop_proc_buff_copy[idx].
				...
				len += snprintf(buf+len, sizeof(buf) - len, "%-9.9s %4d",
				...
				len += snprintf(buf+len, sizeof(buf) - len, "(%3d) ",
						JIFFI_TO_PERCENT(
							ptop_proc_buff_copy
	...
	default:
	{
		...
		len += snprintf(buf+len, sizeof(buf) - len, "%3d ",  total_cpu);
		...
		while (loop < TOPTHREAD_MAX_ITEM) {
			if (ptop_proc_buff_copy[idx].item_per_cpu[cpu].cpu_table[loop].pid !=  0) {
				...
				len += snprintf(buf+len, sizeof(buf) - len, "%-9.9s %4d",
						...
						item_per_cpu[cpu].
						cpu_table[loop].pid);
				len += snprintf(buf+len, sizeof(buf) - len, "(%3d) ", tick_cnt);
			} else {
				len += snprintf(buf+len, sizeof(buf) - len, "              (  0) ");
			}
...
static void topthread_show_given_entry(int idx, int disp_option)
{
	...
	len += snprintf(buf+len, sizeof(buf) - len, "(%3d)", JIFFI_TO_PERCENT(excess_usage));
	PRINT_KD("%s\n", buf);
}
...
void sec_topthread_on_off(void)
{
	...
	mutex_lock (&change_state_lock);
	state = sec_topthread_get_state();

#ifdef CONFIG_KDEBUGD_AGENT_SUPPORT
	if (CM_START == agent_topthread_get_status()) {
		/* If agent state is already init no need to do anything*/
		if ((state > E_NONE && state < E_DESTROYING)) {
			PRINT_KD("Already Running\n");
			mutex_unlock (&change_state_lock);
			return;
		}
	}

	if (CM_STOP == agent_topthread_get_status()) {
		/* If agent state is already stopp
		 * If state is E_RUN_N_PRINT kdebugd is printing the data
		 * in that case no need to do anything
		 * FIXME: kdebugd background running feature will be hindered.
		 * */
		if (state == E_NONE || state >= E_DESTROYING ||
				state == E_RUN_N_PRINT) {
			PRINT_KD("Already Stopped or Kdebugd printing the data\n");
			mutex_unlock (&change_state_lock);
			return;
		}
	}
...
#define TOPTHREAD_MAX_PRINT 800
