...
{
	static u64 old_rx_bytes = 0, old_tx_bytes;
	u64 rx_persec, tx_persec;
	...
	if (sec_netusage_init_flag) {
		...
		if (net_dev->netdev_ops->ndo_get_stats64) {
			...
			const struct rtnl_link_stats64 *stats =
				net_dev->netdev_ops->ndo_get_stats64(net_dev, &temp);
...
void sec_netusage_agent_interrupt(void)
{
	...
	if (dev->netdev_ops->ndo_get_stats64) {
		struct rtnl_link_stats64 temp;
		const struct rtnl_link_stats64 *stats =
			dev->netdev_ops->ndo_get_stats64(dev, &temp);

		netusage.sec = kdbg_get_uptime();
		netusage.rx = stats->rx_bytes;
		netusage.tx = stats->tx_bytes;
