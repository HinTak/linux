...
#define SMART_DEADLOCK_PRINT(smart_deadlock_type, fmt , args ...)						\
	if(!smart_deadlock_check_worker()) {												\
		PRINT_KD(fmt , ##args);															\
	} else if( smart_deadlock_type > 0 ) {												\
		char kernel_bt_buf[300];														\
		__kernel_size_t alloc_size = 0;																\
	    snprintf(kernel_bt_buf, 100, fmt , ##args);										\
	    alloc_size = strlen(kernel_bt_buf) + 1;											\
	    smart_deadlock_set_bt(kernel_bt_buf, alloc_size, smart_deadlock_type);	\
	}
#else
	#define SMART_DEADLOCK_PRINT(smart_deadlock_type, fmt , args ...) PRINT_KD(fmt , ##args)
#endif
...
void kdebugd_send_fd_detail(int32_t task_id);
