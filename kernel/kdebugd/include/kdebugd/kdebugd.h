...
typedef enum {
	...
	KDBG_MENU_COUNTER_MONITOR_STACK_USAGE,
	KDBG_MENU_ELF,
	KDBG_MENU_AOP,
...
enum kubt_bt_symbol_type {
    KDEBUGD_BACKTRACE_ONLY_ADDR,
    KDEBUGD_BACKTRACE_ELF,
    KDEBUGD_BACKTRACE_DWARF
};

struct bt_frame {
    unsigned long addr;
    enum kubt_bt_symbol_type type;
#ifdef CONFIG_ELF_MODULE
    char sym_name[KDBG_ELF_SYM_NAME_LENGTH_MAX];
    char lib_name[KDBG_ELF_SYM_MAX_SO_LIB_PATH_LEN];
...
#else
#define PRINT_KD_STR KERN_WARNING
#endif
...
#define FD_BT_LIMIT 50
#define FD_BT_SEND_BIT (1 << 1)
#define FD_BT_SAVE_BIT (1 << 0)
