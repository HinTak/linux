...
/* API's for sending FD Tracer data*/
struct kdbg_pid_fd_bt {
	enum agent_cm event;
	pid_t pid;
};
/* Send complete details of FD open in reguler interval*/
void kdebugd_send_fd_detail(int32_t task_id);
/* Send Types of FD's Open for each process*/
void send_proc_fd_detail(pid_t pid);
