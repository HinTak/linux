...
#define kdml_mod_end_write(data, len) \
	agent_write(KDBG_CMD_KDML_MODULE_END, data, len)

#define kdml_mod_start_write(data, len) \
	agent_write(KDBG_CMD_KDML_MODULE_START, data, len)
...
struct kdml_module_info {
	...
	int64_t timestamp; /* time when module started */
	char name[MAX_MODULE_NAME_LEN];
	uint32_t unused; /* alignment to 8 bytes */
};

struct kdml_module_info_end {
...
extern void kdml_send_alloc_summary(void);
extern void kdml_send_slabinfo(void);
extern void kdml_handle_start_stop(int stop);
extern void kdbg_module_info_write(void);
