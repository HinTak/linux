		...
		const char __user *buffer, size_t count, loff_t *data)
{
	...
	temp_buf[count - 1] = '\0';
	if (kstrtol(temp_buf, base, &command) < 0)
		return -1;
	...
	/* follow through */
	case 1:
		kdbg_smack_enable = command;
		break;

	case 2:
#ifdef CONFIG_SECURITY_SMACK
		skp = smk_import_entry("@", strlen("@"));
		if (skp) {
			tsp = current->cred->security;
			tsp->smk_task = skp;
		} else {
			pr_emerg("Agent process smack setting fail!!");
			ret = 1;
		}
#endif
		break;
