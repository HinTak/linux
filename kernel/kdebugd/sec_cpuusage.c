...
static void cpuusage_show_header(void)
{
	...
	for_each_online_cpu(cpu) {
		len += snprintf(buf+len, sizeof(buf) - len, "sys i/o wait          ");
	}
	PRINT_KD("%s", buf);
	PRINT_KD("\n");
	len = 0;
	len += snprintf(buf+len, sizeof(buf) - len, "time ");
	PRINT_KD("time ");
	for_each_online_cpu(cpu) {
		len += snprintf(buf+len, sizeof(buf) - len, " CPU%d   user  |    |  ", cpu);
	}
	PRINT_KD("%s", buf);
	PRINT_KD("\n");
	len = 0;
...
void sec_cpuusage_show_buffer(int index)
{
	...
	len += snprintf(buf, sizeof(buf) - len, "%04ld", sec_count);

	for_each_online_cpu(cpu) {

		len += snprintf(buf+len, sizeof(buf) - len, " [%03d%%]", (user_perc[cpu] + sys_perc[cpu]));
		len += snprintf(buf+len, sizeof(buf) - len, " %03d%% %03d%% %03d%%", user_perc[cpu],
			 sys_perc[cpu], io_perc[cpu]);
#ifndef CONFIG_SMP
		/* Since only one CPU print character string also */
		pict_str[0] = pict_str[PIC_STR_LEN - 1] = '\0';
		len += snprintf(buf+len, sizeof(buf) - len, "\t%s", sec_cpuusage_create_string(pict_str,
							    user_perc[cpu],
							    sys_perc[cpu],
							    io_perc[cpu]));
#endif
...
void sec_cpuusage_agent_interrupt(void)
{
	...
	for_each_online_cpu(cpu) {
		...
		cpuusage.cpu_data[cpu].system = (int)
			...
			kcpustat_cpu(cpu).cpustat[CPUTIME_SOFTIRQ] +
			kcpustat_cpu(cpu).cpustat[CPUTIME_IRQ]);
		cpuusage.cpu_data[cpu].io = (int)kcpustat_cpu(cpu).cpustat[CPUTIME_IOWAIT];
	}
...
void sec_cpuusage_dump(void)
{
	...
	for (i = 0; i < buffer_count; ++i, ++idx) {
		...
		len += snprintf(buf, sizeof(buf) - len, "%04ld", g_psec_cpuusage_buffer_[idx].sec_count);
		for_each_online_cpu(cpu) {
			...
			len += snprintf(buf+len, sizeof(buf) - len, " [%03d%%]", (user_perc + sys_perc));
			len += snprintf(buf+len, sizeof(buf) - len, " %03d%% %03d%% %03d%%", user_perc,
				 sys_perc, io_perc);
