...
#define SMART_DEADLOCK_PRINT_ELF   "0x%08lx in %s () from %s\n"
#define SMART_DEADLOCK_PRINT_DWARF "0x%08lx in %s () at %s:%d\n"
...
static void kubt_print_symbol(struct kubt_kdbgd_sym *sym, unsigned long addr)
{
	...
	if (sym->is_valid && (sym->pdf_info->df_line_no != 0) ) {
		...
		if (smart_deadlock_check_worker()) {
				SMART_DEADLOCK_PRINT(2, SMART_DEADLOCK_PRINT_DWARF, addr,
					sym_name,
...
#ifdef CONFIG_SMART_DEADLOCK_PROFILE_MODE
		if (smart_deadlock_check_worker()) {
			SMART_DEADLOCK_PRINT(2, SMART_DEADLOCK_PRINT_ELF, addr,
					sym_name,
					elf_name);
		} else 
#endif
...
int kubt_kdbgd_show_trace(struct mm_struct *tsk_mm, struct kdbgd_bt_regs *regs)
{
	...
	SMART_DEADLOCK_PRINT(0, KUBT_TRACE_HEADER, (unsigned int)ar->fp, (unsigned int)ar->pc,
	...
	while (1) {
		...
		if (ar->pc < start) {
			...
			if (ret != 0)
				return ret;
			if (last_sp >= ar->sp) {
				SMART_DEADLOCK_PRINT(1, "KUBT: abort backtracing at SP: %x\n",
						(unsigned int)ar->sp);
				return 0;
			}
			last_sp = ar->sp;
