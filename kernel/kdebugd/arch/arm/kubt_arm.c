...
static int kubt_lookup_save_sym(struct mm_struct *tsk_mm, unsigned long pc,
		unsigned long *start, struct kdbg_bt_buffer *trace)
{
	struct aop_symbol_info symbol_info;
	char *sym_name = NULL, *lib_name = NULL;
	unsigned int start_addr = 0;
...
#endif
	int ret = -1;
	struct bt_frame *bt_sym;
	int idx = trace->nr_entries;

	if (idx >= trace->max_entries || trace->symbol == NULL)
		return -1;

	trace->nr_entries++;
	bt_sym = &(trace->symbol[idx]);

	sym_name = bt_sym->sym_name;
	sym_name[0] = '\0';
	lib_name = bt_sym->lib_name;
	lib_name[0] = '\0';
	...
	ret = kdbg_elf_get_symbol_and_lib_by_mm(tsk_mm,
			...
			&start_addr);
	if (ret && start_addr) {
		/* it would be 2 if line no. is found */
		bt_sym->type = KDEBUGD_BACKTRACE_ELF;
		*start = start_addr;
#ifdef CONFIG_DWARF_MODULE
		if (df_info.df_line_no) {
			bt_sym->type = KDEBUGD_BACKTRACE_DWARF;
			strncpy(bt_sym->df_file_name, df_info.df_file_name,
					AOP_DF_MAX_FILENAME);
			bt_sym->df_line_no = df_info.df_line_no;
		}
#endif
	} else {
		bt_sym->type = KDEBUGD_BACKTRACE_ONLY_ADDR;
		return -EINVAL;
	}

	if (kubt_filter_sym_name(tsk_mm, sym_name))
		return 1; /* filtered, stop... */

	return 0;
}
#else
static int kubt_lookup_save_sym(struct mm_struct *tsk_mm, unsigned long pc,
		unsigned long *start, struct kbdg_bt_trace *trace)
{
	trace->type = 0;
	pr_err("Not supported, enable CONFIG_ELF_MODULE!!\n");
	return -1;
}

static int kubt_lookup_sym(struct mm_struct *tsk_mm, unsigned long pc, unsigned long *start)
{
	pr_emerg(KDEBUGD_PRINT_ELF, call_depth, pc, "??", "??");
	return -1;
}
#endif
		...
		struct kdbgd_bt_regs *ar, unsigned int insn, unsigned long start)
{
	if ((insn & 0xfffff000) == 0xe24dd000) {
	...
	} else if ((insn & 0xffff0000) == 0xe98d0000) {
		int mask = insn & 0xffff;
		int rn = 0;
		int tmp_sp = ar->sp;
		...
		/* mov pc */
		ar->pc = kubt_jumpover_pc(flags, ar->pc, start);
	} else if ((insn & 0xffff00f0) == 0xe16d00f0) {
		/* strd Rt, [sp, #imm]! */
...
int kubt_kdbgd_show_trace(struct mm_struct *tsk_mm, struct kdbgd_bt_regs *regs, struct kdbg_bt_buffer *trace)
{
	...
	if (!trace)
		pr_emerg(KUBT_TRACE_HEADER, ar->fp, ar->pc, ar->lr,
			ar->sp, regs->sp_end);

	if (trace)
		ret = kubt_lookup_save_sym(tsk_mm, ar->pc, &start, trace);
	else
	...
	if (ar->pc > 0xffff0000) {
		if (trace)
			ret = kubt_lookup_save_sym(tsk_mm, ar->lr, &start, trace);
		else
			ret = kubt_lookup_sym(tsk_mm, ar->lr, &start);
		return ret;
	}
