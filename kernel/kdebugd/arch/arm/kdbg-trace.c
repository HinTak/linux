...
void show_user_backtrace_task(struct task_struct *tsk, int load_elf, struct mm_struct *tsk_mm)
{
...
#ifdef CONFIG_SMP
	SMART_DEADLOCK_PRINT(0, "\n\n");
	SMART_DEADLOCK_PRINT(1, "Pid: %d, Tid: %d, comm: %20s[%d] exec_start[%u.%09u]",
				tsk->tgid, tsk->pid, tsk->comm, task_cpu(tsk), sec, nsec);
	SMART_DEADLOCK_PRINT(0,"\n");
#else
	SMART_DEADLOCK_PRINT(0, "\n\n");
	SMART_DEADLOCK_PRINT(1, "Pid: %d, Tid: %d, comm: %20s", tsk->tgid, tsk->pid, tsk->comm);
	SMART_DEADLOCK_PRINT(0, "\n");
#endif
	...
	if (tsk_user_ssp < regs->ARM_sp) {
		SMART_DEADLOCK_PRINT(1, "Invalid Stack Address\n");
		SMART_DEADLOCK_PRINT(1, "REGS: FP: 0x%08lx, PC: 0x%08lx, LR: 0x%08lx, SP: 0x%08lx, \
				Stack End: 0x%08lx\n",
	...
	if (vma) {
		...
		if (regs->ARM_sp < vma->vm_start) {
			SMART_DEADLOCK_PRINT(0, "pid(%d) : seems stack overflow.\n"
		...
		SMART_DEADLOCK_PRINT(0, "\n");
		SMART_DEADLOCK_PRINT
			("================================================================================\n");
		SMART_DEADLOCK_PRINT(0, "Stack area (0x%08lx to 0x%08lx)\n", regs->ARM_sp,
				vma->vm_end);
		SMART_DEADLOCK_PRINT
			(0, "================================================================================\n");
		...
		SMART_DEADLOCK_PRINT
			(0, "task stack info : stack area (0x%08lx ~ 0x%08lx)\n"
