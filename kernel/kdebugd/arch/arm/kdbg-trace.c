...
int kubt_store_user_bt(struct task_struct *tsk, struct mm_struct *tsk_mm,
		struct kdbg_bt_buffer *trace)
{
	struct pt_regs *regs;
	struct kdbgd_bt_regs bt_regs;

	if (!tsk || !trace || !tsk_mm) {
		dprintk("Error: tsk %p trace %p tsk_mm %p\n", tsk, trace,
				tsk_mm);
		return -1;
	}

	if (!trace->max_entries) {
		dprintk("Error: trace->max_entries %d\n", trace->max_entries);
		return -1;
	}

	regs = task_pt_regs(tsk);
	bt_regs.fp = regs->ARM_fp;
	bt_regs.pc = instruction_pointer(regs);
	bt_regs.lr = regs->ARM_lr;
	bt_regs.sp = regs->ARM_sp;
	bt_regs.sp_end = tsk->user_ssp;

	return  kubt_kdbgd_show_trace(tsk_mm, &bt_regs, trace);
}
...
void show_user_backtrace_task(struct task_struct *tsk, int load_elf,
		struct mm_struct *tsk_mm, struct kdbg_bt_buffer *trace)
{
	struct pt_regs *regs;
	...
	if (!trace) {
#ifdef CONFIG_SMP
		PRINT_KD("\n\nPid: %d, Tid: %d, comm: %20s[%d] exec_start[%u.%09u]\n",
				tsk->tgid, tsk->pid, tsk->comm, task_cpu(tsk), sec, nsec);
#else
		PRINT_KD("\n\nPid: %d, Tid: %d, comm: %20s\n", tsk->tgid, tsk->pid, tsk->comm);
#endif
	} else {
		if (!trace->symbol) {
		...
		trace->pid = tsk->tgid;
		trace->tid = tsk->pid;
		get_task_comm(trace->comm, tsk);
#ifdef CONFIG_SMP
		trace->cpu_number = task_cpu(tsk);
		trace->exec_start = tsk->se.exec_start;
#endif
	}
...
#else
		kubt_kdbgd_show_trace(tsk_mm, &tsk_regs, trace);
#endif
	}
}
