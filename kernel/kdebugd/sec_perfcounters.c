...
{									\
	if (cpu_to_dump == -1 && cpu_current < (num_online_cpus() - 1))	\
		buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "%c", c);	\
	cpu_current++;							\
}
...
static void show_summary_header(void)
{
	...
	for_each_online_cpu(cpu) {
		...
		buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "  CPU%d   ", cpu);
		repeat_char(' ',
	...
	buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "Time ");
	cpu_current = 0;
	for_each_online_cpu(cpu) {
		CHECK_MISS_CPU(cpu);
		for (i = 0; i < NUM_PERF_GROUPS; i++)
			buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "%s ", perf_groups[i].name);
		CHECK_END_CPU_LOOP(cpu);
		CHECK_SEPARATOR('|');
	}
	PRINT_KD("%s", buf);
...
static void show_detail_header(void)
{
	...
	if (perfcounter_num == -1) {
		if (perfcounters_type != PER_THREAD_COUNTERS) {
			/* Output CPUs */
			buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "%*c", 5, ' ');
		...
		for (i = 0; i < NUM_PERF_COUNTERS; i++) {
			...
			for_each_online_cpu(cpu) {
				...
				if (i < NUM_PERF_COUNTERS - 1) {
					repeat_char(' ', 9 / 2 + 1);
					buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "_");
					repeat_char('-',
						(NUM_PERF_COUNTERS - i)*9-2*9);
					buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "=> ");
				}
				buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "%s ", perf_types[i].name);
		...
		for_each_online_cpu(cpu) {
			CHECK_MISS_CPU(cpu);
			/* Output preceding '|' */
			buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, " %s  ", perf_types[perfcounter_num].name);
	...
	if (perfcounters_type == PER_THREAD_COUNTERS)
		buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "Time  PID  ");
	else
		buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "Time ");
	...
	if (perfcounters_type == PER_THREAD_COUNTERS)
		buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "    task\n");
	else{
		PRINT_KD("%s\n", buf);
		buf_len = 0;
	}
		...
		unsigned int buffer_idx, unsigned int output_idx, int cpu)
{
	...
	case SUMMARY_OUTPUT:
		for (i = 0; i < NUM_PERF_GROUPS; i++) {
			...
			buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "%8lld ", value - prev_value);
		}
		break;
	case DETAIL_OUTPUT:
		if (perfcounter_num == -1)
			for (i = 0; i < NUM_PERF_COUNTERS; i++)	{
				...
				if (current_val > 99999999) {
					...
					buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "%7luk ", current_val);
				} else {
				buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "%8lu ", current_val);
				}
			}
		else {
			/*
			 ...
			 * with 64-bit, we can't tell that counter has wrapped
			 */
			buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "%10lu ",
...
static void dump_perf_result(void)
{
	...
	for (i = 0; i < buffer_count; i++, idx++) {
		...
		if (!idx || i) {
			buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "%4ld ", stat_buffer[idx].sec);
			cpu_current = 0;
			for_each_online_cpu(cpu) {
				CHECK_MISS_CPU(cpu);
...
int sec_perfcounters_dump(void)
{
	...
	case GLOBAL_COUNTERS:
		...
		if (perfcounters_status)
			buf_len += snprintf(buf + buf_len, sizeof(buf)-buf_len, "%4ld ",
	...
	case PER_THREAD_COUNTERS:
		...
		if (!num_active_counters_per_cpu) {
			buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "%4ld ", kdbg_get_uptime());
			buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, " -");
			PRINT_KD("%s\n", buf);
			break;
		}
		...
		do_each_thread(g, t) {
			...
			if (unlikely(first)) {
				...
				buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "     ");
			buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "%5d ", (int)t->pid);

			if ((t->perf_event_ctxp[perf_hw_context] == NULL)) {
				buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "-- No event for %s, ", t->comm);
				buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "skip rest threads for this group\n");
				PRINT_KD("%s\n", buf);
				break;
			}
			...
			list_for_each_entry_reverse(event,
				...
				while (j < NUM_PERF_COUNTERS) {
					if (perf_types[j++].available) {
						buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "%8lld ",
							perf_event_read_value(
								event,
								&enabled,
								&running));
			...
			buf_len += snprintf(buf + buf_len, sizeof(buf) - buf_len, "%s", t->comm);
			PRINT_KD("%s\n", buf);
