...
#include "../../../kernel/kdebugd/kdbg_util.h"
...
static int kdbg_ftrace_print_log_write(struct kdbg_ftrace_conf *pfconf, char *trace_buffer)
{
	...
	ret = vfs_write(fp, trace_buffer, strlen(trace_buffer),  &fp->f_pos);
...
static int kdbg_ftrace_print_log_banner(struct kdbg_ftrace_conf *pfconf, struct trace_iterator *iter, int lat)
{
	...
	struct trace_array_cpu *data = per_cpu_ptr(iter->trace_buffer->data, iter->trace_buffer->cpu);
...
#define BANNER_APPEND(x, ...)	do { if (TRACE_MAX_PRINT - ret > 0) { \
	...
	if (lat) {
		...
		BANNER_APPEND("#    | task: %.16s-%d "
				...
				data->comm, data->pid, (data->uid).val, data->nice,
...
int kdbg_ftrace_dump(struct kdbg_ftrace_conf *pfconf)
{
	...
	while (!trace_empty(&iter)) {
		cnt++;
		/*Check for the next entry, if not present break the loop */
		if (NULL == trace_find_next_entry_inc(&iter))
			break;
...
int kdbg_ftrace_set_trace_name(char *trace_name)
{
	...
	if (lock_count == 1) {
		return tracing_set_tracer(&global_trace, trace_name);
	} else {
...
unsigned long kdbg_ftrace_get_trace_max_latency(void)
{
	...
	if (tr->max_latency)
		latency = nsecs_to_usecs(tr->max_latency);
	else
		latency = 0;
	mutex_unlock(&trace_types_lock);
...
int kdbg_ftrace_get_trace_cpu_mask(void)
{
	...
	len = snprintf(mask_str, NR_CPUS + 1, "%*pb\n",
			cpumask_pr_args(global_trace.tracing_cpumask));
	if (!len) {
	...
	/* convert to integer */
	ret = kstrtoul(mask_str, 16, &cpu_mask);
...
int kdbg_ftrace_set_trace_cpu_mask(char *buf)
{
	...
	for_each_tracing_cpu(cpu) {
		 ...
		 */
		if (cpumask_test_cpu((unsigned int)cpu, global_trace.tracing_cpumask) &&
	...
	cpumask_copy(global_trace.tracing_cpumask, tracing_cpumask_new);
...
int kdbg_ftrace_set_trace_option(char *buf)
{
	...
	if (!trace_options[i]) {
		mutex_lock(&trace_types_lock);
		ret = set_tracer_option(&global_trace, cmp, neg);
		mutex_unlock(&trace_types_lock);
	}
...
int kdbg_ftrace_read_ctrl(struct kdbg_ftrace_conf *pfconf, char *buffer)
{
	...
	ret = vfs_read(fp, buffer, TRACE_MAX_PRINT - 1,  &fp->f_pos);
