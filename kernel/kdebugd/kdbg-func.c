...
static void show_task_prio(struct task_struct *p, enum kdbg_print_menu flag)
{
	...
	case KDBG_PRINT_PROCESS:
		len += snprintf(buf, sizeof(buf) - len, "%-16s", p->comm);
		break;
	...
	if (flag == KDBG_PRINT_PROCESS)
		len += snprintf(buf+len, sizeof(buf) - len, "    [%d] ", task_cpu(p));
	else if (flag == KDBG_PRINT_FIRST_THREAD || flag == KDBG_PRINT_LAST_THREAD)
		len += snprintf(buf+len, sizeof(buf) - len, "[%d] ", task_cpu(p));
#endif
	len += snprintf(buf+len, sizeof(buf) - len, " %c", state < sizeof(kdbg_stat_name) ? kdbg_stat_name[state] : '?');

#if (BITS_PER_LONG == 32)
	len += snprintf(buf+len, sizeof(buf) - len, " %08lX ", KSTK_EIP(p));
#else
	len += snprintf(buf+len, sizeof(buf) - len, " %016lx ", KSTK_EIP(p));
#endif
	mm = get_task_mm(p);
	if (mm) {
		regs = task_pt_regs(p);
...
#error "Architecture Not Supported!!!"
			...
			len += snprintf(buf+len, sizeof(buf) - len, "%6lu/%06lu    ", stack_usage >> 10, stack_size  >> 10);
		} else {
		...
		if (p->state == TASK_DEAD || p->state == EXIT_DEAD ||
				p->state ==	EXIT_ZOMBIE) {
			len += snprintf(buf+len, sizeof(buf) - len, "       [dead]    ");
		} else {
			len += snprintf(buf+len, sizeof(buf) - len, "     [kernel]    ");
		}
	}

	PRINT_KD("%s %4d %4d  %4d    %u       %u\n", buf,
			p->pid,
			p->prio,
			p->static_prio,
			p->policy,
...
int show_state_prio(void)
{
...
#if (BITS_PER_LONG == 32)
	PRINT_KD("                             user(kb/kb)                       \n");
	len += snprintf(buf+len, sizeof(buf) - len, "  task         ");

#ifdef CONFIG_SMP
	len += snprintf(buf+len, sizeof(buf) - len, "[CPU]      ");
#endif
	len += snprintf(buf+len, sizeof(buf) - len, "PC    stack_usage  pid prio sprio policy time_slice[x%dms]", 1000/HZ);
	PRINT_KD("%s\n", buf);
	...
	len += snprintf(buf+len, sizeof(buf) - len, "[CPU]      ");
#endif
	len += snprintf(buf+len, sizeof(buf) - len, "PC    stack_usage  pid prio sprio policy time_slice[x%dms]\n", 1000/HZ);
	PRINT_KD("%s\n", buf);
#endif
...
static void kdbg_show_task(struct task_struct *p, enum kdbg_print_menu flag)
{
...
#if BITS_PER_LONG == 32
	len += snprintf(buf + len, sizeof(buf) - len, " %c [%p]", state < sizeof(kdbg_stat_name) ? kdbg_stat_name[state] : '?', p);
	if (state == TASK_RUNNING)
		len += snprintf(buf+len, sizeof(buf) - len, " running  ");
	else
		len += snprintf(buf+len, sizeof(buf) - len, " %08lx ", thread_saved_pc(p));
#else
	len += snprintf(buf + len, sizeof(buf) - len, " %c [%p]", state < sizeof(kdbg_stat_name) ? kdbg_stat_name[state] : '?', p);
	if (state == TASK_RUNNING)
		len += snprintf(buf+len, sizeof(buf) - len, "  running task    ", thread_saved_pc(p));
	else
		len += snprintf(buf+len, sizeof(buf) - len, " %016lx ", thread_saved_pc(p));
#endif

#ifdef CONFIG_DEBUG_STACK_USAGE
	free = stack_not_used(p);
#endif
	len += snprintf(buf+len, sizeof(buf) - len, "%5lu %5d 0x%08lx %6d", free,
			task_pid_nr(p),
	...
	if (relative)
		len += snprintf(buf+len, sizeof(buf) - len, "%7d", relative->pid);
	else
		len += snprintf(buf+len, sizeof(buf) - len, "       ");
	...
	if (!mm) {
		if (p->state == TASK_DEAD || p->state == EXIT_DEAD ||
			...
			len += snprintf(buf+len, sizeof(buf) - len, "      (kernel thread)");
		}
	} else
			len += snprintf(buf+len, sizeof(buf) - len, "      (user thread)");
...
static void kdbg_show_state(void)
{
	...
	PRINT_KD(KERN_EMERG
			"                                                                                  sibling\n");
	len += snprintf(buf, sizeof(buf) - len, "  task         ");
#ifdef CONFIG_SMP
	len += snprintf(buf+len, sizeof(buf) - len, "    [CPU]  ");
#else
	len += snprintf(buf+len, sizeof(buf) - len, "    ");
#endif
	len += snprintf(buf + len, sizeof(buf) - len, "task_struct   PC    stack  pid  flag         father child   younger older");
	PRINT_KD("%s", buf);
	PRINT_KD("---------------------------------------------------------------------------------------------------\n");
#else
	PRINT_KD(KERN_EMERG
...
#else
		PRINT_KD
			("[kdebugd] [CPU %d] Pid:%4d  PC:0x%08x\t\t(TIME:%ld.%ld)\n",
	...
	while (!kthread_should_stop()) {
		...
		for (ii = 0; ii < buffer_count; ++ii, ++rd_idx) {
			...
			if (trace_func_name) {
				 ...
				 */
				rcu_read_lock();
				task = find_task_by_pid_ns(sec_pc_trace_info[rd_idx].pid, &init_pid_ns);
				if (task)
				...
				if (!task) {
					PRINT_KD("ELF: [ALERT]: %s  pid= 0x%x: No Thread\n",
							__FUNCTION__, sec_pc_trace_info[rd_idx].pid);
					continue;
				}
				/* get the symbol for program counter */
				if (!kdbg_elf_get_symbol_by_task(task,
							sec_pc_trace_info
							[rd_idx].pc,
							trace_func_name)) {
			...
			PRINT_KD
				("[kdebugd] [CPU %d] Pid:%4d tgid:%d PC:0x%08x (TIME:%06ld.%-9ld)  %s\n",
	...
	if (atomic_read(&sched_history_init_flag)) {
		...
		if (buffer_count) {
			...
			for (j = 0; j < buffer_count; ++idx, idx %= QUEUE_LIMIT) {
				 ...
				 */
				rcu_read_lock();
				task = find_task_by_pid_ns(g_pvd_queue_[idx].pid, &init_pid_ns);
				if (task)
					get_task_struct(task);
				rcu_read_unlock();

				if (task)
					get_task_comm(g_pvd_queue_[idx].comm, task);
				else {
					strncpy (g_pvd_queue_[idx].comm, "<th_dead>", sizeof("<th_dead>"));
					g_pvd_queue_[idx].comm[sizeof("<th_dead>") - 1]=0;
				}
				...
				strncpy(func_name, "???", sizeof("???"));
				func_name[sizeof("???") - 1] = '\0';

#ifdef CONFIG_ELF_MODULE
				if (func_name && task)
					kdbg_elf_get_symbol_by_task (task,
...
static atomic_t kdbg_task_status = ATOMIC_INIT(1);
