...
/* to know oprofile stopping status
 * During stop dont let user to browse AOP Menu */
static unsigned int aop_profile_stopping;
...
next:
		...
		if (operation >= AOP_MENU_REPORT_APPL_SAMPLE &&
			...
			(aop_profile_running || aop_profile_stopping)) {
			if (aop_profile_running)
				PRINT_KD("Adv OProfile is running....\n"
						"Stop adv OProfile before view report\n");
			else
				PRINT_KD("Adv OProfile Processing Sample please Wait....\n");
