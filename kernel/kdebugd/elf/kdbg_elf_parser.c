...
static LIST_HEAD(kdbg_elf_usb_head_item);
static DEFINE_MUTEX(elf_db_lock);
...
void get_elf_db_lock(void)
{
	mutex_lock(&elf_db_lock);
}

void put_elf_db_lock(void)
{
	mutex_unlock(&elf_db_lock);
}
...
static int kdbg_elf_chk_status(kdbg_elf_usb_elf_list_item *plist, char *filename)
{
	 ...
	 */
	bytesread = vfs_read(elf_filp, (char *)&pEhdr, sizeof(Elf32_Ehdr),
					&elf_filp->f_pos);
	...
	bytesread = vfs_read(elf_filp, (char *)pShdr,
					sizeof(Elf32_Shdr)*pEhdr.e_shnum, &elf_filp->f_pos);
	if (bytesread < (ssize_t)sizeof(Elf32_Shdr)*pEhdr.e_shnum) {
...
static int kdbg_elf_get_file_crc (kdbg_elf_usb_elf_list_item *plist_elf)
{
	...
	while ((count = vfs_read(elf_filp, buffer, sizeof(buffer), &elf_filp->f_pos)) > 0)
		...
		char *filename, kdbg_elf_usb_elf_list_item *plist, int load_from_usb)
{
	...
	bytesread = vfs_read(elf_filp, (char *)pPhdr,
					sizeof(Elf32_Phdr)*pEhdr.e_phnum, &elf_filp->f_pos);
	if (bytesread < (ssize_t)sizeof(Elf32_Phdr)*pEhdr.e_phnum) {
	...
	for (idx = 0; idx < pEhdr.e_shnum; idx++, lShdr++) {
		...
		if (idx == pEhdr.e_shstrndx) {
			...
			bytesread = vfs_read(elf_filp, (char *)strtab_buf,
							strtab_size, &elf_filp->f_pos);
			if (bytesread < (ssize_t)strtab_size) {
	...
	for (idx = 0; idx < pEhdr.e_shnum; idx++, lShdr++) {
		...
		if (!strcmp(KDBG_ELF_SECTION_NAME(lShdr, strtab_buf, strtab_size), ".gnu_debuglink")) {
			...
			bytesread = vfs_read(elf_filp, plist->gnu_debug_buf, sizeof(char)*lShdr->sh_size, &elf_filp->f_pos);
			if (bytesread < (ssize_t)(sizeof(char)*lShdr->sh_size)) {
				sym_errk("byteread is not accurate %d %u\n", bytesread,
						lShdr->sh_size);
				goto ERR;
			}
	...
	if (sym_str_size) {
		...
		if ((kdbg_elf_elf_load_option == KDBG_ELF_LOAD_DEFAULT && load_from_usb) ||
			...
			bytesread = vfs_read(elf_filp, plist->sym_buff,
							sizeof(char)*sym_str_size, &elf_filp->f_pos);
			if (bytesread < (ssize_t)sizeof(char)*sym_str_size) {
		...
		for (idx = 0; idx < symtab_ent_no; idx++) {
			bytesread = vfs_read(elf_filp, (char *)&isyms, sizeof(Elf32_Sym),
							&elf_filp->f_pos);
			if (bytesread < (ssize_t)sizeof (Elf32_Sym)) {
		...
		if ((kdbg_elf_elf_load_option == KDBG_ELF_LOAD_DEFAULT && strstr(plist->elf_name_actual_path, "usb")) ||
			...
			if (bytesread < (ssize_t)sizeof(char)*dyn_str_size) {
				sym_errk("SectionHeader: %d read bytes out of required %u\n",
						bytesread,
						sizeof(char)*dyn_str_size);
				err = -EIO;
		...
		for (idx = 0; idx < dyn_symtab_ent_no; idx++) {
			bytesread = vfs_read(elf_filp, (char *)&dyn_syms,
		...
		unsigned int symbol_len, unsigned int *start_addr)
{
	...
	if (!plist) {
		sym_printk("List is not availble\n");
		return -EINVAL;
	}
...
/* Caution:
 * This function strictly called inside Mutex lock
 * Please check usage of this function in kdbg_elf_sym_func.c file
 */
