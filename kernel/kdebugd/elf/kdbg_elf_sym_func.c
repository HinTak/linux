...
int kdbg_elf_get_symbol_by_task(struct task_struct *task, unsigned int sym_addr, char *sym_name)
{
	int ret = 0;
	...
	if ((vma->vm_flags & VM_READ) && (vma->vm_flags & VM_EXEC) &&
		...
		retval = kdbg_elf_sym_find(pc, plist, sym_name,
...
kdbg_elf_usb_elf_list_item * kdbg_elf_load_elf_db_by_elf_file(char *elf_file)
{
	...
	if (!plist) {
		plist = load_elf_db_by_elf_file(elf_file, 0, elf_status);
	}
	put_elf_db_lock();
	return plist;
}
...
/*
 * Wrappers for Demangler functions for
 * public user interface
 * */

/* Function expect new buffer for copy the demangled name
 * Mostly used by Advance Oprofile module */
 ...
 * the function returns buffer allocated by demanngler module
 * Make sure that buffer need to be free after using.
 */
