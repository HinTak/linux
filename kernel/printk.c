...
void write_emrg_klog(struct pt_regs *regs)
{
	...
	int sectors_to_write = log_buf_len >> 9;
	unsigned long flags;
	__u32 klog_buf_crc;
...
__init void init_emrg_klog_save(void)
{
	...
	if (!IS_ALIGNED(log_buf_len, 512)) {
		pr_err("System message buffer is not aligned\n");
		return;
	}
	...
	needed_space = WRITE_RAW_KLOG_HEAD_SIZE + log_buf_len;
	...
	emrg_klog_params.buf = kmalloc(log_buf_len, GFP_KERNEL);
	if (!emrg_klog_params.buf) {
	...
	emrg_klog_params.buf_len = log_buf_len;
