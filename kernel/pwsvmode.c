...
#elif defined(CONFIG_ARCH_NVT72172) || defined(CONFIG_ARCH_NVT72673)
int pwsv_get_boot_reason_micom(void)
{
	unsigned int ret = 0;
	unsigned int *p_stbc;
	...
	p_stbc = micom_gpio;
	ret = *(volatile unsigned int*)(p_stbc);
	ret = 0xFF & (ret >> 16);
	pr_info("PWSV] Micom Boot reason: %d 0x%X\n", ret, ret);
...
{
	pwsv_mode_flags &= ~(1 << PWSV_MODE_SUSPEND);
}
...
static void pwsv_notice_pwsvmode_submicom(int mode_flags)
{
...
static unsigned long local_find_symbol(char *name)
{
...
{
	static int (*nvt_micom_write)(unsigned char* pbuffer, unsigned int size) = NULL;
	unsigned char pbuffer[9];
	...
	pr_info("[%03d,%s] [Send][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X]\n", 
			current->pid, current->comm, pbuffer[0],pbuffer[1],pbuffer[2],pbuffer[3],pbuffer[4],pbuffer[5],pbuffer[6],pbuffer[7],pbuffer[8]);
	nvt_micom_write = (void *)local_find_symbol("ndp_messagebox_write");

	if (!nvt_micom_write)
		panic("Failed to get nvt_micom_write!\n");

	nvt_micom_write(pbuffer, 9);
}
#endif
