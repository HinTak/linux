...
#if defined CONFIG_ARCH_SDP1202 || defined(CONFIG_ARCH_SDP1302)
...
#if defined(CONFIG_ARCH_SDP1202)||defined(CONFIG_ARCH_SDP1207)|| defined(CONFIG_ARCH_SDP1302)|| defined( CONFIG_ARCH_SDP1302)	// i2c port interrupt disable
...
static int i2c_set_clock(struct i2c_ctrl_t* pPort, u16 speedKhz)
{
	...
	clk = clk_get(pPort->dev, NULL);
	i2cSrcClk = (clk_get_rate(clk) / 100 + 5) / 10;
...
#elif defined(CONFIG_ARCH_SDP1207)|| defined(CONFIG_ARCH_SDP1302)	|| defined( CONFIG_ARCH_SDP1302)
	regVal |= I2CCONE_FILTER_ENABLE;	// filter enable
	regVal |= I2CCONE_FILTER_MAJORITY;	// filter mode 
	regVal |= I2CCONE_FILTER_N_BUF(3);	// use buffer
#endif
...
// sync condition to false
			...
			//	printk("[%s recover] i2c status write 0x%08x\n",__FUNCTION__, regVal);
				R_I2C_STAT = 0xD0;
 				...
 				printk("[%s recover] i2c status 0x%08x gen dummy clk\n",__FUNCTION__, regVal);
				R_I2C_STAT=0xF0;//start
...
static int __init i2c_init(struct i2c_ctrl_t * pPort)
{
	int retVal = 0;
...
static int __ref sdp_i2c_probe(struct platform_device *dev)
{
	int 	retVal = 0;
	int	i;
	char 	drvName[64];
...
#if  defined(CONFIG_ARCH_SDP1202)||defined(CONFIG_ARCH_SDP1302)|| defined( CONFIG_ARCH_SDP1302)
			pPort->irq =(u32)(I2C_IRQ + i);
...
static int sdp_i2c_remove(struct platform_device *dev)
{
	int retVal = 0;
