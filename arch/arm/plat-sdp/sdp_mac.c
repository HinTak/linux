 ...
 * 2014.01.08,seungjun.heo: change netDev_close sequence v1.09
 * 2014.01.17,drain.lee: fix, calling close when not opened. v1.10
 * 2014.01.29,drain.lee: fix ioctl return error when using fixed phy. v1.11
 * 2014.02.05,drain.lee: change RTL8201x fixup v1.12
 * 2014.02.06,drain.lee: fix, prevent defact v1.13
 * 2015.01.07,drain.lee: supprot RTL8304E switch(for LFD) v1.14
 * 2015.01.13,drain.lee: supprot RTL8304E switch(broadcast disable) v1.15
 */
...
#define GMAC_DRV_VERSION	"v1.15(supprot RTL8304E switch - broadcast disable)"
...
sdp_mac_phy_regdump(SDP_GMAC_DEV_T *pGmacDev, const char *header)
{
	...
	if(!pGmacDev->phydev && pGmacDev->mdiobus) {
		phydev = phy_find_first(pGmacDev->mdiobus);

	} else if(pGmacDev->phydev) {
		phydev = pGmacDev->phydev;
	}
...
sdp_mac_ring_init(SDP_GMAC_DEV_T* pGmacDev)
{
	...
	pGmacDev->rx_ring.size = 0x1 << (fls(NUM_RX_DESC)-1);
	...
	pGmacDev->tx_ring.size = 0x1 << (fls(NUM_TX_DESC)-1);
...
sdpGmac_skbAlign(SDP_GMAC_DEV_T* pGmacDev, struct sk_buff *pSkb) {
	...
	/* IP align */
	skb_reserve(pSkb, reserve_size + NET_IP_ALIGN);
...
static int sdp_mac_regset(struct sdp_mac_reg_set *regset) {
	...
	iomem = ioremap_nocache(regset->addr, sizeof(u32));
	if(iomem) {
...
/* version check */
	...
	/* check B_SW_RESET is cleared */
	udelay(5);
	if(pDmaReg->busMode & B_SW_RESET) {
		DPRINTK_GMAC_ERROR("GMAC SW reset bit is already high.\n");
		return -SDP_GMAC_ERR;
	}
...
sdpGmac_netDev_open (struct net_device *pNetDev)
{
	...
	struct mii_bus *sdp_mdiobus = pGmacDev->mdiobus;
	int phy_addr = 0;
	...
	if(pGmacDev->is_netdev_opened) {
		dev_err(pGmacDev->pDev, "netdev was already opened.!\n");
		return -EILSEQ;
	}
	...
	if (IS_ERR_OR_NULL(phydev)) {
		if(pGmacDev->plat->fixed_phy_added) {
			...
			snprintf(bus_id, MII_BUS_ID_SIZE+2, PHY_ID_FMT, "fixed-0", pGmacDev->plat->fixed_phy_id);
			phydev = phy_connect(pNetDev, bus_id, sdp_mac_adjust_link, 0, pGmacDev->plat->fixed_rgmii?PHY_INTERFACE_MODE_RGMII:PHY_INTERFACE_MODE_RMII);
			if(IS_ERR_OR_NULL(phydev)) {
				dev_err(&pNetDev->dev, "Could not connect to Fixed PHY("PHY_ID_FMT")\n",
						"fixed-0", pGmacDev->plat->fixed_phy_id);
				return PTR_ERR(phydev);
			}
		} else {
			dev_err(pGmacDev->pDev, "%s: Could not find PHY\n", pGmacDev->mdiobus->id);
			return -ENODEV;
		}
	} else {
		/* real phy */
		...
		if (ret < 0) {
			dev_err(&pNetDev->dev, "Could not connect to PHY\n");
			return ret;
		}
		 ...
		 */
		if ( phydev->phy_id == 0) {
			dev_err(&pNetDev->dev, "Could not connect to PHY(phy id is 0x0)\n");
...
//	pGmacReg->configuration |= B_JUMBO_FRAME_EN ;
	...
	pGmacDev->is_netdev_opened = true;
...
sdpGmac_netDev_close (struct net_device *pNetDev)
{
	...
	if(!pGmacDev->is_netdev_opened) {
		dev_err(pGmacDev->pDev, "netdev is not opened!\n");
		return -EILSEQ;
	}
	...
	pGmacDev->is_netdev_opened = false;
...
static void
sdp_mac_selftest_adjust_speed_mac(SDP_GMAC_DEV_T *pGmacDev)
{
...
sdp_mac_selftest_do(struct net_device *pNetDev, enum sdp_mac_selftest_num test)
{
	...
	sdp_mac_selftest_adjust_speed_mac(pGmacDev);
...
sdp_mac_ethtool_selftest(struct net_device *pNetDev, struct ethtool_test *test, u64 *data)
{
	...
	if(test->flags & ETH_TEST_FL_OFFLINE) {
		...
		if( !pGmacDev->mdiobus || !phy_find_first(pGmacDev->mdiobus) ) {
...
static int sdpGmac_drv_suspend(struct platform_device *pDev, pm_message_t state)
{
	...
	/* backup mac address */
	memcpy((void *)pPower->gmac_macAddr, (void *)&pGmacReg->macAddr_00_High,
	 	sizeof(pPower->gmac_macAddr));

	if(netif_running(pNetDev) && pGmacDev->is_netdev_opened)
		sdpGmac_netDev_close(pNetDev);
...
sdpGmac_drv_resume(struct platform_device *pDev)
{
	...
	/* restore mac address */
	memcpy( (void *)&pGmacReg->macAddr_00_High, (void *)pPower->gmac_macAddr,
		sizeof(pPower->gmac_macAddr));
...
static int sdpGmac_rtl8201_clkout_fixup(struct phy_device *phydev)
{
	...
	/* power on  and isolate reg set to 0, normal operation */
	phyVal = (u16)phy_read(phydev, MII_BMCR);
	phyVal = phyVal & ~(BMCR_ISOLATE | BMCR_PDOWN);
	phy_write(phydev, MII_BMCR, (u16)phyVal);
...
// request interrupt resource
	...
	dev_info(pGmacDev->pDev, "IRQ is %d, use %s mode, %dbit BUS\n", pNetDev->irq,
		retVal==PHY_INTERFACE_MODE_RGMII?"RGMII":(retVal==PHY_INTERFACE_MODE_RMII?"RMII":"NA!!"),
...
int sdp_mac_parse_dt(struct device *dev, struct sdp_gmac_plat *plat)
{
