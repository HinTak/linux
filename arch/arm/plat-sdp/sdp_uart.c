 ...
 * 20130530 drain.lee v1.24 fix fill_cnt update
 * 20130612 drain.lee v1.25 add error interrupt print, change timeout value, irq CPU1
 * 20130612 drain.lee v1.26 add error msg(for tty buffer full)
 * 20130613 drain.lee v1.27 fix rx dma buffer overflow error
 * 20130913 drain.lee v1.28 fix uart rx bug
 * 20130922 drain.lee v1.29 fix spinlock recursion in startup
 */
...
#define SDP_UART_VERSION	"1.29(fix spinlock recursion)"
...
#if !defined(CURRENT_BAUD_RATE)
#define CURRENT_BAUD_RATE	(115200)	/* XXX remove this out */
#endif

/* Macro */

/* use max 15: avoid overflow */
static int sdp_uart_available_tx(struct uart_port *port)
{
	SDP_UART_REG_T * reg = (SDP_UART_REG_T*) ((void *) port->membase);

	u32 fstat = readl_relaxed(&reg->ufstat);

	if (fstat & (1 << 9))	/* FIFO full */
		return 0;
	else
		return (int)port->fifosize - ((fstat >> 4) & 0xf) - 1;
}
...
#define SDP_UDMA_RX_TIMEOUT		(4)/* ms */
...
/* return false not updated, true updateed */
static bool
sdp_uart_rx_dma_update(struct uart_port *port)
{
	SDP_UART_PRIV_T * p_uart_priv = UART_PRIV(port);
	int handled = false;
	...
	if(p_uart_priv->used_dma && p_uart_priv->udma_rx) {
		...
		if(udma_rx_reg->intstatus & INTSTAUS_STATUS) {
			...
			int now_head = (int) ((udma_tc-(udma_rx_reg->udmasts&UDMASTS_CURR_TC)) & (udma_tc-1));

			p_uart_priv->udma_rx->fill_cnt += (int) udma_tc;

			dev_dbg(port->dev, "sdp_uart_rx_dma_update(f=%d h=%d(%d) t=%d %s%s)\n",
				p_uart_priv->udma_rx->fill_cnt, now_head, p_uart_priv->udma_rx->head, p_uart_priv->udma_rx->tail,
				in_irq()?"in irq ":"", in_softirq()?"in softirq ":"");

			if(p_uart_priv->udma_rx->fill_cnt < 0 || p_uart_priv->udma_rx->fill_cnt > (int) udma_tc) {
				dev_err(port->dev, "Rx DMA Ring overflow(f=%d h=%d(%d) t=%d)!\n",
					p_uart_priv->udma_rx->fill_cnt, now_head, p_uart_priv->udma_rx->head, p_uart_priv->udma_rx->tail);
			}
...
//	u8 pio_data[16];
	...
	if(bytes < 0) {
		/* dma ring buffer is wrap. update rx fill cnt */
		if( sdp_uart_rx_dma_update(port) == false ) {
			pr_err("%s: rx fill count not updated.\n"
				"(udma_tc=%u, fill_cnt=%d(%d), off_head=%d, off_tail=%d, bytes=%d)\n",
				__FUNCTION__, udma_tc, fill_cnt, udma_rx_priv->fill_cnt, off_head, off_tail, bytes);
		}
		fill_cnt = udma_rx_priv->fill_cnt;
		bytes = fill_cnt + off_head - off_tail;
	}

	if(bytes < 0) {
		pr_err("%s: bytes is not correct! wait next timeout..\n"
	...
	if(bytes > (int) udma_tc) {
		pr_err("%s: bytes is not correct! maybe rx ring overflow!! rx ring reset\n"
			"(udma_tc=%u, fill_cnt=%d, off_head=%d, off_tail=%d, bytes=%d)\n",
			__FUNCTION__, udma_tc, fill_cnt, off_head, off_tail, bytes);
...
//	WARN(bytes < 0 || bytes > udma_tc, "bytes(%#x) f%#x h%#x t%#x\n", bytes, fill_cnt, off_head, off_tail);
	...
	if(bytes > 0) {
		...
		if(bytes > added) {
			pr_err("%s: can't insert whole data into tty buffer!\n"
				"(udma_tc=%u, fill_cnt=%d, off_head=%d, off_tail=%d, bytes=%d, added=%d)\n",
				__FUNCTION__, udma_tc, fill_cnt, off_head, off_tail, bytes, added);
		}
	}
	...
	if((off_tail + added) > (int) (udma_tc-1))
		udma_rx_priv->fill_cnt -= (int) udma_tc;
...
//	spin_lock_irqsave(&port->lock, flags);
	...
	count = sdp_uart_available_tx(port);
        while (count > 0) {
		writel_relaxed(xmit->buf[xmit->tail], &reg->utxh);
...
static irqreturn_t sdp_uart_intr (int irq, void *dev_id)
{
	...
	/* error status */
	if(utrstat & UTRSTAT_EI) {
		u32 error_ststus = reg->uerstat;
		if(error_ststus & 0xF) {
			dev_err(port->dev, "%s%s%s%serrors occur!\n",
				error_ststus&UERSTAT_OE ? "Overrun ":"",
				error_ststus&(1<<1) ? "Parity ":"",
				error_ststus&(1<<2) ? "Frame ":"",
				error_ststus&(1<<3) ? "BreakSig ":"");
		}
		reg->utrstat = UTRSTAT_EI;
	}
...
static struct uart_port* pdev_to_port(struct platform_device *dev)
{
...
static int sdp_serial_probe(struct platform_device *dev)
{
	...
	for (i = 0; i < SDP_UART_NR; i++) {
		...
		platform_set_drvdata(dev, &sdp_uart_port[i]);
...
{
        struct uart_port *port = pdev_to_port(dev);
...
static void sdp_serial_save_state(SDP_UART_REG_T *reg, struct sdp_uart_cfg *cfg)
{
	cfg->ulcon = reg->ulcon;
	cfg->ucon = reg->ucon;
	cfg->ubrdiv = reg->ubrdiv;
}

static void sdp_serial_restore_state(SDP_UART_REG_T *reg, struct sdp_uart_cfg *cfg)
{
	reg->ubrdiv = cfg->ubrdiv;
	reg->ulcon = cfg->ulcon;
	reg->ucon = cfg->ucon;
}
...
static int sdp_serial_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	if(DEBUG_PORT(port)) {
		...
		SDP_UART_REG_T * reg = (SDP_UART_REG_T*) ((void *) port->membase);
		sdp_serial_save_state(reg, &priv->cfg);
...
static int sdp_serial_resume(struct platform_device *dev)
{
	...
	if(DEBUG_PORT(port)) {
		...
		SDP_UART_PRIV_T *priv = (SDP_UART_PRIV_T *) UART_PRIV(port);
		SDP_UART_REG_T * reg = (SDP_UART_REG_T*) ((void *) port->membase);
		sdp_serial_restore_state(reg, &priv->cfg);
...
static const struct of_device_id sdp_uart_dt_match[] = {
	{ .compatible = "samsung,sdp-uart" },
	{},
};
MODULE_DEVICE_TABLE(of, sdp_uart_dt_match);
...
static struct platform_driver  sdp_serial_driver = {
	...
	.driver		= {
		.name 	= "sdp-uart",
#ifdef CONFIG_OF
		.of_match_table = sdp_uart_dt_match,
#endif
		.owner 	= THIS_MODULE,
	},
};

static unsigned int get_sdp_uartclk(void)
{
	static struct clk* sdp_uart_clk = NULL;

	if (!sdp_uart_clk) {
#ifdef CONFIG_OF		
		sdp_uart_clk = clk_get(NULL, "apb_pclk");
#else
		sdp_uart_clk = clk_get(NULL, "sdp_uart");
#endif
		if (sdp_uart_clk)
			printk (KERN_INFO "sdp_uart clock is %ldHZ\n", clk_get_rate(sdp_uart_clk));
		else {
			printk (KERN_ERR "sdp_uart clock is not registered in this system.\n");
			return 0;
		}
	}
	return clk_get_rate(sdp_uart_clk);
}
...
sdp_init_port(void)
{
	...
	for (idx = 0; idx < SDP_UART_NR; idx++) {
		...
		p_init_port->uartclk		= get_sdp_uartclk();
...
static void sdp_console_putchar(struct uart_port* port, const char c)
{
	SDP_UART_REG_T* reg = (SDP_UART_REG_T*) ((void *) port->membase);

	while (sdp_uart_available_tx(port) < 1)
		barrier();
	writel_relaxed(c, &reg->utxh);
}
...
sdp_console_polling_write(struct uart_port* port, const char *s, unsigned count)
{
	...
	while (count > 0) {
		if (*s == '\n')
			sdp_console_putchar(port, '\r');
		sdp_console_putchar(port, *s);
		count--; s++;
	}
#endif
}
