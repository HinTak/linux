...
sdp_hwmem_get(struct file * file, SDP_GET_HWMEM_T* p_hwmem_info)
{
	...
	hwmem_info.phy_addr = addr = (unsigned long) page_to_phys(page);
	//hwmem_info.vir_addr = do_mmap(file, 0, size, PROT_WRITE | PROT_READ, MAP_SHARED, addr);
	hwmem_info.vir_addr = 
		do_mmap_pgoff(file, 0, size, PROT_READ | PROT_WRITE, MAP_SHARED, addr >> PAGE_SHIFT);
...
static int
sdp_hwmem_gpio_control(u32 cmd, SDP_HWMEM_GPIO_T * args)
{
#ifdef CONFIG_GPIOLIB
	int ret_val = 0;
	SDP_HWMEM_GPIO_T gpio_info;
	
	
	ret_val = (int) copy_from_user(&gpio_info, args, sizeof(SDP_HWMEM_GPIO_T));
	if(ret_val){
		PRINT_HWMEM_ERR("get gpio control info failed\n");
		return -EINVAL;
	}

	ret_val = gpio_request(gpio_info.gpionum, "hwmem");
	if(ret_val)
	{
		pr_err("gpio_request fail!! gpionum=%d err=%d\n", gpio_info.gpionum, ret_val);
		return ret_val;
	}
	
	if(cmd == CMD_HWMEM_GPIO_WRITE)
	{
		ret_val = gpio_direction_output(gpio_info.gpionum, gpio_info.vaule);
		if(ret_val)
		{
			pr_err("gpio_direction_output fail!! gpionum=%d value=%d err=%d\n", gpio_info.gpionum, gpio_info.vaule, ret_val);
			gpio_free(gpio_info.gpionum);
			return ret_val;
		}
	}
	else
	{
		ret_val = gpio_direction_input(gpio_info.gpionum);
		if(ret_val)
		{
			pr_err("gpio_direction_input fail!! gpionum=%d err=%d\n", gpio_info.gpionum, ret_val);
			gpio_free(gpio_info.gpionum);
			return ret_val;
		}
		gpio_info.vaule = __gpio_get_value(gpio_info.gpionum);
		ret_val = (int) copy_to_user((void __user *) args, &gpio_info, sizeof(SDP_HWMEM_GPIO_T));
		if(ret_val){
			PRINT_HWMEM_ERR("get gpio control info failed\n");
			return -EINVAL;
		}
...
sdp_hwmem_ioctl(struct file *file, unsigned int cmd, unsigned long args)
{
	...
	switch(cmd){
		...
		case (CMD_HWMEM_SET_CLOCKGATING): 
			...
			break;
		case CMD_HWMEM_GPIO_WRITE:
		case CMD_HWMEM_GPIO_READ:
			ret_val = sdp_hwmem_gpio_control(cmd, (SDP_HWMEM_GPIO_T *) args);
			break;
