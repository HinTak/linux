...
struct _sdp_gpio_pull *gpio_pullmap[624] = {NULL, };
...
void sdp_gpio_set_value(unsigned int gpionum, unsigned int value)
{
	...
	tmp = __raw_readl((void *)addr);
	tmp &= (unsigned int)~(1 << pin);
	tmp |= value << pin;
	__raw_writel(tmp,(void *)addr);
...
unsigned int sdp_gpio_get_value(unsigned int gpionum)
{
	...
	return (__raw_readl((void *)addr) & (1 << pin)) >> pin;
}
