 ...
 * 140206, drain.lee : fix, prevent defact.
 */
...
#include <plat/sdp_gadma.h>

/* define for driver */
#define SDP_GADMA_DRV_NAME "sdp-gadma"
#define SDP_GADMA_DRV_VER  "140206(fix, prevent defact)"
 ...
 */
PRIVATE inline u32 sdp_gadma_read(struct sdp_gadma_device *gdev, u32 reg_off) {
	return readl((void *)((u32)gdev->iobase + reg_off));
}

PRIVATE inline void sdp_gadma_write(struct sdp_gadma_device *gdev, u32 val, u32 reg_off) {
	writel(val, (void *)((u32)gdev->iobase + reg_off));
}

int sdp_gadma_start(struct sdp_gadma_chan *gchan) {
	u32 start = 0;
...
int sdp_gadma_stop(struct sdp_gadma_chan *gchan) {
	u32 start = 0;
...
int sdp_gadma_is_busy(struct sdp_gadma_chan *gchan) {
	u32 start = 0;
...
void sdp_gadma_interrupt_enable(struct sdp_gadma_chan *gchan) {
	u32 mask = 0;
...
void sdp_gadma_interrupt_disable(struct sdp_gadma_chan *gchan) {
	u32 mask = 0;
...
void sdp_gadma_tasklet(unsigned long data) {
...
irqreturn_t sdp_gadma_isr(int irq, void *data)
{
	...
	dma_addr_t dest, dma_addr_t src, size_t len, unsigned long flags)
{
	...
	if(len <= BLT0_SIZE_MAX_WIDTH) {
		...
		for(width = BLT0_SIZE_MAX_WIDTH; width > 0; width--) {
			if((len % width) == 0) {
				height = len / width;
