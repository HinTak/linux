 ...
 */
  ...
  * 20140117	drain.lee	bugfix, sometimes valid_peri value is 0
  * 20140204	drain.lee	bugfix, prevent defect.
  */
...
#define VERSION_SRTING		"20140204"
...
/* XXX: temporary, 3.8.13 has no %pa format yet */
#ifndef PHYS_ADDR_FMT
		...
		size_t sg_len, unsigned long flags)
{
	...
	/* first address*/
	dev_dbg(pch->dmac->pif.dev, "scatterlist from "PHYS_ADDR_FMT" to "PHYS_ADDR_FMT" len 0x%x(%d)",
		...
		curdst = sg_next(curdst), cursrc = sg_next(cursrc))
	{
		dev_dbg(pch->dmac->pif.dev,
			"scatterlist from "PHYS_ADDR_FMT" to "PHYS_ADDR_FMT" len 0x%x(%d)",
...
pl330_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	if (!pdat->nr_valid_peri) {
		dev_err(&adev->dev, "not found valid peri!\n");
		kfree(pdat);
		return -ENODEV;
	}
#else
