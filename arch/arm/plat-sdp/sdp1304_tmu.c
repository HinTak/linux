/* arch/arm/plat-sdp/sdp1304_tmu.c
 *
...
static int sdp1304_enable_tmu(struct sdp_tmu_info * info)
{
	...
	val = readl(info->tmu_base + SDP_TSC_CONTROL5) | TSC_TEM_T_EN;
	writel(val, info->tmu_base + SDP_TSC_CONTROL5);

	val = readl(info->tmu_base + SDP_TSC_CONTROL6) | TSC_TEM_T_EN;
	writel(val, info->tmu_base + SDP_TSC_CONTROL6);
...
static unsigned int sdp1304_get_curr_temp(struct sdp_tmu_info *info)
{
	int temp[3] = {0, };
	...
	temp[0] = (readl(info->tmu_base + SDP_TSC_CONTROL7) >> TSC_TEM_T_TS_8BIT_TS) & 0xFF;
	temp[1] = readl(info->tmu_base + SDP_TSC_CONTROL9) & 0xFF;
	temp[2] = (readl(info->tmu_base + SDP_TSC_CONTROL9) >> 16) & 0xFF;
...
static unsigned int get_temp(struct sdp_tmu_info * info, int id)
{
	unsigned int val;
	int temp;
	...
	/* get temperature from TSC register */
	if (id == 0)
		val = (readl(info->tmu_base + SDP_TSC_CONTROL7) >> TSC_TEM_T_TS_8BIT_TS) & 0xFF;
	else if (id == 1)
		val = readl(info->tmu_base + SDP_TSC_CONTROL9) & 0xFF;
	else if (id == 2)
		val = (readl(info->tmu_base + SDP_TSC_CONTROL9) >> 16) & 0xFF;
	else
		return 0;
	
	temp = val + diff_val[id] - (TSC_DEGREE_25 - 25);
	if (temp < 0)
		temp = 0;
...
int sdp1304_tmu_init(struct sdp_tmu_info * info)
{
	...
	info->get_curr_temp = sdp1304_get_curr_temp;
