	...
	(struct sdp_gdma_chan *gchan, struct sdp_gdma_reqcfg *reqcfg)
{
	...
	if(reqcfg->reqtype == COPY) {
		...
		sdp_gdma_write(gchan->gdev,
			GDMA_CON_DST_INT_ARM | GDMA_CON_FUNC_TYPE_FILL |
			GDMA_CON_BUSTLEN_VAL(reqcfg->bus_burst_size),
			GDMA_CON);
		sdp_gdma_write(gchan->gdev, reqcfg->fill_pattern_h, GDMA_FILL0);
		sdp_gdma_write(gchan->gdev, reqcfg->fill_pattern_l, GDMA_FILL1);
