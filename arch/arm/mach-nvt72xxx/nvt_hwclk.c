...
#if defined(CONFIG_NVT_HW_CLOCK)
...
unsigned long periph_clk;
EXPORT_SYMBOL(periph_clk);
uint32_t nvt_clk_vmap;
...
uint64_t __ca53_mul = 0;
EXPORT_SYMBOL(__ca53_mul);
...
static int gtimer_proc_show(struct seq_file *m, void *v)
{
	...
	seq_puts(m, "1st Get Gtimer time!\n");
	time = hwclock_ns((uint32_t *)hwclock_get_va());
	seq_printf(m, "Gtimer :  time      = %llu\n", time);
	msleep(10000);
	seq_puts(m, "2nd Get Gtimer time!\n");
	time = hwclock_ns((uint32_t *)hwclock_get_va());
	seq_printf(m, "Gtimer :  time      = %llu\n", time);
	msleep(10000);
	seq_puts(m, "3rd Get Gtimer time!\n");
	time = hwclock_ns((uint32_t *)hwclock_get_va());
	seq_printf(m, "Gtimer :  time      = %llu\n", time);
...
{
#if defined(CONFIG_NVT_CA53_HW_CLOCK)
	pr_info("TEST NVT HW CLK");
#else
	pr_info("TEST NVT HW CLK : periph_clk = %lu", periph_clk);
#endif
