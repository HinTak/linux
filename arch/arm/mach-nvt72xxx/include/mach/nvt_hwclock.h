...
#define MPLL_GetData(u8Offset) \
({	\
readl((u32 *)((u32)MPLL_REG_VA + ((u8Offset) * 4))); \
})
...
extern void __iomem *mpll_reg_base;
...
static inline unsigned long hwclock_get_freq(void)
{
	...
	if (periph_clk == 0) {
		/**
		 * Ref arm/arch/mach-nvt72xxx/clk.c
		 * Get ratio of ARM PLL
		 */
		if (nvt_clk_vmap == 0) {
			retVal = (MPLL_GetData(EN_MPLL_OFF_ARM));
			retVal |= (MPLL_GetData(EN_MPLL_OFF_ARM + 1) << 8);
			retVal |= (MPLL_GetData(EN_MPLL_OFF_ARM + 2) << 16);
		} else{
...
static inline uint64_t hwclock_raw_ns(uint32_t *addr)
{
	...
	do {
		value_tmp = readl(addr+1);
		value_l = readl(addr);
		value_h = readl(addr+1);
