...
static void __iomem *l2_base, *clk_gen_base;
...
static void __cpuinit set_secondary_entry(unsigned int cpu_id)
{
	switch (cpu_id) {
	case 1:
		writel(virt_to_phys(nvt_secondary_startup),
				clk_gen_base+NVTCORE1_JUMP_OFFSET);
		break;
	case 2:
		writel(virt_to_phys(nvt_secondary_startup),
				clk_gen_base+NVTCORE2_JUMP_OFFSET);
		break;
	case 3:
		writel(virt_to_phys(nvt_secondary_startup),
				clk_gen_base+NVTCORE3_JUMP_OFFSET);
		break;
	default:
		break;
	}
...
static const struct of_device_id nvt_l2c_match[] = {
	{ .compatible = "arm,pl310-cache", },
	{}
};

static const struct of_device_id nvt_clk_gen_match[] = {
...
int __cpuinit nvt_boot_secondary(unsigned int cpu, struct task_struct *idle)
{
	...
	struct device_node *l2c = of_find_matching_node(NULL,
			nvt_l2c_match);
	struct device_node *clk_gen = of_find_matching_node(NULL,
			nvt_clk_gen_match);

	if (!clk_gen_base) {
		clk_gen_base = of_iomap(clk_gen, 0);
		BUG_ON(!clk_gen_base);
	}
	...
	if (!l2_base) {
		l2_base = of_iomap(l2c, 0);
		BUG_ON(!l2_base);
	}

	writel_relaxed(__pa(&secondary_data), (l2_base + L2X0_CLEAN_LINE_PA));
	writel_relaxed((__pa(&secondary_data) + 32),
			(l2_base + L2X0_CLEAN_LINE_PA));
...
int __cpuinit nvt_boot_secondary_ca53(unsigned int cpu, struct task_struct *idle)
{
	unsigned long timeout;
	/*
	 * Set synchronisation state between this boot processor
