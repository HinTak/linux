...
DEFINE_MUTEX(imx8_bus_freq_mutex);
asmlinkage int __invoke_psci_fn_smc(u32, u32, u32, u32);

static irqreturn_t wait_in_wfe_irq(int irq, void *dev_id)
{
	__invoke_psci_fn_smc(FSL_SIP_DDR_DVFS, WAIT_BUS_FREQ_DONE, 0, 0);
...
static void update_bus_freq(int target_freq)
{
	...
	__invoke_psci_fn_smc(FSL_SIP_DDR_DVFS, target_freq, online_cpus, 0);
...
{
	mutex_lock(&imx8_bus_freq_mutex);

	if (!cancel_reduce_bus_freq)
		reduce_bus_freq();

	mutex_unlock(&imx8_bus_freq_mutex);
}
...
void imx8_request_bus_freq(enum bus_freq_mode mode)
{
	mutex_lock(&imx8_bus_freq_mutex);
...
void imx8_release_bus_freq(enum bus_freq_mode mode)
{
	mutex_lock(&imx8_bus_freq_mutex);
	if (mode == BUS_FREQ_HIGH) {
		if (high_bus_count == 0) {
...
int imx8_get_bus_freq_mode(void)
{
...
static int busfreq_probe(struct platform_device *pdev)
{
	...
	busfreq_callback.request = &imx8_request_bus_freq;
	busfreq_callback.release = &imx8_release_bus_freq;
	busfreq_callback.get_mode = &imx8_get_bus_freq_mode;

	return 0;
}

static const struct of_device_id imx_busfreq_ids[] = {
	{ .compatible = "fsl,imx8_busfreq", },
	{ /*sentinel */}
};
