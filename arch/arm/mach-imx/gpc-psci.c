...
static void imx_gpc_psci_irq_unmask(struct irq_data *d)
{
	...
	__invoke_psci_fn_smc(FSL_SIP_GPC, FSL_SIP_CONFIG_GPC_UNMASK, d->hwirq, 0);
...
{
	spin_lock(&gpc_psci_lock);
	__invoke_psci_fn_smc(FSL_SIP_GPC, FSL_SIP_CONFIG_GPC_MASK, d->hwirq, 0);
	spin_unlock(&gpc_psci_lock);
...
{
	spin_lock(&gpc_psci_lock);
	__invoke_psci_fn_smc(FSL_SIP_GPC, FSL_SIP_CONFIG_GPC_SET_WAKE, d->hwirq, on);
	spin_unlock(&gpc_psci_lock);
					 ...
					 const struct cpumask *dest, bool force)
{
	...
	spin_lock(&gpc_psci_lock);
	__invoke_psci_fn_smc(FSL_SIP_GPC, 0x4, d->hwirq, cpu);
	spin_unlock(&gpc_psci_lock);
...
static int imx_gpc_psci_domain_xlate(struct irq_domain *domain,
...
static struct irq_domain_ops imx_gpc_psci_domain_ops = {
	.xlate	= imx_gpc_psci_domain_xlate,
...
OF_DECLARE_2(irqchip, imx_gpc_psci, "fsl,imx8mq-gpc", imx_gpc_psci_init);
...
static int imx_gpc_pd_power_on(struct generic_pm_domain *domain)
{
	...
	__invoke_psci_fn_smc(FSL_SIP_GPC, FSL_SIP_CONFIG_GPC_PM_DOMAIN,
		pd->gpc_domain_id, GPC_PD_STATE_ON);
