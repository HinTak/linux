...
void imx6_request_bus_freq(enum bus_freq_mode mode)
{
	mutex_lock(&bus_freq_mutex);
...
EXPORT_SYMBOL(imx6_request_bus_freq);

void imx6_release_bus_freq(enum bus_freq_mode mode)
{
	mutex_lock(&bus_freq_mutex);
...
EXPORT_SYMBOL(imx6_release_bus_freq);

int imx6_get_bus_freq_mode(void)
{
	return cur_bus_freq_mode;
}
EXPORT_SYMBOL(imx6_get_bus_freq_mode);
...
static int busfreq_probe(struct platform_device *pdev)
{
	...
	busfreq_callback.request = &imx6_request_bus_freq;
	busfreq_callback.release = &imx6_release_bus_freq;
	busfreq_callback.get_mode = &imx6_get_bus_freq_mode;
