...
/****************************************
 * SDP1601 Kant-M syscore
 ****************************************/
static void __sdp1601_pm_allusb_clk(u32 onoff)
{
	const u32 USB_CLK_RST = 0x00F405E0;
	const u32 USB_CLK_RST_MASK = 0x00011111;

	int ret = sdp_set_clkrst_mux(USB_CLK_RST,USB_CLK_RST_MASK
		,((onoff == 0) ? 0x0 : 0x11111) );
	if( ret != 0 ) {
		pr_err("[PM] error[0x%X] : USB CLOCK %d\n",ret,onoff);
	}
}

static void sdp1601_pm_suspend(void)
{
	__sdp1601_pm_allusb_clk(0);
}

static void sdp1601_pm_shutdown(void)
{
	__sdp1601_pm_allusb_clk(0);
}
/******************************************/
