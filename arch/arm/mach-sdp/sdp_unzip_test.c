...
static int err_inject_mod = 600;


#define DEBUG		1
#define N_PAGES		32/* max buffer size per one sample  */
#define N_LOOP		0
#define N_THERADS	0
...
static int sdp_unzip_test_zlib_comp(u8 *input, u32 ilen, u8 *output, u32 olen)
{
#ifndef ZLIB_DEFLATE
	...
	res = crypto_compress_setup(tfm, NULL/*params*/, 0/*paramsize*/);
	...
	//print_comp_request(&req);
	if(res < 0) {
			pr_err("crypto_compress_update return error %d\n", res);
	...
	tfm = crypto_alloc_comp("deflate", CRYPTO_ALG_TYPE_COMPRESS, 0);
	...
	res = crypto_comp_compress(tfm, input, ilen, output, &olen);
	if(res < 0) {
		pr_err("crypto_comp_compress() return %d\n", res);
...
static int sdp_unzip_test_zlib_decomp(u8 *input, u32 ilen, u8 *output, u32 olen)
{
#ifndef ZLIB_DEFLATE
	struct crypto_pcomp *tfm;
	//const char *algo = crypto_tfm_alg_driver_name(crypto_pcomp_tfm(tfm));
	...
	res = crypto_comp_decompress(tfm,  input, ilen, output, &olen);
	if(res < 0) {
		pr_err("crypto_comp_compress() return %d\n", res);
		crypto_free_comp(tfm);
		return res;
...
static int sdp_unzip_test_sha256_digest(const u8 *input, u32 ilen, u8 *out_digest)
{
...
static int sdp_unzip_test_sw_gzip(u8 *input, u32 ilen, u8 *output, u32 olen, int is_comp)
{
	struct crypto_comp *tfm;
	...
	if(is_comp) {
		u32 dlen = olen - 10 - 8;
		res = crypto_comp_compress(tfm, input, ilen, output + 10, &dlen);
		if(res < 0) {
			pr_err("crypto_comp_compress() return %d\n", res);
			return res;
		}
		...
		dlen += 10;
		memcpy(output+dlen, &gzip_crc32, 4);
		dlen += 4;
		memcpy(output+dlen, &ilen, 4);
		dlen += 4;
		...
		res = crypto_comp_decompress(tfm,  input + 10, ilen - 18, output, &dlen);
		decomp_len = dlen;


		memcpy(&comp_len, input + ilen-4, 4);
		if(comp_len != decomp_len) {
			pr_err("gzip decomp size error! gzip %d(0x%08x)bytes decomp %dbytes\n", comp_len, comp_len, decomp_len);
			res = -EINVAL;
			goto err;
		}
...
static int sdp_unzip_test_gzip_comp(u8 *input, u32 ilen, u8 *output, u32 olen)
{
	return sdp_unzip_test_sw_gzip(input, ilen, output, olen, 1);
}
static int sdp_unzip_test_gzip_decomp(u8 *input, u32 ilen, u8 *output, u32 olen)
{
	return sdp_unzip_test_sw_gzip(input, ilen, output, olen, 0);
}
...
static int aes_ctr128_crypt(const void *key, int key_len, const void *iv, const char *clear_text, char *cipher_text, size_t size)
{
	...
	//pr_info("aes_ctr128_crypt start\n");

	if(IS_ERR(tfm)) {
		printk("aes_ctr128_crypt: cannot allocate cipher ctr(aes)\n");
		rc = PTR_ERR(tfm);
	goto out;
	}
	...
	if(rc) {
		printk("aes_ctr128_crypt: cannot set key\n");
		goto out;
	}
	...
	if(rc < 0) {
		pr_err("aes_ctr128_crypt: encryption failed %d\n", rc);
		goto out;
	}
...
#define MMC_DEV_PATH	"/dev/mmcblk0p4"
	...
	for(i = 0; (thread->id < nr_thread) && (!N_LOOP || (i < N_LOOP)); i++) {
		flags = 0;
		case_res = PASS;
		err_injected = 0;
		snprintf(prefix, 30, "GZIPTEST%02d #%d", thread->id, i);
...
//		pr_info("%s: GZIP Compress start src size %d\n", src_size);
		...
		if(flags&GZIP_FLAG_ZLIB_FORMAT) {
			res = sdp_unzip_test_zlib_comp(src_buf, src_size, comp_buf, BUF_SIZE);

		} else {
			res = sdp_unzip_test_gzip_comp(src_buf, src_size, comp_buf, BUF_SIZE);
		}
		if(res <= 0) {
...
//		}
		...
		res = aes_ctr128_crypt(sample_key, 16, sample_iv, comp_buf, encrypt_buf, comp_padded_len);
		...
		res = sdp_unzip_test_sha256_digest(gzip_input, comp_padded_len, sha256_ref);
		if(res < 0) {
...
//		printk(KERN_INFO "GZIP SW Input data validate start\n");
		...
		if(auth.sha256_digest) {
			...
			sdp_unzip_test_sha256_digest(gzip_input, comp_padded_len, sha256_cal);
		...
		if(auth.aes_ctr_iv) {
			res = aes_ctr128_crypt(sample_key, 16, sample_iv, encrypt_buf, comp_buf, comp_padded_len);
			if(res < 0)  {
		...
		if(flags & GZIP_FLAG_ZLIB_FORMAT) {
			res = sdp_unzip_test_zlib_decomp(comp_buf, comp_len, decomp_buf, BUF_SIZE);
		} else {
			res = sdp_unzip_test_gzip_decomp(comp_buf, comp_len, decomp_buf, BUF_SIZE);
		}
		if(res < 0)  {
...
//		pr_info("%s: HW Decompress start\n", prefix);
		...
		if(mmc) {
			...
			sdp_unzip_decompress_async_with_auth(&buf, 0, decomp_pages, TO_ALIGNED(src_size, PAGE_SIZE)/PAGE_SIZE, NULL, NULL, true, flags, &auth);
...
#ifdef CONFIG_SDP_UNZIP_AUTH
		res = sdp_unzip_decompress_sync_with_auth(gzip_input, comp_padded_len, decomp_pages, TO_ALIGNED(src_size, PAGE_SIZE)/PAGE_SIZE, true, flags, &auth);
#else
		res = sdp_unzip_decompress_sync(comp_buf, comp_padded_len , decomp_pages, TO_ALIGNED(src_size, PAGE_SIZE)/PAGE_SIZE, true);
#endif
...
valid:
		...
		if(time_after_eq(jiffies, next_print)) {
			...
			pr_info("%s: test running... #%d, total src %llubytes, comp %llubytes, error#%d\n", prefix, i, total_src, total_comp, total_err);
		}
	}
...
#ifdef USE_GZIP_ZLIB
	thread->use_zlib = unziptest->use_zlib;
#endif
