...
static __cpuinit int golfap_install_powerup(unsigned int cpu)
{
	...
	mask = 0x1 << ((cpu - 1) * 4);
	val = readl(base + SDP1304_MISC_POWER_CTL) & ~mask;
	val |= mask;
	writel(val, base + SDP1304_MISC_POWER_CTL);
	val &= ~mask;
	udelay(1);
	writel(val, base + SDP1304_MISC_POWER_CTL);
	dmb();
	while(readl(base) & (1 << (cpu + 20)));	//wait for CPU power up
	iounmap(base);
...
static int golfap_install_powerdown(unsigned int cpu)
{
	...
	base = ioremap(SDP1304_MISC_BASE, 32);
	while(!(readl(base) & (1 << (cpu + 24))));	//wait for WFI enterance
	mask = 0x1 << ((cpu - 1) * 4 + 1);
	val = readl(base + SDP1304_MISC_POWER_CTL) & ~mask;
	val |= mask;
	writel(val, base + SDP1304_MISC_POWER_CTL);
	...
	writel(val, base + SDP1304_MISC_POWER_CTL);
	dmb();
	while(!(readl((void *) base) & (1 << (cpu + 20))));	//wait for CPU power down
	return 0;
}
...
static __cpuinit int golfap_install_warp(unsigned int cpu)
{
	...
	writel_relaxed(virt_to_phys(sdp_secondary_startup)
		, base + SDP1304_MISC_BOOTUP + cpu * 4);
	dmb();	
	udelay(1);	
	iounmap(base);
	return 0;
...
static int proc_read_sdp_kmeminfo(char *page, char **start, off_t off, int count, int *eof, void *bdata)
{
	int len;
...
void __init sdp1304_check_revision(void)
{
	
}


void __init sdp1202_map_io(void)
{
	printk("[%d] %s\n", __LINE__, __func__);
...
void __init sdp1304_map_io(void)
{
	printk("[%d] %s\n", __LINE__, __func__);
	iotable_init(sdp_io_desc, ARRAY_SIZE(sdp_io_desc));
//	sdp_chipid_init(SDP_VA_CHIPID);
...
void __init sdp1304_reserve(void)
{
	/* EHCI workaround */
