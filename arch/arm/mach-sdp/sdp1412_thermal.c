 ...
 *
 * SDP1412 - Thermal Management support
 *
 */
...
static int sdp1412_enable_tmu(struct sdp_tmu_info * info)
{
	u32 val;
	...
	val = readl((void *)((u32)info->tmu_base + TEM_REG1)) | TSC_TEM_T_EN;
	writel(val, (void *)((u32)info->tmu_base + TEM_REG1));
...
static u32 sdp1412_get_temp(struct sdp_tmu_info *info)
{
	int temp = 0;
	...
	temp = (readl((void *)((u32)info->tmu_base + TEM_REG2)) >> TSC_TEM_T_TS_8BIT_TS) & 0xFF;
...
static int sdp1412_cpu_hotplug_down(struct sdp_tmu_info * info)
{
...
static int sdp1412_cpu_hotplug_up(struct sdp_tmu_info * info)
{
#if 0//def CONFIG_HOTPLUG_CPU
	u32 cpu;
	struct device * dev;
...
/* init */
int sdp1412_tmu_init(struct sdp_tmu_info * info)
{
	info->enable_tmu = sdp1412_enable_tmu;
	info->get_temp = sdp1412_get_temp;
	info->cpu_hotplug_down = sdp1412_cpu_hotplug_down;
	info->cpu_hotplug_up = sdp1412_cpu_hotplug_up;
