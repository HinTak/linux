...
struct sdp_extint_t * sdp_get_extint(u32 phy_base)
{
...
{
	writel(0xFFFFFFFF, sdp_int->base + EXTINT_REG_STAT_CLEAR);	// all clear
}
...
static int _sdp_extint_enable(struct sdp_extint_t *sdp_int, int n_mpirq)
{
	...
	val = readl(sdp_int->base + EXTINT_REG_MASK);
	val &= ~(1UL << n_mpirq);
	writel(val, sdp_int->base + EXTINT_REG_MASK);	//enable
	writel(1 << n_mpirq, sdp_int->base + EXTINT_REG_STAT_CLEAR);	//clear
...
static int _sdp_extint_disable(struct sdp_extint_t *sdp_int, int n_mpirq)
{
	...
	val = readl(sdp_int->base + EXTINT_REG_MASK);
	val |= (1UL << n_mpirq);
	writel(val, sdp_int->base + EXTINT_REG_MASK);	//disable
	writel(1 << n_mpirq, sdp_int->base + EXTINT_REG_STAT_CLEAR);	//clear
...
int _sdp_extint_request_irq(struct sdp_extint_t *sdp_int, int n_mpirq, void (*fp)(void*), void* args)
{
	if(!sdp_int) {
...
static irqreturn_t sdp_extint_isr(int irq, void* dev)
{
	...
	status = readl(sdp_int->base + EXTINT_REG_MASKED_STAT);
	...
	writel(status, sdp_int->base + EXTINT_REG_STAT_CLEAR);
	return IRQ_HANDLED;
}
...
static int sdp_extint_probe(struct platform_device *pdev)
{
	...
	sdp_int->handler = devm_kzalloc(dev, sizeof(struct sdp_extint_isr_t) * sdp_int->nr_irqs, GFP_KERNEL);
	if (!sdp_int->handler) {
		dev_err(dev, "failed to allocate memory\n");
		return -ENOMEM;
	}
...
static int sdp_extint_suspend(struct device *dev)
{
	...
	sdp_int->backup = readl(sdp_int->base + EXTINT_REG_MASK);
...
static int sdp_extint_resume(struct device *dev)
{
	...
	writel(sdp_int->backup, sdp_int->base + EXTINT_REG_MASK);	//enable
	writel(0xFFFFFFFF, sdp_int->base + EXTINT_REG_STAT_CLEAR);	//clear
