 ...
 * 2016/01/13, jk97.son :	add sample_shift function to change sample_delay when response crc & error is gernerated
 */
...
#define DRIVER_MMCH_VER  "151229(add function for error)"
...
static int mmch_register_bus_dump(SDP_MMCH_T * sdpmmch)
{
	const u32 SDP_MMCH_MONITER_BASE_1 = 0x00fc0000;//(~0x00fc002f)
	const u32 SDP_MMCH_MONITER_BASE_2 = 0x00fb0000;//(~0x00fb002f)
	const u32 SDP_MMCH_MONITER_BASE_3 = 0x00348800;//(~0x0034889f)
	const u32 SDP_MMCH_MONITER_BASE_4 = 0x00a20000;//(~0x00a2009f)
	const u32 SDP_MMCH_MONITER_BASE_5 = 0x00a20200;//(~0x00a2029f)

	u32 idx=0;
	u32 reg_dump_1[0x40/4];
	u32 reg_dump_2[0x40/4];
	u32 reg_dump_3[0xa0/4];
	u32 reg_dump_4[0xa0/4];
	u32 reg_dump_5[0xa0/4];

	u8 * __iomem iomem;
	
	iomem = ioremap(SDP_MMCH_MONITER_BASE_1, ARRAY_SIZE(reg_dump_1));
	for(idx = 0; idx < ARRAY_SIZE(reg_dump_1); idx++) {
		reg_dump_1[idx] = mmch_readl(iomem + (idx*4));
	}
	iounmap(iomem);

	iomem = ioremap(SDP_MMCH_MONITER_BASE_2, ARRAY_SIZE(reg_dump_2));
	for(idx = 0; idx < ARRAY_SIZE(reg_dump_2); idx++) {
		reg_dump_2[idx] = mmch_readl(iomem + (idx*4));
	}
	iounmap(iomem);

	iomem = ioremap(SDP_MMCH_MONITER_BASE_3, ARRAY_SIZE(reg_dump_3));
	for(idx = 0; idx < ARRAY_SIZE(reg_dump_3); idx++) {
		reg_dump_3[idx] = mmch_readl(iomem + (idx*4));
	}
	iounmap(iomem);

	iomem = ioremap(SDP_MMCH_MONITER_BASE_4, ARRAY_SIZE(reg_dump_4));
	for(idx = 0; idx < ARRAY_SIZE(reg_dump_4); idx++) {
		reg_dump_4[idx] = mmch_readl(iomem + (idx*4));
	}
	iounmap(iomem);

	iomem = ioremap(SDP_MMCH_MONITER_BASE_5, ARRAY_SIZE(reg_dump_5));
	for(idx = 0; idx < ARRAY_SIZE(reg_dump_5); idx++) {
		reg_dump_5[idx] = mmch_readl(iomem + (idx*4));
	}
	iounmap(iomem);

	
	pr_info("====================SDP MMC Moniter DUMP================\n");
	for(idx = 0; idx < ARRAY_SIZE(reg_dump_1); idx+=4) {
		pr_info("0x%08llx: 0x%08x 0x%08x 0x%08x 0x%08x\n",
				((u64)SDP_MMCH_MONITER_BASE_1)+(idx*4),
				reg_dump_1[idx+0],
				reg_dump_1[idx+1],
				reg_dump_1[idx+2],
				reg_dump_1[idx+3]
	...
	pr_info("========================================================\n");
	for(idx = 0; idx < ARRAY_SIZE(reg_dump_2); idx+=4) {
		pr_info("0x%08llx: 0x%08x 0x%08x 0x%08x 0x%08x\n",
				((u64)SDP_MMCH_MONITER_BASE_2)+(idx*4),
				reg_dump_2[idx+0],
				reg_dump_2[idx+1],
				reg_dump_2[idx+2],
				reg_dump_2[idx+3]
	...
	pr_info("========================================================\n");
	for(idx = 0; idx < ARRAY_SIZE(reg_dump_3); idx+=4) {
		pr_info("0x%08llx: 0x%08x 0x%08x 0x%08x 0x%08x\n",
				((u64)SDP_MMCH_MONITER_BASE_3)+(idx*4),
				reg_dump_3[idx+0],
				reg_dump_3[idx+1],
				reg_dump_3[idx+2],
				reg_dump_3[idx+3]
	...
	pr_info("========================================================\n");
	for(idx = 0; idx < ARRAY_SIZE(reg_dump_4); idx+=4) {
		pr_info("0x%08llx: 0x%08x 0x%08x 0x%08x 0x%08x\n",
				((u64)SDP_MMCH_MONITER_BASE_4)+(idx*4),
				reg_dump_4[idx+0],
				reg_dump_4[idx+1],
				reg_dump_4[idx+2],
				reg_dump_4[idx+3]
	...
	pr_info("========================================================\n");
	for(idx = 0; idx < ARRAY_SIZE(reg_dump_5); idx+=4) {
		pr_info("0x%08llx: 0x%08x 0x%08x 0x%08x 0x%08x\n",
				((u64)SDP_MMCH_MONITER_BASE_5)+(idx*4),
				reg_dump_5[idx+0],
				reg_dump_5[idx+1],
				reg_dump_5[idx+2],
				reg_dump_5[idx+3]
...
int sdp_mmch_sample_shift(struct mmc_host *host, u32 opcode)
{
	...
	addr = ioremap_nocache(0x1000A8, sizeof(u32));		
	...
	for(j = 0; j < sample_count; j++) {
		...
		current_sample=mmch_readl(addr)&0x0000000F;
		fixed_sample=(current_sample+3)%4;
		mmch_writel((mmch_readl(addr)&0xFFFFFFF0)|fixed_sample, addr);	
		...
		if(!mrq.cmd->error && !mrq.data->error) {	
			...
			pr_info("sdp-mmc %2d pattern match!\n", fixed_sample);
			iounmap(addr);			
			mmc_release_host(host);
			return 0;
		} else {
			dev_printk(KERN_DEBUG, &sdpmmch->pdev->dev, "%2d pattern miss match!(req ret: CMD %d, DATA %d)\n", fixed_sample, mrq.cmd->error, mrq.data->error);
			pr_info("sdp-mmc %2d pattern miss match!(req ret: CMD %d, DATA %d)\n", fixed_sample, mrq.cmd->error, mrq.data->error);
		}
	}

	pr_info("=========================================================\n");
	pr_info("=============SDP MMC Register DUMP after shift===========\n");
			mmch_register_dump_notitle(sdpmmch);
	pr_info("=========================================================\n");
	...
	mmc_release_host(host);
	dev_printk(KERN_DEBUG, &sdpmmch->pdev->dev, "there is no matched sample value!\n");

	return -1;
}
EXPORT_SYMBOL(sdp_mmch_sample_shift);
...
#endif
	if(sdpmmch->host->successed_rdqs >= 0){
			dev_printk(KERN_DEBUG, &sdpmmch->pdev->dev, "rdqs_value was changed..\nprevious rdqs : %d\ncurrent rdqs : %d\n", 
						platdata->tuning_hs400_sam_default.list[1].value, sdpmmch->host->successed_rdqs);
			platdata->tuning_hs400_sam_default.list[1].value=(u32)sdpmmch->host->successed_rdqs;
	}//jkson 20150629
...
mmch_dbg_drv_tuning(void *data, u64 *val)
{
	...
	*val = sdp_mmch_drv_tuning(host, host->card, NULL);

	return 0;
}
*/
//jkson20150716 partition read&write
...
//jkson20151021 write on boot partition
...
static int mmch_dbg_strength_get(void *data, u64 *val)
{
	struct mmc_host *host = data;
	//SDP_MMCH_T *sdpmmch = mmc_priv(host);
...
static int mmch_dbg_strength_set(void *data, u64 val)
{
	...
	dev_info(&sdpmmch->pdev->dev, "strength settting done\n");
	iounmap(addr);
#else
...
DEFINE_SIMPLE_ATTRIBUTE(mmch_dbg_strength_fops, mmch_dbg_strength_get, mmch_dbg_strength_set,
	"%llu\n");
...
sdp_mmch_do_rdqs_tuning(struct mmc_host *host, struct mmc_card *card, int start, int end, int inc, struct seq_file *s)
{
	...
	for(cur_rdqs = start; cur_rdqs <= end; cur_rdqs += inc) {
		...
		/* check data error */
		ret = mmc_send_ext_csd(card, extcsd);
		if(!ret) {
...
sdp_mmch_execute_hs400_rdqs_tuning(struct mmc_host *host, struct mmc_card *card)
{
	...
	if(ret < 0) {
		...
		host->successed_rdqs=(ret<<2); //jkson 20150629
		sdpmmch->rdqs_tune = MMCH_RDQS_TUNE_DONE;
	}

	err = sdp_mmch_sample_shift(host, MMC_READ_SINGLE_BLOCK);
	if (err < 0) {
		dev_err(mmc_dev(host), "sdp_mmch_sample_shift, err : %d\n", err);
	}
...
static void mmch_add_host_debugfs(struct mmc_host *host)
{
	...
	if (!debugfs_create_file("strength", S_IRUSR, root, host,&mmch_dbg_strength_fops))
	{
