...
struct ehci_sdp_platdata sdp_ehci0_pdata = {
	.swreset_addr = 0x100908f4,
	.swreset_mask = 0x1000,
};

struct ehci_sdp_platdata sdp_ehci1_pdata = {
	.swreset_addr = 0x100908f4,
	.swreset_mask = 0x2000,
};
...
{
	/* SRAM select*/
	writel(1, (u32*)(VA_IO_BASE0+0x00090CC0));
	return  0;
}
...
static int sdp1207_spi_init(void)
{
	...
	/* No MSPI */
	writel(readl(base+OFF_MSPI_TX_CTRL)|MSPI_TX_CTRL_SELSPI,
			(u32*)(base+OFF_MSPI_TX_CTRL));

	/* Pad SPI Selection */
	writel(readl((u32*)REG_PAD_CTRL_10)|PAD_CTRL_10_SPI_SEL,
			(u32*)REG_PAD_CTRL_10);
...
static int __init sdp1207_l2c_init(void)
{
...
/* now we start to use CONFIG_COMMON_CLK */
...
static unsigned long sdp1207_recalc_fclk(unsigned long parent_rate)
{
       u32 pms = R_PMU_CPU_CON;
       return (unsigned long)sdp1207_calc_cpu_pms(pms);
}

int __init sdp1207_init_clocks(void)
{
	struct clk *pclk;
	u32 fclk_rate, dclk_rate, dspclk_rate;
	u32 pms_core = R_PMU_CPU_CON;
	u32 pms_ddr = R_PMU_DDR_CON;
	u32 pms_dsp = R_PMU_DSP_CON;

	fclk_rate	= sdp1207_calc_cpu_pms(pms_core);
	dclk_rate	= sdp1207_calc_ddr_pmsk(pms_ddr);
	dspclk_rate	= sdp1207_calc_dsp_pms(pms_dsp);

	clk_register_sdp_scalable_clk(NULL, "fclk", NULL, sdp1207_recalc_fclk);
	clk_register_fixed_rate(NULL, "dclk", NULL, CLK_IS_ROOT, dclk_rate);
	clk_register_fixed_rate(NULL, "dspclk", NULL, CLK_IS_ROOT, dspclk_rate);

	clk_register_fixed_rate(NULL, "busclk", "dspclk", 0, dspclk_rate / 6);
	clk_register_fixed_rate(NULL, "hclk", "busclk", 0, dspclk_rate / 12);

	pclk = clk_register_fixed_rate(NULL, "pclk", "busclk", 0, dspclk_rate / 12);
	
	/* XXX: add more clkdevs here, or use lookup table instead? */
...
void __init sdp1207_init_early(void)
{
	sdp1207_patch();
}
