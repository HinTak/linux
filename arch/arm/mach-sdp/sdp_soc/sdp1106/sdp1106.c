...
/* plat-sdp/sdp_hwmem.c uses this */
...
static struct usb_ehci_pdata sdp_ehci0_platdata;
static struct usb_ehci_pdata sdp_ehci1_platdata;
static struct usb_ohci_pdata sdp_ohci0_platdata;
static struct usb_ohci_pdata sdp_ohci1_platdata;
...
static struct platform_device sdp_ehci0 = {
		...
		.platform_data		= &sdp_ehci0_platdata,
        },
...
static struct platform_device sdp_ehci1 = {
        ...
        .dev = {
                ...
                .coherent_dma_mask      = 0xFFFFFFFFUL,
		.platform_data		= &sdp_ehci1_platdata,
        },
...
static struct platform_device sdp_ohci0 = {
        ...
        .dev = {
                ...
                .coherent_dma_mask      = 0xFFFFFFFFUL,
		.platform_data		= &sdp_ohci0_platdata,
        },
...
static struct platform_device sdp_ohci1 = {
        ...
        .dev = {
                ...
                .coherent_dma_mask      = 0xFFFFFFFFUL,
		.platform_data		= &sdp_ohci1_platdata,
        },
...
static int sdp1106_mmch_init(SDP_MMCH_T *p_sdp_mmch)
{
	...
	{
		/* save clk delay */
		p_sdp_mmch->pm_clk_delay[0] = readl(VA_SFR0_BASE + 0x00090934);
		p_sdp_mmch->pm_is_valid_clk_delay = true;
	}
	else
	{
		/* restore clk delay */
		writel(p_sdp_mmch->pm_clk_delay[0], VA_SFR0_BASE + 0x00090934);
	}
	return 0;
}
...
#define REG_MSPI_TX_CTRL	((void __iomem *)0x30090268)// set 0x1000 for sel SPI
...
*/
	/* CDMAARUSER0 */
	writel(0x07050301, VA_SFR0_BASE + 0x00b70040);
	/* CDMAARUSER1 */
	writel(0x0f0d0b09, VA_SFR0_BASE + 0x00B70044);

	/* CDMAAWUSER0 */
	writel(0x07050301, VA_SFR0_BASE + 0x00B70050);
	/* CDMAAWUSER2 */
	writel(0x0f0d0b09, VA_SFR0_BASE + 0x00B70058);

	/* DMAMODE */
	writel(0x00000101, VA_SFR0_BASE + 0x00B7004c);

	//dma330 reset for dma menager thread Secure state
	val = readl(VA_SFR0_BASE + 0x00b70034);
	writel(val&~(1<<13), VA_SFR0_BASE + 0x00b70034);
	writel(val|(1<<13), VA_SFR0_BASE + 0x00b70034);
...
static int __init sdp1106_l2c_init(void)
{
...
static int __init sdp1106_l2c_init(void)
{
	pr_info ("L2x0 off kernel.\n");
	return 0;
}
#endif

static void __init sdp1106_patch(void)
{
...
void __init sdp1106_init_early(void)
{
	sdp1106_patch();
}
...
early_initcall(sdp1106_l2c_init);
