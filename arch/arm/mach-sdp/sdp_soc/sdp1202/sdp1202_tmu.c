/* linux/arch/arm/mach-sdp/sdp_soc/sdp1202/sdp1202_tmu.c
*
...
static unsigned int get_curr_temp(struct sdp_tmu_info *info)
{
	...
	temp = (__raw_readl(info->tmu_base + SDP1202_TSC_CONTROL5) >> TSC_TEM_T_TS_8BIT_TS) & 0xFF;
...
static ssize_t show_lot_id(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	void *base = (void *)(VA_SFR0_BASE + 0x80000);
	int diff;
...
static int sdp1202_tmu_init(struct sdp_tmu_info *info)
{
	...
	tmp = __raw_readl(info->tmu_base + SDP1202_TSC_CONTROL4);
	tmp |= (1 << TSC_TEM_T_EN);
	__raw_writel(tmp, info->tmu_base + SDP1202_TSC_CONTROL4);
...
static int sdp_tmu_probe(struct platform_device *pdev)
{
	...
	INIT_DEFERRABLE_WORK(&info->polling, sdp_handler_tmu_state);
