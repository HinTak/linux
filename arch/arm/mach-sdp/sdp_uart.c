...
#define CONFIG_SERIAL_SDP_UARTS	4
...
#if 0	/* not used functions, disabled for avoid compile warnings */
static void sdp_serial_rx_enable(struct uart_port *port)
{
	unsigned long flags;
	...
	while (--count && !sdp_serial_txempty_nofifo(port))
		udelay(100);

	ufcon = rd_regl(port, SDP_UFCON);
	ufcon |= SDP_UFCON_RESETRX;
	wr_regl(port, SDP_UFCON, ufcon);

	ucon = rd_regl(port, SDP_UCON);
	ucon |= SDP_UCON_RXIRQMODE;
	wr_regl(port, SDP_UCON, ucon);

	spin_unlock_irqrestore(&port->lock, flags);
}

static void sdp_serial_rx_disable(struct uart_port *port)
{
	unsigned long flags;
	unsigned int ucon;
...
static int sdp_serial_startup(struct uart_port *port)
{
	...
	if(cpu_online((u32) ourport->irq_affinity)) {
