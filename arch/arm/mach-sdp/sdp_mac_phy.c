/*
 * arch/arm/mach-sdp/sdp_mac_phy.c
 *
...
//#define SDP_MAC_PHY_VER		"20150729(Support EPHY WOL)"
//#define SDP_MAC_PHY_VER		"20151102(Support EPHY AnyPacket WOL)"
//#define SDP_MAC_PHY_VER		"20150107(fix EPHY DSP Register init value for Jazz)"
//#define SDP_MAC_PHY_VER		"20150111(Change sw mdix switch code, add debug log)"
#define SDP_MAC_PHY_VER		"20160307(bugfix, Jazz-L rev0 WOL is not work)"
...
static int ephy_set_wol(struct phy_device *phydev, struct ethtool_wolinfo *wol) {
	...
	if (wol->wolopts & WAKE_MAGIC) {
		...
		ephy_wolreg_write(phydev, 0x3, 0x7);

		/* WOL INT UnMask */
		phy_write(phydev, 30, phy_read(phydev, 30) | 0x3<<9);

		dev_info(&phydev->dev, "%s: WOL On(%pM)\n", __FUNCTION__, dev_addr);
	} else {
		/* WOL INT Mask */
...
static ssize_t ephy_dbg_anypacket_wol_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	while(token) {
		...
		} else if(strnicmp("src-match", opt, strlen(opt)) == 0) {
		...
		} else if(strnicmp("dst-match", opt, strlen(opt)) == 0) {
		...
		} else if(strnicmp("bypass-crc", opt, strlen(opt)) == 0) {
		...
		} else if(strnicmp("src-offset", opt, strlen(opt)) == 0) {
		...
		} else if(strnicmp("dst-offset", opt, strlen(opt)) == 0) {
		...
		} else if(strnicmp("src-length", opt, strlen(opt)) == 0) {
		...
		} else if(strnicmp("dst-length", opt, strlen(opt)) == 0) {
		...
		} else if(strnicmp("src-ipaddr", opt, strlen(opt)) == 0) {
			const char *end;
		...
		} else if(strnicmp("dst-ipaddr", opt, strlen(opt)) == 0) {
			const char *end;
...
static int common_phy_resume(struct phy_device *phydev)
{
	...
	PM_DEV_DBG(&phydev->dev, "%s: call phy_scan_fixups\n", __FUNCTION__);
	phy_scan_fixups(phydev);

	if(phydev->drv->config_init) {
		PM_DEV_DBG(&phydev->dev, "%s: call config_init\n", __FUNCTION__);
