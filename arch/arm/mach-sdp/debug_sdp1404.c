...
static void restore_dbgecr(int cpu)
{
	...
	for (i = 0; i < CORE_CNT; i++)
		reg_ECR |= readl(dbg_regs[i + CORE_CNT * CA15] + OFFSET_ECR) & 0x1;
...
{
	writel(UNLOCK_MAGIC, dbg_regs[core + (cluster * CORE_CNT)] + OFFSET_LAR);
}
...
static int __init armdebug_sdp1404_init(void)
{
	int cluster_boot = 0;
	int i;
	int ret;
...
//		cluster_boot = !(read_cpuid(CPUID_MPIDR) >> 8 & 0xf);
	...
	for (i = 0; i < CORE_CNT * CLUSTER_CNT; i++)
		dbg_regs[i] = ioremap(
				PHYS_OFFSET - (SZ_4K*(CORE_CNT*CLUSTER_CNT+1)) + (SZ_4K * i),
				SZ_4K);
...
subsys_initcall(armdebug_sdp1404_init);
