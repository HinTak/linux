...
#elif defined(CONFIG_ARCH_SDP1501)
		if( soc_is_sdp1501() || soc_is_sdp1511() ) //Jazz-MU / Jazz-MT
			ret_val = of_property_read_u32_array(np_mem, "samsung,align3", data, 3);		
		else if ( soc_is_sdp1521() || soc_is_sdp1531() ) //Jazz-ML / Jazz-L
			ret_val = of_property_read_u32_array(np_mem, "samsung,pvr", data, 3);
#else
		PRINT_HWMEM_ERR("ERROR: None of HAWKM_UHD_1.5 or HAWKM_UHD_2.5 SOC chosen for DDR Check test\n");
		ret = -ENOMEM;
...
#ifdef CONFIG_ARCH_SDP1501
		if( soc_is_sdp1501() || soc_is_sdp1511() ) //Jazz-MU / Jazz-MT
			ret_val = of_property_read_u32_array(np_mem, "samsung,align5", data, 3);
		else if ( soc_is_sdp1521() || soc_is_sdp1531() ) //Jazz-ML / Jazz-L
