...
#define GMAC_DRV_VERSION	"v1.35"
#define GMAC_DRV_VERSION_DESC	"increase budget when rx buffer is unavailable"
...
static inline u32 sdp_mac_phyreadl(phys_addr_t phyaddr)
{
	...
	addr = ioremap_nocache(phyaddr, sizeof(u32));
...
static void
sdp_mac_ephy_config_dump(SDP_GMAC_DEV_T *pGmacDev)
{
	int i = 0;
	u32 value;
	...
	for(i = 0; i < ARRAY_SIZE(dump_info); i++) {
		value = sdp_mac_phyreadl((phys_addr_t)dump_info[i][1]);
		dev_err(pGmacDev->pDev, "%s: 0x%p[%u] == %u(raw 0x%08x)\n",
			(const char *)dump_info[i][0], dump_info[i][1], (u32)dump_info[i][2],
			(value >> (u32)dump_info[i][2])&0x1, value);
	}
}
