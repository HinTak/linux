...
#if defined(CONFIG_MMC_SDHCI_NVT) && !defined(CONFIG_VD_RELEASE)
extern void dump_nvt_mmc_reg(void);
#endif
...
void show_info(struct task_struct *task, struct pt_regs *regs, unsigned long addr)
{
	...
	if (i == ALLOWED_TASK_NUM) {
		...
		printk("[show_info] It's not show_info() case for Process Name[%s], Thread Name[%s], PC = 0x%08lx, LR = 0x%08lx .\n", task->group_leader->comm, current->comm, regs->ARM_pc, regs->ARM_lr);
		return;
	...
	sep_printk_end();
#endif
	console_default();	/* VDLinux patch : revert to console loglevel 15 -> 7 */
}
