...
void __show_user_stack(struct task_struct *task, unsigned long sp)
{
	...
	if (!vma) {
		pr_alert("pid(%d) : printing user stack failed.\n", (int)task->pid);
		return;
	}

	if (sp < vma->vm_start) {
		pr_alert("pid(%d) : seems stack overflow.\n"
				 "  sp(0x%08lx), stack vma (0x%08lx ~ 0x%08lx)\n",
				 (int)task->pid, sp, vma->vm_start, vma->vm_end);
		return;
	}

	pr_alert("pid(%d) stack vma (0x%08lx ~ 0x%08lx)\n",
	...
	if (task->user_ssp > sp_end)
		dump_mem(KERN_ALERT, "User Stack: ", sp, sp_end);
	else
		dump_mem(KERN_ALERT, "User Stack: ", sp, task->user_ssp);
}
...
void __show_user_stack_tg(struct task_struct *task)
{
	...
	pr_alert("--------------------------------------------------------\n");
	pr_alert("* dump all user stack of pid(%d) thread group\n", (int)task->pid);
	pr_alert("--------------------------------------------------------\n");
...
void show_user_stack(struct task_struct *task, struct pt_regs *regs)
{
	...
	vma = find_vma(task->mm, task->user_ssp);
	if (vma) {
		pr_alert("task stack info : pid(%d) stack area (0x%08lx ~ 0x%08lx)\n",
			         (int)task->pid, vma->vm_start, vma->vm_end);
	}

	pr_alert("-----------------------------------------------------------\n");
	pr_alert("* dump user stack\n");
	pr_alert("-----------------------------------------------------------\n");
...
void show_pc_lr(struct task_struct *task, struct pt_regs *regs)
{
	...
	pr_alert("\nLast Exception point\n");
	pr_alert("--------------------------------------------------------------------------------------\n");
	pr_alert("PC, LR MEMINFO\n");
	pr_alert("--------------------------------------------------------------------------------------\n");
	pr_alert("PC:%lx, LR:%lx\n", regs->ARM_pc, regs->ARM_lr);
	...
	if (vma && (regs->ARM_pc >= vma->vm_start))
		dump_mem(KERN_ALERT, "PC meminfo ", addr_pc_start, addr_pc_end);
	else
		pr_alert("No VMA for ADDR PC\n");

	pr_alert("--------------------------------------------------------------------------------------\n");
	vma = find_vma(task->mm, regs->ARM_lr);

	if (vma && (regs->ARM_lr >= vma->vm_start))
		dump_mem(KERN_ALERT, "LR meminfo ", addr_lr_start, addr_lr_end);
	else
		pr_alert("No VMA for ADDR LR\n");
...
void show_register_memory_kernel(struct pt_regs * regs)
{
	...
	for (register_num = 0; register_num <16 /* from r0 to r15 */; register_num++) {
		printk("\n\n* REGISTER : r%d\n",register_num);
...
void dump_backtrace_entry(unsigned long where, unsigned long from, unsigned long frame)
{
	...
	if (frame < irq_stack_p && (frame + sizeof(struct pt_regs)) > irq_stack_p)
		frame = IRQ_STACK_TO_TASK_STACK(irq_stack_base_p);
#endif

#ifdef CONFIG_KALLSYMS
	printk("[<%08lx>] (%ps) from [<%08lx>] (%pS)\n", where, (void *)where, from, (void *)from);
#else
	pr_err("Function entered at [<%08lx>] from [<%08lx>]\n", where, from);
#endif
...
/*
 * Stack pointers should always be within the vmalloc area
 * or irq_stack array area if CONFIG_VMAP_STACK is enabled.
 * If it is not there, then we can't dump out any information
 * relating to the stack.
 */
static int verify_stack(unsigned long sp)
{
	...
	if (sp > (unsigned long)irq_stack &&
	    sp < (((unsigned long)irq_stack) + sizeof(irq_stack)))
		return 0;
...
static void dump_backtrace(struct pt_regs *regs, struct task_struct *tsk)
{
	...
	if (!fp) {
		...
		ok = 0;
#ifdef CONFIG_DUMP_RANGE_BASED_ON_REGISTER
                dump_mem(KERN_EMERG, "Print full Stack: ", (unsigned long)task_stack_page(tsk),
...
static inline void dump_mem_svc(struct pt_regs *regs, struct task_struct *tsk)
{
...
#ifdef CONFIG_IRQ_STACK
static inline void dump_mem_irq(struct pt_regs *regs, struct task_struct *tsk)
{
	unsigned long irq_stack_base_p;

	irq_stack_base_p = IRQ_STACK_BASE_PTR(raw_smp_processor_id());

	if (regs->ARM_sp > irq_stack_base_p) {
		dump_mem(KERN_EMERG, "Stack: ", regs->ARM_sp,
				THREAD_SIZE + irq_stack_base_p);
	} else {
		pr_alert("[VDLP] stack dump range change!!\n");
		pr_alert("[VDLP] regs->ARM_sp(0x%lx) -> task->irq_stack(0x%lx)!!\n",
				regs->ARM_sp, irq_stack_base_p);
		dump_mem(KERN_EMERG, "Stack: ", irq_stack_base_p,
				THREAD_SIZE + irq_stack_base_p);
	}
}
#else
...
void die(const char *str, struct pt_regs *regs, int err)
{
	...
	/* Oops - =>  Oops - bad syscall, Oops - bad syscall(2), Oops - undefined instruction exclude BUG_ON */
	if (!strcmp(str1, "Oops - bad syscall(2)") ||
                (!strcmp(str1, "Oops - undefined instruction") && bug_type != BUG_TRAP_TYPE_BUG) ||
...
/**
 * do_stackoverflow - stack overflow handler
 *
 * Allocate new page and map it to satck guard page and return
 * for normal execution.
 *
 * die on any issue while handling overflow.
 */
#ifdef CONFIG_KERNEL_STACK_SMALL
asmlinkage void __exception do_stackoverflow(struct pt_regs *regs, unsigned long far, unsigned int fsr)
{
	int err, old_lvl;
	struct page *page;
	unsigned long addr = (unsigned long)current->stack;
	bool print = check_printk_lock();

	/*
	 * check if overflow is beyond 2 PAGES (8 KB).
	 */
	if (regs->ARM_sp < addr)
		goto error;
	...
	old_lvl = console_loglevel;
	console_verbose();
	set_tsk_thread_flag(current, TIF_VMAP_STACK_ALLOC);
	if (print)
		pr_alert("[STACK EXTENDED] kernel stack for Comm: %s PID: %d sp: %p FAR: %lx FSR: %x\n",
					current->comm, current->pid, (void *)regs->ARM_sp, far, fsr);

	page = alloc_page(GFP_ATOMIC | __GFP_HIGHMEM | __GFP_MEMALLOC | __GFP_NO_KSWAPD);
	if (!page)
		goto error;

	err = map_kernel_range_noflush(addr, PAGE_SIZE, PAGE_KERNEL, &page);
	if (err < 0)
		goto error;

	flush_cache_vmap(addr, addr + PAGE_SIZE);
	account_kernel_stack(current, 1, page);
	if (print)
		pr_alert("[STACK EXTENDED] Address of page allcoated: %p stack mapped: %p\n",
					page, (void *)addr);

	*(unsigned long *)addr = (unsigned long)page;
	set_tsk_thread_flag(current, TIF_VM_STACK);
	clear_tsk_thread_flag(current, TIF_VMAP_STACK_ALLOC);
	console_revert(old_lvl);
	return;

error:
	die("Kernel stack overflow", regs, 0);
}
#endif /* CONFIG_KERNEL_STACK_SMALL */
...
#endif
	if (user_mode(regs) && !do_instn_recovery(regs))
		return;
...
#ifdef CONFIG_NVT_ARM_ERRATUM_858921
static int get_cntvct_trap(struct pt_regs *regs, unsigned int instr)
{
	int reg_high = (instr >> 16) & 15;
	int reg_low = (instr >> 12) & 15;
	u64 ts;

	if ((reg_high == 15) || (reg_low == 15) || (reg_high == reg_low))
		return 1;

	ts = arch_counter_get_cntvct();

	regs->uregs[reg_high] = (u32)((ts >> 32) & 0xffffffff);
	regs->uregs[reg_low] = (u32)(ts & 0xffffffff);
...
static struct undef_hook arm_cntvct_hook = {
...
static int __init arm_cntvct_hook_init(void)
{
	register_undef_hook(&arm_cntvct_hook);
	return 0;
}

late_initcall(arm_cntvct_hook_init);
