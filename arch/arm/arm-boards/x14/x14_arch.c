...
/*------------------------------------------------------------------------------
	...
	FILE NAME: arch/arm/arm-boards/x14/x14_arch.c
...
-------------------------------------------------------------------------------*/
/* initialize x14's IRQ */
extern void x14_init_irq(void);
/* Add SD Card driver */
extern void __init x14_add_device_mmc(short mmc_id);
extern void __init x14_add_device_memstick(s16 memstick_id);
//extern int Mstar_ehc_platform_init(void);
/*------------------------------------------------------------------------------
    Local Function Prototypes
-------------------------------------------------------------------------------*/
static void __init x14_map_io(void);
static void __init serial_init(void);
static void __init boot_clear(void);
static void __init x14_l2c_init(void);
...
-------------------------------------------------------------------------------*/
/* x14's system timer */
extern struct sys_timer x14_timer;
extern struct sys_timer x14_ptimer;
extern struct smp_operations x14_smp_ops;
/*------------------------------------------------------------------------------
    Global Variable
-------------------------------------------------------------------------------*/
/* platform device for x14_board_init() */
struct platform_device x14_device_usbgadget =
{
	.name		  = "x14-usbgadget",
	.id		  = -1,
};
...
static struct platform_device x14_device_pmu =
{
        .name      = "arm-pmu",
        .id        = -1,
...
/* platform device array for x14_board_init() */
static struct platform_device *x14_devices[] __initdata =
{
	&x14_device_usbgadget,
	#if defined(CONFIG_OPROFILE)
	&x14_device_pmu,
	#endif
};
...
static struct map_desc x14_io_desc[] __initdata =
{
...
/*------------------------------------------------------------------------------
    ...
    Remark:
        None.
-------------------------------------------------------------------------------*/
void __init x14_map_io(void)
{
    iotable_init(x14_io_desc, ARRAY_SIZE(x14_io_desc) );
	serial_init();
    boot_clear();
...
void __init boot_clear(void)
{
	*(volatile unsigned short *)(0xfd201208) = 0x4004 ;  // Src: Fill / Dst: Miu0
	*(volatile unsigned short *)(0xfd201218) = 0x1000 ;  //addr low
	*(volatile unsigned short *)(0xfd20121C) = 0x2000 ;  //addr hi  >> onboot addr: 0x20001000
	*(volatile unsigned short *)(0xfd201220) = 0x0000 ;  //size low
	*(volatile unsigned short *)(0xfd201224) = 0x0002 ;  //size hi  >> 128K

	*(volatile unsigned short *)(0xfd201228) = 0x0000 ;  //fill pattern lo
	*(volatile unsigned short *)(0xfd20122C) = 0x0000 ;  //fill pattern hi

	*(volatile unsigned short *)(0xfd201200) = 0x0001 ;  //Fire
}
...
#ifdef CONFIG_MSTAR_X14_BD_FPGA
    uart0.uartclk = 12000000; //FPGA
#elif  CONFIG_MSTAR_X14_BD_GENERIC  
...
static irqreturn_t x14_xiu_timeout_interrupt(int irq, void *dev_id)
{
...
static struct irqaction x14_xiu_timeout_irq = {
    .name = "X14 Timeout IRQ",
    .flags = IRQF_TIMER | IRQF_IRQPOLL | IRQF_DISABLED,
    .handler = x14_xiu_timeout_interrupt,

};

void __init x14_l2c_init(void)
{
#ifdef CONFIG_CACHE_L2X0
...
/*------------------------------------------------------------------------------
    Function: x14_board_init

    Description:
        x14 board init function
    Input: (The arguments were used by caller to input data.)
        None.
    Output: (The arguments were used by caller to receive data.)
        None.
    ...
    Remark:
        None.
-------------------------------------------------------------------------------*/
static void __init x14_board_init(void)
{

	platform_add_devices(x14_devices, ARRAY_SIZE(x14_devices));

#if defined(CONFIG_USB_EHCI_HCD) || defined(CONFIG_USB_EHCI_HCD_MODULE)
	Mstar_ehc_platform_init();
#endif

#if defined(CONFIG_MEMSTICK_X14_MS)
	x14_add_device_memstick(0);
#endif
	...
	*(volatile unsigned short *)(0xfd200254) = 0xFFFF ;
    ...
    setup_irq(E_FIQ_XIU_TIMEOUT , &x14_xiu_timeout_irq);
}
...
inline void _chip_flush_miu_pipe(void)
{
	...
	*(volatile unsigned short *)(0xfd203114) &= ((unsigned short)(~(0x0001)));
...
static void __init x14_reserve(void)
{
...
MACHINE_START(X14, "x14")
//#endif
//    .atag_offset    = PHYS_OFFSET + 0x00000100,
    .atag_offset    = 0x00000100,
    .map_io = x14_map_io,
    .init_irq = x14_init_irq,
    .init_machine = x14_board_init,
    .handle_irq = gic_handle_irq,
    .smp = smp_ops(x14_smp_ops),
