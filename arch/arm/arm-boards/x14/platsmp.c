...
void __init x14_smp_init_cpus(void);
void __init x14_smp_prepare_cpus(unsigned int max_cpus);
void __cpuinit x14_secondary_init(unsigned int cpu);
int __cpuinit x14_boot_secondary(unsigned int cpu, struct task_struct *idle);
#if defined(CONFIG_MP_MSTAR_STR_BASE)
void __cpuinit platform_smp_boot_secondary_init(void);
#endif
...
struct smp_operations __initdata x14_smp_ops = {
	.smp_init_cpus		= x14_smp_init_cpus,
	.smp_prepare_cpus	= x14_smp_prepare_cpus,
	.smp_secondary_init	= x14_secondary_init,
	.smp_boot_secondary	= x14_boot_secondary,
#ifdef CONFIG_HOTPLUG_CPU
	.cpu_kill			= smp_platform_cpu_kill,
	.cpu_die			= smp_platform_cpu_die,
	.cpu_disable		= smp_platform_cpu_disable,
#endif
};
