...
-------------------------------------------------------------------------------*/
void x14_irq_ack(unsigned int irq);
void x14_irq_mask(unsigned int irq);
void x14_irq_unmask(unsigned int irq);
void __init x14_init_irq(void);

/* Clear FIQ (Clear is not supported for IRQ) */
void x14_irq_ack(unsigned int irq) {
    unsigned short tmp;
//printk(KERN_WARNING "x14_irq_ack(irq=%d)\n",irq);
        if(irq <16)
	{
...
void x14_irq_mask(unsigned int irq) {
    unsigned short tmp;
//printk(KERN_WARNING "x14_irq_mask(irq=%d)\n",irq);
...
void x14_irq_unmask(unsigned int irq) {
   unsigned short tmp;
    ...
    //printk(KERN_WARNING "x14_irq_unmask(irq=%d)\n",irq);
...
extern void arm_interrupt_chain_setup(unsigned int chain_num);
