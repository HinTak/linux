...
-------------------------------------------------------------------------------*/
void __init x14_clocksource_init(unsigned int base);
void __init x14_clockevents_init(unsigned int base,unsigned int irq);
void __init x14_init_timer(void);
...
#if defined(CONFIG_MSTAR_X14_FPGA)
#define CLOCK_TICK_RATE         (12*1000*1000)
#endif


#ifndef CONFIG_GENERIC_CLOCKEVENTS
static irqreturn_t x14_timer_interrupt(int irq, void *dev_id)
{
    ...
    //stop timer
    CLRREG16(X14_BASE_REG_TIMER0_PA, TIMER_TRIG);
    //set interval
    //interval = (CLOCK_TICK_RATE / HZ);
    OUTREG16(X14_BASE_REG_TIMER0_PA + ADDR_TIMER_MAX_LOW, (interval &0xffff));
    OUTREG16(X14_BASE_REG_TIMER0_PA + ADDR_TIMER_MAX_HIGH, (interval >>16));

    //trig timer0
    SETREG16(X14_BASE_REG_TIMER0_PA, TIMER_TRIG);

    return IRQ_HANDLED;
}

static struct irqaction x14_timer_irq = {
    .name = "X14 Timer Tick",
    .flags = IRQF_TIMER | IRQF_IRQPOLL | IRQF_DISABLED,
    .handler = x14_timer_interrupt,
};
#endif
...
void __init x14_clocksource_init(unsigned int base)
{
	...
	cs->mult = clocksource_hz2mult((u32)(GLB_TIMER_FREQ_HZ), cs->shift);//PERICLK = CPUCLK/2
#else	
	...
	//calculate the value of mult    //cycle= ( time(ns) *mult ) >> shift
	cs->mult = clocksource_hz2mult((u32)(GLB_TIMER_FREQ_HZ), cs->shift);  //Mstar timer =>12Mhz, 
#endif                  
...
static DEFINE_TWD_LOCAL_TIMER(twd_local_timer, X14_MPCORE_TWD, IRQ_LOCALTIMER);

static void __init x14_twd_init(void)
{
...
#else
#define x14_twd_init()  do {} while(0)
#endif

static int __init x14_init_timer_clocks(void)
{
    struct clk *fclk;
...
void __init x14_init_timer(void)
{
	...
	printk("CPU Clock Frequency = %d MHz\n", GLB_TIMER_FREQ_HZ / 1000 / 1000 * 2);
	
	x14_init_timer_clocks();
	...
	x14_clocksource_init(X14_BASE_REG_TIMER1_PA);
	x14_clockevents_init(X14_BASE_REG_TIMER0_PA,E_FIQ_EXTIMER0);

	x14_twd_init();
}

struct sys_timer x14_timer = {
    .init = x14_init_timer,
};
