...
void arm_interrupt_chain_setup(unsigned int chain_num);
...
static void arm_ack_irq(struct irq_data *d)
{
 ...
 x14_irq_ack( d->irq - VIRTUAL_FIQ_START);
...
static void arm_mask_irq(struct irq_data *d)
{
    ...
    x14_irq_mask( d->irq - VIRTUAL_FIQ_START);
...
static void arm_unmask_irq(struct irq_data *d)
{
    //handle MSTAR IRQ controler 
    spin_lock(&irq_controller_lock);
    x14_irq_unmask( d->irq - VIRTUAL_FIQ_START);
    spin_unlock(&irq_controller_lock);
