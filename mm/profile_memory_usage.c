...
static unsigned long get_kernel_mem(bool print)
{
	 ...
	 * Unreclaimable Slab + PageTable + Vmalloc(except ioremap) +
	 * Kernel Stack
	 */
	...
	if (print) {
		...
		printk(KERN_INFO " Vmalloc(except ioremap):  %5lu  kbyte\n",
			    ...
			    struct smap_mem *rss)
{
	...
	while (vma) {
		...
		if (vma->vm_mm && !(vma->vm_flags & VM_HUGETLB)) {
			...
			/* We need to release this spin_lock otherwise this
			 * leads to scheduling while atomic bugs by virtue of
			 * walk_page_range() -> smaps_pte_range() ->
			 * _cond_resched() */
			task_unlock(p);
...
static unsigned long get_pss_from_tasks(bool print)
{
	...
	if (print && toggle) {
		printk(KERN_INFO "===============================\n");
		printk(KERN_INFO "      Current Memory Usage\n");
		printk(KERN_INFO "===============================\n");
	}
	...
	do_each_thread(g, p) {
		...
		if (print && toggle) {
			...
			printk(KERN_INFO "              Pss    Rss\n");
			printk(KERN_INFO " Total   :  %5lu  %5lu  kbyte\n",
			       pss.total, rss.total);
			printk(KERN_INFO " Code    :  %5lu  %5lu  kbyte\n",
			       pss.vmag[0], rss.vmag[0]);
			printk(KERN_INFO " Data    :  %5lu  %5lu  kbyte\n",
			       pss.vmag[1], rss.vmag[1]);
			printk(KERN_INFO " LibCode :  %5lu  %5lu  kbyte\n",
			       pss.vmag[2], rss.vmag[2]);
			printk(KERN_INFO " LibData :  %5lu  %5lu  kbyte\n",
			       pss.vmag[3], rss.vmag[3]);
			printk(KERN_INFO " Stack   :  %5lu  %5lu  kbyte\n",
			       pss.vmag[4], rss.vmag[4]);
			printk(KERN_INFO " Other   :  %5lu  %5lu  kbyte\n\n",
			       pss.vmag[5], rss.vmag[5]);
		}
	} while_each_thread(g, p);

	if (print && toggle) {
		printk(KERN_INFO " * User(pss) :  %5lu kbyte\n", sum_of_pss);
		kernel_mem = get_kernel_mem(true);
		printk(KERN_INFO " * Kernel    :  %5lu kbyte\n", kernel_mem);
		printk(KERN_INFO "===============================\n");
	}
...
static void profiling_stop(void)
{
	...
	printk(KERN_INFO "\n\n==========================\n");
	printk(KERN_INFO "       Final Report\n");
	printk(KERN_INFO "==========================\n");
	printk(KERN_INFO " * START *\n");
	printk(KERN_INFO " USER(PSS) : %lu kbyte\n", g_start_pss);
	printk(KERN_INFO " KERNEL    : %lu kbyte\n\n", g_start_kernel_mem);
