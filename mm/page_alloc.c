...
#ifndef CONFIG_LOWMEM_RESERVE
#define CONFIG_LOWMEM_RESERVE 2560
#endif
...
unsigned long normal_fallback(void)
{
	int cpu;
	unsigned long ret = 0;
	...
	for_each_online_cpu(cpu) {
		...
		ret += this->event[PG_LOWMEM_FALLBACK];
	}
...
static void warn_pagetypeinfo_show(void)
{
	...
	pr_emerg("%-43s ", "Free pages count per migrate type at order");
	...
	for (zone = node_zones; zone - node_zones < MAX_NR_ZONES; ++zone) {
		...
		for (mtype = 0; mtype < MIGRATE_TYPES; mtype++) {
			pr_emerg("Node %4d, zone %8s, type %12s ",
			pgdat->node_id,
			zone->name,
		...
		unsigned long bad_flags)
{
	...
	write_emrg_klog(NULL, WRITE_RAW_KLOG_TYPE_BAD_PAGE);
			...
			gfp_t gfp_flags, int migratetype)
{
	...
	VM_BUG_ON_PAGE(bad_range(zone, page), page);
#ifdef CONFIG_PAGE_OWNER
	if ((gfp_flags & __GFP_HIGHMEM) && is_normal(zone)
					&& page_owner_inited) {
		for (i = 0; i < (1 << order); i++) {
			page_ext = lookup_page_ext(page + i);
			__set_bit(PAGE_EXT_FALLBACK, &page_ext->flags);
		}
						...
						const struct alloc_context *ac)
{
	...
	for_each_zone_zonelist_nodemask(zone, z, zonelist, ac->high_zoneidx,
		...
		if((gfp_mask & __GFP_NONE_CMA) && is_zone_cma(zone))
			continue;
...
static inline void check_and_do_stackoverflow(void)
{
	unsigned long remaining_stack;
	unsigned long sp = current_stack_pointer;

	remaining_stack = sp - ((unsigned long)current->stack + PAGE_SIZE);
	if (remaining_stack < 500) {
		/*
		 * If remaining stack is less than 500 bytes, do map extra stack
		 * page in advance as we are going into section which can cause
		 * recursion if overflow happened afterwards.
		 * e.g overflow happened in page_allocation, can't afford that.
		 */
		set_tsk_thread_flag(current, TIF_VMAP_STACK_ALLOC);
		READ_ONCE(*((unsigned long *)(current->stack)));
...
#ifdef CONFIG_CMA
	enum zone_type zone_cma = MAX_NR_ZONES;
#endif
	do {
		zone_type--;
...
#ifdef CONFIG_CMA
			if(is_zone_cma(zone)) {
				zone_cma = zone_idx(zone);
				continue;
			}
			else if(!is_highmem_idx(zone_idx(zone)) && zone_cma != MAX_NR_ZONES) {
				zoneref_set_zone(pgdat->node_zones + zone_cma,
						&zonelist->_zonerefs[nr_zones++]);
				check_highest_zone(zone_cma);
			}
#endif
	...
	for_each_online_pgdat(pgdat) {
		for (j = 0; j < MAX_NR_ZONES; j++) {
			...
			if (is_zone_cma_idx(j)) {
				/* zone cma doesn't set lowmem ratio */
				zone->lowmem_reserve[ZONE_HIGHMEM] = 0;
				continue;
			}
			 ...
			 bool skip_hwpoisoned_pages)
{
	 ...
	 * If ZONE_MOVABLE or ZONE_CMA, the zone never contains unmovable pages
	 */
