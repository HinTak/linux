...
#if defined(_ECHOP) || defined(__ECHOP__) || defined(_FOXP) || defined(__FOXP__)
struct fi_system_config {
	...
	int support_3d;
	int frc_vx1_rx_eq;
	int duty_start_posi_dyn;
	int duty_del_offset_dyn;
	int duty_start_posi_mov;
	int duty_del_offset_mov;
	int fdisplay_3d_on_off;
	int dither_bypass;
	int reserved1;
	int reserved2;
	char model_name[20];
...
#ifdef _ECHOP
/*This is used by EchoP TCON's.*/
struct ti_tcon_ctr_t {
	int DynCtrOn;
	int MovCtrOn;
	int CoefGainHigh;
	int IirSlpfac;
	...
	int RHighHistoWT_4;
	int RHighHistoWT_5;
	int RHsizeCutOff_Low_L;
	int RHsizeCutOff_High_L;
	int RHsizeCutOff_Low_R;
	int RHsizeCutOff_High_R;
	int glimitMax;
};

struct ti_tcon_factory_data {
	struct ti_tcon_ctr_t ctr;
	struct ti_TconDCCDebug_t dcc_Dyn;
	struct ti_TconDCCDebug_t dcc_Movie;
	struct ti_TconPartion_t partition_dyn;
	struct ti_TconPartion_t partition_movie;
	struct ti_TconTemperature_t temp_Still_Dyn;
	struct ti_TconTemperature_t temp_Motion_Dyn;
	struct ti_TconTemperature_t temp_Still_Movie;
	struct ti_TconTemperature_t temp_Motion_Movie;
	struct ti_TconGlimitLBT_t glimit_lbt_dyn;
	struct ti_TconGlimitLBT_t glimit_lbt_Movie;
};
#else
struct ti_tcon_factory_data {
...
/* Set Panorama mode */
#define FRC_IOCTL_SET_PANORAMA_MODE			FRC_IOWR(0xD2, int)
...
/* Set MOtion State, Still or Movie */
#define TCON_IOCTL_SET_MOTION_STATE                             TCON_IOWR(0x6F, int)
