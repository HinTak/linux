...
#if defined (CONFIG_ARCH_SDP1601)  && defined(__KANTM_REV_0__)		// Keep OCL Hub only for Kant.M, For rest(Kant-M2 and MUSE etc.) remove it
	#define MAX_PRIORITY_DEVICES	22
#else
	#define MAX_PRIORITY_DEVICES	21
#endif
...
struct snapshot_hcd{
    char name[MAX_HCD_NAME_SIZE];
    usbdev_speed udev_speed;
    int valid;
    };

struct resume_devnode {
	int (*reset_resume)	(resume_args *arg1);
	void (*enumerate_device)	(struct usb_hub *hub);
...
struct instant_resume_control {
	...
	volatile long unsigned int      wifi_ctrl;
        struct instant_resume_tree      *instant_tree[MAX_INSTANT_TREES];
	struct completion		event_end;
	struct completion		parent_end;
	struct completion		kernel_resume_end;
	struct mutex			defer_lock;
	bool				access_once;	
	void (*iot_func)(void);
	spinlock_t			off_khubd_lock;
...
EXTERN void instant_iotmode_decision(void);
