...
unsigned int board_information;
EXPORT_SYMBOL(board_information);
static int __init set_board_information(char *str)
{
#ifdef CONFIG_ARCH_MXC
	int i=0;
	/* for parm "bdinfo=" val should be b1010 format.*/
	for(i=4;i>=0;i--) {
			if(str[i] == '1')
					board_information |= 1<<(4-i);	
	}
#else
	if (!str || !strlen(str))
		return 0;

	if (kstrtouint(str, 16, &board_information))
		return 0;
#endif
	return 1;
}
__setup("bdinfo=", set_board_information);

static char * __initdata model_names = NULL;
static struct file_operations model_names_fops = { };
static int __init set_model_names(char *str)
{
	model_names = str;
	return 1;
}
__setup("model=", set_model_names);

static int __init model_name_procfs_init(void)
{
	if (!model_names || !strlen(model_names))
		return 0;

	if (!proc_create(model_names, 0, NULL, &model_names_fops))
		pr_warn("/proc/%s registration failed.\n", model_names);

	return 0;
}
late_initcall(model_name_procfs_init);

static int __initdata pba_boot;
static struct file_operations pba_boot_fops = { };
static int __init set_pba_boot(char *str)
{
	pba_boot = 1;
	return 1;
}
__setup("pba_boot", set_pba_boot);

static int __init pba_boot_procfs_init(void)
{
	if (!pba_boot)
		return 0;

	if (!proc_create("pba_boot", 0, NULL, &pba_boot_fops))
		pr_warn("/proc/pba_boot registration failed.\n");

	return 0;
}
late_initcall(pba_boot_procfs_init);
...
#ifndef CONFIG_KERNEL_STACK_SMALL
void __init __weak thread_info_struct_cache_init(void)
{
}
#endif
