...
#ifdef CONFIG_MINIMAL_CORE_COMP
			...
			struct minimal_coredump_params *cprm)
{
	...
	while (len_available) {
		if (len_available < (AES_ENC_BUF_SIZE - cprm->bufs_index)) {
			...
			memcpy(p_aes_buf + cprm->bufs_index,
...
static int set_gzip_header(struct minimal_coredump_params *cprm)
{
	...
	ret = encrypt_write(filename, strlen(filename)+1, !LAST_WRITE, cprm);
	if (ret) {
		pr_alert("Can't encrypt_write minimal corefile\n");
		/* goto out; */
		/* FALLTHROUGH to out */
...
#else
	/* gzip_header_t write */
	if (!dump_write(cprm->file, &gzip_hdr, sizeof(struct gzip_header))) {
		pr_alert("Can't write gzip header\n");
		ret = -EIO;
		goto out;
	}

	/* write original file name */
	if (!dump_write(cprm->file, filename, strlen(filename)+1)) {
		pr_alert("Can't write minimal core filename\n");
		ret = -EIO;
		/* goto out; */
		/* FALLTHROUGH to out */
	}
...
{
	cprm->def_strm.workspace = vmalloc(zlib_deflate_workspacesize(
						MAX_WBITS,
						 MAX_MEM_LEVEL));
...
#ifndef CONFIG_MINIMAL_CORE_COMP
	if (!dump_write(cprm->file, addr, nr)) {
		pr_alert("Can't write minimal corefile\n");
		ret = -EIO;
		goto out;
	}
	goto out;
	...
	if (MINIMAL_CORE_COMP_STATE == cprm->minimal_core_state) {
		...
		pr_alert("Deflated... %p %d bytes\n", addr, nr);
			...
			ret = encrypt_write(cprm->comp_buf,
				cprm->def_strm.total_out, !LAST_WRITE, cprm);
			if (ret) {
				pr_alert("mincore : write fail\n");
				goto free_workspace;
			}
#else
			if (!dump_write(cprm->file, cprm->comp_buf,
				cprm->def_strm.total_out)) {
				pr_alert("mincore : content write fail\n");
				ret = -EIO;
				goto free_workspace;
			}
#endif
...
#else
		if (!dump_write(cprm->file, &cprm->crc,
			sizeof(unsigned long))) {
			pr_alert("mincore: crc write fail\n");
			ret = -EIO;
			goto free_workspace;
		}

		if (!dump_write(cprm->file, &cprm->total_size,
			sizeof(unsigned long))) {
			pr_alert("mincore: file len write fail\n");
			ret = -EIO;
			goto free_workspace;
		}
#endif
...
static int minimal_dump_seek(struct minimal_coredump_params *cprm, loff_t off)
{
...
#ifndef CONFIG_MINIMAL_CORE_COMP
		if (!dump_seek(cprm->file, off)) {
			pr_alert("Cannot lseek minimal corefile\n");
			return -EIO;
		}
#else
	...
	while (off > 0) {
		...
		if (n > PAGE_SIZE)
			n = PAGE_SIZE;
		ret = minimal_dump_write(cprm, buf, n);
		if (ret)
			break;
		off -= n;
	}
		...
		struct minimal_coredump_params *cprm, loff_t *foffset)
{
	...
	MINIMAL_CORE_WRITE(men->name, en.n_namesz, foffset);
	/* align to 4 byte */
	MINIMAL_CORE_WRITE(buf, roundup(*foffset, 4) - *foffset, foffset);
	MINIMAL_CORE_WRITE(men->data, men->datasz, foffset);
	/* align to 4 byte */
	MINIMAL_CORE_WRITE(buf, roundup(*foffset, 4) - *foffset, foffset);
		...
		struct minimal_coredump_params *cprm, loff_t *foffset)
{
	...
	do {
		...
		if (first && write_mini_note(&info->auxv, cprm, foffset))
			return -1;
		if (first && write_mini_note(&info->files, cprm, foffset))
			return -1;
	...
	for (i = 0; i < nregs; i++) {
		...
		if (!found) {
			addrvalue[naddrvalue].start = rounddown(score_regs[i],
					PAGE_SIZE) - PAGE_SIZE;
			addrvalue[naddrvalue].end = roundup(score_regs[i],
					PAGE_SIZE) + PAGE_SIZE;
...
#endif /* CONFIG_MINIMAL_CORE_ENCRYPT */
...
static int elf_minimal_core_dump(struct minimal_coredump_params *cprm)
{
	...
	*/
	...
	/* sz = get_note_info_size(&info); */ /* FIXED, copied*/
#ifdef CORE_DUMP_USE_REGSET
	sz = info.size;
#else
	...
	minimal_core_fill_phdr(phdr4note, sz, offset);
	offset += sz;
				...
				loff_t offset, u64 ino, unsigned int d_type)
{
	...
	*/
	if (namelen <= strlen(MINCORE_NAME))
		return 0;
	tmp = name + (namelen - strlen(MINCORE_NAME));
	...
	entry->name = kmalloc(namelen+1, GFP_KERNEL);
	if (NULL == entry->name) {
		kfree(entry);
		...
		struct list_head *b)
{
	...
	* filename not in expected format, let it be */
	...
	if (((tmp - entry->name) > 0) && ((tmp - entry->name) <= ULONG_LEN))
		strncpy(num_str, entry->name, (tmp - entry->name));
	else
		return 0;
...
static int open_mincore_dir(struct file **dir_file)
{
	...
	*dir_file = filp_open(MINIMAL_CORE_LOCATION,
	...
	strcpy(onelup_dir_name, MINIMAL_CORE_LOCATION);
	tmp = strchr(onelup_dir_name, '/');
	if (tmp) {
		tmp++;
		tmp = strchr(tmp, '/');
	}
	...
	*tmp = '\0';
	...
	tmp = strchr(file_dir_name, '/');
	if (NULL == tmp) {
		pr_alert(
			"FATAL : mincore :dirname not wrong format\n");
		error = -EIO;
		goto close_dir;
	}
	...
	*tmp = '\0';
	...
	subdir = lookup_one_len(file_dir_name, parent, strlen(file_dir_name));
	if (IS_ERR(subdir)) {
		pr_alert(
...
{
	/* now it is /mtd_rwcommon/error_log/mcore-<comm>.<tgid>.<pid>.<index>
	...
	*/
	...
	snprintf(small_corename, sizeof(small_corename),
			"%s%lu.%s.%d.%d%s", MINIMAL_CORE_LOCATION, (max + 1),
			current->group_leader->comm, current->pid,
			current->tgid, MINCORE_NAME);

	score_file = filp_open(small_corename, flags, 0600);
