...
//    }
    ...
    has_dumped = 1;
    pr_alert ("[Extended-Mincore] has_dumped = 1\n");

end_coredump:
    set_fs(fs);

cleanup:
    //Shreyas To-Do : Need to free note_info below..
    //free_note_info(&info);
    kfree(shdr4extnum);
    kfree(vma_filesz);
    kfree(phdr4note);
...
skip_cred_change:
    ...
    while (idx--) 
	{
		...
		pr_alert("[Extended-Mincore] Waiting for get_coredump_config to return..\n");
		msleep(200);
	}

    if ((coreType == NO_CORE) || (coreType == -1))
    {
        pr_alert("[Extended-Mincore] get_coredump_config returned with core_type = %d\n", coreType);
        kfree(mcprm);
        kfree(small_corename);
        return;
    }

    pr_alert("[Extended-Mincore] get_coredump_config return coreType = %d,  process name as %s and signal type as %d\n", coreType, procNameFromConfig,signalFromConfig);
    if (isDumpRequired(procNameFromConfig, signalFromConfig, mcprm->siginfo->si_signo) == false)
    {
        pr_alert("[Extended-Mincore] isDumpRequired returned false.. Skipping (min/mid/full)core generation..\n");
        kfree(mcprm);
        kfree(small_corename);
        return;
    }
