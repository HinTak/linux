					...
					const void *addr, int nr)
{
	...
	/* check for invalid input size to updcrc, Fix: DF170228-01384 */
	if (cprm->def_strm.avail_in > MINIMAL_CORE_COMP_IN_BUFSIZE) {
		pr_alert("mincore: invalid size to updcrc 0x%lx\n",
...
static int open_mincore_dir(struct file **dir_file)
{
	...
	*tmp = '\0';
	...
	parent = parent_dir->f_path.dentry;
...
skip_cred_change:
	...
	small_corename = kmalloc(CORENAME_MAX_SIZE + 1, GFP_KERNEL);
	...
	*/
	...
	snprintf(small_corename, CORENAME_MAX_SIZE + 1,
	...
	if (strncmp(target_version, "", sizeof(""))) {
		strncat(small_corename, ".", CORENAME_MAX_SIZE
			- strlen(small_corename));
		strncat(small_corename, target_version, CORENAME_MAX_SIZE
			- strlen(small_corename));
...
#endif
	strncat(small_corename, pc_lr, CORENAME_MAX_SIZE
			- strlen(small_corename));
#ifdef CONFIG_ARM64
	...
	if (pc_vma && mcprm->regs->user_regs.pc > pc_vma->vm_start) {
		...
		strncat(small_corename, pc_offset, CORENAME_MAX_SIZE
					- strlen(small_corename));
	}
#else
	...
	strncat(small_corename, MINCORE_NAME, CORENAME_MAX_SIZE
			- strlen(small_corename));
