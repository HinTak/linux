...
static int open_mincore_dir(struct file **dir_file, char *dir_path)
{
	...
	/* max wait of 20 seconds for success of
	 * get_new_folder_name
	 */
	while (idx--) {
	...
	if (ret) {
		*dir_file = filp_open(dir_path, O_RDONLY | O_DIRECTORY, 0);
		...
		pr_alert("minimal core: get_new_folder_name() timeout\n");
		error = -EIO;
	}
...
skip_cred_change:
	...
	*/
	...
	if (strncmp(target_version, "", sizeof(""))) {
		strncat(small_corename, ".", MINICORENAME_MAX_SIZE
			- strlen(small_corename));
		strncat(small_corename, target_version, MINICORENAME_MAX_SIZE
			- strlen(small_corename));
...
#endif
	strncat(small_corename, pc_lr, MINICORENAME_MAX_SIZE
			- strlen(small_corename));
#ifdef CONFIG_ARM64
