...
#ifndef CONFIG_REGISTER_PROCESSLIST_BY_PROC
static struct process_list pax_process_list[] = {
	/* add process list like under format */
	/*{"/mtd_exe/exeAPP" , 1 , 1},*/
	};
#endif
...
static unsigned long randomize_stack_top(unsigned long stack_top)
{
	...
	if ((current->flags & PF_RANDOMIZE) &&
		...
		if ((current->mm->pax_flags & MF_PAX_ASLR))
#endif /* CONFIG_ASLR_PER_PROCESS */
...
#if defined CONFIG_PER_PROCESS
/**
 * dep_aslr_get_exe_path - get absolute path of the current process
 * @path:  pointer to struct path (mm->exe_file->f_path)
 *
 * Return path of exe on Success or NULL otherwise
 */
static char *dep_aslr_get_exe_path(struct path *path)
{
	char *exe_path = NULL;
	char *buf = NULL;
...
/**
 * get_exe_path - Get realpath of current process.
 *
...
static const char *get_exe_path(void)
{
	...
	if (mm->exe_file)
		cp = dep_aslr_get_exe_path(&mm->exe_file->f_path);
	up_read(&mm->mmap_sem);
	return cp;
}

/**
 * set_flag_by_process_list - set PAX flags on the process
 * @pax_flag - flag that need to be set
 *
 * Return pax_flags on SUCCESS
 */
unsigned long set_flag_by_process_list(unsigned long pax_flag)
{
	...
	if (exe_path != NULL) {
		...
		if (!strcmp(plist->process_name, exe_path)) {
				aslr_enable = plist->aslr_enable;
				dep_enable  = plist->dep_enable;
				break;
			}
		}
		rcu_read_unlock();
		...
		int i = 0;
		int count_loop =
			sizeof(pax_process_list) / sizeof(struct process_list);
		for (i = 0 ; i < count_loop ; i++) {
			if (!strcmp(pax_process_list[i].process_name,
						exe_path)) {
				aslr_enable = pax_process_list[i].aslr_enable;
				dep_enable  = pax_process_list[i].dep_enable;
				break;
			}
		}
#endif
...
#ifdef CONFIG_MPROTECT_PERPROCESS
	if (!dep_enable)
		pax_flag &= ~(MF_PAX_PAGEEXEC | MF_PAX_MPROTECT);
#endif
#ifdef CONFIG_ASLR_PER_PROCESS
	if (!aslr_enable)
		pax_flag &= ~MF_PAX_ASLR;
#endif
	if (exe_path != NULL)
		kfree(exe_path);
...
/**
 * assign_pax_flags - Assign PAX flag based on the ASLR & DEP in the system
 *
 * Return pax_flags on success
 */
static unsigned long assign_pax_flags(void)
{
...
#ifdef CONFIG_ASLR_PER_PROCESS
	pax_flags |= MF_PAX_ASLR;
#endif
	return pax_flags;
}

/**
 * pax_parse_pax_flags - parse pax flags and set for current process
 *
 * Sets pax_flags on current process on success
 */
static void pax_parse_pax_flags(void)
{
	unsigned long pax_flags ;
	pax_flags = assign_pax_flags();
#ifdef CONFIG_PER_PROCESS
	pax_flags = set_flag_by_process_list(pax_flags);
#endif
	current->mm->pax_flags = pax_flags;
}
#endif /*#if defined(CONFIG_PAX) */
...
static int load_elf_binary(struct linux_binprm *bprm)
{
	...
	for(i = 0, elf_ppnt = elf_phdata;
		...
		if (loc->elf_ex.e_type == ET_EXEC || load_addr_set) {
			...
			/* PaX: randomize base address at the default exe
			 * base if requested */
