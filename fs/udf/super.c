...
void bdrom_update_call_counts(int pos_012)
{
	...
	static int units[10] = {10000, 1000, 1000, 1000, 1000,
...
static inline int bdrom_metadata_cache_invalid_bh(void *bh)
{
	if( (((unsigned int)bh) & (unsigned int)0xFFFFfffc) == (unsigned int)0 )
		return 1;
...
int bdrom_metadata_cache_cleanup(bdrom_metadata_cache* p_this)
{
	...
	if ((p_this->m_isBdRom_01 == 1) &&
				...
				(p_this->m_nPhysicalReads > 0))
		{
			...
			for( n = SB_READ_CACHE_MIN; n < SB_READ_CACHE_MAX; n++ )
			{
				if (bdrom_metadata_cache_check_bh
					(p_this->sb_array[n]) != -1)
				{
				...
				else if( p_this->sb_array[n] ==
					(void*) BH_ACCESS_IS_NON_MDATA)
				{
					nEmptySlots++;
				}
				else {
			...
			udf_debug("Cached:Empty = %d:%d, ratio = %d\n",
					nCachedSlots, nEmptySlots,
					100*nCachedSlots/(nCachedSlots + nEmptySlots)
				 );
			printk("############################################\n\n");
			return 1;
		}
...
int bdrom_metadata_cache_get_read_area_V01 (bdrom_metadata_cache* p_this,
				int given_lbn, int* read_from_lbn,
				int* read_until_lbn)
{
...
int bdrom_metadata_cache_get_read_area_V02(bdrom_metadata_cache* p_this,
					int given_lbn, int* read_from_lbn,
					int* read_until_lbn)
{
	int start, until;
...
int bdrom_metadata_cache_get_read_area_V03(bdrom_metadata_cache* p_this,
					int given_lbn, int* read_from_lbn,
					int* read_until_lbn)
{
	int start, until;
...
int bdrom_metadata_cache_get_read_area(bdrom_metadata_cache* p_this,
					int given_lbn, int* read_from_lbn,
					int* read_until_lbn)
{
...
int bdrom_metadata_cache_update_reads(bdrom_metadata_cache* p_this,
					int n_lbs_read)
{
...
int udf_identify_descriptor(struct super_block *sb,
			struct buffer_head *bh, uint16_t *ident)
{
	...
	if (!bh || bdrom_metadata_cache_invalid_bh(bh))
	{
	...
	*ident = le16_to_cpu(tag_p->tagIdent);
	...
	if (le16_to_cpu(tag_p->descCRCLength) + sizeof(struct tag) >
			sb->s_blocksize ||
			le16_to_cpu(tag_p->descCRC) ==
			udf_crc(bh->b_data + sizeof(struct tag),
...
void udf_release_data(struct buffer_head *bh)
{
	...
	if (!bdrom_metadata_cache_invalid_bh(bh))
		brelse(bh);
	return;
}
#endif
...
restart:
	...
	if (IS_ERR(fe) && !mdata_number_iscorrect) {
		for (i = 0; i < sbi->s_partitions ; i++)
			if(sbi->s_partmaps[i].s_partition_type ==
					UDF_TYPE1_MAP15 ||
					sbi->s_partmaps[i].s_partition_type ==
					UDF_SPARABLE_MAP15)
				break;
		udf_debug("Correct Metadata Partition no: from: %d, to %d\n",
