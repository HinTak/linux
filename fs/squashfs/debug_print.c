				  ...
				  int fail_block)
{
	...
	check_unit = all_block_read ? d->b : 1;
	...
	for (i = fail_block; i < fail_block + check_unit; i++) {
		...
		/* Set bh's data to the initial value before Reread*/
		memset(d->bh[i]->b_data, 0xadef, msblk->devblksize);
	...
	for (blk_n = fail_block; blk_n < fail_block + check_unit; blk_n++) {
		...
		if (all_block_read)
			cur = (u32 *) (block_buffer + msblk->devblksize * blk_n);
		else
			cur = (u32 *) block_buffer;
