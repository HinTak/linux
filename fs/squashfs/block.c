...
static int compression_type_checking(int type)
{
	int ret=HW_IOVEC_COMP_GZIP;

	switch(type)
	{
		case ZLIB_COMPRESSION :
			ret = HW_IOVEC_COMP_ZLIB;
			break;
		case LZO_COMPRESSION :
			ret = HW_IOVEC_COMP_LZO;
			break;
		case GZIP_COMPRESSION :
			ret = HW_IOVEC_COMP_GZIP;
			break;
		default :
			printk("[HW2 squashfs] No matched one!!!!!\n");
			printk("Please refer enum hw_iovec_comp_type in include/linux/blkdev.h\n");
			printk("default setting as HW_IOVEC_COMP_GZIP!!!! \n");
	}
			...
			int length, u64 *next_index, int srclength, int pages)
{
	...
	int (*do_hw)(struct block_device *bdev,
		     ...
		     unsigned int out_cnt, struct req_hash *rq_hash, enum hw_iovec_comp_type comp_type, enum hw_iovec_encrypt_type enc_type) = NULL;
	...
	if (compressed) {
		...
		if (do_hw && !is_metablock) {
			...
			err = do_hw(sb->s_bdev, &iovec, 1, out_pages, pages,
					NULL, compression_type_checking(msblk->decompressor->id), 0);
