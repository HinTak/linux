			...
			int length, u64 *next_index, int srclength, int pages)
{
	...
	if (compressed) {
		...
		if (do_hw && !is_metablock) {
			...
			sector_t sector_off =
				(cur_index << msblk->devblksize_log2) >> 9;

			/* Strong HW requirements */
			BUG_ON(msblk->block_size != 128<<10);
			BUG_ON(msblk->devblksize_log2 != 12);

			out_pages = kcalloc(hw_pages_cnt, sizeof(struct page *),
					    GFP_KERNEL);
			if (!out_pages)
				goto read_failure;

			for (i = 0; i < hw_pages_cnt; ++i) {
				BUG_ON(!virt_addr_valid(buffer[i]));
				out_pages[i] = virt_to_page(buffer[i]);
			}

			err = do_hw(sb->s_bdev, out_pages, hw_pages_cnt,
				    sector_off, offset, length);
			kfree(out_pages);
		...
		/* Do software decompression */
			...
			if (length < 0) {
				/* Everything should be freed in decompressor */
				b = 0;
				decompress_err = 1;
				goto read_failure;
			}
