		...
		struct writeback_control *wbc, void *data)
{
	...
	if (page_has_buffers(page)) {
		...
		if (buffer_mapped(bh)) {
			...
			*/
			if (buffer_dirty(bh)) {
				ret = vdfs4_get_int_block(inode, block_in_file, bh, 1, 0);
				if (ret) {
					mutex_unlock(&inode_info->truncate_mutex);
#if defined(CONFIG_VDFS4_DEBUG)
					VDFS4_DEBUG_TMP("get_int_block failed %s %llu",
							inode_info->name, block_in_file);
#endif
					goto confused;
				}
				if (buffer_new(bh))
		...
		*/
		...
		ret = vdfs4_get_block(inode, block_in_file, bh, 1);
		if (ret) {
#if defined(CONFIG_VDFS4_DEBUG)
			VDFS4_DEBUG_TMP("vdfs4_get_block failed %s %llu",
						inode_info->name, block_in_file);
#endif
			goto confused;
		}
...
#if defined(CONFIG_VDFS4_DEBUG)
			VDFS4_DEBUG_TMP("file expanded more than 1page %s %ld %ld %u",
				inode_info->name, page->index, end_index, offset);
#endif
			goto confused;
		}
...
alloc_new:
	...
	if (IS_ERR_OR_NULL(bio)) {
		...
		if (IS_ERR_OR_NULL(bio)) {
#if defined(CONFIG_VDFS4_DEBUG)
			VDFS4_DEBUG_TMP("__allocate_new_bio failed %p", bio);
#endif
			goto confused;
		}
	}
...
#if defined(CONFIG_VDFS4_DEBUG)
	if (err)
		VDFS4_DEBUG_TMP("err = %d, ino#%lu name=%s, page index: %lu, "
