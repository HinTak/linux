...
int emmcfs_fill_cattree_value(struct inode *inode, void *value_area)
{
	...
	if (comm_rec->record_type &&
			rec_type != le16_to_cpu(comm_rec->record_type)) {
		EMMCFS_ERR("mismatch record type was %d new %d",
				le16_to_cpu(comm_rec->record_type), rec_type);
		EMMCFS_BUG();
	}

	if (comm_rec->object_id &&
			le64_to_cpu(comm_rec->object_id) != inode->i_ino) {
		EMMCFS_ERR("mismatch i_ino: ino#%llu becomes ino#%lu",
			le64_to_cpu(comm_rec->object_id), inode->i_ino);
		EMMCFS_BUG();
	}
