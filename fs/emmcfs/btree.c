...
{
	struct emmcfs_gen_node_descr *bnode_desc = EMMCFS_BNODE_DSCR(bnode);
		...
		struct emmcfs_btree *btree)
{
	...
	while(pagevec_lookup_tag(&pvec, mapping, &index, PAGECACHE_TAG_DIRTY,
			btree->pages_per_node)) {
		struct emmcfs_gen_node_descr *bnode_desc;

		bnode_desc = vmap(pvec.pages, btree->pages_per_node, VM_MAP,
				PAGE_KERNEL);
		BUG_ON(!bnode_desc);

		if (pvec.pages[0]->index)
		...
		void *new_data, int level)
{
	...
	adding_record = kzalloc(btree->max_record_len, GFP_KERNEL);
	if (!adding_record) {
		err = -ENOMEM;
		goto exit_no_ar_clean;
	}
	first_ins_root_rec = kzalloc(btree->max_record_len, GFP_KERNEL);
		...
		int left_bnode_id, int right_bnode_id, int level)
{
...

	{
		...
		struct emmcfs_generic_key *key =
		 kzalloc(((struct emmcfs_generic_key *)record)->key_len,
		 GFP_KERNEL);
...
static struct query emmcfs_init_query(void)
{
	 ...
	 q.start = 0;
	 q.end = 0;
	 q.data = kzalloc(sizeof(struct emmcfs_bnode *) *
