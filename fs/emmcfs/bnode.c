...
int bnode_sanity_check(struct emmcfs_bnode *bnode);
...
void emmcfs_sign_bnode(void *bnode_data,
		struct emmcfs_btree *btree)
{
		...
		__u32 node_id, enum emmcfs_get_bnode_mode mode)
{
	...
	EMMCFS_LOG_FUNCTION_START(btree->sbi);
	mutex_lock(&btree->lock_get_bnode);
	...
	if (!ret) {
		...
		if (IS_ERR(ret)) {
			mutex_unlock(&btree->lock_get_bnode);
			EMMCFS_LOG_FUNCTION_END(btree->sbi, 0);
			return ret;
		}
		add_bnode_cache(ret);
