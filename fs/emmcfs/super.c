...
#define EMMCFS_WRITE_TIMEOUT	30000
...
static void update_super_forks(struct emmcfs_sb_info *sbi)
{
	...
	if (!is_fork_valid(&exsb->catalog_tree_fork) ||
			!is_fork_valid(&exsb->extents_overflow_tree_fork) ||
			!is_fork_valid(&exsb->xattr_fork) ||
			!is_fork_valid(&exsb->inode_num_bitmap_fork) ||
			!is_fork_valid(&exsb->hlinks_fork)) {
		BUG();
	}
#endif
	...
	/* Extents overflow tree fork */
	emmcfs_form_fork(&exsb->xattr_fork, sbi->xattr_tree->inode);
 ...
 */
int emmcfs_sync_fs(struct super_block *sb, int wait)
{
	struct emmcfs_sb_info *sbi = sb->s_fs_info;
	struct emmcfs_inode_info *inode_info;
	if (wait) {
		down_write(sbi->snapshot_info->transaction_lock);
		spin_lock(&EMMCFS_SB(sb)->dirty_list_lock);
		while (!list_empty(&sbi->dirty_list_head)) {
			...
			spin_unlock(&EMMCFS_SB(sb)->dirty_list_lock);
			filemap_write_and_wait(inode_info->vfs_inode.i_mapping);
			spin_lock(&EMMCFS_SB(sb)->dirty_list_lock);
		}
		spin_unlock(&EMMCFS_SB(sb)->dirty_list_lock);
		update_metadata(sbi);
		...
		struct super_block *sb)
{
	...
	if ((!test_option(sbi, STRIPPED) && total_block_count >
...
static int emmcfs_sb_read(struct super_block *sb, int silent)
{
	...
	if (EMMCFS_IS_READONLY(sb) && !check_sb)
		goto skip_restoring;

	check_sb_copy = emmcfs_verify_sb(esb_copy, "SB_COPY", 1);

	if (check_sb && (check_sb_copy || EMMCFS_IS_READONLY(sb))) {
		/*both superblocks are corrupted or fisrt superblock is
		 * corrupted and it's readonly mode*/
		ret = check_sb;
		if (!silent || debug_mask & EMMCFS_DBG_SB)
...
static int emmcfs_extended_sb_read(struct super_block *sb)
{
	...
	if (EMMCFS_IS_READONLY(sb) && !check_exsb)
		goto skip_restoring;
	...
	if (check_exsb && (check_exsb_copy || EMMCFS_IS_READONLY(sb))) {
		...
		struct emmcfs_btree *btree, struct inode *inode)
{
	...
	EMMCFS_LOG_FUNCTION_START(sbi);
	mutex_init(&btree->lock_get_bnode);
	btree->sbi = sbi;
	btree->inode = inode;
...
static int emmcfs_fill_cat_tree(struct emmcfs_sb_info *sbi)
{
	...
	err = emmcfs_fill_btree(sbi, cat_tree, inode);
	if (err)
		goto err_put_inode;
...
static int emmcfs_fill_ext_tree(struct emmcfs_sb_info *sbi)
{
	...
	err = emmcfs_fill_btree(sbi, ext_tree, inode);
	if (err)
		goto err_put_inode;
...
/*
 * wakes up periodically and does emmcfs sync.
 */
int emmcfs_writeback_worker(void *data)
{
	struct emmcfs_sb_info *sbi = data;
	set_user_nice(current, 0);
	while (!kthread_should_stop()) {
		set_current_state(TASK_INTERRUPTIBLE);
		schedule_timeout(msecs_to_jiffies(EMMCFS_WRITE_TIMEOUT));
		if (!kthread_should_stop()) {
			if (sbi->snapshot_info->transactions_count) {
...
static int emmcfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (!EMMCFS_IS_READONLY(sb)) {
		sbi->writeback_thread = kthread_run(emmcfs_writeback_worker,
			sbi, "emmcfs-writeback");

		if (!sbi->writeback_thread) {
			ret = PTR_ERR(sbi->writeback_thread);
			goto btree_not_verified;
		}
	}
...
void emmcfs_kill_block_super(struct super_block *sb)
{
	struct emmcfs_sb_info *sbi = sb->s_fs_info;

	if (sbi && !EMMCFS_IS_READONLY(sb))
		kthread_stop(sbi->writeback_thread);
	kill_block_super(sb);
}
...
static struct file_system_type emmcfs_fs_type = {
	...
	BUILD_EMMCFS_BUG();
#endif
	.kill_sb	= emmcfs_kill_block_super,
