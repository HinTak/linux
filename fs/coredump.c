...
#endif
	/*
	 * On robustness team request, adding one more
	 * stack variable.
	 */
	int max_retries = USB_CON_MAX_RETRIES;

#if !defined(CONFIG_VD_RELEASE) && defined(CONFIG_KPI_SYSTEM_SUPPORT)
	if (isRobustnessMode) {
		pr_alert("[Pid:%d] robustness mode is set, reducing USB checking count\n",
				current->pid);
		max_retries = ROBUSTNESS_RETRIES;
	}
#endif
