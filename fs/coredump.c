...
#if (!defined(CONFIG_VD_RELEASE) || defined(CONFIG_VD_PERFORMANCE_MODE))
...
#endif /* (!defined(CONFIG_VD_RELEASE) || defined(CONFIG_VD_PERFORMANCE_MODE)) */
...
#if defined(CONFIG_ELF_CORE) && (!defined(CONFIG_VD_RELEASE) || defined(CONFIG_VD_PERFORMANCE_MODE))
	binfmt = mm->binfmt;
...
#endif
		if(core_dumped)
			printk(KERN_ALERT "***** CoreDump: finished dumping core for %s(%d)\n", current->comm, current->pid);
		else
