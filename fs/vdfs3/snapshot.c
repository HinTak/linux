...
static unsigned long free_blocks_in_bitmap(unsigned long *bitmap,
		unsigned long size);
static long meta_area_free_blocks(struct vdfs_sb_info *sbi);
		...
		sector_t *ext_table_start, __u64 *table_version_out)
{
	...
	table = vdfs_vmalloc((long unsigned int)(DIV_ROUND_UP(tables_size_in_bytes,
...
static int load_extended_table(struct vdfs_sb_info *sbi, sector_t start)
{
	...
	if (!snapshot->extended_table) {
		snapshot->extended_table = vdfs_vmalloc(PAGE_CACHE_SIZE);
		if (!snapshot->extended_table)
			return -ENOMEM;
	}
...
static __u64 get_new_meta_iblock(struct vdfs_sb_info *sbi, 
...
static __u32 get_minimal_meta_expand_step(struct vdfs_sb_info *sbi)
{
...
static __u32 calculate_expand_step(struct vdfs_sb_info *sbi)
{
	...
	if (is_volume_small)
		...
		/* Expand step is now related with partition size. It should be
		high enough that filling partition with directories only is possible.
		That value must be also power of 2, due to fsm hash table
		implementation.
		Additionally this will help with fragmentation issues - expansion
		will be done less often, so chance to allocate whole desired
		chunk is higher */
		expand_step = 
			((__u32)sbi->volume_blocks_count / VDFS_META_BTREE_EXTENTS);
		/* never go below minimum (for relatively small volumes) */
		expand_step = MAX(expand_step, minimal_expand_step << 1);
		pow2_align = __fls(expand_step);
		if (expand_step != (1 << pow2_align))
			pow2_align++;
		expand_step = ALIGN(expand_step, 1 << pow2_align);
	}
	return expand_step;
}
		...
		__u64 new_table_index)
{
	...
	if (((tables_size & (PAGE_SIZE - 1)) +
		...
		void *new_table = vdfs_vmalloc(tables_in_pages << PAGE_SHIFT);
		...
		__u64 new_table_index, int force_insert)
{
	...
	if ((free_blocks - (long)new_blocks_count) < ((long)meta_tbc >> 1)) {
		...
		if (ret) {
			...
			free_blocks = free_blocks_in_bitmap(snapshot_bitmap,
...
unsigned long free_blocks_in_bitmap(unsigned long *bitmap, unsigned long size)
{
...
int vdfs_is_enospc(struct vdfs_sb_info *sbi, int threshold)
{
	struct vdfs_layout_sb *vdfs_sb = sbi->raw_superblock;
...
int vdfs_check_meta_space(struct vdfs_sb_info *sbi)
{
	long free_blocks, ttb;
	struct vdfs_layout_sb *vdfs_sb = sbi->raw_superblock;
	struct vdfs_snapshot_info *snapshot = sbi->snapshot_info;
