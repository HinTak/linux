 ...
 */
static int emmcfs_readdir(struct file *filp, struct dir_context *ctx)
{
...
static int vdfs_dir_fsync(struct file *file, loff_t start, loff_t end, int datasync)
{
		...
		struct vdfs_comp_file_descr *descr)
{
	...
	if (PAGE_CACHE_SIZE - (descr_offset -
		...
		data = vdfs_vmap(pages, 2, VM_MAP, PAGE_KERNEL);
		...
		struct vdfs_comp_extent_info *cext)
{
	...
	hash_offset = inode_i->comp_table_start_offset +
		...
		sizeof(struct vdfs_comp_extent) + inode_i->hash_len * chunk_idx;
	...
	if (PAGE_CACHE_SIZE - (hash_offset - ((hash_offset >> PAGE_CACHE_SHIFT)
			<< PAGE_CACHE_SHIFT)) < inode_i->hash_len) {
		struct page *pages[2];
		ret = vdfs_read_comp_pages(&inode_i->vfs_inode, page_idx,
			2, pages, VDFS_FBASED_READ_M);
		if (ret)
		...
		data = vdfs_vmap(pages, 2, VM_MAP, PAGE_KERNEL);
		memcpy(hash, (char *)data + pos, inode_i->hash_len);
		vunmap(data);
		for (page_idx = 0; page_idx < 2; page_idx++) {
			mark_page_accessed(pages[page_idx]);
			page_cache_release(pages[page_idx]);
...
{
	unsigned char hash_orig[inode_i->hash_len];
	unsigned char hash_calc[inode_i->hash_len];
	void *data = NULL;
	int ret = 0;

	data = vdfs_vmap(comp_pages, (unsigned int)cext->blocks_n,
			VM_MAP, PAGE_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		return ret;
	...
	ret = inode_i->hash_fn((unsigned char *)
	...
	if (ret)
		return ret;
	if (memcmp(hash_orig, hash_calc, inode_i->hash_len)) {
...
retry_uncompressed:
	...
	comp_pages = kmalloc(sizeof(struct page *) *
			((unsigned long)pages_per_chunk + 2lu), GFP_NOFS);
	if ((!comp_pages)) {
		ret = -ENOMEM;
		goto out_kfree;
	}
...
again:
		...
		extent.first_block = emmcfs_fsm_get_free_block(sbi, entry->
				alloc_hint, &count, count, VDFS_FSM_ALLOC_DELAYED);
...
static int vdfs_file_open(struct inode *inode, struct file *filp)
{
	...
	if (is_dlink(inode) && is_vdfs_inode_flag_set(
			inode_info->data_link.inode, VDFS_COMPRESSED_FILE)) {
		struct inode *data_inode = EMMCFS_I(inode)->data_link.inode;
		struct emmcfs_inode_info *data_inode_info = EMMCFS_I(data_inode);
#ifdef CONFIG_VDFS_RETRY
		int retry_count = 0;
retry_dlink:
#endif
		if (data_inode_info->compr_type == VDFS_COMPR_UNDEF) {
			mutex_lock(&data_inode->i_mutex);
			if (data_inode_info->compr_type == VDFS_COMPR_UNDEF)
				rc = init_file_decompression(
						data_inode_info, 1);
			mutex_unlock(&data_inode->i_mutex);
		}
		if (rc) {
	...
	if (is_vdfs_inode_flag_set(inode, VDFS_COMPRESSED_FILE) &&
			inode_info->compr_type == VDFS_COMPR_UNDEF) {
#ifdef CONFIG_VDFS_RETRY
		int retry_count = 0;
retry:
#endif
		rc = init_file_decompression(inode_info, 1);
		if (rc) {
#ifdef CONFIG_VDFS_RETRY
			if (retry_count < 3) {
				retry_count++;
 ...
 */
static ssize_t emmcfs_direct_IO(struct kiocb *iocb, struct iov_iter *iter, loff_t offset)
{
	...
	rc = blockdev_direct_IO(iocb, inode, iter, offset, vdfs_get_block);
	if (!iov_iter_rw(iter))
		return rc;
 ...
 */
static ssize_t vdfs_file_write_iter(struct kiocb *iocb, struct iov_iter * iov_iter)
{
	ssize_t ret = 0;
 ...
 */
static ssize_t vdfs_file_read_iter(struct kiocb *iocb, struct iov_iter * iov_iter)
{
#if defined(CONFIG_VDFS_EXEC_ONLY_AUTHENTICATED) || defined(CONFIG_VDFS_DEBUG)
...
static const struct file_operations emmcfs_file_operations = {
	...
	.read_iter	= vdfs_file_read_iter,
	.write_iter	= vdfs_file_write_iter,
	.mmap		= vdfs_file_mmap,
		...
		int offset, size_t table_len)
{
	...
	BUG_ON(!pages_num);
	data = vdfs_vmap(pages, (unsigned int)pages_num, VM_MAP, PAGE_KERNEL);
		...
		unsigned int extents_num, loff_t start_offset)
{
	void *data = NULL;
	int ret = 0;
	data = vdfs_vmap(pages, pages_num, VM_MAP, PAGE_KERNEL);
	ret = inode_i->verify_fn((char *)data + (start_offset
			& (PAGE_SIZE - 1))
			+ extents_num * sizeof(struct vdfs_comp_extent),
			(extents_num + 1) * inode_i->hash_len,
			inode_i->hash_len,
			(char *)data +
		...
		struct vdfs_comp_file_descr *descr)
{
	...
	ret = inode_i->hash_fn((unsigned char *)descr,
...
int prepare_compressed_file_inode(struct emmcfs_inode_info *inode_i)
{
	int ret = 0;
...
int init_file_decompression(struct emmcfs_inode_info *inode_i, int debug)
{
	...
	unsigned long table_size_bytes = 0;
	u32 crc = 0;
	...
	case VDFS_FORCE_DESCR_START:
		set_vdfs_inode_flag(inode, VDFS_READ_ONLY_AUTH);
		/* fallthrough */
	case VDFS_AUTHE_DESCR_START:
#ifdef CONFIG_VDFS_DATA_AUTHENTICATION
		inode_i->verify_fn = vdfs_verify_rsa_sha1_signature;
		inode_i->hash_fn = calculate_hash_sha1;
		inode_i->hash_len = VDFS_SHA1_LEN;
		table_size_bytes = VDFS_SUPERBLOCK_CRYPTED_HASH_LEN;
		break;
#endif
	case VDFS_AUTHE1024_DESCR_START:
#ifdef CONFIG_VDFS_DATA_AUTHENTICATION
		inode_i->verify_fn = vdfs_verify_rsa_md5_signature;
		inode_i->hash_fn = calculate_hash_md5;
		inode_i->hash_len = VDFS_MD5_LEN;
		table_size_bytes = VDFS_FILE_CRYPTED_HASH_LEN;
		break;
#endif
	case VDFS_COMPR_DESCR_START:
		inode_i->verify_fn = NULL;
		break;
	case VDFS_FORCE1024_DESCR_START:
	default:
		if (!debug)
			return -EOPNOTSUPP;
	...
	if (inode_i->verify_fn != NULL)
		table_size_bytes += (unsigned long)inode_i->hash_len *
			(extents_num + 1lu);
		...
		struct inode *parent)
{
	...
	tree = VDFS_BTREE_REC_I((void *) record)->rec_pos.bnode->host;
	sbi = tree->sbi;
...
retry:
#endif
		ret = prepare_compressed_file_inode(EMMCFS_I(inode));
		if (ret) {
#ifdef CONFIG_VDFS_RETRY
			if (retry_count < 3) {
				retry_count++;
