...
static struct fsm_node *
fsm_get_longest_node(struct emmcfs_fsm_info *fsm)
{
	struct fsm_node *prev, *node;
		...
		__u32 *length_in_blocks, unsigned int min_blk_count, int fsm_flags)
{
	...
	if (fsm_flags & VDFS_FSM_ALLOC_ALIGNED) {
		...
		EMMCFS_BUG_ON((length < minimal_length) || (length > optimal_length));
...
int emmcfs_fsm_build_management(struct super_block *sb)
{
	...
	fsm = kzalloc(sizeof(struct emmcfs_fsm_info), GFP_NOFS);
	if (!fsm)
		return -ENOMEM;
	...
	fsm->data = vdfs_vmap(fsm->pages, fsm->page_count, VM_MAP, PAGE_KERNEL);
