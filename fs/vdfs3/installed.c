		...
		struct inode *root_inode)
{
	...
	btree = kzalloc(sizeof(struct vdfs_btree), GFP_NOFS);
	if (!btree)
		...
		struct inode *root)
{
	...
	installed_info = kzalloc(sizeof(*installed_info), GFP_NOFS);
	if (!installed_info)
		return ERR_PTR(-ENOMEM);
		...
		struct vdfs_image_install *vdfs_image)
{
	...
	struct inode *image_inode = image_file->f_path.dentry->d_inode;
...
static int vdfs_install_check_layout_version(struct inode *image_inode)
{
	struct page *superblock_page;
	void *data = NULL;
	int ret = 0;
	ret = vdfs_read_or_create_pages(image_inode, 0, 1, &superblock_page,
			VDFS_PACKTREE_READ, 0, 0);
	if (ret)
		return ret;
	data = kmap_atomic(superblock_page);
	if (!data) {
		ret = -ENOMEM;
		goto exit;
	}

	if (memcmp(((struct emmcfs_super_block *)
			data)->layout_version, VDFS_LAYOUT_VERSION,
			strlen(VDFS_LAYOUT_VERSION))) {
			EMMCFS_ERR("Wrong image layout version - %.4s. "
					"VDFS filesystem layout version is %.4s",
	...
	kunmap_atomic(data);
exit:
	mark_page_accessed(superblock_page);
	page_cache_release(superblock_page);
	return ret;
}
		...
		struct file *parent_dir, unsigned long arg)
{
	...
	vdfs_image = kmalloc(sizeof(*vdfs_image), GFP_NOFS);
	if (!vdfs_image)
		return ERR_PTR(-ENOMEM);
	...
	image_inode = image_file.file->f_path.dentry->d_inode;
	filemap_write_and_wait(image_inode->i_mapping);
	invalidate_bdev(sbi->sb->s_bdev);
	ret = vdfs_install_check_layout_version(image_inode);
	if (ret)
		goto exit_dput;
...
{
	struct inode *install_point_inode = install_dir->f_path.dentry->d_inode;
