...
retry:
#endif
		ret = prepare_compressed_file_inode(inode_i);
		if (!ret)
...
long vdfs_dir_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case VDFS_IOC_DATA_LINK: {
		...
		ret = vdfs_data_link_create(filp->f_path.dentry,
	...
	case VDFS_IOC_UNINSTALL: {
		struct inode *parent = filp->f_path.dentry->d_inode;
