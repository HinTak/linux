		...
		int unzip_error)
{
	...
	ibuff += mapped_offset;
	file_name = kmalloc(VDFS_FILE_NAME_LEN, GFP_NOFS);
	...
	struct inode *inode, __u32 chunk_index, __u32 index, int is_fragment)
{
	...
	mapped_packed_pages = vdfs_vm_map_ram(packed_pages, packed_page_count, -1,
			PAGE_KERNEL);
	...
	mapped_opages = vdfs_vm_map_ram(opages, (unsigned)total_pages, -1,
			PAGE_KERNEL);
	if (!mapped_opages) {
		ret = -ENOMEM;
		goto exit;
	...
	if (chunk.compressed) {
		strm.workspace =
			vdfs_vmalloc((unsigned long)zlib_inflate_workspacesize());
		if (!strm.workspace) {
		...
		unsigned nr_pages)
{
	...
	mapped_pages = vdfs_vmap(pages_for_vmap, nr_pages, VM_MAP, PAGE_KERNEL);
	if (!mapped_pages) {
...
again:
	...
	unpacked_mapped_pages = vdfs_vmap(unpacked_pages, TOTAL_PAGES, VM_MAP,
				PAGE_KERNEL);
	...
	unsigned nr_pages)
{
	...
	pages_for_vmap =
		kzalloc(page_count * sizeof(struct page *), GFP_NOFS);
	if (!pages_for_vmap)
		return -ENOMEM;
		...
		struct emmcfs_cattree_key *key)
{
	...
	new_name = kmalloc(key->name_len + 1lu, GFP_NOFS);
	if (!new_name)
		return -ENOMEM;
	...
	void *buffer, size_t buf_size)
{
	...
	mapped_pages = vdfs_vmap(pages, pages_count, VM_MAP, PAGE_KERNEL);
		...
		struct emmcfs_inode_info *inode_i)
{
	...
	void *packed = vdfs_vmap(pages, (unsigned)pages_num, VM_MAP, PAGE_KERNEL);
	void *chunk;
	int count;
	char *file_name = NULL;
	int name_length;
		...
		enum compr_type compr_type)
{
	...
	strm.workspace = vdfs_vmalloc((unsigned long)zlib_inflate_workspacesize());
	if (!strm.workspace)
		return -ENOMEM;

	packed = vdfs_vm_map_ram(src, (unsigned)pages_num, -1, PAGE_KERNEL);
	if (!packed) {
		vfree(strm.workspace);
		return -ENOMEM;
	}

	unpacked = vdfs_vm_map_ram(dst, (unsigned)pages_per_chunk, -1, PAGE_KERNEL);
	if (!unpacked) {
		...
		int pages_per_chunk, int offset, int len_bytes)
{
	int res = 0;
	size_t out_len;
	void *packed = vdfs_vm_map_ram(src, (unsigned)pages_num, -1, PAGE_KERNEL);
	void *unpacked = NULL;
