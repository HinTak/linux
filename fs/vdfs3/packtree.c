		...
		struct inode *packtree_root)
{
	...
	packtree_info = kzalloc(sizeof(*packtree_info), GFP_NOFS);
	if (!packtree_info)
		return ERR_PTR(-ENOMEM);

	packtree_info->tree = kzalloc(sizeof(struct vdfs_btree), GFP_NOFS);
		...
		struct ioctl_install_params *pm)
{
	...
	struct emmcfs_inode_info *image_inode =
			EMMCFS_I(image_file->f_path.dentry->d_inode);
	...
	ipv->source_image.name_len = (u8)image_file->f_path.dentry->d_name.len;
	memcpy(ipv->source_image.name, image_file->f_path.dentry->d_name.name,
		...
		void *data)
{
	...
	struct page **pages =
			kzalloc(pages_cnt * sizeof(struct page *), GFP_NOFS);
	if (!pages)
		return -ENOMEM;
	...
	mapped_pages = vdfs_vmap(pages, pages_cnt,  VM_MAP, PAGE_KERNEL);
	if (!mapped_pages) {
		ret = -ENOMEM;
		goto exit;
	}
