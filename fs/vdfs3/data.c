...
#if !defined(CONFIG_VD_RELEASE) && defined(CONFIG_ORSAY_DEBUG) && defined (CONFIG_ARCH_SDP1404)
		// For Orsay HawkP AV debuging, force oops happens when crc can't be updated because of no enough VM
		BUG();
#endif
		return -ENOMEM;
	}
		...
		unsigned int pages_count, int start_block)
{
	...
	for (count = 0; count < pages_count; count++) {
		...
		if (last_block + blocks_per_page != block) {
			int pages_left = pages_count - count;
			sector_t pages_on_device = blocks_num - block;
			int max_pages_num = min_t(sector_t, pages_left,
					pages_on_device);
			if (bio)
				submit_bio(READ, bio);
again:
		...
		struct writeback_control *wbc, void *data)
{
	...
	if (page_has_buffers(page)) {
		...
		if (buffer_mapped(bh)) {
			...
			*/
			if (buffer_dirty(bh)) {
				if (vdfs_get_int_block(inode, block_in_file, bh, 1, 0)) {
...
confused:
	...
	/*
	 * vdfs_writepage cannot perform delayed allocations
	 */
		...
		const char *name, unsigned int length)
{
	...
	if (!IS_ERR(fd)) {
		...
		if (written < 0) {
			EMMCFS_ERR("cannot write to file %s err:%zd",
					path, written);
			ret = (int)written;
			goto exit;
		}
