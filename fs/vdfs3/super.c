...
void vdfs_init_inode(struct emmcfs_inode_info *inode)
{
	...
	inode->compr_type = VDFS_COMPR_UNDEF;
...
static struct inode *vdfs_alloc_inode(struct super_block *sb)
{
	...
	inode = kmem_cache_alloc(vdfs_inode_cachep, GFP_NOFS);
	if (!inode)
		return NULL;
...
int vdfs_sync_fs(struct super_block *sb, int wait)
{
	...
	if (wait) {
		...
		list_for_each(iter, &sbi->orphan_inodes) {
			struct emmcfs_inode_info *inode_i = list_entry(iter,
			...
			if ((inode->i_state & (I_FREEING|I_WILL_FREE|I_NEW)) ||
				...
				if ((S_ISREG(inode->i_mode) ||
					...
					mutex_w_lock(sbi->catalog_tree->
							rw_tree_lock);
			/* an error is handled in __vdfs_write_inode */
					__vdfs_write_inode(sbi, inode);
...
static void vdfs_save_debug_area(struct vdfs_sb_info *sbi)
{
	...
	debug_area = vdfs_vmap(debug_pages, debug_page_count, VM_MAP, PAGE_KERNEL);
	if (!debug_area)
		goto exit_free_page;
		...
		unsigned char *hash)
{
	...
	sdescsha1 = kmalloc(size, GFP_NOFS);
	if (!sdescsha1) {
...
int calculate_hash_md5(unsigned char *buf, unsigned int buf_len,
		unsigned char *hash)
{
	int ret = 0;
	unsigned int size;
	...
	sdescmd5 = kmalloc(size, GFP_NOFS);
	if (!sdescmd5) {
	...
	ret = crypto_shash_init(&sdescmd5->shash);
	if (ret)
		goto exit;
	ret = crypto_shash_update(&sdescmd5->shash, buf, buf_len);
	if (ret)
		goto exit;
	ret = crypto_shash_final(&sdescmd5->shash, hash);
exit:
	crypto_free_shash(md5);
	kfree(sdescmd5);
	return ret;
...
int vdfs_verify_rsa_sha1_signature(unsigned char *buf, unsigned int buf_len,
		int hash_len, unsigned char *signature)
{
	int ret = 0;
	u8 *hash = kzalloc(VDFS_SUPERBLOCK_CRYPTED_HASH_LEN, GFP_NOFS);
	if (!hash)
		return -ENOMEM;

	ret = calculate_hash_sha1(buf, buf_len,
			hash + VDFS_SUPERBLOCK_CRYPTED_HASH_LEN - hash_len);
	if (ret)
		goto exit;
	ret = verify_rsa_signature_2048(hash, VDFS_SUPERBLOCK_CRYPTED_HASH_LEN,
			signature, VDFS_SUPERBLOCK_CRYPTED_HASH_LEN);
exit:
	kfree(hash);
	return ret;
}

int vdfs_verify_rsa_md5_signature(unsigned char *buf, unsigned int buf_len,
		int hash_len, unsigned char *signature)
{
	int ret = 0;
	u8 *hash = kzalloc(VDFS_FILE_CRYPTED_HASH_LEN, GFP_NOFS);
	if (!hash)
		return -ENOMEM;

	ret = calculate_hash_md5(buf, buf_len,
			hash + VDFS_FILE_CRYPTED_HASH_LEN - hash_len);
	if (ret)
		goto exit;
	ret = verify_rsa_signature(hash, VDFS_FILE_CRYPTED_HASH_LEN,
			signature, VDFS_FILE_CRYPTED_HASH_LEN);
exit:
	kfree(hash);
	return ret;
}
				...
				int silent, int check_signature)
{
	...
	if (check_signature &&
			...
			- sizeof(esb->sb_hash), VDFS_SHA1_LEN, esb->sb_hash)) {
...
static int allocate_hw_input_buffers(void)
{
	...
	if (!hw_buf_addr1) {
		EMMCFS_ERR("Cannot allocate first input buffer for HW1");
	...
	if (!hw_buf_addr2) {
		EMMCFS_ERR("Cannot allocate second input buffer for HW1");
		hw_buf_pages2[0] = NULL;
		return 0;
	}
