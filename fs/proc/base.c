...
#if defined(CONFIG_SECURITY_SFD) && defined(CONFIG_SECURITY_SFD_SECURECONTAINER)
	if(!sf_proc_authorized(current,task))
	{
		return false;
	}
#endif
...
#ifdef CONFIG_SLP_LOWMEM_NOTIFY
		pr_warn_once("%s (%d): /proc/<pid>/oom_adj is deprecated,"
...
enum tv_scenario_process_score
{
	SCENARIO_MGR_TV_DISABLED = 0,
	SCENARIO_MGR_TV_RESTART_SKIP,
	SCENARIO_MGR_TV_BIN_PERM,
	SCENARIO_MGR_TV_BIN_NOT_PERM,
	SCENARIO_MGR_TV_MAX,
};

#define SCNEARIO_MGR_TV_BIN_PATH				"/usr/bin/scenario-mgr-tv"
#define SCENARIO_MGR_TV_PROC_PATH				"tv_scenario"
#define SCENARIO_MGR_TV_PROC_POLICY				SCENARIO_MGR_TV_PROC_PATH"/policy_score"
#define SCENARIO_MGR_TV_PROC_CUR_SCENARIO		SCENARIO_MGR_TV_PROC_PATH"/current_tv_scenario"
#define SCENARIO_MGR_TV_PROC_SCORE_SKIP_RESTART	SCENARIO_MGR_TV_PROC_POLICY"/skip_restart"

#define SCENARIO_MGR_TV_BIN_SIZE		256
#define INT32_STR_SIZE					12

int dynamic_scenario_mgr_tv_enabled;

int scenario_check_permission(void)
{
	struct mm_struct *mm;
	...
	if (!dynamic_scenario_mgr_tv_enabled)
		return 0;

	if (lock_task_sighand(current, &flags)) {
		int scenario_score = current->signal->scenario_score_adj;
		unlock_task_sighand(current, &flags);
		if (scenario_score & (1<<SCENARIO_MGR_TV_BIN_PERM))
			return 1;
		else if(scenario_score & (1<<SCENARIO_MGR_TV_BIN_NOT_PERM))
			return 0;
	}
	...
	mmput(mm);
	if (exe_file) {
		binary = kmalloc(SCENARIO_MGR_TV_BIN_SIZE, GFP_KERNEL);
		if (binary)	{
			char *p;
			p = d_path(&exe_file->f_path, binary, SCENARIO_MGR_TV_BIN_SIZE);

			if (!IS_ERR(p) && !strcmp(p, SCNEARIO_MGR_TV_BIN_PATH)) {
				ret = 1;
				if (lock_task_sighand(current, &flags))
				{
					if (!strcmp(p, SCNEARIO_MGR_TV_BIN_PATH))
						current->signal->scenario_score_adj |= (1<<SCENARIO_MGR_TV_BIN_PERM);
					else
						current->signal->scenario_score_adj |= (1<<SCENARIO_MGR_TV_BIN_NOT_PERM);
					unlock_task_sighand(current, &flags);
				}
			}
...
static ssize_t scenario_score_adj_read(struct file *file, char __user *buf,
					size_t count, loff_t *ppos)
{
	struct task_struct *task = get_proc_task(file_inode(file));
	char buffer[INT32_STR_SIZE];
	size_t len;
	int scenario_score_adj = 0;
	unsigned long flags;
    ...
    if (lock_task_sighand(task, &flags)) {
        scenario_score_adj = task->signal->scenario_score_adj;
        unlock_task_sighand(task, &flags);
    }
    put_task_struct(task);
	len = snprintf(buffer, sizeof(buffer), "%d", scenario_score_adj);
	return simple_read_from_buffer(buf, count, ppos, buffer, len);
}

static ssize_t scenario_score_adj_write(struct file *file, const char __user *buf,
					size_t count, loff_t *ppos)
{
	struct task_struct *task;
	char buffer[INT32_STR_SIZE];
	unsigned long flags;
	int scenario_score_adj;
	int err;

	if (!scenario_check_permission())
		return -EPERM;
	...
	err = kstrtoint(strstrip(buffer), 0, &scenario_score_adj);
	if (err)
		goto out;
	...
	task->signal->scenario_score_adj = scenario_score_adj;
...
static const struct file_operations proc_source_adj_operations = {
	.read		= scenario_score_adj_read,
	.write		= scenario_score_adj_write,
	.llseek		= generic_file_llseek,
};

int current_tv_scenario;

static ssize_t current_tv_scenario_read(struct file *file, char __user *buf,
					size_t count, loff_t *ppos)
{
	char buffer[INT32_STR_SIZE];
	size_t len;
	len = snprintf(buffer, sizeof(buffer), "%d", current_tv_scenario);
	return simple_read_from_buffer(buf, count, ppos, buffer, len);
}

static ssize_t current_tv_scenario_write(struct file *file, const char __user *buf,
					size_t count, loff_t *ppos)
{
	char buffer[INT32_STR_SIZE];
	int val;
	...
	current_tv_scenario = val;

out:
	return err < 0 ? err : count;
}

static int __init set_dynamic_scenario_mgr_tv_disable(char *str)
{
	dynamic_scenario_mgr_tv_enabled = 1;
	return 0;
}
early_param("only_entry_model", set_dynamic_scenario_mgr_tv_disable);


static ssize_t policy_skip_restart(struct file *file, char __user *buf,
					size_t count, loff_t *ppos)
{
	char buffer[INT32_STR_SIZE];
	size_t len;
	...
	if (dynamic_scenario_mgr_tv_enabled)
		len = snprintf(buffer, sizeof(buffer), "%d", SCENARIO_MGR_TV_RESTART_SKIP);
	else
		len = snprintf(buffer, sizeof(buffer), "%d", SCENARIO_MGR_TV_DISABLED);

	return simple_read_from_buffer(buf, count, ppos, buffer, len);
}

static const struct file_operations proc_current_tv_scenario_operations = {
    .read       = current_tv_scenario_read,
    .write      = current_tv_scenario_write,
    .llseek     = generic_file_llseek,
};

static const struct file_operations proc_policy_skip_restart_operations = {
    .read       = policy_skip_restart,
    .llseek     = generic_file_llseek,
};


static int __init current_tv_scenario_init(void)
{
	proc_mkdir(SCENARIO_MGR_TV_PROC_PATH, NULL);
	proc_create(SCENARIO_MGR_TV_PROC_CUR_SCENARIO, 0, NULL, 
			&proc_current_tv_scenario_operations);
	proc_mkdir(SCENARIO_MGR_TV_PROC_POLICY, NULL);
	proc_create(SCENARIO_MGR_TV_PROC_SCORE_SKIP_RESTART, 0, NULL, 
			&proc_policy_skip_restart_operations);
	return 0;
}
module_init(current_tv_scenario_init);
...
#ifdef CONFIG_SLP_LOWMEM_NOTIFY
		if (memnotify_check_oom_score_adj_admin()) {
			if (memnotify_add_oom_score_adj_item(task,
				oom_score_adj) == -ENOMEM) {
				put_task_struct(task);
				return -ENOMEM;
			}
			put_task_struct(task);
...
static int show_rss(struct seq_file *m, void *v)
{
	struct inode *inode = m->private;
	struct task_struct *task = get_proc_task(inode);
	struct mm_struct *mm;
	...
	seq_printf(m,   " =======================\n"
		...
		"  KernelStack : %7luK\n"
		" Kernel Total : %7luK\n",
		K(get_max_total_rss(mm)), K(max_cnt[0]), K(max_cnt[1]),
		K(max_cnt[2]), K(max_cnt[3]), K(max_cnt[4]), K(max_cnt[5]),
		K(max_cnt[6]),
		K(tot_cur_cnt), K(cur_cnt[0]), K(cur_cnt[1]), K(cur_cnt[2]),
		K(cur_cnt[3]), K(cur_cnt[4]), K(cur_cnt[5]), K(cur_cnt[6]),
		kernel_mem.slab, kernel_mem.slab_unreclaimable,
		kernel_mem.page_table, kernel_mem.vmalloc_used,
		kernel_mem.kernel_stack, kernel_mem.kernel_total);
...
static int show_cpu_affinity_mask(struct seq_file *m, void *v)
{
	int ret = 0;
		...
		size_t count, loff_t *ppos)
{
	...
	if (count >= sizeof(mask_str)) {
		ret = -EINVAL;
		pr_err("Invalid input, max lengh allowed %zd\n", sizeof(mask_str) - 1);
	...
	if (copy_from_user(mask_str, buf, count)) {
		ret = -EINVAL;
		pr_debug("copy_from_user failed, buf %p, count %zd.\n", buf, count);
		goto err;
	}
	...
	pr_debug("mask_str %s, strlen(mask_str) %zd, tmp_mask_str %s  strlen(tmp_mask_str) %zd.\n",
...
#endif
#ifdef CONFIG_CPU_MEMORY_INFO
extern struct file_operations proc_pid_cpu_memory_operations;
#endif
#ifdef CONFIG_GPU_MEMORY_INFO
extern struct file_operations proc_pid_gpu_memory_operations;
#endif
...
#if defined(CONFIG_SECURITY_SFD) && defined(CONFIG_SECURITY_SFD_SECURECONTAINER)
static int proc_pid_zone(struct seq_file *m, struct pid_namespace *ns,
			  struct pid *pid, struct task_struct *task)
{
	#ifdef CONFIG_SECURITY_SFD_SECURECONTAINER_MODE_ENFORCE        
	#define  SECURECONTAINER_PID_ZONE_UEP_LEVEL_ALLOW	'D'
	if(SECURECONTAINER_PID_ZONE_UEP_LEVEL_ALLOW <= current->uepLevel)
	#endif
	{
		seq_printf(m, "%d %d",task->uepLevel, get_ns_type(task->nsproxy->mnt_ns));
	}
...
static const struct pid_entry tgid_base_stuff[] = {
...
#ifdef CONFIG_CPU_MEMORY_INFO
	REG("cpu_memory",	S_IRUGO, proc_pid_cpu_memory_operations),
#endif
#ifdef CONFIG_GPU_MEMORY_INFO
	REG("gpu_memory",	S_IRUGO, proc_pid_gpu_memory_operations),
#endif
	...
	REG("scenario_score_adj", S_IRUGO|S_IWUSR, proc_source_adj_operations),
#ifdef CONFIG_AUDITSYSCALL
...
#endif
#if defined(CONFIG_SECURITY_SFD) && defined(CONFIG_SECURITY_SFD_SECURECONTAINER)
	ONE("zone",      S_IRUSR, proc_pid_zone),
#endif
