...
#ifdef CONFIG_KNBD_ZRAM_DEBUG
extern void print_zram_info(struct seq_file*);
#endif
...
#define K(x) ((x) << (PAGE_SHIFT-10))
	...
	pr_info("tmpfs used memory     : %8lld (kB)\n" 
		...
		"Cached                : %8lu (kB)\n"
		"Shmem                 : %8lu (kB)\n"
		"VmallocUsed           : %8lu (kB) [%6lu kB ioremap, %8lu kB vmalloc, %6lu kB vmap, %6lu kB usermap, %6lu kB vpages]\n"
		,
		total_used,
		K(j.bufferram),
		K(cached),
