...
static struct rw_semaphore rw_target_version;

#ifdef CONFIG_NOT_ALLOW_REBOOT_DURING_COREDUMP

#define MAX_COREDUMP_IN_PROGRESS_BUF		(8)
...
static ssize_t proc_coredump_in_progress_read(struct file *file, char __user *buf,
					size_t count, loff_t *ppos)
{
	char buffer[MAX_COREDUMP_IN_PROGRESS_BUF] = {0};
	size_t len;

	len = (size_t)snprintf(buffer, sizeof(buffer), "%d\n", atomic_read(&coredump_in_progress));

	return simple_read_from_buffer(buf, count, ppos, buffer, len);
}

const struct file_operations proc_coredump_in_progress_ops = {
	.read = proc_coredump_in_progress_read,
};
#endif

#define MAX_COREDUMP_TARGET_VERSION_WRITE_BUF		(128)
					...
					size_t count, loff_t *ppos)
{
	...
	down_read(&rw_target_version);
	ret = simple_read_from_buffer(buf, count, ppos, target_version, strlen(target_version));
	up_read(&rw_target_version);
	return ret;
}

/*
 * read_target_version - provides target version info for external modules.
 *
 * @buffer: destination buffer to be filled with target version info.
 * @size: size of destination buffer.
 * (To avoid trimming of target version info ,pass maximum size of buffer as 128).
 *
 * Return: Number of bytes copied to destination buffer on success and 0 on failure.
 */
int read_target_version(char *buffer, size_t size)
{
	size_t len;

	if (size < 2) {
		pr_err("%s: Buffer size passed is less than required size\n", __func__);
		return 0;
	}
	...
	strncpy(buffer, target_version, len);
	up_read(&rw_target_version);
	/* remove the extra '\n' character at end of buffer */
	if (buffer[len-1] == '\n')
		buffer[--len] = '\0';
	else
		buffer[len] = '\0';
...
EXPORT_SYMBOL(read_target_version);
...
static int __init init_vd_proc_coredump(void)
{
	...
	entry = proc_create("coredump_in_progress", S_IRUSR | S_IRGRP | S_IROTH, NULL,
		&proc_coredump_in_progress_ops);
	if (!entry)
		pr_err("Failed to create coredump_in_progress proc entry\n");
#endif
	init_rwsem(&rw_target_version);
