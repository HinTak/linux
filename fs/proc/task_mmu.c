...
extern long get_mm_rss_swap(struct task_struct *p);
static ssize_t cpu_memory_read(struct file *file, char __user *buf, size_t count,
                            loff_t *ppos)
{
	...
	sync_task_rss(p);
	if (likely(p->mm)) {
		cpu_rss = K(get_mm_rss(p->mm)) + K(get_mm_counter(p->mm, MM_SWAPENTS));
	}
	task_unlock(p);
	put_task_struct(p);
...
const struct file_operations proc_pid_cpu_memory_operations = {
	.read	= cpu_memory_read,
};
#endif

#ifdef CONFIG_GPU_MEMORY_INFO
unsigned long (*show_gpu_mem_pid_fn)(pid_t pid);
static ssize_t gpu_memory_read(struct file *file, char __user *buf, size_t count,
                            loff_t *ppos)
{
	...
	pid = task->pid;
	put_task_struct(task);
	if (show_gpu_mem_pid_fn != NULL) {
		gpu_mem = (*show_gpu_mem_pid_fn)(pid);
	}
...
/* Function to register call back to show given pid
 * graphich memory, called once.
 */
int graphic_memory_pid_register(unsigned long(*func)(pid_t pid))
{
	show_gpu_mem_pid_fn = func;
	pr_info("Graphic func.to show pid gpu memory is registered\n");
	return 0;
}
EXPORT_SYMBOL(graphic_memory_pid_register);
const struct file_operations proc_pid_gpu_memory_operations = {
	.read	= gpu_memory_read,
};
#endif
