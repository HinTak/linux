...
static int kpi_fault_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m,"%lu.%02lu|%s|0x%08lx|0x%08lx|%s|%s|dummy\n",
			(unsigned long) kpi_fault_time.tv_sec,
			(kpi_fault_time.tv_nsec / (NSEC_PER_SEC / 100)),
			kpi_fault_comm,	kpi_fault_pc, kpi_fault_lr, kpi_fault_process_comm, kpi_fault_info_status);
...
static int kpi_fault_proc_3rd_show(struct seq_file *m, void *v)
{
	seq_printf(m,"%lu.%02lu|%s|0x%08lx|0x%08lx|%s|%s|dummy\n",
			...
			kpi_fault_3rd_comm,	kpi_fault_3rd_pc, kpi_fault_3rd_lr, kpi_fault_3rd_process_comm, kpi_fault_info_status);
	return 0;
}
		...
		size_t count, loff_t *ppos)
{
	...
	sscanf(buffer,"%s %s",kpi_fault_info_status, kpi_fault_process_comm);	
	strncpy(kpi_fault_comm, kpi_fault_process_comm, TASK_COMM_LEN_BUF);
