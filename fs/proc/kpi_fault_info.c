...
*/
void set_kpi_fault(unsigned long pc, unsigned long lr, char* thread_name, char* process_name, char* type)
{
	//int i;
	
	printk(KERN_ERR "[kpi_fault_info] [k] %d -> p:0x%08lx l:0x%08lx th:%s pro:%s ty:%s\n", write, pc, lr, thread_name, process_name, type);
...
static ssize_t kpi_fault_proc_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
	...
	if(k_fault_info[read].used == 1)
	{
		...
		//if(kpi_fault_call)
		{
			snprintf(buffer, 512, "%lu %lu %s %s %s %c", k_fault_info[read].pc, k_fault_info[read].lr,
						k_fault_info[read].thread_name, k_fault_info[read].process_name, k_fault_info[read].type, '\0');
