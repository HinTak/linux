	...
	else
	{
		...
		else
		{
			...
			else
			{
                ...
                sscanf(buffer, "%s %s ", k_fault_info[write].type , k_fault_info[write].type2 );
				if(!strlen(k_fault_info[write].type2))
				    strncpy(k_fault_info[write].thread_name, k_fault_info[write].type, sizeof(k_fault_info[write].thread_name));
                else 
				    strncpy(k_fault_info[write].thread_name, k_fault_info[write].type2, sizeof(k_fault_info[write].thread_name));
...
int get_robustness_mode(void)
{
    return isRobustnessMode;
}
EXPORT_SYMBOL(get_robustness_mode);
...
int tagerpro_get_nr_threads(void)
{
	return nr_threads;
}

unsigned long tagerpro_nr_running(void)
{
	return nr_running();
}
unsigned long tagerpro_nr_iowait(void)
{
	return nr_iowait();
}
unsigned long tagerpro_nr_iowait_cpu(int cpu)
{
	return nr_iowait_cpu(cpu);
}

#ifdef CONFIG_SWAP
void tagerpro_si_swapinfo(struct sysinfo *val)
{
	return si_swapinfo(val);
}
EXPORT_SYMBOL_GPL(tagerpro_si_swapinfo);
#endif

#ifdef CONFIG_CMA
unsigned long tagerpro_get_totalcma_pages(void)
{
	return totalcma_pages;
}
EXPORT_SYMBOL_GPL(tagerpro_get_totalcma_pages);
#endif

unsigned long tagerpro_vm_commit_limit(void)
{
	return vm_commit_limit();
}

unsigned long tagerpro_total_swapcache_pages(void)
{
	return total_swapcache_pages();
}


EXPORT_SYMBOL_GPL(tagerpro_get_nr_threads);
EXPORT_SYMBOL_GPL(tagerpro_nr_running);
EXPORT_SYMBOL_GPL(tagerpro_nr_iowait);
EXPORT_SYMBOL_GPL(tagerpro_nr_iowait_cpu);
EXPORT_SYMBOL_GPL(tagerpro_vm_commit_limit);
EXPORT_SYMBOL_GPL(tagerpro_total_swapcache_pages);
...
static ssize_t robustness_mode_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
	return simple_read_from_buffer(buf, count, ppos, &isRobustnessMode, sizeof(int));
}

static ssize_t robustness_mode_write(struct file *filp, const char __user *buf, size_t count, loff_t *ppos)
{
    char buffer[BUFFER_SIZE_128];
    mutex_lock(&coredump_control_lock);
    ...
    if(copy_from_user(buffer, buf, count))
    {
        ...
        printk(KERN_ERR "[kpi_fault_info] [Rob Mode] Robustness Mode write function copy_from_user fail\n");
    ...
    isRobustnessMode = (int)simple_strtol(buffer, 0, 10);
...
const struct file_operations robustness_mode_fops = {
    .read  = robustness_mode_read,
    .write = robustness_mode_write,
};
#endif
...
static int __init proc_kpi_fault_info_init(void)
{
    ...
    proc_create("robustness_mode" , 0666, NULL, &robustness_mode_fops);
#endif
