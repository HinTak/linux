...
do_reread:
	...
	if (bnode->mode == EMMCFS_BNODE_MODE_RO)
		prot = PAGE_KERNEL_RO;
#endif
	data = vdfs_vm_map_ram(bnode->pages, btree->pages_per_node, -1, prot);
	if (!data) {
	...
	if (!create && bnode->node_id != 0 &&
		...
		EMMCFS_ERR("Wrong bnode magic: %#x inode=%lu bnode=%d",
		...
		struct vdfs_btree *btree, __u64 version)
{
	...
	bnode_data = vdfs_vm_map_ram(page, btree->pages_per_node,
				-1, PAGE_KERNEL);
	if (!bnode_data) {
