                                       ...
                                       const Uint8* duidHash )
{
    ...
    if ( 0x00 != em[PS_end] )
    {
        // SF_LOG_E( "%s(): Bad signature format: S[ PS_end ] = 0x%02X != 0x00", __FUNCTION__, em[PS_end] );
        return SF_STATUS_UEP_SIGNATURE_INCORRECT;
    }
    ...
    if ( memcmp( hash, em + PS_end + 1 + derLength, hashLength ) )
    {
        ...
        if ( 0 == memcmp( (void *)duidHash, (void *)(em + PS_end + 1 + derLength), HASH_ALGO_LENGTH ) )
        {
...
{
    SF_STATUS r = SF_STATUS_UEP_SIGNATURE_INCORRECT;
    ...
    do
    {
        ...
        r = VerifyEncodedMessage( hash, hLength, em, emLength, RSA_der_enc,
                                  sizeof(RSA_der_enc), duidHash );
        sf_free( em );
    }
    while ( FALSE );
...
SF_STATUS SetupSignatureContext( UepSignatureContext* pCtx )
{
    ...
    if ( pCtx )
    {
        ...
        pCtx->hashCtx.tfm   = crypto_alloc_hash( HASH_ALGO_NAME, 0, CRYPTO_ALG_ASYNC );
        if ( IS_ERR( pCtx->hashCtx.tfm ) )
        {
            SF_LOG_E( "%s(): failed to allocate hash algorithm", __FUNCTION__ );
...
{
    SF_STATUS r = SF_STATUS_BAD_ARG;
    Uint8 hash [ HASH_ALGO_LENGTH ] = { 0 };
    if ( pCtx && pKey && pSignature )
    {
        crypto_hash_final( &pCtx->hashCtx, hash );
        r = CheckSignature( pKey, pSignature, length, hash, HASH_ALGO_LENGTH, duidHash );
        if (r != SF_STATUS_UEP_SIGNATURE_CORRECT)
        {
            SF_LOG_E("%s():[%02x, %02x, %02x, %02x, %02x, %02x, %02x, %02x, \
                             %02x, %02x, %02x, %02x, %02x, %02x, %02x, %02x]", __FUNCTION__, 
