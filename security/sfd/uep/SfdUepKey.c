...
UepKey* CreateKey( const Char* pubRsaN, const Char* pubRsaE )
{
    ...
    if ( NULL == pKey )
    {
        SF_LOG_E( "%s(): Not enough memory", __FUNCTION__ );
        return NULL;
    }
    ...
    if ( NULL == pKey->rsaN || NULL == pKey->rsaE )
    {
        SF_LOG_E( "%s(): failed to mpi_alloc", __FUNCTION__ );
        DestroyKey( pKey );
        return NULL;
    }
    ...
    {
        SF_LOG_E( "%s(): failed to create RSA key from strings", __FUNCTION__ );
        DestroyKey( pKey );
        return NULL;
    }
    return pKey;
