...
void SfdCacheTest()
{
	...
	for( i = 0 ; i < 5 ; i++ )
	{
		SfdCacheAdd( i+1, SFD_MODULE_TYPE_DISPATCHER, SF_STATUS_FAIL );
	}

	printk( "SfdCacheTest test nodes added\n" );
	SfdCacheTestListup();

	if( SF_SUCCESS( SfdCacheCheck( 1, SFD_MODULE_TYPE_DISPATCHER, &status ) ) && SF_STATUS_PENDING != status )
		printk( "SfdCacheTest Test Case #1 PASSED, status:%u\n", status );
	else
		printk( "SfdCacheTest Test Case #1 FAILED\n");

	if( SF_SUCCESS( SfdCacheCheck( 1, SFD_MODULE_TYPE_NOTIFIER, &status ) ) && SF_STATUS_PENDING == status )
	...
	if( SF_FAILED( SfdCacheCheck( 1, SFD_MODULE_TYPE_DISPATCHER, &status ) ) )
		printk( "SfdCacheTest Test Case #3 PASSED, status:%u\n", status );
	else
...
****************************************************************************************************
*/
SF_STATUS SFAPI SfdCacheAdd(unsigned long long i_ino, SFD_MODULE_TYPE moduleType, SF_STATUS status, Uint8 uepLevel)
{
	...
	hash_for_each_possible_safe( g_SfdCacheHashTable, pCache, tmp, hlist, i_ino )
	{
		if( i_ino == pCache->i_ino )
		{
			...
			SF_LOG_I( "SfdCacheAdd Update type:%u, i_ino:%u, status:%u\n",
						moduleType, pCache->i_ino, pCache->results[moduleType] );
	...
	if( SF_FAILED( result ) )
	{
		if( NULL != pNewCache )
		{
			...
			result = SF_STATUS_OK;
#ifdef SFD_CACHE_DEBUG
			SF_LOG_I( "SfdCacheAdd Added type:%u, i_ino:%u, status:%u\n",
						moduleType, pNewCache->i_ino, status );
...
****************************************************************************************************
*/
SF_STATUS SFAPI SfdCacheCheck(unsigned long long i_ino, SFD_MODULE_TYPE moduleType, SF_STATUS * pStatus, Uint8 *pUepLevel)
{
	struct SfdCacheNode * pCache = NULL;
	struct hlist_node *tmp;
	SF_STATUS result = SF_STATUS_FAIL;
	...
	hash_for_each_possible_safe( g_SfdCacheHashTable, pCache, tmp, hlist, i_ino )
	{
		if( i_ino == pCache->i_ino )
		{
			*pStatus = pCache->results[moduleType];
			...
			/*
			SF_LOG_I( "SfdCacheCheck type:%u, Found i_ino:%u, pStatus:%u\n",
					moduleType, pCache->i_ino, *pStatus );
			*/
			result = SF_STATUS_OK;
			break;
		}
