...
static int sf_process_authorized(struct task_struct *cur, struct task_struct *tsk)
{
	...
	if(current_ns->mnt_ns== host_ns->mnt_ns)
	{
		...
		{
			if((target_ns->type >= NS_TYPE_HYBRID) && (cur->uepLevel < SFD_UEP_LEVEL_CONTAINER_PROC))
			{
				return FALSE;
			}
		}
...
int sf_syscall_ns_authorized(struct task_struct *tsk, char* func, int nstype, unsigned long flags)
{
	...
	if ((ns->type >= nstype) || (flags & CLONE_CONTAINER_ISOLATE) || ((flags & CLONE_CONTAINER_HYBRID) && (tsk->uepLevel < SFD_UEP_LEVEL_CONTAINER_API)))
	{
