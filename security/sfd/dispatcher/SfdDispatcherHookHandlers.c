...
{
    // Remove the spec which filter fopen in Tizen5.0
    return 0;
    /*
...
static int sf_process_authorized(struct task_struct *cur, struct task_struct *tsk)
{
	...
	if((current_ns == NULL) || (target_ns == NULL) || (current_ns->mnt_ns == NULL) || (target_ns->mnt_ns == NULL))
		return TRUE;

	if(get_ns_type(current_ns->mnt_ns) == NS_TYPE_DEFAULT)
	{
		if (get_ns_type(target_ns->mnt_ns)>= NS_TYPE_HYBRID)
		{
			if(cur->uepLevel < SFD_UEP_LEVEL_CONTAINER_PROC)
			{
				return FALSE;
			}
		}
...
static int sf_set_report_log(char **cur_cmdline, char **par_cmdline, char **description,struct task_struct *cur)
{

	if((*cur_cmdline != NULL) || (*par_cmdline != NULL) || (*description != NULL))
	{
		return 0;
	}

	*cur_cmdline = (char*)sf_malloc(ZONE_REPORT_LEN_PATH);
	...
	*par_cmdline = (char*)sf_malloc(ZONE_REPORT_LEN_PATH);
	...
	*description = (char*)sf_malloc(ZONE_REPORT_LEN_DESC);
	...
	if(!get_cmdline(cur,*cur_cmdline,ZONE_REPORT_LEN_PATH-1))
	{
		return 0;
	}

	if(!get_cmdline(cur->parent,*par_cmdline,ZONE_REPORT_LEN_PATH-1))
	{
		return 0;
	}
...
static void sf_log_free(char **cur_cmdline, char **par_cmdline,char **description)
{
	if(*cur_cmdline)
	{
		sf_free(*cur_cmdline);
...
int SfdPathFilter(const char *name, const struct path * const path, const char *func)
{
    ...
    if(sip->smk_inode->smk_known[0] == SECURECONTAINER_LABEL)
    {
        if( SFD_UEP_LEVEL_CONTAINER_PATH > current->uepLevel )
        {
			if(sf_set_report_log(&cur_cmdline, &par_cmdline ,&description, current))
			{
				snprintf(description,ZONE_REPORT_LEN_DESC-1,"call:lv%c,path:%s,parent:%s",current->uepLevel,name,par_cmdline);
				SF_LOG_I("[%s] send zone report %s,%s", __FUNCTION__, cur_cmdline, description);

				#ifdef CONFIG_SECURITY_SFD_SECURECONTAINER_MODE_ENFORCE
				if(sf_send_report_file("SECUREZONE","DATA_SZ_PATH",cur_cmdline,description,0))
				{
					SF_LOG_I("[%s] failed to send report %s,%s", __FUNCTION__, cur_cmdline, description);
				}
				#endif
			}

			sf_log_free(&cur_cmdline,&par_cmdline,&description);
...
int sf_syscall_task_authorized(struct task_struct *cur, const char *func)
{
	...
	{
		if(sf_set_report_log(&cur_cmdline, &par_cmdline, &description, cur))
		{
			snprintf(description,ZONE_REPORT_LEN_DESC-1,"syscall:%s,caller:lv%c,parent:%s",func,cur->uepLevel,par_cmdline);
			SF_LOG_I("[%s] send zone report %s,%s", __FUNCTION__, cur_cmdline, description);

			#ifdef CONFIG_SECURITY_SFD_SECURECONTAINER_MODE_ENFORCE
			if(sf_send_report_process("SECUREZONE","DATA_SZ_SYSCALL",cur_cmdline,description,0))
			{
				SF_LOG_I("[%s] failed to send report %s,%s", __FUNCTION__, cur_cmdline, description);
			}
			#endif
...
int sf_syscall_ns_authorized(struct task_struct *tsk, char* func, int nstype, unsigned long flags)
{
	...
	if ((get_ns_type(ns->mnt_ns) >= nstype) || (flags & CLONE_CONTAINER_ISOLATE) || ((flags & CLONE_CONTAINER_HYBRID) && (tsk->uepLevel < SFD_UEP_LEVEL_CONTAINER_API)))
	{
...
int sf_security_bprm_check( struct linux_binprm* pBinaryParameters )
{
    ...
    if ( pBinaryParameters && pBinaryParameters->file )
    {
        ...
        // if ( SF_SUCCESS( result ) || (pBinaryParameters->buf[0] == "#" && pBinaryParameters->buf[1] =="!") )
        {
            ...
            sf_memset(args.szMntInfo, 0x00, sizeof(args.szMntInfo));
            sf_memset(args.szMntFlag, 0x00, sizeof(args.szMntFlag));
            sf_memset(args.szRetInfo, 0x00, sizeof(args.szRetInfo));
            result = SfdProcessOperationThroughModules( &args.header );
            if (args.sysError < 0)
            {
                return args.sysError;
            }
            else if (args.sysError == (int)SF_STATUS_NOT_ENOUGH_BUFFER)
            {
                return -EIO;
            }
        }
...
#define SF_REPORT_DATE_STR_SIZE  12
/*
****************************************************************************************************
*
****************************************************************************************************
...
SF_STATUS SFAPI SfdMakeSysTime(char *pszDate, int size)
{
    ...
    struct rtc_time tm_time_utc;
    struct timespec tspec;

    if ((pszDate == NULL) || (size < 1))
    {
        return SF_STATUS_BAD_ARG;
    }
 
    tspec = current_kernel_time();
    utc_time = (u32)tspec.tv_sec;
    micro_seconds = tspec.tv_nsec / 1000;
    rtc_time_to_tm(utc_time, &tm_time_utc);
    snprintf(pszDate, size, "%02d%02d%02d%06ld", 
        tm_time_utc.tm_hour, tm_time_utc.tm_min, tm_time_utc.tm_sec, micro_seconds);
    return result;
}
...
{
    QueueFormat* pItem = NULL;
    QueueFormat* pTemp = NULL;
    write_lock( &g_SfdSendQueueLock );
    if (g_pDispatcher)
    {
        list_for_each_entry_safe(pItem, pTemp, &g_pDispatcher->queue.noti_que, list)
        {
            if (pItem)
            {
                list_del(&pItem->list);
