...
#define GET_LOW16_MASK  0x0000FFFF
...
/**
****************************************************************************************************
* @brief Check The file exists in RW mount point. only Release mode.
* @param [in] pFile Pointer to the file to be processed.
* @param [in] pName file struct's file path
* @return SF_STATUS_OK if the file is in responsibility of SFD, SF_STATUS_FAIL otherwise
****************************************************************************************************
*/
...
/**
****************************************************************************************************
* @brief		Get unique scramble id using structure address containing device's mount info.
* @param [out] 	scramble_id		
* @param [in]  	sb	
* @param [in]  	bdi		
* @return 		void
****************************************************************************************************
*/
static inline unsigned int SfdGetScrambleId(unsigned int sb, unsigned int bdi)
{
	BUILD_BUG_ON_MSG(!(sizeof(unsigned int) == (sizeof(unsigned long long) / 2)), 
		"This compiling environment is not suit to SFD kernel module. Please contact to euijin.je@samsung.com");
	BUILD_BUG_ON_MSG(sizeof(unsigned int) != 4, 
		"This compiling environment is not suit to SFD kernel module. Please contact to euijin.je@samsung.com");

	return (((sb & GET_LOW16_MASK) << 16) | (bdi & GET_LOW16_MASK));
}
...
static inline unsigned long long SfdGetUniqueIno(struct inode * inode)
{
	...
	unsigned int uscramble_id = 0;

	if ( unlikely(NULL == inode) )
		return 0;
	if ( (NULL == inode->i_sb) || (NULL == inode->i_sb->s_bdi) )
		return 0;

	uscramble_id = SfdGetScrambleId((unsigned int)inode->i_sb, (unsigned int)inode->i_sb->s_bdi);
	uino = (unsigned long long)(int)uscramble_id;
