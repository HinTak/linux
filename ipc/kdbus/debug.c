...
struct kmem_cache *kdbus_trace_dump_cachep;
...
static struct kdbus_trace *kdbus_trace_pickup_bin(struct kdbus_debug* dbg)
{
	...
	if (cand->data)
		...
		cand->data = kmem_cache_zalloc(kdbus_trace_dump_cachep,
						GFP_KERNEL);
...
static void kdbus_trace_free(struct kdbus_debug *dbg)
{
	...
	for (i = 0; i < KDBUS_TRACE_MAX; i++)
		if (trace_bin(dbg->trace, i).data)
			kmem_cache_free(kdbus_trace_dump_cachep,
				(trace_bin(dbg->trace, i).data));
	kfree(dbg->trace);
}
...
#ifdef CONFIG_KDBUS_DEBUG_TRACE
	kdbus_trace_dump_cachep = kmem_cache_create("kdbus_trace_dump_cache",
						sizeof(struct kdbus_trace_dump),
						0, 0, NULL);
	BUG_ON(!kdbus_trace_dump_cachep);
#endif
