					 ...
					 const char *conn_description)
{
	...
	if (atomic_inc_return(&conn->user->connections) > KDBUS_USER_MAX_CONN) {
		pr_err("kdbus: Exceed maximum number of connections (%s/%d)\n",
			current->comm, current->pid);
		kdbus_debug_log("Exceed maximum number of connections (%s/%d)\n",
			current->comm, current->pid);
		kdbus_debug_stat_show(bus, KDBUS_STAT_CONN_NUM);
	...
	conn->tgid = current->tgid;
	strncpy(conn->proc_name, current->group_leader->comm, TASK_COMM_LEN);
#endif

	kdbus_debug_conn_init(conn);

	return conn;

exit_unref:
	pr_err("kdbus: kdbus_conn_new return err, code : %d\n", ret);
	kdbus_pool_slice_release(slice);
...
exit_disconnect:
	pr_err("kdbus: kdbus_conn_connect err, code: %d\n", ret);
	kdbus_conn_release(conn);
			 ...
			 size_t memory, size_t fds)
{
	...
	if (quota->msgs >= KDBUS_CONN_MAX_MSGS) {
		struct task_struct *p = pid_task(c->pid, PIDTYPE_PID);
		pr_err("kdbus: message queue overflow at [P:%s(%d) C:%lld] by [P:%s(%d)]\n", 
			p ? p->comm : "unknown", pid_vnr(c->pid), c->id, current->comm, current->tgid);
		return -ENOBUFS;
	}
			    ...
			    const struct kdbus_name_entry *name)
{
	...
	kdbus_trace_add(conn_dst, KDBUS_TRACE_POOL_INSERTED, staging->msg);
	...
	if (reply) {
#if defined(CONFIG_SMART_DEADLOCK_PROFILE_MODE) || defined(CONFIG_KDBUS_DEBUG)
                reply->timestamp = jiffies;
#endif
#ifdef CONFIG_SMART_DEADLOCK_PROFILE_MODE 
                if (staging->dst_name)
                        strncpy(reply->service_name, staging->dst_name, KDBUS_NAME_MAX_LEN);
                else
                        reply->service_name[0] = '\0';
#endif
...
#ifdef CONFIG_SMART_DEADLOCK_PROFILE_MODE
extern void write_dbus_info(const char *fmt, ...);
extern struct kdbus_domain *kdbus_domain_v4;
#define CONVERT_MS_INTO_SEC(time)  (int)(time / 1000), (int)(time % 1000)
#define kdbus_bus_from_node(_node) \
	container_of((_node), struct kdbus_bus, node)
static void kdbus_conn_dump_all_pending_reply(void)
{
		if (kdbus_domain_v4 != NULL) {
				struct rb_node *n = NULL;
				struct kdbus_node *parent = NULL;
				
				mutex_lock(&kdbus_domain_v4->lock);
				parent = &kdbus_domain_v4->node;
				for (n = rb_first(&parent->children) ; n ; n =  rb_next(n)) {
						...
						if (pos->type != KDBUS_NODE_BUS)
								continue;

						bus = kdbus_bus_from_node(pos);
						down_read(&bus->conn_rwlock);
						hash_for_each(bus->conn_hash, i, conn, hentry) {
								struct kdbus_reply *r, *r_tmp;
								...
								list_for_each_entry_safe(r, r_tmp,
												&conn->reply_list,
												entry) {
										write_dbus_info("|%16s(%-5d) |%16s(%-5d) |%32s\t|   %7llu   |  %2ld.%3ld  |  %-16s |  %16s |\n",
														r->reply_dst->proc_name, r->reply_dst->tgid, r->reply_src->proc_name,
														r->reply_src->tgid, r->service_name[0] != '\0' ? r->service_name : "--------------------",
														r->cookie, CONVERT_MS_INTO_SEC(jiffies_to_msecs(jiffies - r->timestamp)),
														bus->node.name ? bus->node.name : "none", (r->sync && r->waiting) ? "sync_waiting" : "none");
								}
								mutex_unlock(&conn->lock);
						}
						up_read(&bus->conn_rwlock);
				}
				mutex_unlock(&kdbus_domain_v4->lock);
		}
}
EXPORT_SYMBOL(kdbus_conn_dump_all_pending_reply);
#endif
				 ...
				 ktime_t expire)
{
	...
	for (;;) {
		...
		if (!poll_schedule_timeout(&pwq, TASK_INTERRUPTIBLE,
					   &expire, 0)) {
			kdbus_reply_timeout_info(reply_wait);

			kdbus_debug_log("Synchronous call timed out (cookie: %lld)\n",
				reply_wait->cookie);
			kdbus_debug_log("+ Source connection info:\n");
			kdbus_debug_conn_show(reply_wait->reply_dst);
			kdbus_debug_log("+ Destination connection info:\n");
			kdbus_debug_conn_show(reply_wait->reply_src);
			ret = -ETIMEDOUT;
			break;
		}
	...
	if (entry) {
		...
		kdbus_trace_add(conn_src, KDBUS_TRACE_RECV, entry, conn_src->pool);
			    ...
			    struct kdbus_staging *staging)
{
	...
	kdbus_trace_add(src, KDBUS_TRACE_SEND_REPLY, dst, msg);
exit:
					   ...
					   ktime_t exp)
{
	...
	kdbus_trace_add(src, KDBUS_TRACE_SEND_SYNC, dst, msg);
			      ...
			      struct kdbus_staging *staging)
{
	...
	kdbus_trace_add(src, KDBUS_TRACE_SEND_ASYNC, dst, msg);
exit:
	up_read(&bus->name_registry->rwlock);
	kdbus_reply_unref(wait);
	kdbus_conn_unref(dst);
...
int kdbus_cmd_recv(struct kdbus_conn *conn, void __user *argp)
{
	...
	if (!entry) {
		...
		kdbus_trace_add(conn, KDBUS_TRACE_RECV, entry, conn->pool);
