...
char *kdbus_metadata_pid_comm(struct kdbus_conn *conn) {
	return conn->meta_proc->pid_comm;
}
char *kdbus_metadata_tid_comm(struct kdbus_conn *conn) {
	return conn->meta_proc->tid_comm;
}
int kdbus_metadata_tgid(struct kdbus_conn *conn) {
	return pid_vnr(conn->meta_proc->tgid);
}
#endif
