	...
	list_for_each_entry_safe(reply, reply_tmp, &conn->reply_list, entry) {
		...
		if (reply->deadline_ns != 0 && !reply->interrupted) {
			kdbus_notify_reply_timeout(conn->ep->bus, conn->id,
						   reply->cookie);
			kdbus_reply_timeout_info(reply);

			kdbus_debug_log("Asynchronous call timed out (cookie: %lld)\n",
				reply->cookie);
			kdbus_debug_log("+ Source connection info:\n");
			kdbus_debug_conn_show(reply->reply_dst);
			kdbus_debug_log("+ Destination connection info:\n");
			kdbus_debug_conn_show(reply->reply_src);
		}
...
/**
 * kdbus_reply_timeout_info - shows kdbus timeout information.
 * @reply: kdbus_reply object keeps information of message reply.
 *
 * Extracts the usefule information for reply object and logs the same in 
 * case of timeout.
 */

void kdbus_reply_timeout_info(struct kdbus_reply *reply)
{
#ifdef CONFIG_KDBUS_DEBUG
        unsigned int send_sec = 0;
        unsigned int recv_sec = 0;
        unsigned int send_ms = 0;
        unsigned int recv_ms = 0;

        send_ms = jiffies_to_msecs(reply->timestamp);
        send_sec = send_ms / 1000;
        send_ms = ((send_ms) / 10) % 100;

        recv_ms = jiffies_to_msecs(reply->recv_time);
        recv_sec = recv_ms / 1000;
        recv_ms = ((recv_ms) / 10) % 100;

        if (recv_sec > 0) {
                pr_err("kdbus: timeout(%s), S[P:%d(%s) C:%lld @%u.%02u]->D[P:%d(%s) C:%lld @%u.%02u]\n",
                        reply->sync ? "sync" : "async", pid_vnr(reply->reply_dst->pid),
                        kdbus_metadata_pid_comm(reply->reply_dst), reply->reply_dst->id,
                        send_sec, send_ms, pid_vnr(reply->reply_src->pid),
                        kdbus_metadata_pid_comm(reply->reply_src), reply->reply_src->id,
                        recv_sec, recv_ms);
        } else {
                pr_err("kdbus: timeout(%s), S[P:%d(%s) C:%lld @%u.%02u]->D[P:%d(%s) C:%lld,-]\n",
                        reply->sync ? "sync" : "async", pid_vnr(reply->reply_dst->pid),
                        kdbus_metadata_pid_comm(reply->reply_dst), reply->reply_dst->id,
                        send_sec, send_ms, pid_vnr(reply->reply_src->pid),
                        kdbus_metadata_pid_comm(reply->reply_src), reply->reply_src->id);
        ...
        pr_err("kdbus: timeout(%s), src(P:%d C:%lld)->dest(P:%d C:%lld)\n",
                reply->sync ? "sync" : "async", pid_vnr(reply->reply_dst->pid), reply->reply_dst->id,
                pid_vnr(reply->reply_src->pid), reply->reply_src->id);
#endif
}
