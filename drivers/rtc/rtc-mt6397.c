...
static int rtc_regmap_bulk_read(struct regmap *map, unsigned int reg,
				u16 *val, size_t val_count)
{
	int ret=0;
	size_t i;
	unsigned int value;
	for (i = 0; i < val_count; i++) {
		ret = regmap_read(map, reg + (i << 1), &value);
		if (ret < 0)
			return ret;
		*(val + i) = value;
	}
...
static int rtc_regmap_bulk_write(struct regmap *map, unsigned int reg,
				const u16 *val, size_t val_count)
{
	int ret=0;
	size_t i;
	unsigned int value;
	for (i = 0; i < val_count; i++) {
		value = *(val + i);
		ret = regmap_write(map, reg + (i << 1), value);
		if (ret < 0)
			return ret;
	}
	return ret;
...
static void _rtc_get_tick(struct rtc_time *tm)
{
	...
	ret = rtc_regmap_bulk_read(mt_rtc->regmap, mt_rtc->addr_base + RTC_TC_SEC,
			       data, RTC_OFFSET_COUNT);
	if (ret < 0)
		goto exit;
			       ...
			       struct rtc_time *tm, int *sec)
{
	...
	mutex_lock(&rtc->lock);
	ret = rtc_regmap_bulk_read(rtc->regmap, rtc->addr_base + RTC_TC_SEC,
			       data, RTC_OFFSET_COUNT);
	if (ret < 0)
		goto exit;
...
void rtc_read_pwron_alarm(struct rtc_wkalrm *alm)
{
	...
	ret = rtc_regmap_bulk_read(mt_rtc->regmap, mt_rtc->addr_base + RTC_AL_SEC,
			       data, RTC_OFFSET_COUNT);
	if (ret < 0)
		goto exit;
...
static int mtk_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alm)
{
	...
	ret = rtc_regmap_bulk_read(rtc->regmap, rtc->addr_base + RTC_AL_SEC,
			       data, RTC_OFFSET_COUNT);
	if (ret < 0)
		goto err_exit;
