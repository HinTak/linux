...
static void sdp_i2c_message_start(struct sdp_i2c *i2c, struct i2c_msg *msg)
{
	...
	writeb((u8) addr, i2c->regs + SDP_IICDS);
			      ...
			      struct i2c_msg *msgs, int num)
{
	...
	iicstat=readl(i2c->regs + SDP_IICSTAT)&SDP_IICSTAT_BUSBUSY;
	if(iicstat) dev_err(i2c->dev, "port %d recovery is failed, check bus line %x\n", i2c->adap.nr,readl(i2c->regs + SDP_IICSTAT));
			      ...
			      struct i2c_msg *msgs, int num)
{
	...
	if (timeout == 0) {
		pr_err("sdp-i2c CH:%d ID:0x%x timeout\n",i2c->adap.nr,(msgs->addr<<1));
		sdp_i2c_disable_irq(i2c);
		ret = -ETIMEDOUT;
	} else if (ret != num)
		pr_err("sdp-i2c CH:%d ID:0x%x(%d)\n",i2c->adap.nr,(msgs->addr<<1), ret);
...
static int sdp_i2c_bus_init(struct sdp_i2c *i2c)
{
	...
	/* write slave address */
	writeb((u8) pdata->slave_addr, i2c->regs + SDP_IICADD);
...
#ifdef CONFIG_PM
static int sdp_i2c_suspend(struct platform_device *pdev, pm_message_t state)
{
//	pr_info("[%s:%d]\n",__func__,__LINE__);
	is_suspend = true;
...
static int sdp_i2c_resume(struct platform_device *pdev)
{
	struct sdp_i2c *i2c = platform_get_drvdata(pdev);
	int ret;
//	pr_info("[%s:%d]\n",__func__,__LINE__);
	...
	ret = sdp_i2c_bus_init(i2c);
	if (ret != 0)		dev_err(&pdev->dev, "failed to init i2c bus\n");
