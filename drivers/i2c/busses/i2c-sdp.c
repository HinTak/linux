...
static int sdp_eeprom_sw_reset(struct sdp_i2c *i2c){
		...
		int count= timeout;
		int baseaddr_flag=0;
		int baseaddr_value=0;
		
        vbase1 = ioremap(0x580500, 0x100);
		baseaddr_value = (readl(vbase1) >> 8) & 0xFF;
        val = readl(vbase1 + 0xc);
		...
		//EEPROM read base address veriy
        if(baseaddr_value != 0xFC){
			baseaddr_flag=1;
		}

        //pr_info("start  sdp_eeprom_sw_reset function\n");
        if(!trdone_flag || baseaddr_flag)
        {        	 
           // EEPROM SW reset 
           //pr_info("trdone_flag disable and eeprom reset start\n");
           if(baseaddr_flag){
			   val = (readl(vbase1) & 0xFFFF00FF) | 0xFC20;
			   writel(val, vbase1);
           	}
           //switch gpio to CPU
           if(!trdone_flag){
	           ddc_val = readl(vbase2+0x78); //get ddc value
	           writel(ddc_val | 0x08, vbase2+0x78); //scl high
	           val = readl(vbase2+0x74) | 0x00023000; //GPIO control reg : scl output mode
	           writel(val, vbase2+0x74);

	           for(i=0; i<9; i++)
	           {
	                    udelay(1);
	                    writel(ddc_val & ~0x8, vbase2+0x78); //scl low
	                    udelay(1);
	                    writel(ddc_val | 0x8, vbase2+0x78); //scl high
	                    udelay(1);

	                    val = readl(vbase2 + 0x7C);
	                    if(val & 0x10)
	                    {
	           ...
	           ddc_val = readl(vbase2+0x78); //get ddc value
	           writel(ddc_val | 0x18, vbase2+0x78); //scl high
	           val = readl(vbase2+0x74) | 0x00033000; //GPIO control reg : scl/sda output mode
	           writel(val, vbase2+0x74);

	           udelay(1);

	           //start condition
	           writel(ddc_val & ~0x10, vbase2+0x78); //sda low
	           udelay(1);
	           ...
	           writel(ddc_val | 0x8, vbase2+0x78); //scl high
	           udelay(1);
	           writel(ddc_val | 0x10, vbase2+0x78); //sda high
	           udelay(1);

	           val = readl(vbase2+0x74) & (~0x00033000); //GPIO control -> normal
	           writel(val, vbase2+0x74);

           	}
		   
		   //switch CPU to EDID module
           val = readl(vbase2 + 0x10) & (~0x4000);
           writel(val, vbase2 + 0x10);

           //EDID re-load by reset         
           val = readl(vbase1 + 0x8);
           writel(val & ~(0xff), vbase1 + 0x8);
           udelay(1);
           writel(val | 0xff, vbase1 + 0x8);
           
			/* waiting for EDID load done */
			while (count--) {
				if (readl(vbase1 + 0x0c) & 0x100){
			...
			if(baseaddr_flag){
			   val = (readl(vbase1) & ~0x00000020);
			   writel(val, vbase1);
           	}  

			/* I2C4_MAIN_SEL */
			val = readl(vbase2 + 0x10) | (0x1 << 14);
			writel(val, vbase2 + 0x10);
        }
...
static int sdp_i2c_probe(struct platform_device *pdev)
{
     ...
     * switch it to main */
#ifdef CONFIG_ARCH_SDP1501
    if (soc_is_jazz() && (((ulong)i2c->regs & 0xfff) == 0x180) ) {
