...
static bool ntiic_bus_init(enum EN_KER_IIC_BUS enBusId,
	enum EN_KER_IIC_CLOCK bus_clk, enum EN_KER_IIC_DUTY_LEVEL bus_duty,
	enum EN_KER_IIC_ACK_CTRL enAckCtrl, u8 enAckCtrlCounter);
static bool ntiic_set_clk(enum EN_KER_IIC_BUS bus,
	enum EN_KER_IIC_CLOCK bus_clk);
static u32 ntiic_count_timeout(enum EN_KER_IIC_CLOCK bus_clk,
...
static int nt726xx_i2c_xfer_rd(enum EN_KER_IIC_BUS bus,
	enum EN_KER_IIC_CLOCK clkSpd, struct i2c_msg *pmsg,
	struct ST_KEEP_MSGINFO stKpt, bool stop);
static int nt726xx_i2c_xfer_wr(enum EN_KER_IIC_BUS bus,
	enum EN_KER_IIC_CLOCK clkSpd, struct i2c_msg *pmsg,
	struct ST_KEEP_MSGINFO stKpt, bool stop);
...
static void _nt726xx_i2c_err_hdl(unsigned long timeout,
	enum EN_KER_IIC_BUS bus, struct i2c_msg *pmsg);
...
{
	if (timeout == 0)
		I2C_INF_LOG("[i2c fail] Timeout\n");
	else if ((gstI2CIntMailBox[bus].irq_flag & BIT_9) != 0)
		I2C_INF_LOG("[i2c fail] intr_tx_empty\n");
	else if ((gstI2CIntMailBox[bus].irq_flag & BIT_10) != 0)
		I2C_INF_LOG("[i2c fail] intr_rx_full\n");
	else if ((gstI2CIntMailBox[bus].irq_flag & BIT_11) != 0)
		I2C_INF_LOG("[i2c fail] intr_nack\n");
	else if ((gstI2CIntMailBox[bus].irq_flag & BIT_12) != 0)
		I2C_INF_LOG("[i2c fail] intr_clk_str_timeout\n");

	I2C_INF_LOG(
	"[i2c fail][i2c-%d][s=0x%02x][girq=0x%08x][0xc0=0x%08x][0xe8=0x%08x]\n",
	bus, (pmsg->addr << 1),
	gstI2CIntMailBox[bus].irq_flag,
	nt726xx_i2c_rd_reg(bus, _REG_CONTROLREG),
	nt726xx_i2c_rd_reg(bus, _REG_INTCTRL));
	/*_i2c_reset_slaveDev(bus);*/
	_busReset(bus);
}
		...
		enum EN_KER_IIC_ACK_CTRL enAckCtrl, u8 enAckCtrlCounter)
{
	...
	if (enBusId >= EN_KER_IIC_BUS_6)	{
		I2C_INF_LOG("[i2c fail][i2c-%d]\n", enBusId);
		return 0;
	}
	...
	nt726xx_i2c_wr_reg(enBusId, _REG_CLOCK, (clockvalue<<1) | (0<<16));

	FLUSH_REG(nt726xx_i2c_rd_reg(enBusId, _REG_CLOCK));

	I2C_DBG_LOG("@@@@@@before 0xfd000xc8=0x%x\n",
		nt726xx_i2c_rd_reg(enBusId, _REG_ACKCTL));
	nt726xx_i2c_wr_reg(enBusId, _REG_ACKCTL,
		(enAckCtrl << 2) | (enAckCtrlCounter<<3));
	I2C_DBG_LOG("@@@@@@after 0xfd000xc8=0x%x\n",
		nt726xx_i2c_rd_reg(enBusId, _REG_ACKCTL));
	FLUSH_REG(nt726xx_i2c_rd_reg(enBusId, _REG_ACKCTL));
	...
	enum EN_KER_IIC_CLOCK bus_clk)
{
	...
	nt726xx_i2c_wr_reg(bus,
			_REG_CLOCK, (clockvalue<<1) |
			(0<<16));
	FLUSH_REG(nt726xx_i2c_rd_reg(bus, _REG_CLOCK));

	duty = (clockvalue*9 + 10)/20;

	nt726xx_i2c_wr_reg(bus, _REG_CLKDUTY, (duty<<16));
	FLUSH_REG(nt726xx_i2c_rd_reg(bus, _REG_CLKDUTY));
	ret = true;
	return ret;
}
...
static int nt726xx_i2c_hwinit(struct platform_device *pdev, int channel)
{
	...
	*/
	...
	for (busId = 0; busId < NUM_OF_I2C_MASTER; busId++)	{
		ret = ntiic_bus_init(busId, EN_KER_IIC_CLOCK_200K,
		...
		ret = ntiic_set_clk(busId, EN_KER_IIC_CLOCK_200K);
		if (!ret) {
...
static irqreturn_t nt726xx_i2c_isr(int irq, void *dev_info)
{
	...
	unsigned int temp, j, k, count, busId;
	unsigned char *pu8Buffer;
	u32    tempBuffer;

	for (busId = 0; busId < NUM_OF_I2C_MASTER; busId++) {
		temp = (nt726xx_i2c_rd_reg(busId, _REG_INTCTRL) & 0x001F00);
		I2C_DBG_LOG("temp = 0x%08x\n", temp);
		if (temp != 0) {
			if ((temp & BIT_10) != 0) {/*RX full*/
				if (i2c[busId].NumOfBytes >= 16) {
					i2c[busId].NumOfBytes -= 16;
					count = 16;
				} else {
					count = i2c[busId].NumOfBytes;
					i2c[busId].NumOfBytes = 0;
				}
				pu8Buffer = i2c[busId].wPtr;

				if (i2c[busId].PingPongIndex == 0) {
					for (j = 0; j < 4; j++) {
					...
					nt726xx_i2c_wr_reg(busId,
					_REG_PINGPONGCTRL,
					nt726xx_i2c_rd_reg(busId,
					_REG_PINGPONGCTRL) |
					BIT_7);
					i2c[busId].PingPongIndex = 1;
				} else {
				for (j = 0; j < 4; j++) {
					tempBuffer = nt726xx_i2c_rd_reg(
					busId,
					...
					_REG_PINGPONGCTRL) |
					BIT_23);
					i2c[busId].PingPongIndex = 0;
				}
				i2c[busId].wPtr = pu8Buffer;
	...
	struct ST_KEEP_MSGINFO stKpt, bool stop)
{
	...
	u32SubAddress, byteNumber, clkSpd);
	...
	/*down(&g_iic_sem_HW[bus]);*/

	/*set global channel*/
	I2C_DBG_LOG("bus = %d\n", bus);

	gstI2CIntMailBox[bus].irq_flag = 0;

	waitQueue = &gstHWI2C.wait;

	/*bit2: I2C Enable*/
	nt726xx_i2c_wr_reg(bus, _REG_CONTROLREG, BIT_2);
	...
	ntiic_set_clk(bus, __rtnFilterBusClock(clkSpd));
	...
	/*bit count*/
	nt726xx_i2c_wr_reg(bus, _REG_SIZE, ((byteNumber) * 8)<<8);
	/*set "byte alignment*/
	nt726xx_i2c_wr_reg(bus, _REG_PINGPONGCTRL,
	nt726xx_i2c_rd_reg(bus, _REG_PINGPONGCTRL) |
	(BIT_7 | BIT_23 | BIT_13));
	...
	if (byteNumber > 32) {
		...
		nt726xx_i2c_rd_reg(bus, _REG_INTCTRL) |
		BIT_2);
		gstI2CIntMailBox[bus].wPtr = pu8Data;
		gstI2CIntMailBox[bus].NumOfBytes = byteNumber;
		gstI2CIntMailBox[bus].PingPongIndex = 0;
	}
	...
	if ((byteNumber) <= 16) {
		...
		(gstI2CIntMailBox[bus].irq_flag & BIT_8) != 0, HZ * 3);
		/*INTR_FINISH*/
		if ((gstI2CIntMailBox[bus].irq_flag & BIT_8) != 0) {
			if ((nt726xx_i2c_rd_reg(bus,
				_REG_CONTROLREG) &
				BIT_24) != 0) {
				I2C_INF_LOG(
			...
			_nt726xx_i2c_err_hdl(timeout, bus, pmsg);
			up(&g_iic_sem_HW[bus]);
			return -1;
		}
		...
		timeout = wait_event_timeout(*waitQueue,
			(gstI2CIntMailBox[bus].irq_flag & BIT_8) != 0,
			HZ * 6);
		/*INTR_FINISH*/
		if ((gstI2CIntMailBox[bus].irq_flag & BIT_8) != 0) {
			if ((nt726xx_i2c_rd_reg(bus,
		...
		timeout = wait_event_timeout(*waitQueue,
			(gstI2CIntMailBox[bus].irq_flag & BIT_8) != 0,
			HZ * ntiic_count_timeout(__rtnFilterBusClock(clkSpd),
			byteNumber));
		/*INTR_FINISH*/
		if ((gstI2CIntMailBox[bus].irq_flag & BIT_8) != 0) {
			if ((nt726xx_i2c_rd_reg(bus,
			...
			if (gstI2CIntMailBox[bus].NumOfBytes != 0) {
				pu8Data = gstI2CIntMailBox[bus].wPtr;
			if (gstI2CIntMailBox[bus].PingPongIndex == 0) {
				j = NT726xx_FIFO_1;
				I2C_DBG_LOG("PingPongIndex ==0\n");
			} else {
				j = NT726xx_FIFO_2;
				I2C_DBG_LOG("PingPongIndex ==1\n");
			}
				for (i = 0;
				i < gstI2CIntMailBox[bus].NumOfBytes;
				i++)
					*pu8Data++ =
						(nt726xx_i2c_rd_reg(
						bus,
	...
	struct ST_KEEP_MSGINFO stKpt, bool stop)
{
	...
	/*8clear FIFO data*/
	for (i = 0; i < 4; i++) {
		nt726xx_i2c_wr_reg(bus, rtnFIFO_Reg(NT726xx_FIFO_1, i), 0);
		nt726xx_i2c_wr_reg(bus, rtnFIFO_Reg(NT726xx_FIFO_2, i), 0);
	}
	...
	case EN_KER_IIC_NO_SUBADDR:
	{
			nt726xx_i2c_wr_reg(bus, _REG_SBADDR, 0);
			nt726xx_i2c_wr_reg(bus, _REG_CONTROLREG,
			nt726xx_i2c_rd_reg(bus, _REG_CONTROLREG) &
			(~_I2C_CTRL_SUBADDR_ENABLE));
			break;
	}
	case EN_KER_IIC_SUBADDR_8BITS:
	{
			nt726xx_i2c_wr_reg(bus, _REG_SBADDR,
			u32SubAddress);
			nt726xx_i2c_wr_reg(bus, _REG_CONTROLREG,
			nt726xx_i2c_rd_reg(bus, _REG_CONTROLREG) |
			(_I2C_CTRL_SUBADDR_ENABLE));
	...
	case EN_KER_IIC_SUBADDR_TOTAL:
	{
		...
		/*bit count*/
		nt726xx_i2c_wr_reg(bus, _REG_SIZE,
				((byteNumber) * 8)<<8);
		/*set "byte alignment*/
		nt726xx_i2c_wr_reg(bus, _REG_PINGPONGCTRL,
		nt726xx_i2c_rd_reg(bus, _REG_PINGPONGCTRL) |
		...
		nt726xx_i2c_rd_reg(bus, _REG_CONTROLREG) |
		u32CtrlMask);
		nt726xx_i2c_wr_reg(bus, _REG_INTCTRL,
		(BIT_0 | BIT_3 | BIT_4 | BIT_16 |
		BIT_17 | BIT_18 | BIT_19 | BIT_20));
		if ((byteNumber) > 32) {
			/*enable tx empty INT.*/
	...
	if (byteNumber <= 16) {
		...
		/*INTR_FINISH*/
		if ((gstI2CIntMailBox[bus].irq_flag & BIT_8) != 0) {
			if ((nt726xx_i2c_rd_reg(bus, _REG_CONTROLREG) &
				BIT_24) != 0) {
				I2C_INF_LOG(
					"[i2c fail][i2c-%d][s=0x%02x] Dev NACK\n",
					bus, (pmsg->addr << 1));
			...
			int num)
{
	...
	bus = (u32)adap->nr;
	clkSpd = (u32)i2c_dev->bus_clk_rate;
	/*20170425 add dts param bus enable/disable*/
	/* Check if i2c Bus is enable, if not enable, will do nothing*/
	if (!i2c_dev->bus_enable) {
		I2C_INF_LOG("[i2c-%d] DTS is not enable. No support\n",
			adap->nr);
		return 0;
	}
	...
	if (num == 1) {/*no SubAddress, only read or write*/
		...
		if (pmsg->flags & I2C_M_RD) {/*read*/
			r = nt726xx_i2c_xfer_rd(bus, clkSpd,
									pmsg,
									stKpt,
									stop);
		} else {/*write*/
			r = nt726xx_i2c_xfer_wr(bus, clkSpd,
									pmsg,
									stKpt,
									stop);
		}
...
static int _nt726xx_init_md(struct platform_device *pdev)
{
	...
	u32 bus_clock = 0;
	u32 u32_i2c_irq = 0;
	u32 bus_enable = 0;/*20170425 add dts param bus enable/disable*/
	int i;
	...
	/*read DTS(bus_enable)*//*20170425*/
	err = of_property_read_u32(pdev->dev.of_node,
	"bus-enable", &bus_enable);
	if (err) {
		i2c->bus_enable = 0; /* default Disable */
		I2C_DBG_LOG("use default bus_enable set Disable");
	} else {
		i2c->bus_enable = bus_enable;
		I2C_DBG_LOG("use dts bus_enable = %d\n", bus_enable);
	}
...
static void I2C_IRQ_init_timer(void)
{
	pr_crit("I2C IRQ init_timer start(0517_Tizen4.0)\n");
	g_iic_irq_num = 0;
	/* Timer init */
