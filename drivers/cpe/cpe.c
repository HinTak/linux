...
#define u32_IPC_BUF_Addr	0xFD7A8020
#else
...
#if	defined(CONFIG_ARCH_NVT72671D)
    static u32 g_u32DRV_IPC_CMD_OFFSET = 0xFD7A8018;	
#else
...
#define TRIM_DATA_ERROR  (-1)
	...
	if (m_u8trim_init == C_NOT_OK) {
        ...
        CPE_LOG("[CPE] load trim done");     // cy...test		
	}
#endif	
...
#define PACKAGE_TYPE8_ADDR 90      //0x5A
int ntcpe_get_pkgtype(void)
{
	u32 u32Type;

	cpe_read_otp(PACKAGE_TYPE8_ADDR, 1, &u32Type);

	//CPE_LOG("[CPE]PacketType=%x\n",u32Type);
	return (u8)u32Type;

}
EXPORT_SYMBOL(ntcpe_get_pkgtype);

#define TRAN_SEL2_VALID            0x00000080
#define FT85C_TEMPTRIM_VALID       0x00000000
#define CP25C_TEMPTRIM_VALID       0x00000001

#define TRAN_SEL_ADDR              (1989)      //0x7C5, bit[7], 0:use transit_1; 1:use transit_2
#define TRAN1_TEMPTRIM_ADDR        (1993)      //0x7C9, bit[0~1]
#define TRAN2_TEMPTRIM_ADDR        (1995)      //0x7CB, bit[0~1]

#if	defined(CONFIG_ARCH_NVT72172)
#define TEMP_TRIM1_VALID           0x00000008
#define TEMP_TRIM2_VALID           0x00000010
#define TEMP_TRIM3_VALID           0x00000020
#define TEMP_TRIM_VALID_ADDR       (1991)      //0x7C7
#define TEMP_TRIM1_ADDR            (1875)      //0x753
#define TEMP_TRIM2_ADDR            (1883)      //0x75B
#define TEMP_TRIM3_ADDR            (1891)      //0x763
#else
#define TEMP_TRIM1_VALID           0x00000040
#define TEMP_TRIM2_VALID           0x00000080
#define TEMP_TRIM3_VALID           0x00000100
#define TEMP_TRIM_VALID_ADDR       (152)       //0x98
#define TEMP_TRIM1_ADDR            (130)       //0x82
#define TEMP_TRIM2_ADDR            (138)       //0x8A
#define TEMP_TRIM3_ADDR            (146)       //0x92
#endif
int ntcpe_get_thermal_trim(void)
{
	u32   u32Valid = 0, u32Data = 0;

	cpe_read_otp(TEMP_TRIM_VALID_ADDR, 2, &u32Valid);
	CPE_LOG("Thermal Valid=%x\n", u32Valid);

	if ((u32Valid&TEMP_TRIM3_VALID)==TEMP_TRIM3_VALID)
	{
		cpe_read_otp(TEMP_TRIM3_ADDR, 1, &u32Data);
		CPE_LOG("TEMP Trim3 =%x\n", u32Data);
	}
	else if ((u32Valid&TEMP_TRIM2_VALID)==TEMP_TRIM2_VALID)
	{
		cpe_read_otp(TEMP_TRIM2_ADDR, 1, &u32Data);
		CPE_LOG("TEMP Trim2 =%x\n", u32Data);
	}
	else if((u32Valid&TEMP_TRIM1_VALID)==TEMP_TRIM1_VALID)
	{
		cpe_read_otp(TEMP_TRIM1_ADDR, 1, &u32Data);
		CPE_LOG("TEMP Trim1 =%x\n", u32Data);
	}
	else
	{
		CPE_LOG("No Temp(thermal) trim VALID\n");
	    return -1;
	}
...
EXPORT_SYMBOL(ntcpe_get_thermal_trim);

int ntcpe_get_thermal_trimtype(void)
{
	u32   u32Valid = 0, u32Data = 0;

	cpe_read_otp(TRAN_SEL_ADDR, 1, &u32Valid);

	if ((u32Valid&TRAN_SEL2_VALID)==TRAN_SEL2_VALID)
	{
		cpe_read_otp(TRAN2_TEMPTRIM_ADDR, 1, &u32Data);
	}
	else
	{
		cpe_read_otp(TRAN1_TEMPTRIM_ADDR, 1, &u32Data);
	}

	return u32Data;
}
EXPORT_SYMBOL(ntcpe_get_thermal_trimtype);
