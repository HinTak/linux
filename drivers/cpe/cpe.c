...
static u32 g_u32DRV_IPC_CMD_OFFSET;
...
static void check_cpe_revision(void)
{
	u32 m_u32chip_revision = (ioremap_read(IC_REVISION))&0xFFF;

	if ((m_u32chip_revision == 0x458) || (m_u32chip_revision == 0x658)) {
		g_u32DRV_IPC_CMD_OFFSET = 0xFD058B40;
	} else if (m_u32chip_revision == 0x670) {
		g_u32DRV_IPC_CMD_OFFSET = 0xFD058018;
	} else { /*563 or else*/
		g_u32DRV_IPC_CMD_OFFSET = 0xFD058018;
	}
}
int drv_ipc_command(u32 cmdid, u32 cmdoffset, u32 cmdlength, u32 cmdto)
{
	...
	u32 m_cmdoffset = (cmdoffset&0x7F)<<17|((cmdoffset&0x180)>>7)<<6|
					((cmdoffset&0x600)>>9)<<8;
	u32 m_cmdto = (cmdto&0x3F)<<10;
	u32 m_cmdlength = ((cmdlength&0x100)>>8)<<16|(cmdlength&0xFF)<<24;
	u32 m_command = m_cmdid|m_cmdoffset|m_cmdto|m_cmdlength;
	u32 m_u32tmp[4] = {0};
	int timeout;

	if (g_u8ipc_cmd_init == C_NOT_OK) {
		check_cpe_revision();
		g_u8ipc_cmd_init = C_OK;
	}
