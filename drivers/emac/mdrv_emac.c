...
int loopback_write( struct file *filp, const char *buff, unsigned long len, void *data )
{
    ...
    if (buff && !copy_from_user(tmpbuf, buff, len))
    {
		...
		tx_cnt += (Tbuf[cnt] * loopback_pow(10, (int)(len - cnt - 1)));
            }
	    rx_cnt = tx_cnt;
            // start test
...
int loopback_read( char *page, char **start, off_t off, int count, int *eof, void *data )
{
    ...
    result[0] = (rx_cnt <= 0 ? '0' : '1');
    len = snprintf(page, sizeof(result), "%c\n", result[0]);
...
static int MDev_EMAC_rx (struct net_device *dev)
{
    ...
    #else
	if(loop_back_test && (rx_cnt > 0))
        {
            u32 bytes;
	    rx_cnt -= 1; //for loop back test
...
static int MDev_EMAC_setup (struct net_device *dev, unsigned long phy_type)
{
    ...
    if (request_irq(dev->irq, MDev_EMAC_interrupt, IRQF_DISABLED, dev->name, dev))
        return -EBUSY;
...
static void MDev_EMAC_timer_callback(unsigned long value)
{
    ...
    if(ret == -1 && ThisBCE.connected == 1){
        printk(KERN_ALERT "--------PHY Disconnect happen--------\n");
        printk(KERN_ALERT "- KDISCONN: %d=,%d\n",ret,ThisBCE.connected);
        printk(KERN_ALERT "-------------------------------------\n");
    }
    else if(ret == 0 && ThisBCE.connected == 0){
        printk(KERN_ALERT "----------PHY Connect again----------\n");
        printk(KERN_ALERT "- KCONNECT: %d=,%d\n",ret,ThisBCE.connected);
        printk(KERN_ALERT "-------------------------------------\n");
    }
...
static int mstar_emac_drv_suspend(struct platform_device *dev, pm_message_t state)
{
    ...
    //Disable Receiver and Transmitter //
    uRegVal = MHal_EMAC_Read_CTL();
    uRegVal &= (unsigned)(~(EMAC_TE | EMAC_RE));
    MHal_EMAC_Write_CTL(uRegVal);
...
static int mstar_emac_drv_resume(struct platform_device *dev)
{
    ...
    LocPtr = (struct EMAC_private*) netdev_priv(netdev);;
    LocPtr->ep_flag &= (unsigned)(~EP_FLAG_SUSPENDING);
