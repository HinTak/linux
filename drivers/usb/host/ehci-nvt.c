...
void create_ew_debugfs_node(struct ehci_hcd *ehci)
{
	...
	if (ehci->debug_dir) {
		...
		debugfs_create_file("hanshake_fails", S_IRUSR | S_IRGRP,
					ehci->debug_dir, ehci,
					&debug_hanshake_fails_fops);

		debugfs_create_file("device_detect_fails", S_IRUSR | S_IRGRP,
					ehci->debug_dir, ehci,
					&debug_device_detect_fails_fops);

		debugfs_create_file("strength_level", S_IRUSR | S_IRGRP,
					ehci->debug_dir, ehci,
					&debug_strength_level_fops);

		debugfs_create_file("disconnect_threshold", S_IRUSR | S_IRGRP,
					ehci->debug_dir, ehci,
