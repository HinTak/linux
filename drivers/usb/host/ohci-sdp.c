...
struct sdp_ohci_hcd {
	...
	u32	uart_sel_port;
	void __iomem *addr_chk_uart_sel;
	u32	uart_sel_enabled;
	struct delayed_work	work_monitor;	
#endif	
#endif
};
...
#if defined(CONFIG_ARCH_SDP1803)
	if(!of_property_read_u32(np,"uart_sel_port",&cmd)) {
		sdp_ohci->uart_sel_port = cmd;
		dev_info(sdp_ohci->dev,"[OHCI]uart_sel_port[%d]\n",cmd);
	}

	if(!of_property_read_u32(np,"reg_chk_uart_sel",&cmd)) {
		if(cmd)
			sdp_ohci->addr_chk_uart_sel = ioremap(cmd, 0x4);
		dev_info(sdp_ohci->dev,"[OHCI]reg_chk_uart_sel[0x%x]\n",cmd);
	}
#endif
...
#if defined(CONFIG_ARCH_SDP1803)
/******************************/  
//when usb uart bypass function is running,  uart tx data makes DP signal toggle unexpectedly.
//
//in order to prevent incorrect ohci interrupt from occuring,   
//polling the uart sel information  and disable interrupt mask.
/******************************/
static void sdp_ohci_work_monitor(struct work_struct *work)
{
	u32 reg_chk;
	const u32 REG_CHK_MASK = 0x9;
	u32 val, uart_sel_enabled, intrenable;
	unsigned long	flags;
	u32 changed;
	struct delayed_work *delayed_work 
		= container_of(work, struct delayed_work, work);
	struct sdp_ohci_hcd *sdp_ohci 
		= container_of(delayed_work, struct sdp_ohci_hcd,work_monitor);
	struct ohci_hcd	*ohci = hcd_to_ohci(sdp_ohci->hcd);

	if(!sdp_ohci->addr_chk_uart_sel) return;
	
	if (ohci->rh_state != OHCI_RH_RUNNING) {			
		schedule_delayed_work(&sdp_ohci->work_monitor, msecs_to_jiffies(2000));
		return;
	}
	changed = 0;
	

	val = *((volatile unsigned int *) (sdp_ohci->addr_chk_uart_sel));
	uart_sel_enabled = (val&REG_CHK_MASK)?1:0;
	
	if(uart_sel_enabled != sdp_ohci->uart_sel_enabled){
		changed=1;
		if(uart_sel_enabled){
			/*usb_serial is enabled         */ 
			/*disable ochi master interrupt */
			...
			/*usb_serial is disabled        */
			/*enalbe ochi master interrupt  */				
			val = ohci_readl(ohci, &ohci->regs->intrenable);
			spin_lock_irqsave(&ohci->lock, flags);
			if(!(val&OHCI_INTR_MIE)){
	...
	sdp_ohci->uart_sel_enabled = uart_sel_enabled;


	//dev_err(sdp_ohci->dev,"[OHCI]uart_sel doing[%d] \n", uart_sel_enabled);
	if(changed){
		dev_err(sdp_ohci->dev,"[OHCI]uart_sel changed to[%d] \n", uart_sel_enabled);	
	}
	schedule_delayed_work(&sdp_ohci->work_monitor, msecs_to_jiffies(2000));
	
	return;
}
#endif	//defined(CONFIG_ARCH_SDP1803)
...
#if defined(CONFIG_ARCH_SDP1803)	
	sdp_ohci->uart_sel_port = 0;
	sdp_ohci->addr_chk_uart_sel = 0;
	sdp_ohci->uart_sel_enabled = 0;
#endif		
...
#if defined(CONFIG_ARCH_SDP1803)
	if(sdp_ohci->uart_sel_port) {
		INIT_DELAYED_WORK(&sdp_ohci->work_monitor, sdp_ohci_work_monitor);
		schedule_delayed_work(&sdp_ohci->work_monitor, msecs_to_jiffies(1000));
	}
#endif	//defined(CONFIG_ARCH_SDP1803)
...
#if defined(CONFIG_PM) && defined(CONFIG_ARCH_SDP)
...
#if defined(CONFIG_ARCH_SDP1803)
	if(sdp_ohci->uart_sel_port) {
		cancel_delayed_work_sync(&sdp_ohci->work_monitor);
	}
#endif

	ret = ohci_suspend(hcd, device_may_wakeup(dev));
