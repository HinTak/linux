...
void reset_tvkey_dongle(int bus_num)
{
	...
	else {					// Nike.M/L Built-in
		if(bus_num == TVKEY_BUILTIN_USB_1) {		// usb 2-1
			if(soc_is_nikel()) {					// Nike.L
...
int usb_is_ocl(void)
{
	if((_ext_tztv_sys_get_platform_info() == 4) || (_ext_tztv_sys_get_platform_info() == 6)) {
		return 1;
	}
	else {
		return 0;
