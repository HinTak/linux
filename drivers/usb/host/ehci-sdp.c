...
enum sdp_ehci_mode
{
	SDP_NORMAL	= 0x0, 	/* normal */
	SDP_NO_COMP,		/* no companion */
	SDP_NO_OWNER,		/* set no-owner : don't use ehci */ 
	SDP_MODE_TYPE_MAX
};
...
struct sdp_ehci_hcd {
	...
	u32 			mcm_set_val;
	struct sdp_ehci_mcm_cmd *mcm_cmd;

	struct workqueue_struct *wq;
	struct work_struct 	work_thr;
	u32 			w_param;

	int conn_usb_port;			/* port: 0 = BT, 1 = WIFI */
	int (*conn_usbon_fn)(int port);

	u32			irq_affinity;
	bool			async_suspend;
	enum sdp_ehci_mode 	mode;		/* No Companion */
#else
	u32 	owner_flag;
#endif	
};
		...
		,const struct sdp_ehci_mcm_cmd *cmd ,u8 val)
{
	...
	msleep(80); //for MuseM wifi/bt combo
	do {
...
extern unsigned int _ext_tztv_sys_is_serdes_model(void);
extern int _ext_tztv_system_serdes_lock_check(void);
...
static bool __serdes_check_lock(struct device *dev)
{
	...
	for( cnt = 0; cnt < MAX_CNT ; cnt++ ) {
		if (_ext_tztv_system_serdes_lock_check() == 0 ) {
...
static void __cmd_dev_execute(struct sdp_ehci_hcd *sdp_ehci,u32 val)
{
	struct sdp_ehci_mcm_cmd *mcmd = sdp_ehci->mcm_cmd;
	
	if( !mcmd ) return;
	if( sdp_ehci->mcm_set_val == val ) return;
	if( __mcm_get_ctrl_fn(sdp_ehci) ) return;

	/* prepare serdes control */
#ifdef SDP_EHCI_CHK_SERDES
	if(sdp_ehci->conn_usbon_fn 
		&& sdp_ehci->conn_usb_port != SDP_SERDES_INV_PORT) {
		int timeout=500;
		...
		dev_err(sdp_ehci->dev, "serdes usb block port%d %s\n"
				, sdp_ehci->conn_usb_port
				,((timeout >= 0) ? "prepared" : "reset fail."));
	} else {
...
static void sdp_ehci_cmd_worker(struct work_struct *work)
{
	...
	__cmd_dev_execute(sdp_ehci,sdp_ehci->w_param);
}

static void sdp_ehci_cmd_dev(struct sdp_ehci_hcd *sdp_ehci,u32 val,bool use_wq)
{
	if( !sdp_ehci->mcm_cmd ) return;
	
	if( use_wq ) {
		sdp_ehci->w_param = val;
		queue_work(sdp_ehci->wq, &sdp_ehci->work_thr);
	} 
	else {
		__cmd_dev_execute(sdp_ehci,val);
	}
}

/* get platform data from device tree */
static int sdp_ehci_parse_dt
		(struct sdp_ehci_hcd *sdp_ehci,struct device_node* np)
{	
	const resource_size_t A_PORT_ADDR = 0x480000;
	u32 cmd;

	if (!np) {
		dev_err(sdp_ehci->dev,"device node is null.\n");
		return -EINVAL;
	}
	...
	if (!of_property_read_u32(np, "conn_dev", &cmd)) {
		...
		if( !mcmd ) {
			dev_err(sdp_ehci->dev,"no-MEM.\n");
			return -ENOMEM;
		}
		
		mcmd->ctrl = cmd;
		dev_info(sdp_ehci->dev,"[EHCI-conn_dev:0x%X]\n",mcmd->ctrl);
		...
		sdp_ehci->mcm_cmd = mcmd;
#ifdef SDP_EHCI_CHK_SERDES
#ifdef CONFIG_ARCH_SDP1601
		if(!soc_is_sdp1701()		/* No Kant-M2 */
			&& _ext_tztv_sys_is_serdes_model() ) {
			sdp_ehci->conn_usbon_fn = 
				(void*)__find_symbol(sdp_ehci->dev,
						"sdp_conn_usb_on");
			switch(cmd) {
			case SDP_WIFI_CTRL:
				sdp_ehci->conn_usb_port = 1;
				break;
			case SDP_BT_CTRL:
				sdp_ehci->conn_usb_port = 0;
				break;
			default:
				sdp_ehci->conn_usb_port = SDP_SERDES_INV_PORT;
				break;
	...
	if (!of_property_read_u32(np, "mode", &cmd)) {
		enum sdp_ehci_mode v = (enum sdp_ehci_mode)cmd;
		sdp_ehci->mode = (v < SDP_MODE_TYPE_MAX) ? v : SDP_NORMAL;
	}
	else if (!of_property_read_u32(np, "no_companion", &cmd)) {
		sdp_ehci->mode = (cmd == 0x1) ? SDP_NO_COMP : SDP_NORMAL;
	}

#ifdef CONFIG_ARCH_SDP1601
	/* checking bt dev. in Kant-M(No M2) */
	if ( !soc_is_sdp1701() 				/* No Kant-M2 */
		&& sdp_ehci->mode == SDP_NORMAL		/* No Conn. Dev. */
		&& sdp_ehci->hcd			/* A port */
		&& sdp_ehci->hcd->rsrc_start == A_PORT_ADDR ) 
	{	/* BT is connected to OHCI --> don't use BT */
		sdp_ehci->mode = SDP_NO_OWNER;
	}
#endif

	dev_err(sdp_ehci->dev,"[EHCI]MODE - [%s]\n",
		((sdp_ehci->mode == SDP_NORMAL) ? "Normal" :
		((sdp_ehci->mode == SDP_NO_COMP) ? "No Comp." : 
		((sdp_ehci->mode == SDP_NO_OWNER) ? "No Owner(No Use)" : 
			"Invalid Value")))
		);

	if(!of_property_read_u32(np,"irq-affinity",&cmd)) {
		sdp_ehci->irq_affinity = cmd;
		dev_info(sdp_ehci->dev,"[EHCI]irq-affinity[%d]\n",cmd);
	}

	sdp_ehci->async_suspend = of_property_read_bool(np,"async-suspend");
	dev_info(sdp_ehci->dev,"[EHCI]async_suspend[%d]\n"
					,sdp_ehci->async_suspend);
	
	return 0;
}

static void sdp_ehci_set_irq_affinity(struct sdp_ehci_hcd *sdp_ehci,u32 irqnum)
{	
	u32 affinity = sdp_ehci->irq_affinity;
...
static int sdp_ehci_run (struct usb_hcd *hcd)
{
	...
	if( ret == 0 ) {
		...
		sdp_ehci_cmd_dev(sdp_ehci,SDP_DEV_RUN,false);		
	}
	else {
...
static void sdp_ehci_relinquish_port(struct usb_hcd *hcd,int portnum)
{
	...
	if( sdp_ehci != NULL && sdp_ehci->mode == SDP_NO_COMP ) {
...
#ifdef CONFIG_ARCH_SDP
	sdp_ehci->mode = SDP_NORMAL;
	sdp_ehci->mcm_ctrl_fn = NULL;
	sdp_ehci->mcm_cmd = NULL;
	sdp_ehci->mcm_set_val = SDP_DEV_STANDBY;
	sdp_ehci->conn_usbon_fn = NULL;
	sdp_ehci->conn_usb_port = SDP_SERDES_INV_PORT;
	sdp_ehci->irq_affinity = 0;
	sdp_ehci->async_suspend = false;
	
	/* get info. : have connected device / info. */
	if( sdp_ehci_parse_dt(sdp_ehci,pdev->dev.of_node) ) {
...
#ifdef CONFIG_ARCH_SDP
	sdp_ehci_set_irq_affinity(sdp_ehci,(u32)irq);

	if( sdp_ehci->async_suspend ) {
		device_enable_async_suspend(sdp_ehci->dev);
		dev_info(&pdev->dev, "[EHCI]set async suspend/resume\n");
	}

	platform_set_drvdata(pdev, sdp_ehci);

	/* set no-ower : don't use EHCI */
	if( sdp_ehci->mode == SDP_NO_OWNER )
...
static int sdp_ehci_remove(struct platform_device *pdev)
{
	...
	destroy_workqueue(sdp_ehci->wq);
	if(sdp_ehci->mcm_cmd)
		devm_kfree(sdp_ehci->dev,sdp_ehci->mcm_cmd);
#endif
...
#ifdef CONFIG_USB_MODULE	
	device_remove_file(&pdev->dev, &dev_attr_handshake_fails);
        device_remove_file(&pdev->dev, &dev_attr_device_detect_fails);
        device_remove_file(&pdev->dev, &dev_attr_cerr_count);
...
#ifdef CONFIG_ARCH_SDP
	sdp_ehci_cmd_dev(sdp_ehci,SDP_DEV_RUN,true);	
#endif	
