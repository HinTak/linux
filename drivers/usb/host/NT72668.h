...
struct nvt_ehci_platform_data {
	enum EN_SYS_CLK_RST  hclk_rst;
	enum EN_SYS_CLK_RST  pclk_rst;
	enum EN_SYS_CLK_RST  aclk_rst;
	enum EN_SYS_CLK_RST  uclk_rst;
	enum EN_SYS_CLK_RST  hclk_rst_u3;	/* combo with usb30 u3phy */
	enum EN_SYS_CLK_SRC  clk_src;
};

struct nvt_xhci_platform_data {
	enum EN_SYS_CLK_RST  hclk_rst;
	enum EN_SYS_CLK_RST  pclk_rst;
	enum EN_SYS_CLK_RST  aclk_rst;
	enum EN_SYS_CLK_SRC  clk_src_u2;
	enum EN_SYS_CLK_SRC  clk_src_u3;
};

#if defined(CONFIG_USB_EW_FEATURE)
extern int EHCI_TUNE_CERR;
#endif
#endif
