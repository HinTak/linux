...
static int nt72668_u2_phy_init(struct usb_phy *phy)
{
	...
	if (!pphy->usb3_controller) {
		...
		set(0x1 << 11, pphy->apb_regs);		/* enable park mode revision */
		set(0x1 << 12, pphy->apb_regs); 	/* enable short packet interrupt revision */
		set(0x1 << 17, pphy->apb_regs); 	/* enable split transaction MDATA revision */
#endif
	}
