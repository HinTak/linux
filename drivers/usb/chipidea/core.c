...
static irqreturn_t ci_irq(int irq, void *data)
{
	...
	if (ci->in_lpm) {
		if (ci->wakeup_int) {
			/*
			 * This can happen on extcon case, which directly
			 * call ci_irq() if there is ID state change before
			 * the previous real wakeup irq hanlding is done(i.e
			 * runtime resumed).
			 */
			dev_dbg(ci->dev, "Wake up is pending, just wait...\n");
			return IRQ_HANDLED;
		}
		...
		struct ci_hdrc_platform_data *platdata)
{
	...
	if (!IS_ERR(ext_id)) {
		ret = extcon_get_state(cable->edev);
		if (ret)
...
static int ci_extcon_register(struct ci_hdrc *ci)
{
	...
	if (!IS_ERR_OR_NULL(id->edev)) {
		ret = extcon_register_interest(&id->extcon_id_dev,
					id->edev->name, "USB-HOST",
					&id->nb);
		if (ret < 0) {
...
static int ci_hdrc_probe(struct platform_device *pdev)
{
	...
	if (ci->platdata->id_extcon.edev)
		extcon_unregister_interest(
			&ci->platdata->id_extcon.extcon_id_dev);
stop:
	ci_role_destroy(ci);
deinit_phy:
	ci_usb_phy_exit(ci);
