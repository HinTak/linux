...
#define SAM_MULTI_STREAMING_GADGET
...
static int __ref sam_multi_bind(struct usb_composite_dev *cdev)
{
    ...
    if (use_streaming) {
        ...
        // strcpy(the_dev->net->name, "usb2"); 
#ifdef CBR_CHANNEL
	the_dev->cbr = cbr_setup(1, the_dev);
#endif	    
        multi_eem.dev[2] = the_dev;
        ...
        // strcpy(the_dev->net->name, "usb3"); 
#ifdef CBR_CHANNEL
	the_dev->cbr = cbr_setup(2, the_dev);
#endif	    
        multi_eem.dev[3] = the_dev;
    }
#endif
...
#ifdef SAM_MULTI_STREAMING_GADGET
	if(use_streaming) {
        the_dev = multi_eem.dev[3];
#ifdef CBR_CHANNEL
        cbr_cleanup(the_dev->cbr);
#endif
         gether_cleanup();
        multi_eem.dev[3] = NULL;
    }
fail3:
	if(use_streaming) {
        the_dev = multi_eem.dev[2];
#ifdef CBR_CHANNEL
        cbr_cleanup(the_dev->cbr);
#endif
         gether_cleanup();
        multi_eem.dev[2] = NULL;
    }
#endif
fail2:
	if(use_eem) {
...
{
    if(!((use_eem == 1) || (use_storage == 1) || (use_streaming == 1) )) {
