...
typedef int (*cas_filter_t)(struct Scsi_Host *, int *, int *, int *);
...
void register_scsi_cas_filter(int (*filter)(struct Scsi_Host *, int *vid, int *pid, int *serial))
{
...
static struct cas_proto_dongle_ctx dongle_ctx[] = {
						{.idVendor = 0x0, .idProduct = 0x0, .iSerialNumber = 0x0, .valid = 0, .dongle_id = 0},
						{.idVendor = 0x0, .idProduct = 0x0, .iSerialNumber = 0x0, .valid = 0, .dongle_id = 1},
						{.idVendor = 0x0, .idProduct = 0x0, .iSerialNumber = 0x0, .valid = 0, .dongle_id = 2},
						{.idVendor = 0x0, .idProduct = 0x0, .iSerialNumber = 0x0, .valid = 0, .dongle_id = 3}
			};
typedef void (*cas_callback_t)(struct cas_proto_evt_ctx *);
static cas_callback_t cas_callback = NULL;

int find_dongle_update(__u8 serial);
int find_next_dongle(__u8 serial);
...
int find_dongle_update(__u8 serial)
{
	...
	for(index=0;index<MAX_NUM_DONGLE;index++) {
		if(dongle_ctx[index].iSerialNumber == serial){
...
void hcas_probe(struct scsi_device *sdp, struct gendisk *gd)
{
	...
	host = sdp->host;
	cas_serial = 0x0;
	if(host &&  cas_device_filter && cas_device_filter(host, &cas_vid, &cas_pid, &cas_serial)) {
		//add probe event
		...
		evt_ctx->ctx.dongle_id = find_next_dongle(cas_serial);
...
void hcas_remove(struct device *dev)
{
	...
	cas_serial = 0x0;
	if(host &&  cas_device_filter && cas_device_filter(host, &cas_vid, &cas_pid, &cas_serial)) {
		dongle_id = find_dongle_update(cas_serial);
