...
#define VERSION_STR		"v0.7(20181206 fix IO Unit size 4byte, add io delay attrebute)"
...
static unsigned long mcmif_chunk_delay = 300;//delay us
...
static int sdp_chunk_memcpy(void *dst, const void *src, size_t num) {
	...
	for(cur = 0; cur < chunks_len; cur += CHUNK_SIZE) {
		...
		udelay(mcmif_chunk_delay);/* for bus release */
			...
			size_t *retlen, u_char *buf)
{
	...
	if(remap_offset) {		
		remap_offset = (typeof(loff_t))0x10000 * (0x1 << remap_offset);
...
static DEVICE_ULONG_ATTR(chunk_delay, 0644, mcmif_chunk_delay);

static struct device_attribute *sdp_mc_memif_attrs[] = {
	&dev_attr_chunk_delay.attr,
};
...
static int sdp_mc_memif_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(sdp_mc_memif_attrs) && sdp_mc_memif_attrs[i]; i++) {
		device_create_file(&pdev->dev, sdp_mc_memif_attrs[i]);
	}
