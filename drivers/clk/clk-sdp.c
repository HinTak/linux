...
static void __iomem *reg_base_mp = NULL;
static phys_addr_t reg_phy_base_mp = 0;
static phys_addr_t reg_phy_end_mp = 0;
...
static struct sdp_gate sdp1202_gate[] __initdata = {
	...
	{ .name = "rstn_gzip", .parent = "ahb_clk",
		.flags = CLK_SET_RATE_PARENT | CLK_IGNORE_UNUSED,
		.offset = SW_RESET0, .bit_idx = 23, },
	{ .name = "clk_gzip", .parent = "ahb_clk",
		.flags = CLK_SET_RATE_PARENT | CLK_IGNORE_UNUSED,
		.offset = MASK_CLK0, .bit_idx = 29, },	
	{ .name = "clk_dmap", .parent = "ahb_clk",
		.flags = CLK_SET_RATE_PARENT,
		.offset = MASK_CLK0, .bit_idx = 6, },
	{ .name = "clk_dma", .parent = "ahb_clk",
		.flags = CLK_SET_RATE_PARENT,
		.offset = MASK_CLK0, .bit_idx = 7, },	
	{ .name = "rstn_dma", .parent = "ahb_clk",
		.flags = CLK_SET_RATE_PARENT,
...
static struct sdp_fixed_div sdp1412_fixed_div[] __initdata = {
	...
	{ .name = "spi_clk", .parent = "pll3_ephy",
		.mult = 1, .div = 2*6, .alias = "spi_clk", },
...
static struct sdp_fixed_div sdp1500fpga_fixed_div[] __initdata = {
        { .name = "apb_pclk", .parent = "fin_pll",
                .mult = 1, .div = 64, .alias = "apb_pclk", },
        { .name = "emmc_clk", .parent = "fin_pll",
                .mult = 1, .div = 1, .alias = "emmc_clk", },
};

static struct sdp_fixed_rate sdp1500fpga_fixed_rate[] __initdata = {
        { .name = "fin_pll", .flags = CLK_IS_ROOT, },
};



#define SDP1501_MASK_CTRL0		0x734
#define SDP1501_MASK_CTRL1		0x738
#define SDP1501_MASK_CTRL2		0x73C
#define SDP1501_MASK_CTRL3		0x740
#define SDP1501_MASK_CTRL4		0x744
#define SDP1501_MASK_CTRL5		0x748
#define SDP1501_MASK_CTRL6		0x74C
#define SDP1501_MASK_CTRL7		0x750
#define SDP1501_MASK_CTRL8		0x754
#define SDP1501_MASK_CTRL9		0x758
#define SDP1501_MASK_CTRL10		0x75C
#define SDP1501_MASK_CTRL11		0x760
#define SDP1501_MASK_CTRL12		0x764
#define SDP1501_MASK_CTRL13		0x768

#define SDP1501_SW_RESET0		0x7A8
#define SDP1501_SW_RESET1		0x7AC
#define SDP1501_SW_RESET2		0x7B0
#define SDP1501_SW_RESET3		0x7B4
#define SDP1501_SW_RESET4		0x7B8
#define SDP1501_SW_RESET5		0x7BC
#define SDP1501_SW_RESET6		0x7C0
#define SDP1501_SW_RESET7		0x7C4
#define SDP1501_SW_RESET8		0x7C8
#define SDP1501_SW_RESET9		0x7CC
#define SDP1501_SW_RESET10		0x7D0
#define SDP1501_SW_RESET11		0x7D4
#define SDP1501_SW_RESET12		0x7D8
#define SDP1501_SW_RESET13		0x7DC
#define SDP1501_SW_RESET14		0x7E0


/* Defines and declaration for the clocks belonging to SDP1501 Soc */

static struct sdp_fixed_rate sdp1501_fixed_rate[] __initdata = {
	{ .name = "fin_pll", .flags = CLK_IS_ROOT, },
};

static __initdata struct sdp_pll sdp1501_pll[] = {
	{ .name = "CPUPLL", .parent = "fin_pll",
		.off_ams = 0x000, },

	{ .name = "DDRPLL0", .parent = "fin_pll",
		.off_ams = 0x018, .off_k = 0x01C, },
	{ .name = "DDRPLL1", .parent = "fin_pll",
		.off_ams = 0x034, .off_k = 0x038, },
	{ .name = "DDRPLL2", .parent = "fin_pll",
		.off_ams = 0x050, .off_k = 0x054, },
	{ .name = "DDRPLL3", .parent = "fin_pll",
		.off_ams = 0x06C, .off_k = 0x070, },
	{ .name = "DDRPLL4", .parent = "fin_pll",
		.off_ams = 0x088, .off_k = 0x08C, },

	{ .name = "PULLPLL0", .parent = "fin_pll",
		.off_ams = 0x0A4, .off_k = 0x0A8, },
	{ .name = "PULLPLL1", .parent = "fin_pll",
		.off_ams = 0x0C0, .off_k = 0x0C4, },

	{ .name = "AUDPLL0", .parent = "PULLPLL0",
		.off_ams = 0x0DC, .off_k = 0x0E0, .div = 68, },
	{ .name = "AUDPLL1", .parent = "PULLPLL1",
		.off_ams = 0x0F8, .off_k = 0x0FC, .div = 68, },

	{ .name = "COREPLL0", .parent = "fin_pll",
		.off_ams = 0x114, },
	{ .name = "COREPLL1", .parent = "fin_pll",
		.off_ams = 0x12C, .off_k = 0x130, },

	{ .name = "DPPLL1", .parent = "PULLPLL0",
		.off_ams = 0x148, .off_k = 0x14C, .div = 68, },

	{ .name = "VBY1PLL", .parent = "fin_pll",
		.off_ams = 0x164, .off_k = 0x168, },

	{ .name = "USBPLL", .parent = "fin_pll",
		.off_ams = 0x180, .off_k = 0x184, },

	{ .name = "FRCPLL", .parent = "fin_pll",
		.off_ams = 0x19C, .off_k = 0x1A0, },

	{ .name = "EMACPLL", .parent = "fin_pll",
		.off_ams = 0x1B8, .off_k = 0x1BC, },
};

static struct sdp_fixed_div sdp1501_fixed_div[] __initdata = {
	{ .name = "arm_clk", .parent = "CPUPLL",
		.mult = 1, .div = 1, .alias = "arm0_clk", },

	{ .name = "ddr0_clk", .parent = "DDRPLL0",
		.mult = 1, .div = 2, .alias = "ddr0_clk", },
	{ .name = "ddr1_clk", .parent = "DDRPLL1",
		.mult = 1, .div = 2, .alias = "ddr1_clk", },
	{ .name = "ddr2_clk", .parent = "DDRPLL2",
		.mult = 1, .div = 2, .alias = "ddr2_clk", },
	{ .name = "ddr3_clk", .parent = "DDRPLL3",
		.mult = 1, .div = 2, .alias = "ddr3_clk", },
	{ .name = "ddr4_clk", .parent = "DDRPLL4",
		.mult = 1, .div = 2, .alias = "ddr4_clk", },

	{ .name = "apb_pclk", .parent = "COREPLL0",
		.mult = 1, .div = 8, .alias = "apb_pclk", },
	{ .name = "dma_clk", .parent = "COREPLL0",
		.mult = 1, .div = 8, .alias = "dma_clk", },
	{ .name = "usb2_clk", .parent = "USBPLL",
		.mult = 1, .div = 2, .alias = "usb2_clk", },
	{ .name = "gpu_clk", .parent = "COREPLL0",
		.mult = 1, .div = 4, .alias = "gpu_clk", },
	{ .name = "ahb_clk", .parent = "COREPLL0",
		.mult = 1, .div = 8, .alias = "ahb_clk", },
	{ .name = "emmc_clk", .parent = "PULLPLL1",
		.mult = 1, .div = 2, .alias = "emmc_clk", },
	{ .name = "emac_clk", .parent = "EMACPLL",
		.mult = 1, .div = 8, .alias = "emac_clk", },
};

static struct sdp_gate sdp1501_gate[] __initdata = {
	{ .name = "ga2d_clk", .parent = "PULLPLL0",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1501_MASK_CTRL1, .bit_idx = 0, },

	{ .name = "jpeg1_clk", .parent = "COREPLL0",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1501_MASK_CTRL1, .bit_idx = 16, },
	{ .name = "jpeg0_clk", .parent = "COREPLL0",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1501_MASK_CTRL1, .bit_idx = 20, },
	{ .name = "mfd_pclk", .parent = "COREPLL0",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1501_MASK_CTRL1, .bit_idx = 24, },

	{ .name = "hen_pclk", .parent = "VBY1PLL",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1501_MASK_CTRL3, .bit_idx = 4, },
		
	{ .name = "dvde_hclk", .parent = "PULLPLL0",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1501_MASK_CTRL5, .bit_idx = 12, },

	{ .name = "srp_clk", .parent = "COREPLL0",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1501_MASK_CTRL5, .bit_idx = 20, },


	/* unzip(gzip) clock gating */
	{ .name = "gzip_clk", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1501_MASK_CTRL3, .bit_idx = 8, },
	{ .name = "gzip_rst", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1501_SW_RESET2, .bit_idx = 12, },

/*
	{ .name = "rstn_i2c", .parent = "apb_pclk",
		.flags = CLK_SET_RATE_PARENT,
		.offset = SDP1501_SW_RESET0, .bit_idx =19, },
*/
};

static struct sdp_fixed_div sdp1600fpga_fixed_div[] __initdata = {
        { .name = "apb_pclk", .parent = "fin_pll",
                .mult = 1, .div = 64, .alias = "apb_pclk", },
        { .name = "emmc_clk", .parent = "fin_pll",
...
static struct sdp_fixed_rate sdp1600fpga_fixed_rate[] __initdata = {
        { .name = "fin_pll", .flags = CLK_IS_ROOT, },
};


/* for KantM ES */
#define SDP1601_MASK_CTRLS0		0x590
#define SDP1601_MASK_CTRLS1		0x594
#define SDP1601_MASK_CTRLS2		0x598
#define SDP1601_MASK_CTRLS3		0x59c
#define SDP1601_MASK_CTRLS4		0x5a0
#define SDP1601_MASK_CTRLS5		0x5a4
#define SDP1601_MASK_CTRLS12		0x5a8
#define SDP1601_MASK_CTRLS13		0x5ac

#define SDP1601_SW_RESETS0		0x5e0
#define SDP1601_SW_RESETS1		0x5e4
#define SDP1601_SW_RESETS2		0x5e8
#define SDP1601_SW_RESETS3		0x5ec
#define SDP1601_SW_RESETS4		0x5f0
#define SDP1601_SW_RESETS13		0x5f4
#define SDP1601_SW_RESETS14		0x5f8
#define SDP1601_SW_RESETN0		0x805E0
#define SDP1601_SW_RESETN1		0x805E4
#define SDP1601_SW_RESETN2		0x805E8
#define SDP1601_SW_RESETN3		0x805EC
#define SDP1601_SW_RESETN4		0x805F0
#define SDP1601_SW_RESETN5		0x805F4
#define SDP1601_SW_RESETN6		0x805F8
#define SDP1601_SW_RESETN7		0x805FC
#define SDP1601_SW_RESETN8		0x80600
#define SDP1601_SW_RESETN9		0x80604
#define SDP1601_SW_RESETN10		0x80608
#define SDP1601_SW_RESETN11		0x8060C
#define SDP1601_SW_RESETN12		0x80610
#define SDP1601_SW_RESETN13		0x80614
#define SDP1601_SW_RESETN14		0x80618

/* for kantm clock gate*/
#define SDP1601_MASK_CLOCK0  0x80590
#define SDP1601_MASK_CLOCK2  0x80598
#define SDP1601_MASK_CLOCK7  0x805AC


static struct sdp_fixed_rate sdp1601_fixed_rate[] __initdata = {
        { .name = "fin_pll", .flags = CLK_IS_ROOT, },
};

static __initdata struct sdp_pll sdp1601_pll[] = {
	{ .name = "CPUPLL", .parent = "fin_pll",
		.off_ams = 0x000, },
	{ .name = "COREPLL0", .parent = "fin_pll",
		.off_ams = 0x06C, .off_k = 0x070, },
	...
	{ .name = "DDRPLL2", .parent = "fin_pll",
		.off_ams = 0x050, .off_k = 0x054, },
	{ .name = "PULLPLL0", .parent = "fin_pll",
		.off_ams = 0x80038, .off_k = 0x8003C, },
	{ .name = "PULLPLL1", .parent = "fin_pll",
		.off_ams = 0x80054, .off_k = 0x80058, }, 
	{ .name = "VBY1PLL", .parent = "fin_pll",   
		.off_ams = 0x800E0, .off_k = 0x800E4,},
	{ .name = "FRCPLL", .parent = "fin_pll", 	
		.off_ams = 0x80000, .off_k = 0x80004,},
	{ .name = "GRAHPLL1", .parent = "fin_pll",
		.off_ams = 0x8001C, .off_k = 0x80020,}, 
	{ .name = "USITPLL", .parent = "fin_pll", 
		.off_ams = 0x80118, .off_k = 0x8011C,}, 		
/*	{ .name = "OCMPLL", .parent = "VBY1PLL",  
		.off_ams = 0x800FC, .off_k = 0x80100,},//27.728??*/
	{ .name = "AFEPLL", .parent = "fin_pll",  
		.off_ams = 0x800A8, .off_k = 0x800AC,},
	{ .name = "CHPLL", .parent = "fin_pll",   
		.off_ams = 0x80134, .off_k = 0x80138,},
	{ .name = "DPPLL1", .parent = "PULLPLL0", 		
		.off_ams = 0x800C4, .off_k = 0x800C8, .div = 34,}, 
	{ .name = "AUDPLL0", .parent = "PULLPLL0",			
		.off_ams = 0x80070, .off_k = 0x80074, .div = 34,}, 
	{ .name = "AUDPLL1", .parent = "PULLPLL1",			
		.off_ams = 0x8008c, .off_k = 0x80090, .div = 40,}, 
/*
	{ .name = "USBPLL", .parent = "fin_pll",  //????ams¾îµ÷åÁ?
		.off_ams = 0x800E4, .off_k = 0x800E8, },
		*/
};
/* sample 
static struct sdp_fixed_div sdp1501_fixed_div[] __initdata = {
...
*/
static struct sdp_fixed_div sdp1601_fixed_div[] __initdata = {///ÀÛ¾÷Áß........................
	{ .name = "arm_clk", .parent = "CPUPLL",
		.mult = 1, .div = 1, .alias = "arm0_clk", }, 
	{ .name = "apb_pclk", .parent = "COREPLL0",
		.mult = 1, .div = 8, .alias = "apb_pclk", },
	...
	{ .name = "ddr2_clk", .parent = "DDRPLL2",
		.mult = 1, .div = 2, .alias = "ddr2_clk", },
    { .name = "emmc_clk", .parent = "GRAHPLL1",
        .mult = 1, .div = 1, .alias = "emmc_clk", },
};


static struct sdp_gate sdp1601_gate[] __initdata = {
	{ .name = "ga2d_clk", .parent = "PULLPLL0",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1601_MASK_CLOCK2, .bit_idx = 0, },

	{ .name = "mfd_pclk", .parent = "COREPLL0",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1601_MASK_CLOCK0, .bit_idx = 12, },

	{ .name = "jpeg0_clk", .parent = "FRCPLL",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1601_MASK_CLOCK0, .bit_idx = 8, },

	{ .name = "hen_pclk", .parent = "VBY1PLL",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1601_MASK_CLOCK0, .bit_idx = 4, },

	{ .name = "dvde_hclk", .parent = "PULLPLL1",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1601_MASK_CLOCK7, .bit_idx = 8, },
	{ .name = "srp_clk", .parent = "COREPLL0",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1601_MASK_CLOCK0, .bit_idx = 0, },
	{ .name = "gzip_clk", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1601_MASK_CTRLS2, .bit_idx = 4, },
	{ .name = "gzip_rst", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1601_SW_RESETS3, .bit_idx = 16, },
	{ .name = "spi_rst", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE,
		.offset = SDP1601_SW_RESETN11, .bit_idx = 0, },
	{ .name = "dma_clk", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE,
		.offset = SDP1601_MASK_CLOCK7, .bit_idx = 4, },
	{ .name = "dma_rst", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE,
		.offset = SDP1601_SW_RESETN10, .bit_idx = 28, },
};
...
/* for Muse-M ES */
#define SDP1803_SW_RESET00	0x7d0
#define SDP1803_SW_RESET01	0x7d4
#define SDP1803_SW_RESET02	0x7d8
#define SDP1803_SW_RESET03	0x7dc
#define SDP1803_SW_RESET04	0x7e0
#define SDP1803_SW_RESET05	0x7e4
#define SDP1803_SW_RESET06	0x7e8
#define SDP1803_SW_RESET07	0x7ec
#define SDP1803_SW_RESET08	0x7f0
#define SDP1803_SW_RESET09	0x7f4
#define SDP1803_SW_RESET10	0x7f8
#define SDP1803_SW_RESET11	0x7fc
#define SDP1803_SW_RESET12	0x800
#define SDP1803_SW_RESET13	0x804
#define SDP1803_SW_RESET14	0x808
#define SDP1803_SW_RESET15	0x80c
#define SDP1803_SW_RESET16	0x810
#define SDP1803_SW_RESET17	0x814
#define SDP1803_SW_RESET18	0x818
#define SDP1803_SW_RESET19	0x81c
#define SDP1803_SW_RESET20	0x820
#define SDP1803_SW_RESET21	0x824
#define SDP1803_SW_RESET22	0x828
#define SDP1803_SW_RESET23	0x82c
#define SDP1803_SW_RESET24	0x830

#define SDP1803_CLK_MASK00	0x870
#define SDP1803_CLK_MASK01	0x874
#define SDP1803_CLK_MASK02	0x878
#define SDP1803_CLK_MASK03	0x87c
#define SDP1803_CLK_MASK04	0x880
#define SDP1803_CLK_MASK05	0x884
#define SDP1803_CLK_MASK06	0x888
#define SDP1803_CLK_MASK07	0x88c
#define SDP1803_CLK_MASK08	0x890
#define SDP1803_CLK_MASK09	0x894
#define SDP1803_CLK_MASK10	0x898
#define SDP1803_CLK_MASK11	0x89c
#define SDP1803_CLK_MASK12	0x8a0


static struct sdp_fixed_rate sdp1803_fixed_rate[] __initdata = {
        { .name = "fin_pll", .flags = CLK_IS_ROOT, },
};

static __initdata struct sdp_pll sdp1803_pll[] = {
	{ .name = "COREPLL0", .parent = "fin_pll",
		.off_ams = 0x000, .off_k = 0x004, },
	{ .name = "CPUPLL", .parent = "fin_pll",
		.off_ams = 0x01C, .off_k = 0x020, },
	{ .name = "PULLPLL0", .parent = "fin_pll",
		.off_ams = 0x038, .off_k = 0x03C, },
	{ .name = "PULLPLL1", .parent = "fin_pll",
		.off_ams = 0x054, .off_k = 0x058, },
	{ .name = "FRCPLL", .parent = "fin_pll",
		.off_ams = 0x070, .off_k = 0x074,},
	{ .name = "COREPLL1", .parent = "fin_pll",
		.off_ams = 0x08C, .off_k = 0x090, },
	{ .name = "COREPLL2", .parent = "fin_pll",
		.off_ams = 0x0A8, .off_k = 0x0AC, },
	{ .name = "USITPLL", .parent = "fin_pll", 
		.off_ams = 0x0C4, .off_k = 0x0C8,},
	{ .name = "DPPLL", .parent = "fin_pll",
		.off_ams = 0x0E0, .off_k = 0x0E4,},
	{ .name = "DPPVEPLL", .parent = "fin_pll",
		.off_ams = 0x0FC, .off_k = 0x100,},
	{ .name = "AUDPLL0", .parent = "fin_pll",
		.off_ams = 0x118, .off_k = 0x11C,},
	{ .name = "AUDPLL1", .parent = "fin_pll",
		.off_ams = 0x134, .off_k = 0x138,},
	{ .name = "DDRPLL2", .parent = "fin_pll",
		.off_ams = 0x150, .off_k = 0x154, },
	{ .name = "DDRPLL3", .parent = "fin_pll",
		.off_ams = 0x16C, .off_k = 0x170, },
};

static struct sdp_fixed_div sdp1803_fixed_div[] __initdata = {
	{ .name = "arm_clk", .parent = "CPUPLL",
		.mult = 1, .div = 1, .alias = "arm0_clk", }, 
	{ .name = "apb_pclk", .parent = "COREPLL0",
		.mult = 1, .div = 8, .alias = "apb_pclk", },
	...
	{ .name = "ddr3_clk", .parent = "DDRPLL3",
		.mult = 1, .div = 2, .alias = "ddr3_clk", },
    { .name = "emmc_clk", .parent = "COREPLL2",
        .mult = 1, .div = 2, .alias = "emmc_clk", },
};


static struct sdp_gate sdp1803_gate[] __initdata = {
	{ .name = "mfd_pclk", .parent = "COREPLL0",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1803_CLK_MASK11, .bit_idx = 28, },
	{ .name = "jpeg0_clk", .parent = "FRCPLL",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1803_CLK_MASK10, .bit_idx = 4, },
	{ .name = "hen_pclk", .parent = "FRCPLL",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1803_CLK_MASK10, .bit_idx = 8, },
	{ .name = "dvde_hclk", .parent = "FRCPLL",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1803_CLK_MASK07, .bit_idx = 24, },
	{ .name = "gzip_clk", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1803_CLK_MASK10, .bit_idx = 16, },

	{ .name = "gzip_rst", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1803_SW_RESET16, .bit_idx = 16, },
	{ .name = "sdp_spi", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1803_SW_RESET15, .bit_idx = 28, },
};



/* for Muse-L ES */
#define SDP1804_SW_RESET00	0x7d0
#define SDP1804_SW_RESET01	0x7d4
#define SDP1804_SW_RESET02	0x7d8
#define SDP1804_SW_RESET03	0x7dc
#define SDP1804_SW_RESET04	0x7e0
#define SDP1804_SW_RESET05	0x7e4
#define SDP1804_SW_RESET06	0x7e8
#define SDP1804_SW_RESET07	0x7ec
#define SDP1804_SW_RESET08	0x7f0
#define SDP1804_SW_RESET09	0x7f4
#define SDP1804_SW_RESET10	0x7f8
#define SDP1804_SW_RESET11	0x7fc
#define SDP1804_SW_RESET12	0x800
#define SDP1804_SW_RESET13	0x804
#define SDP1804_SW_RESET14	0x808
#define SDP1804_SW_RESET15	0x80c
#define SDP1804_SW_RESET16	0x810
#define SDP1804_SW_RESET17	0x814
#define SDP1804_SW_RESET18	0x818
#define SDP1804_SW_RESET19	0x81c
#define SDP1804_SW_RESET20	0x820
#define SDP1804_SW_RESET21	0x824
#define SDP1804_SW_RESET22	0x828
#define SDP1804_SW_RESET23	0x82c
#define SDP1804_SW_RESET24	0x830
#define SDP1804_SW_RESET25	0x870
#define SDP1804_SW_RESET26	0x874

#define SDP1804_CLK_MASK00	0x878
#define SDP1804_CLK_MASK01	0x87c
#define SDP1804_CLK_MASK02	0x880
#define SDP1804_CLK_MASK03	0x884
#define SDP1804_CLK_MASK04	0x888
#define SDP1804_CLK_MASK05	0x88c
#define SDP1804_CLK_MASK06	0x890
#define SDP1804_CLK_MASK07	0x894
#define SDP1804_CLK_MASK08	0x898
#define SDP1804_CLK_MASK09	0x89c
#define SDP1804_CLK_MASK10	0x8a0
#define SDP1804_CLK_MASK11	0x8f4
#define SDP1804_CLK_MASK12	0x8f8


static struct sdp_fixed_rate sdp1804_fixed_rate[] __initdata = {
        { .name = "fin_pll", .flags = CLK_IS_ROOT, },
};

static __initdata struct sdp_pll sdp1804_pll[] = {
	{ .name = "COREPLL0", .parent = "fin_pll",
		.off_ams = 0x000, .off_k = 0x004, },
	{ .name = "CPUPLL", .parent = "fin_pll",
		.off_ams = 0x01C, .off_k = 0x020, },
...
static struct sdp_fixed_div sdp1804_fixed_div[] __initdata = {
	{ .name = "arm_clk", .parent = "CPUPLL",
		.mult = 1, .div = 1, .alias = "arm0_clk", }, 
	{ .name = "apb_pclk", .parent = "COREPLL0",
		.mult = 1, .div = 8, .alias = "apb_pclk", },
...
static struct sdp_gate sdp1804_gate[] __initdata = {
	{ .name = "mfd_pclk", .parent = "COREPLL0",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1804_CLK_MASK07, .bit_idx = 28, },
	{ .name = "jpeg0_clk", .parent = "FRCPLL",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1804_CLK_MASK08, .bit_idx = 4, },
	{ .name = "hen_pclk", .parent = "FRCPLL",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1804_CLK_MASK08, .bit_idx = 8, },
	{ .name = "dvde_hclk", .parent = "FRCPLL",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1804_CLK_MASK05, .bit_idx = 24, },
	{ .name = "gzip_clk", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1804_CLK_MASK08, .bit_idx = 16, },

	{ .name = "gzip_rst", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1804_SW_RESET16, .bit_idx = 16, },
	{ .name = "sdp_spi", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP1804_SW_RESET15, .bit_idx = 28, },
};



/* for Nike-M ES */
#define SDP_NIKEM_CLK_RST_REG500_SWRESET	0x7D0
#define SDP_NIKEM_CLK_RST_REG501_SWRESET	0x7D4
#define SDP_NIKEM_CLK_RST_REG502_SWRESET	0x7D8
#define SDP_NIKEM_CLK_RST_REG503_SWRESET	0x7DC
#define SDP_NIKEM_CLK_RST_REG504_SWRESET	0x7E0
#define SDP_NIKEM_CLK_RST_REG505_SWRESET	0x7E4
#define SDP_NIKEM_CLK_RST_REG506_SWRESET	0x7E8
#define SDP_NIKEM_CLK_RST_REG507_SWRESET	0x7EC
#define SDP_NIKEM_CLK_RST_REG508_SWRESET	0x7F0
#define SDP_NIKEM_CLK_RST_REG509_SWRESET	0x7F4
#define SDP_NIKEM_CLK_RST_REG510_SWRESET	0x7F8
#define SDP_NIKEM_CLK_RST_REG511_SWRESET	0x7FC
#define SDP_NIKEM_CLK_RST_REG512_SWRESET	0x800
#define SDP_NIKEM_CLK_RST_REG513_SWRESET	0x804
#define SDP_NIKEM_CLK_RST_REG514_SWRESET	0x808
#define SDP_NIKEM_CLK_RST_REG515_SWRESET	0x80C
#define SDP_NIKEM_CLK_RST_REG516_SWRESET	0x810
#define SDP_NIKEM_CLK_RST_REG517_SWRESET	0x814
#define SDP_NIKEM_CLK_RST_REG518_SWRESET	0x818
#define SDP_NIKEM_CLK_RST_REG519_SWRESET	0x81C
#define SDP_NIKEM_CLK_RST_REG520_SWRESET	0x820
#define SDP_NIKEM_CLK_RST_REG521_SWRESET	0x824
#define SDP_NIKEM_CLK_RST_REG522_SWRESET	0x828
#define SDP_NIKEM_CLK_RST_REG523_SWRESET	0x82C
#define SDP_NIKEM_CLK_RST_REG524_SWRESET	0x830
#define SDP_NIKEM_CLK_RST_REG525_SWRESET	0x834
#define SDP_NIKEM_CLK_RST_REG526_SWRESET	0x838
#define SDP_NIKEM_CLK_RST_REG527_SWRESET	0x83C
#define SDP_NIKEM_CLK_RST_REG528_SWRESET	0x840
#define SDP_NIKEM_CLK_RST_REG529_SWRESET	0x844
#define SDP_NIKEM_CLK_RST_REG530_SWRESET	0x848
#define SDP_NIKEM_CLK_RST_REG531_SWRESET	0x84C
#define SDP_NIKEM_CLK_RST_REG532_SWRESET	0x850
#define SDP_NIKEM_CLK_RST_REG540_SWRESET	0x870
#define SDP_NIKEM_CLK_RST_REG541_SWRESET	0x874
#define SDP_NIKEM_CLK_RST_REG542_SWRESET	0x878
#define SDP_NIKEM_CLK_RST_REG543_SWRESET	0x87C
#define SDP_NIKEM_CLK_RST_REG544_SWRESET	0x880
#define SDP_NIKEM_CLK_RST_REG545_SWRESET	0x884
#define SDP_NIKEM_CLK_RST_REG546_SWRESET	0x888
#define SDP_NIKEM_CLK_RST_REG547_SWRESET	0x88C
#define SDP_NIKEM_CLK_RST_REG548_SWRESET	0x890
#define SDP_NIKEM_CLK_RST_REG549_SWRESET	0x894
#define SDP_NIKEM_CLK_RST_REG550_SWRESET	0x898
#define SDP_NIKEM_CLK_RST_REG551_SWRESET	0x89C
#define SDP_NIKEM_CLK_RST_REG554_SWRESET	0x8A8

#define SDP_NIKEM_CLK_RST_REG552_MASK	0x8A0
#define SDP_NIKEM_CLK_RST_REG553_MASK	0x8A4
#define SDP_NIKEM_CLK_RST_REG554_MASK	0x8A8
#define SDP_NIKEM_CLK_RST_REG555_MASK	0x8AC
#define SDP_NIKEM_CLK_RST_REG556_MASK	0x8B0
#define SDP_NIKEM_CLK_RST_REG557_MASK	0x8B4
#define SDP_NIKEM_CLK_RST_REG558_MASK	0x8B8
#define SDP_NIKEM_CLK_RST_REG559_MASK	0x8BC
#define SDP_NIKEM_CLK_RST_REG560_MASK	0x8C0
#define SDP_NIKEM_CLK_RST_REG561_MASK	0x8C4
#define SDP_NIKEM_CLK_RST_REG562_MASK	0x8C8
#define SDP_NIKEM_CLK_RST_REG580_MASK	0x910
#define SDP_NIKEM_CLK_RST_REG581_MASK	0x914
#define SDP_NIKEM_CLK_RST_REG582_MASK	0x918
#define SDP_NIKEM_CLK_RST_REG583_MASK	0x91C
#define SDP_NIKEM_CLK_RST_REG584_MASK	0x920
#define SDP_NIKEM_CLK_RST_REG585_MASK	0x924
#define SDP_NIKEM_CLK_RST_REG586_MASK	0x928
#define SDP_NIKEM_CLK_RST_REG587_MASK	0x92C
#define SDP_NIKEM_CLK_RST_REG588_MASK	0x930
#define SDP_NIKEM_CLK_RST_REG589_MASK	0x934
#define SDP_NIKEM_CLK_RST_REG590_MASK	0x938
#define SDP_NIKEM_CLK_RST_REG591_MASK	0x93C
#define SDP_NIKEM_CLK_RST_REG592_MASK	0x940
#define SDP_NIKEM_CLK_RST_REG593_MASK	0x944
#define SDP_NIKEM_CLK_RST_REG594_MASK	0x948
#define SDP_NIKEM_CLK_RST_REG595_MASK	0x94C
#define SDP_NIKEM_CLK_RST_REG596_MASK	0x950
#define SDP_NIKEM_CLK_RST_REG597_MASK	0x954
#define SDP_NIKEM_CLK_RST_REG598_MASK	0x958
#define SDP_NIKEM_CLK_RST_REG599_MASK	0x95C
#define SDP_NIKEM_CLK_RST_REG600_MASK	0x960
#define SDP_NIKEM_CLK_RST_REG601_MASK	0x964
#define SDP_NIKEM_CLK_RST_REG602_MASK	0x968
#define SDP_NIKEM_CLK_RST_REG620_MASK	0x9B0
#define SDP_NIKEM_CLK_RST_REG621_MASK	0x9B4
#define SDP_NIKEM_CLK_RST_REG622_MASK	0x9B8
#define SDP_NIKEM_CLK_RST_REG623_MASK	0x9BC


static struct sdp_fixed_rate sdp_nikem_fixed_rate[] __initdata = {
        { .name = "fin_pll", .flags = CLK_IS_ROOT, },
};

static __initdata struct sdp_pll sdp_nikem_pll[] = {
	{ .name = "COREPLL0", .parent = "fin_pll",
		.off_ams = 0x000, .off_k = 0x004, },
	{ .name = "CPUPLL", .parent = "fin_pll",
		.off_ams = 0x01C, .off_k = 0x020, },
	...
	{ .name = "DDRPLL2", .parent = "fin_pll",
		.off_ams = 0x16C, .off_k = 0x170, },
	{ .name = "DDRPLL3", .parent = "fin_pll",
		.off_ams = 0x188, .off_k = 0x18C, },
};

static struct sdp_fixed_div sdp_nikem_fixed_div[] __initdata = {
	{ .name = "arm_clk", .parent = "CPUPLL",
		.mult = 1, .div = 1, .alias = "arm0_clk", }, 
	{ .name = "apb_pclk", .parent = "COREPLL0",
		.mult = 1, .div = 8, .alias = "apb_pclk", },
...
static struct sdp_gate sdp_nikem_gate[] __initdata = {
	{ .name = "mfd_pclk", .parent = "COREPLL0",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP_NIKEM_CLK_RST_REG554_SWRESET, .bit_idx = 12, },
	{ .name = "jpeg0_clk", .parent = "FRCPLL",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP_NIKEM_CLK_RST_REG554_SWRESET, .bit_idx = 8, },
	{ .name = "hen_pclk", .parent = "FRCPLL",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP_NIKEM_CLK_RST_REG554_SWRESET, .bit_idx = 4, },
	{ .name = "dvde_hclk", .parent = "FRCPLL",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP_NIKEM_CLK_RST_REG548_SWRESET, .bit_idx = 0, },
	{ .name = "gzip_rst", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP_NIKEM_CLK_RST_REG529_SWRESET, .bit_idx = 16, },
	{ .name = "gzip_clk", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP_NIKEM_CLK_RST_REG561_MASK, .bit_idx = 8, },
	{ .name = "sdp_spi", .parent = "apb_pclk",
		.flags = CLK_SET_PARENT_GATE | CLK_IGNORE_UNUSED,
		.offset = SDP_NIKEM_CLK_RST_REG531_SWRESET, .bit_idx = 12, },
};
		...
		struct of_device_id *clk_matches)
{
	...
	for_each_matching_node_and_match(np, clk_matches, &match) {
		if (of_property_read_u32(np, "clock-frequency", &freq))
			continue;
		clk[(uintptr_t) match->data].fixed_rate = freq;
	}
		...
		unsigned long parent_rate)
{
	...
	if (pll->reg_k)
	{
		/* Shifting done to handle the case of two 16 bit K values packed in one
		    32 bit register */
		val = __raw_readl((void *)(((uintptr_t)pll->reg_k) & ~((uintptr_t) 0x3)));
		if(((uintptr_t) pll->reg_k) & 0x3)
			pll_k = (s16) (val >> 16);
		else
		...
		struct sdp_pll *clks, unsigned int nr_clk)
{
	...
	for (i = 0; i < (int) nr_clk; i++, clks++) {
		clk = sdp_clk_register_pll_all(clks->name, clks->parent,
				(void *) ((uintptr_t) reg_base + clks->off_ams),
				clks->off_k ? (void *) ((uintptr_t) reg_base + clks->off_k) : NULL, clks->div ? clks->div : 1, 
				clks->pll_type);
		sdp_clk_add_provider(np, clks->name, clk);
		...
		struct sdp_gate *clks, unsigned int nr_clk)
{
	...
	for (i = 0; i < (int) nr_clk; i++, clks++) {
		clk = clk_register_gate(NULL, clks->name, clks->parent,
				clks->flags, (void *) ((uintptr_t) reg_base + clks->offset),
		...
		struct sdp_clocks *sdp_clk)
{
	...
	if(sdp_clk->pll_clk) {
		...
		pr_info("%s: skip pll clock register\n", __func__);
	}

	/* TODO: register mux */

	if(sdp_clk->fixed_div) {
		sdp_clk_register_fixed_div(np, sdp_clk->fixed_div,
			sdp_clk->size_of_fixed_div);
	} else {
		pr_info("%s: skip fixed div register\n", __func__);
	}
	...
	if(sdp_clk->gate_clk) {
		...
		pr_info("%s: skip gate clock register\n", __func__);
	}

}

static void __init sdp_1202_pllclk_init(struct device_node *np,
		struct sdp_clocks *sdp_clk)
{
	void __iomem *base;
	void __iomem *base_mp;

	/* Temoporary: should be removed*/
	struct resource res;
	struct resource pll_res;
	if (of_address_to_resource(np, 0, &res)) {
		pr_err("%s: failed get the clock resource \n", __func__);
		return ;
	}
	...
	if (of_address_to_resource(np, 1,&pll_res)) {
		pr_err("%s: failed get the pll clock resource \n", __func__);
		return ;
	}
	reg_phy_base_mp = pll_res.start;
	reg_phy_end_mp = pll_res.end;

	base_mp = of_iomap(np, 1);

	if (!base_mp) {
		pr_err("%s: failed to map pll clock controller registers,"
				" aborting pll clock initialization\n", __func__);
		return;
	}
	reg_base_mp = base_mp;
...
static void __init sdp1202_clk_init(struct device_node *np)
{
	...
	sdp_1202_pllclk_init(np, &sdp_soc_clk);
...
static void __init sdp1500fpga_clk_init(struct device_node *np)
{
        struct sdp_clocks sdp_soc_clk = {0};

        sdp_soc_clk.fixed_rate = sdp1500fpga_fixed_rate;
        sdp_soc_clk.size_of_fixed_rate = ARRAY_SIZE(sdp1500fpga_fixed_rate);

	sdp_soc_clk.fixed_div = sdp1500fpga_fixed_div;
	sdp_soc_clk.size_of_fixed_div = ARRAY_SIZE(sdp1500fpga_fixed_div);

        sdp_clk_init(np, &sdp_soc_clk);
        set_ap_reg();
        pr_info ("SDP1500fpga %-12s:%10lu Hz\n", "fin_pll", _get_rate("fin_pll"));
}
CLK_OF_DECLARE(sdp_1500fpga_clk, "samsung,sdp1500fpga-clock", sdp1500fpga_clk_init);


static void __init sdp1501_clk_init(struct device_node *np)
{
	struct sdp_clocks sdp_soc_clk;

	sdp_soc_clk.fixed_rate = sdp1501_fixed_rate;
	sdp_soc_clk.size_of_fixed_rate = ARRAY_SIZE(sdp1501_fixed_rate);
	sdp_soc_clk.pll_clk = sdp1501_pll;
	sdp_soc_clk.size_of_pll_clk = ARRAY_SIZE(sdp1501_pll);
	sdp_soc_clk.fixed_div = sdp1501_fixed_div;
	sdp_soc_clk.size_of_fixed_div = ARRAY_SIZE(sdp1501_fixed_div);
	sdp_soc_clk.gate_clk = sdp1501_gate;
	sdp_soc_clk.size_of_gate_clk = ARRAY_SIZE(sdp1501_gate);

	sdp_clk_init(np, &sdp_soc_clk);
	set_ap_reg();
	pr_info ("SDP1501 %-12s:%10lu Hz\n", "fin_pll", _get_rate("fin_pll"));
}
CLK_OF_DECLARE(sdp_1501_clk, "samsung,sdp1501-clock", sdp1501_clk_init);

static void __init sdp1600fpga_clk_init(struct device_node *np)
{
        struct sdp_clocks sdp_soc_clk = {0};

        sdp_soc_clk.fixed_rate = sdp1600fpga_fixed_rate;
        sdp_soc_clk.size_of_fixed_rate = ARRAY_SIZE(sdp1600fpga_fixed_rate);

	sdp_soc_clk.fixed_div = sdp1600fpga_fixed_div;
	sdp_soc_clk.size_of_fixed_div = ARRAY_SIZE(sdp1600fpga_fixed_div);

        sdp_clk_init(np, &sdp_soc_clk);
        set_ap_reg();
        pr_info ("SDP1600fpga %-12s:%10lu Hz\n", "fin_pll", _get_rate("fin_pll"));
}
CLK_OF_DECLARE(sdp_1600fpga_clk, "samsung,sdp1600fpga-clock", sdp1600fpga_clk_init);

static void __init sdp1601_clk_init(struct device_node *np)
{
	struct sdp_clocks sdp_soc_clk;

	sdp_soc_clk.fixed_rate = sdp1601_fixed_rate;
	sdp_soc_clk.size_of_fixed_rate = ARRAY_SIZE(sdp1601_fixed_rate);
	sdp_soc_clk.pll_clk = sdp1601_pll;
	sdp_soc_clk.size_of_pll_clk = ARRAY_SIZE(sdp1601_pll);
	sdp_soc_clk.fixed_div = sdp1601_fixed_div;
	sdp_soc_clk.size_of_fixed_div = ARRAY_SIZE(sdp1601_fixed_div);
	sdp_soc_clk.gate_clk = sdp1601_gate;
	sdp_soc_clk.size_of_gate_clk = ARRAY_SIZE(sdp1601_gate);

	sdp_clk_init(np, &sdp_soc_clk);
	set_ap_reg();
	pr_info ("SDP1601 %-12s:%10lu Hz\n", "fin_pll", _get_rate("fin_pll"));
}
CLK_OF_DECLARE(sdp_1601_clk, "samsung,sdp1601-clock", sdp1601_clk_init);



static void __init sdp1803_clk_init(struct device_node *np)
{
	struct sdp_clocks sdp_soc_clk;

	sdp_soc_clk.fixed_rate = sdp1803_fixed_rate;
	sdp_soc_clk.size_of_fixed_rate = ARRAY_SIZE(sdp1803_fixed_rate);
	sdp_soc_clk.pll_clk = sdp1803_pll;
	sdp_soc_clk.size_of_pll_clk = ARRAY_SIZE(sdp1803_pll);
	sdp_soc_clk.fixed_div = sdp1803_fixed_div;
	sdp_soc_clk.size_of_fixed_div = ARRAY_SIZE(sdp1803_fixed_div);
	sdp_soc_clk.gate_clk = sdp1803_gate;
	sdp_soc_clk.size_of_gate_clk = ARRAY_SIZE(sdp1803_gate);

	sdp_clk_init(np, &sdp_soc_clk);
	set_ap_reg();
	pr_info ("SDP1803 %-12s:%10lu Hz\n", "fin_pll", _get_rate("fin_pll"));
}
/* Muse-M clock declare. */
CLK_OF_DECLARE(sdp_1803_clk, "samsung,sdp1803-clock", sdp1803_clk_init);


static void __init sdp1804_clk_init(struct device_node *np)
{
	struct sdp_clocks sdp_soc_clk;

	sdp_soc_clk.fixed_rate = sdp1804_fixed_rate;
	sdp_soc_clk.size_of_fixed_rate = ARRAY_SIZE(sdp1804_fixed_rate);
	sdp_soc_clk.pll_clk = sdp1804_pll;
	sdp_soc_clk.size_of_pll_clk = ARRAY_SIZE(sdp1804_pll);
	sdp_soc_clk.fixed_div = sdp1804_fixed_div;
	sdp_soc_clk.size_of_fixed_div = ARRAY_SIZE(sdp1804_fixed_div);
	sdp_soc_clk.gate_clk = sdp1804_gate;
	sdp_soc_clk.size_of_gate_clk = ARRAY_SIZE(sdp1804_gate);

	sdp_clk_init(np, &sdp_soc_clk);
	set_ap_reg();
	pr_info ("SDP1804 %-12s:%10lu Hz\n", "fin_pll", _get_rate("fin_pll"));
}
/* Muse-M clock declare. */
CLK_OF_DECLARE(sdp_1804_clk, "samsung,sdp1804-clock", sdp1804_clk_init);


static void __init sdp_nikem_clk_init(struct device_node *np)
{
	struct sdp_clocks sdp_soc_clk;

	sdp_soc_clk.fixed_rate = sdp_nikem_fixed_rate;
	sdp_soc_clk.size_of_fixed_rate = ARRAY_SIZE(sdp_nikem_fixed_rate);
	sdp_soc_clk.pll_clk = sdp_nikem_pll;
	sdp_soc_clk.size_of_pll_clk = ARRAY_SIZE(sdp_nikem_pll);
	sdp_soc_clk.fixed_div = sdp_nikem_fixed_div;
	sdp_soc_clk.size_of_fixed_div = ARRAY_SIZE(sdp_nikem_fixed_div);
	sdp_soc_clk.gate_clk = sdp_nikem_gate;
	sdp_soc_clk.size_of_gate_clk = ARRAY_SIZE(sdp_nikem_gate);

	sdp_clk_init(np, &sdp_soc_clk);
	set_ap_reg();
	pr_info ("Nike-M %-12s:%10lu Hz\n", "fin_pll", _get_rate("fin_pll"));
}
/* Nike-M clock declare. */
CLK_OF_DECLARE(sdp_nikem_clk, "samsung,sdp-nikem-clock", sdp_nikem_clk_init);


// static void __init sdp_nikel_clk_init(struct device_node *np)
// {
// 	struct sdp_clocks sdp_soc_clk;

// 	sdp_soc_clk.fixed_rate = sdp_nikel_fixed_rate;
// 	sdp_soc_clk.size_of_fixed_rate = ARRAY_SIZE(sdp_nikel_fixed_rate);
// 	sdp_soc_clk.pll_clk = sdp_nikel_pll;
// 	sdp_soc_clk.size_of_pll_clk = ARRAY_SIZE(sdp_nikel_pll);
// 	sdp_soc_clk.fixed_div = sdp_nikel_fixed_div;
// 	sdp_soc_clk.size_of_fixed_div = ARRAY_SIZE(sdp_nikel_fixed_div);
// 	sdp_soc_clk.gate_clk = sdp_nikel_gate;
// 	sdp_soc_clk.size_of_gate_clk = ARRAY_SIZE(sdp_nikel_gate);

// 	sdp_clk_init(np, &sdp_soc_clk);
// 	set_ap_reg();
// 	pr_info ("Nike-L %-12s:%10lu Hz\n", "fin_pll", _get_rate("fin_pll"));
// }
// /* Nike-L clock declare. */
// CLK_OF_DECLARE(sdp_nikel_clk, "samsung,sdp-nikel-clock", sdp_nikel_clk_init);


/* for Zebu kernel */
static struct sdp_fixed_div zebu_fixed_div[] __initdata = {
        { .name = "apb_pclk", .parent = "fin_pll",
                .mult = 1, .div = 1, .alias = "apb_pclk", },
        { .name = "emmc_clk", .parent = "fin_pll",
                .mult = 1, .div = 1, .alias = "emmc_clk", },
...
static struct sdp_fixed_rate zebu_fixed_rate[] __initdata = {
        { .name = "fin_pll", .flags = CLK_IS_ROOT, },
};

static void __init zebu_clk_init(struct device_node *np)
{
        struct sdp_clocks sdp_soc_clk = {0};

        sdp_soc_clk.fixed_rate = zebu_fixed_rate;
        sdp_soc_clk.size_of_fixed_rate = ARRAY_SIZE(zebu_fixed_rate);

	sdp_soc_clk.fixed_div = zebu_fixed_div;
	sdp_soc_clk.size_of_fixed_div = ARRAY_SIZE(zebu_fixed_div);

        sdp_clk_init(np, &sdp_soc_clk);
        set_ap_reg();
        pr_info ("Zebu %-12s:%10lu Hz\n", "fin_pll", _get_rate("fin_pll"));
}
CLK_OF_DECLARE(zebu_clk, "samsung,zebu-clock", zebu_clk_init);
...
u32 sdp_read_clkrst_mux(u32 phy_addr)
{
	void __iomem *addr;
	u32 val;
	u32 val1 = (unsigned int)reg_phy_base_ap & 0xFFF00000;
	u32 val2 = (unsigned int)reg_phy_base_mp & 0xFFF00000;
	
	if(((phy_addr < reg_phy_base_ap) || (phy_addr > reg_phy_end_ap)) && ((phy_addr < reg_phy_base_mp) || (phy_addr > reg_phy_end_mp))) {
		panic("%s: invalid address for clockgating! addr=0x%08X\n", __func__, phy_addr);
		return (u32) -EINVAL;
	}
	if ((phy_addr&0xFFF00000) == val1)
	{		
		addr = (void *) ((uintptr_t) reg_base_ap + ((uintptr_t) phy_addr - (uintptr_t) reg_phy_base_ap));
	}
	else if ((phy_addr&0xFFF00000) == val2) 
	{		
		addr = (void *) ((uintptr_t) reg_base_mp + ((uintptr_t) phy_addr - (uintptr_t) reg_phy_base_mp));
	}
	else
		return -EINVAL;
