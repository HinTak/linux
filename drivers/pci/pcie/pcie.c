...
extern int sdp_read_config (struct pci_bus *bus, unsigned int devfn, int where, int size, u32 *val);
extern int sdp_write_config (struct pci_bus *bus, unsigned int devfn, int where, int size, u32 val);
...
int __init sdp_pcie_map_irq(const struct pci_dev *dev, u8 slot, u8 pin)
{
	return IRQ_PCI;
}
