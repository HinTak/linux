...
/* Tone Control Block */
struct _btpcm_tone_cb
{
    struct btpcm_tone_ccb tone_ccb[BTPCM_NB_STREAM_MAX];
};

/*
 * Global variables
 */
struct _btpcm_tone_cb btpcm_tone_cb; /* BTPCM control block */
...
static int btpcm_tone_init(void);
static void btpcm_tone_exit(void);
static int btpcm_tone_open(int tone_stream);
static int btpcm_tone_close(int tone_stream);
static int btpcm_tone_config(int tone_stream, void *p_opaque, int frequency, int nb_channel,
        int bits_per_sample, void (*pcm_callback) (int tone_stream, void *p_opaque, void *p_buf, int nb_pcm_frames));
static int btpcm_tone_start(int tone_stream, int nb_pcm_frames, int nb_pcm_packets, int synchronization);
static int btpcm_tone_stop(int tone_stream);
static void btpcm_tone_synchronization(int tone_stream);
...
static int btpcm_tone_init(void)
{
    ...
    BTPCM_INFO("The Linux time reference (HK) is %d\n", HZ);

    memset(&btpcm_tone_cb, 0, sizeof(btpcm_tone_cb));
...
{
    int stream;
    struct btpcm_tone_ccb *p_tone_ccb = &btpcm_tone_cb.tone_ccb[0];

    BTPCM_DBG("Enter\n");

    /* Stop every started tone */
    for (stream = 0 ; stream < BTPCM_NB_STREAM_MAX ; stream++, p_tone_ccb++)
    {
        if (atomic_read(&p_tone_ccb->started))
        {
            btpcm_tone_stop(stream);
        }
    }
...
{
    if ((tone_stream < 0) ||
        (tone_stream >= BTPCM_NB_STREAM_MAX))
    {
 ...
 **
 *******************************************************************************/
static int btpcm_tone_config(int tone_stream, void *p_opaque, int frequency, int nb_channel, int bits_per_sample,
        void (*callback) (int tone_stream, void *p_opaque, void *p_buf, int nb_pcm_frames))
{
    struct btpcm_tone_ccb *p_tone_ccb;

    BTPCM_DBG("stream=%d freq=%d nb_channel=%d bps=%d cback=%p\n",
            tone_stream, frequency, nb_channel, bits_per_sample, callback);
    ...
    p_tone_ccb = &btpcm_tone_cb.tone_ccb[tone_stream];
    if (atomic_read(&p_tone_ccb->started))
    {
    ...
    p_tone_ccb->callback = callback;
    p_tone_ccb->p_opaque = p_opaque;
    p_tone_ccb->frequency = frequency;
    p_tone_ccb->bits_per_sample = bits_per_sample;
    p_tone_ccb->nb_channel = nb_channel;
...
static int btpcm_tone_start(int tone_stream, int nb_pcm_frames, int nb_pcm_packets, int synchronization)
{
    ...
    BTPCM_DBG("tone_stream=%d nb_pcm_frames=%d\n", tone_stream, nb_pcm_frames);
    ...
    {
        if (p_tone_ccb->hr_timer == NULL)
        {
            p_tone_ccb->hr_timer =  btpcm_hrtimer_alloc(btpcm_tone_hrtimer_callback,
                    (void *)(uintptr_t)tone_stream);
            if (p_tone_ccb->hr_timer == NULL)
            {
    ...
    case BTPCM_TONE_NB_FRAMES:  /* 96 frames, 2 ms */
        p_tone_ccb->timer_duration = 1; /* 10 ms timer */
        p_tone_ccb->nb_frames = nb_pcm_frames;
        p_tone_ccb->nb_packets = 5; /* 5 packets every time */
        break;

    case (BTPCM_TONE_NB_FRAMES * 5):  /* 480 frames, 10 ms */
        p_tone_ccb->timer_duration = 1; /* 10 ms timer */
        p_tone_ccb->nb_frames = nb_pcm_frames;
        p_tone_ccb->nb_packets = 1; /* 1 packet every time */
        break;

    case (BTPCM_TONE_NB_FRAMES * 10):  /* 960 frames, 20 ms */
        p_tone_ccb->timer_duration = 2; /* 20 ms timer */
        p_tone_ccb->nb_frames = nb_pcm_frames;
        p_tone_ccb->nb_packets = 1; /* 1 packet every time */
        break;

    case (BTPCM_TONE_NB_FRAMES * 20):  /* 1920 frames, 40 ms */
        p_tone_ccb->timer_duration = 4; /* 40 ms timer */
        p_tone_ccb->nb_frames = nb_pcm_frames;
        p_tone_ccb->nb_packets = 1; /* 1 packet every time */
        break;
    ...
    /* 2ms per SBC frames */
    p_tone_ccb->timer_duration = nb_pcm_frames / BTPCM_TONE_NB_FRAMES * 2;
    p_tone_ccb->nb_frames = nb_pcm_frames;
    p_tone_ccb->nb_packets = 1; /* 1 packet every time */
#endif /* HZ == 1000 */
    BTPCM_DBG("Timer_duration=%d (ms) nb_frames=%d nb_packets=%d\n",
            p_tone_ccb->timer_duration,
            p_tone_ccb->nb_frames,
            p_tone_ccb->nb_packets);
...
/* If High Resolution Timer */
    ...
    do_div(period_ns, p_tone_ccb->frequency);
    p_tone_ccb->timer_duration = period_ns / 1000000; /* for debug */
    ...
    p_tone_ccb->p_buf = kmalloc(p_tone_ccb->nb_frames * BTPCM_FRAME_SIZE, GFP_KERNEL);
    if (p_tone_ccb->p_buf == NULL)
    {
        BTPCM_ERR("Unable to allocate buffer (size=%d)\n", (int)(p_tone_ccb->nb_frames * BTPCM_FRAME_SIZE));
        return -ENOMEM;
    }

    /* Mark the tone as started */
    atomic_set(&p_tone_ccb->started, 1);

    p_tone_ccb->jiffies = jiffies;  /* Save the current timestamp (for debug) */
...
#ifdef USE_BTPCM_HRTIMER
        /* Start the HR timer */
        err = btpcm_hrtimer_start(p_tone_ccb->hr_timer, period_ns);
        if (err < 0)
        {
            BTPCM_ERR("Unable to start timer\n");
            kfree(p_tone_ccb->p_buf);
            atomic_set(&p_tone_ccb->started, 0);
            return -EINVAL;
        }
#else
        init_timer(&p_tone_ccb->timer);
        p_tone_ccb->timer.function = btpcm_tone_timer_routine;
        p_tone_ccb->timer.data = tone_stream;
        p_tone_ccb->timer.expires = jiffies + p_tone_ccb->timer_duration; /* 10 or 20 ms */
        add_timer(&p_tone_ccb->timer); /* Starting the timer */
#endif
    }
    return 0;
}
...
#ifdef USE_BTPCM_HRTIMER
    if (p_tone_ccb->hr_timer)
    {
        err = btpcm_hrtimer_stop(p_tone_ccb->hr_timer);
        if (err < 0)
        {
            BTPCM_ERR("Unable to stop timer\n");
            return -EINVAL;
        }
        btpcm_hrtimer_free(p_tone_ccb->hr_timer);
        p_tone_ccb->hr_timer = NULL;
    }
#else
...
/*******************************************************************************
 **
 ** Function         btpcm_tone_synchronization
 **
...
static void btpcm_tone_synchronization(int tone_stream)
{
    ...
    delta_error = (int)jiffies_to_msecs(jiffies - p_tone_ccb->jiffies);
    delta_error -= p_tone_ccb->timer_duration;
    p_tone_ccb->jiffies = jiffies;
    ...
    btpcm_tone_read(tone_stream,
            (short *)p_tone_ccb->p_buf,
            p_tone_ccb->nb_frames * BTPCM_FRAME_SIZE,
            &p_tone_ccb->sinus_index);

    p_tone_ccb->callback(tone_stream, p_tone_ccb->p_opaque, p_tone_ccb->p_buf, p_tone_ccb->nb_frames);
}
...
static void btpcm_tone_hrtimer_callback(void *p_opaque)
{
    int tone_stream = (int)(uintptr_t)p_opaque;
    int nb_packets;
    struct btpcm_tone_ccb *p_tone_ccb;
    int delta_error;
    ...
    for (nb_packets = 0 ; nb_packets < p_tone_ccb->nb_packets; nb_packets++)
    {
        /* Fill the buffer with the requested number of frames */
        btpcm_tone_read(tone_stream,
                (short *)p_tone_ccb->p_buf,
...
static void btpcm_tone_timer_routine(unsigned long data)
{
    int tone_stream = (int)data;
    int nb_packets;
    struct _btpcm_tone_ccb *p_tone_ccb;
    int delta_error;
    ...
    p_tone_ccb = &btpcm_tone_cb.tone_ccb[tone_stream];
    if (p_tone_ccb->started == 0)
    {
        BTPCM_DBG("stream=%d stopped.\n", tone_stream);
        /* del_timer(&p_tone_ccb->timer);*/ // is it needed?
        kfree(p_tone_ccb->p_buf);
        p_tone_ccb->p_buf = 0;
        return;
    }
    ...
    /* Restart the timer */
    mod_timer(&p_tone_ccb->timer,
            jiffies + p_tone_ccb->timer_duration - delta_error); /* restarting timer */

    /* Send the requested number of packets */
    for (nb_packets = 0 ; nb_packets < p_tone_ccb->nb_packets; nb_packets++)
    {
        /* fill the buffer with the requested number of frames */
        btpcm_tone_read(stream,
                (short *)p_tone_ccb->p_buf,
                p_tone_ccb->nb_frames * BTPCM_FRAME_SIZE,
                &p_tone_ccb->sinus_index);
        p_tone_ccb->callback(stream, p_tone_ccb->p_opaque, p_tone_ccb->p_buf, p_tone_ccb->nb_frames);
    }
}
#endif
