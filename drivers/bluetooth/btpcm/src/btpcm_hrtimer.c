...
#include "../inc/btpcm.h"
#include "../inc/btpcm_hrtimer.h"

/*
 * Defines
 */
struct btpcm_hrtimer_cb_
{
    int resolution;
};
 ...
 * Globals
 */
static struct btpcm_hrtimer_cb_ btpcm_hrtimer_cb = { .resolution  = 0 };
...
int btpcm_hrtimer_init(void)
{
    ...
    if (tp.tv_sec > 0 || !tp.tv_nsec) {
        BTPCM_ERR("hrtimer: Invalid resolution %u.%09u",
    ...
    BTPCM_INFO("HR Timer resolution=%d\n", btpcm_hrtimer_cb.resolution);
