...
static void synopGMAC_linux_cable_unplug_function(u32 notused)
{
	...
	synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_STATUS_REG, &data2);
	
//if((data & Mii_phy_status_link_up) == 0){
	// jay hsu
	if( (0 == (data & Mii_Link)) && (0 == (data2 & Mii_Link)))
	{  
#ifdef DEBUG 
		...
		if(!gmacdev->LinkState){
			...
			if(gmacdev->PHY_ID == RTL8201F_PHY_ID){
				...
				synopGMAC_RTL8201F_disable_EEE_Cab(gmacdev);
			}
#endif
...
static void synopGMAC_linux_reset_mac(struct net_device *netdev,synopGMACdevice *gmacdev)
{
	...
	if(done_MAC_reset == 1){
		printk(KERN_CRIT "[NTKETHMAC] Reset PHY for checking invaild packets\n");
#ifndef CONFIG_NVT_INTERNAL_PHY
	#ifdef USE_DEVICE_TREE		
		// External Phy reset.
		GPIO_MUX_reg_val = readl(gmacdev->GpioMuxReg + 0x8);
		GPIO_MUX_reg_val |= (u32)(1UL << NVT_PHY_RESET_GPIO_BIT);
		writel(GPIO_MUX_reg_val, gmacdev->GpioMuxReg + 0x8);

		GPIO_MUX_reg_val = 0;
		GPIO_MUX_reg_val |= (u32)(1UL << NVT_PHY_RESET_GPIO_BIT);
		writel(GPIO_MUX_reg_val, gmacdev->GpioMuxReg);
		mdelay(1);

		writel(GPIO_MUX_reg_val, gmacdev->GpioMuxReg + 0x4);
		mdelay(100);
...
static struct platform_driver synopGMAC_plat_driver = {
	.driver = {
		...
		.of_match_table = of_platform_synopGMAC_table,
	},
