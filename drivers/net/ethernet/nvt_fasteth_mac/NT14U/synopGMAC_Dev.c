...
u32 ioremap_write(u32 address, u32 value)
{
	u32 *remap_address;
...
u32 ioremap_AND_value(u32 address, u32 and_value)
{
	u32 *remap_address;
	u32 value;
...
u32 ioremap_OR_value(u32 address, u32 or_value)
{
	u32 *remap_address;
	u32 value;
...
addr = ((PhyBase << GmiiDevShift) & GmiiDevMask) | ((RegOffset << GmiiRegShift) & GmiiRegMask) | g_MDC_CLOCK;
...
// jay hsu : set ahb clock by auto detect function @ 20100329
addr = ((PhyBase << GmiiDevShift) & GmiiDevMask) | ((RegOffset << GmiiRegShift) & GmiiRegMask) | GmiiWrite | g_MDC_CLOCK;
...
void synopGMAC_mac_addr_setFilter_enable(synopGMACdevice *gmacdev)
{
	...
	data = synopGMACReadReg((u32 *)gmacdev->MacBase, 0x0048);	// GmacAddr1High

	data |= 0xC0000000;		// AE: Address Enable, and SA: Source Address

	synopGMACWriteReg((u32 *)gmacdev->MacBase, 0x0048, data);
...
{
	synopGMACSetBits((u32 *)gmacdev->MacBase, GmacFrameFilter, GmacSrcInvaAddrFilter);
	return;
}
...
{
	synopGMACClearBits((u32 *)gmacdev->MacBase, GmacFrameFilter, GmacSrcInvaAddrFilter);
	return;
}
...
s32 synopGMAC_set_phy_page_reg(synopGMACdevice * gmacdev,u8 page)
{
	...
	data =  page;
	status = synopGMAC_write_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase, PHY_PAGE_REG, data);
	return status;		
}
...
s32 synopGMAC_get_phy_test_reg(synopGMACdevice * gmacdev)
{
	...
	synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_TEST_REG, &data);
	printk("PHY Test Reg = 0x%8x\n",data);
...
s32 synopGMAC_get_phy_rxer_reg(synopGMACdevice * gmacdev)
{
        ...
        synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_RXER_REG, &data);

        printk("PHY RX_ER Reg = 0x%8x\n",data);
...
void  synopGMAC_get_phy_LBREM_reg(synopGMACdevice * gmacdev)
{
        ...
        synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_LBREM_REG, &data);
        printk("PHY LBREM Reg = 0x%8x\n",data);
        return;
}
/* For RTL8201 EL */
...
u16 synopGMAC_RTL8201F_get_phy_RMII_mode_setting_reg(synopGMACdevice * gmacdev)
{
	...
	synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_RTL8201F_RMII_MODE_SET_REG, &data);
...
s32 synopGMAC_RTL8201F_set_phy_TXC_direction(synopGMACdevice *gmacdev, u8 input)
{
	...
	status = synopGMAC_write_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_RTL8201F_RMII_MODE_SET_REG, data);
	synopGMAC_set_phy_page_reg(gmacdev,0);
	return status;
}
...
u16 synopGMAC_RTL8201F_get_phy_fibermode_loopback_reg(synopGMACdevice * gmacdev)
{
        ...
        synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_RTL8201F_FIBER_MODE_LOOPBACK_REG, &data);
	data &= (u16)(~(0x0004));
	synopGMAC_write_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_RTL8201F_FIBER_MODE_LOOPBACK_REG, data);
        return data;
}
...
u16 synopGMAC_RTL8201F_get_phy_interrupt_status(synopGMACdevice * gmacdev)
{
        ...
        synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase, PHY_RTL8201F_INT_SNR_REG, &data);
        return data;
}

s32 synopGMAC_RTL8201F_disable_EEE_Cab(synopGMACdevice *gmacdev)
{
	u16 data=0;
	s32 status = -ESYNOPGMACNOERR;

	synopGMAC_set_phy_page_reg(gmacdev,4);
	synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_RTL8201F_EEE_CAP_ENABLE_REG, &data);
  data &= 0xCCFF;	
	status = synopGMAC_write_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_RTL8201F_EEE_CAP_ENABLE_REG, data);
	synopGMAC_set_phy_page_reg(gmacdev,0);
	return status;
}
/* For RTL8201 F */
...
s32 synopGMAC_set_phy_speed(synopGMACdevice * gmacdev, u8 speed10)
{
 ...
 #if 0
        data &=0xefff;          // disable auto-negotiation
        //data |= Mii_Duplex;
        data &= (~Mii_Duplex);
        data &= 0xdfff;         // set in 10M
        //      data |= 0x200;  // restart auto-negotiation
#endif
        status = synopGMAC_write_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_CONTROL_REG,      data );
        if (status)
                return status;
...
//      gmacdev->DuplexMode = HALFDUPLEX;
        ...
        printk("PHY STATUS Reg = %04x\n",data);

        if((data & Mii_Link) == 0)
        {
                printk("Ethernet Phy No Link\n");
        }
        else
        {
                printk("Ethernet Phy Link UP\n");
        }
...
s32 synopGMAC_check_phy_init_for_10_100M(synopGMACdevice * gmacdev)
{
	...
	if (status)	
 	...
 	status = synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase,PHY_STATUS_REG, &data2);
	if (status)	
		return status;
		
	if((data & Mii_Link) == 0 && (data2 & Mii_Link) == 0)
    {
		TR("No Link\n");
	...
	if ( loop_count < 0 )
	{
		printk(KERN_CRIT "[NTKETHMAC] : Ethernet Phy Auto-Negotiation Failed\n");
		return -ESYNOPGMACPHYERR;
	}
...
#elif defined(CONFIG_NVT_RTL_PHY_SUPPORT) || defined(CONFIG_NVT_ICPLUS_PHY_SUPPORT) || defined(CONFIG_NVT_INTERNAL_PHY)
...
//	gmacdev->DmaBase = dmaBase;
	gmacdev->MacBase = ioremap(SYNOP_GMAC_BASE_ADDR, SYNOP_GMAC_REG_MEMMAP_LEN);
	gmacdev->DmaBase = ioremap(SYNOP_DMA_BASE_ADDR, SYNOP_DMA_REG_MEMMAP_LEN);;
	...
	if (node) {
		...
		gmacdev->ClockCtrlReg = of_iomap(node, 4);
...
{
	if (desc == NULL) {
		printk(KERN_CRIT "%s: desc is NULL pointer !\n", __FUNCTION__);
		return -1;
	}
...
{
	u32 i=0, dmastatus;
	DmaDesc * rxdesc = gmacdev->RxDesc + index;

	if(gmacdev == NULL){
		printk(KERN_CRIT "gmacdev = NULL\n");
		return;
	}

	printk(KERN_CRIT "######### synopGMAC_dbg_print_rxdes_addr index=%d ###########\n",index);
	//printk(KERN_CRIT "gmacdev = 0x%08x\n",gmacdev);
	
	//for(i =0; i < gmacdev->RxDescCount; i++){
		printk(KERN_CRIT "---------------------------------\n");
		printk(KERN_CRIT "Desc index %d addr=0x%p\n",i,rxdesc);
		printk(KERN_CRIT "Status=0x%08x\n",rxdesc->status);
		printk(KERN_CRIT "Length=0x%08x\n",rxdesc->length);
		printk(KERN_CRIT "Data1=0x%08x\n",rxdesc->data1);
		printk(KERN_CRIT "Buffer1=0x%08x\n",rxdesc->buffer1);
	//	rxdesc++;
	//}
	printk(KERN_CRIT "---------------------------------\n");
	//printk(KERN_CRIT "RX BUSY Desc %d addr=0x%08x\n",gmacdev->RxBusy,gmacdev->RxBusyDesc);
	//printk(KERN_CRIT "RX NEXT Desc %d addr=0x%08x\n",gmacdev->RxNext,gmacdev->RxNextDesc);
	printk(KERN_CRIT "MAC current RX Desc =0x%08x\n",synopGMAC_get_dma_current_RX_desc(gmacdev));
	dmastatus = synopGMACReadReg((u32 *)gmacdev->DmaBase, DmaStatus);
	printk(KERN_CRIT "MAC DMA status =0x%08x\n",dmastatus);
}

void synopGMAC_dbg_print_txdes_addr(synopGMACdevice * gmacdev, u32 index)
{
	u32 i=0,dmastatus;
	DmaDesc * txdesc = gmacdev->TxDesc + index;
	...
	printk(KERN_CRIT "######### synopGMAC_dbg_print_txdes_addr index=%d###########\n",index);
	printk(KERN_CRIT "gmacdev = 0x%p\n",gmacdev);
	...
	//}
	printk(KERN_CRIT "---------------------------------\n");
	//printk(KERN_CRIT "TX BUSY Desc %d addr=0x%08x\n",gmacdev->TxBusy,gmacdev->TxBusyDesc);
	//printk(KERN_CRIT "TX NEXT Desc %d addr=0x%08x\n",gmacdev->TxNext,gmacdev->TxNextDesc);
	printk(KERN_CRIT "MAC current TX Desc =0x%08x\n",synopGMAC_get_dma_current_TX_desc(gmacdev));
	dmastatus = synopGMACReadReg((u32 *)gmacdev->DmaBase, DmaStatus);
	printk(KERN_CRIT "MAC DMA status =0x%08x\n",dmastatus);
}
#endif
...
//		synopGMAC_dbg_print_txdes_addr(gmacdev,txover);
//#endif
		printk(KERN_CRIT "%s: txdesc is NULL\n",__FUNCTION__);
		return -1;	
	}

	// jay hsu : do cache sync, dram sync to cpu cache
	dma_sync_single_for_cpu(NULL, virt_to_phys(txdesc),sizeof(DmaDesc), DMA_FROM_DEVICE);
...
//#endif
	...
	dma_sync_single_for_device(NULL, virt_to_phys(txdesc),sizeof(DmaDesc), DMA_TO_DEVICE);
...
//		synopGMAC_tx_checksum_offload_tcp_pseudo(gmacdev, txdesc);
	...
	dma_sync_single_for_device(NULL, Buffer1,Length1, DMA_TO_DEVICE);
	dma_sync_single_for_device(NULL, virt_to_phys(txdesc),sizeof(DmaDesc), DMA_TO_DEVICE);
...
s8 synopGMAC_set_rx_qptr(synopGMACdevice * gmacdev, u32 Buffer1, u32 Length1, u32 Data1, u32 Buffer2, u32 Length2, u32 Data2, u32 *now_desc)
{
	...
	dma_sync_single_for_cpu(NULL, virt_to_phys(rxdesc),sizeof(DmaDesc), DMA_FROM_DEVICE);
	if(!synopGMAC_is_desc_empty(rxdesc))
		return -1;
	...
	// jay hsu : do cache sync, cpu cache sync to dram
	dma_sync_single_for_device(NULL, virt_to_phys(rxdesc),sizeof(DmaDesc), DMA_TO_DEVICE);
...
s8 synopGMAC_get_rx_qptr(synopGMACdevice * gmacdev, u32 * Status, u32 * Buffer1, u32 * Length1, u32 * Data1, u32 * Buffer2, u32 * Length2, u32 * Data2, u32 *now_desc)
{
	...
	if(rxdesc == NULL){
//#ifndef CONFIG_VD_RELEASE
//		synopGMAC_dbg_print_rxdes_addr(gmacdev, rxnext);
//#endif
		printk(KERN_CRIT "%s: rxdesc is NULL\n", __FUNCTION__);
		return -1;
	}
	...
	if(Status != NULL)
		*Status = rxdesc->status;// send the status of this descriptor
		...
		printk(KERN_CRIT "%s ,%d DESC =0x%p status is NULL!! \n",__FUNCTION__,rxnext,rxdesc);
//#ifndef CONFIG_VD_RELEASE
//		synopGMAC_dbg_print_rxdes_addr(gmacdev, rxnext);
//#endif
	}
	...
	// jay hsu : do cache sync, dram sync to cpu cache
	dma_sync_single_for_cpu(NULL, rxdesc->buffer1,((rxdesc->length & DescSize1Mask) >> DescSize1Shift), DMA_FROM_DEVICE);

//#ifndef CONFIG_VD_RELEASE
//	if(!synopGMAC_is_rx_desc_valid(rxdesc->status)){
//		synopGMAC_dbg_print_rxdes_addr(gmacdev, rxnext);
//	}
//#endif
...
{
	u32 data;
	data = synopGMACReadReg((u32 *)gmacdev->DmaBase, DmaTxCurrDesc);
	return data;
}
...
{
	u32 data;
	data = synopGMACReadReg((u32 *)gmacdev->DmaBase, DmaRxCurrDesc);
	return data;
}
