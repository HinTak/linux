...
void *sys_plat_alloc_memory(u32 size, dma_addr_t *);
void sys_plat_free_memory(u32 *addr, dma_addr_t, size_t len);
...
static inline void synopGMACWriteReg(
	u32 *RegBase, u32 RegOffset, u32 RegData)
{
	u32 addr = (u32)RegBase + RegOffset;
	/* debug
	* TR("%s RegBase = 0x%08x RegOffset = 0x%08x
...
static inline void synopGMACSetBits(
	u32 *RegBase, u32 RegOffset, u32 BitPos)
{
	u32 addr = (u32)RegBase + RegOffset;
	u32 data = readl((void *)addr);
