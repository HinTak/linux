...
s32 synopGMAC_set_mdc_clk_div(struct synop_device *gmacdev, u32 clk_div_val)
{
	...
	orig_data = synopGMACReadReg(
			(u32 *)gmacdev->MacBase, GmacGmiiAddr);
...
s32 synopGMAC_phy_loopback(struct synop_device *gmacdev, bool loopback)
{
	...
	if (loopback) {
		...
		status = synopGMAC_write_phy_reg((u32 *)gmacdev->MacBase,
						 gmacdev->PhyBase,
						 PHY_CONTROL_REG, data);
	} else {
		synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,
...
{
	synopGMACClearBits(
			(u32 *)gmacdev->MacBase, GmacConfig, GmacPadCrcStrip);
}
...
{
	synopGMACSetBits(
			(u32 *)gmacdev->MacBase, GmacConfig, GmacDeferralCheck);
}
...
{
	synopGMACClearBits(
			(u32 *)gmacdev->MacBase, GmacFrameFilter, GmacFilter);
}
...
void synopGMAC_mac_addr_setFilter_enable(struct synop_device *gmacdev)
{
	...
	/*GmacAddr1High*/
	data = synopGMACReadReg((u32 *)gmacdev->MacBase, 0x0048);
...
s32 synopGMAC_mac_init(struct synop_device *gmacdev, u32 promisc)
{
	...
	pr_warn("[NTKETHMAC] MAC init duplex= %d, speed= %d\n",
	...
	if (gmacdev->DuplexMode == FULLDUPLEX) {
		...
		PHYreg = synopGMACReadReg((u32 *)gmacdev->MacBase,
					  GmacGmiiData);
	...
	if (promisc) {
		...
		pr_warn("[NTKETHMAC] Promiscuous Mode is enabled.\n");
	} else {
		...
		pr_warn("[NTKETHMAC] Promiscuous Mode is disabled.\n");
	}
	return 0;
}
	...
	if (data & Mii_En_Auto_Neg) {
		...
		while (loop_count-- > 0) {
			...
			if ((data & Mii_AutoNegCmplt) != 0) {
				pr_warn("[NTKETHMAC] AN Completed\n");
				break;
			}
		     ...
		     u32 macBase, u32 dmaBase, u32 phyBase)
{
	...
	if (node) {
		...
		gmacdev->rx_padin_reg = of_iomap(node, 3);
		gmacdev->clock_ctrl_reg = gmacdev->rx_padin_reg + 0x4;
		gmacdev->IRQNum = irq_of_parse_and_map(node, 0);

		/* cpu affinity */
		irq = gmacdev->IRQNum;
		if (!of_property_read_u32(
			node, "irq-affinity", &irq_affinity)) {
			if (irq_affinity > 3) {
				pr_err("[NTKETHMAC_ERROR] irq-affinity %u\n",
				       irq_affinity);
			} else {
				retval = irq_set_affinity(
					irq, cpumask_of(irq_affinity));

				if (retval)
					pr_info("[NTKETHMAC]%d can't irq_set_affinity %u\n",
						irq, irq_affinity);
			}
		}
	} else {
...
void synopGMAC_get_desc_data(struct dma_desc *desc,
			     u32 *Status, u32 *Buffer1,
			     u32 *Length1, u32 *Data1,
...
s8 synopGMAC_get_tx_qptr(
		struct synop_device *gmacdev,
		u32 *Status, u32 *Buffer1, u32 *Length1,
...
s8 synopGMAC_set_tx_qptr(
		...
		u32 Buffer2, u32 Length2, u32 Data2,
		u32 offload_needed, u32 *now_desc)
{
...
#if 0 /* We replace dma_sync_single_for_device with dma_map_single */
...
s8 synopGMAC_get_rx_qptr(struct synop_device *gmacdev,
			 u32 *Status, u32 *Buffer1, u32 *Length1, u32 *Data1,
		u32 *Buffer2, u32 *Length2, u32 *Data2, u32 *now_desc)
{
...
{
	synopGMACClearBits((u32 *)gmacdev->MacBase,
...
{
	synopGMACClearBits((u32 *)gmacdev->DmaBase,
...
{
	synopGMACSetBits((u32 *)gmacdev->DmaBase,
			 DmaControl, DmaDisableDropTcpCs);
}
		...
		struct synop_device *gmacdev, u32 clk_type)
{
	...
	clkval = synopGMACReadReg(
			(u32 *)gmacdev->MacBase, GmacTSControl);
...
{
	*higher_sec_val =
		(u16)(synopGMACReadReg((u32 *)gmacdev->MacBase,
			GmacTSHighWord) & GmacTSHighWordMask);
	*sec_val =
		synopGMACReadReg((u32 *)gmacdev->MacBase, GmacTSHigh);
