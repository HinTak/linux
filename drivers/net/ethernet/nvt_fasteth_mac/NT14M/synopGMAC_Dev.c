...
u32 g_MDC_CLOCK;
struct semaphore sem_phy_rw; /*lock for read/write PHY by MAC register,in order to avoid race condition*/
...
s32 synopGMAC_mac_init(synopGMACdevice * gmacdev, u32 promisc)
{
	...
	printk(KERN_CRIT "[NTKETHMAC] MAC init duplex=%d, speed=%d\n",gmacdev->DuplexMode,gmacdev->Speed);
...
s32 synopGMAC_check_phy_init_for_10_100M(synopGMACdevice * gmacdev)
{
	...
	if ( data & Mii_En_Auto_Neg )
	{
		...
		while(loop_count-- > 0)
		{
	        ...
	        {
				printk(KERN_CRIT "[NTKETHMAC] Autonegotiation Complete\n");
				break;
			}
	...
	if((data & Mii_Link) == 0)
    {
		printk(KERN_CRIT "\033[1;31m[NTKETHMAC] No Link\033[0m\n");
	...
	{
		gmacdev->LinkState = LINKUP; 
		printk(KERN_CRIT "[NTKETHMAC] Link UP\n");
	}	
	...
	if ( loop_count < 0 )
	{
		...
		printk(KERN_CRIT "\033[1;31m[NTKETHMAC_ERROR]: Ethernet Phy Auto-Negotiation Failed\033[0m\n");
		return -ESYNOPGMACPHYERR;
	}
