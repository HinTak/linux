...
static void iot_work_fn(struct work_struct *work);
static DECLARE_WORK(iot_work, iot_work_fn);
...
/*  memory alloc for non-cache */
void *sys_plat_alloc_memory(u32 size, dma_addr_t *phyAddr)
{
	return dma_alloc_coherent(p_device, size, phyAddr, GFP_ATOMIC);
}

/*  memory free for non-cache */
void sys_plat_free_memory(u32 *addr, dma_addr_t phyAddr, size_t len)
{
	dma_free_coherent(p_device, len, addr, phyAddr);
}
...
#define pwsv_running()	pwsv_is_running()
#else
#define pwsv_running()	1	/* force to 1 to make sure the PMT Control and Status Register is read. */
#endif
...
static void synopGMAC_linux_cable_unplug_function(u32 notused)
{
	...
	if (netif_running(netdev)) {
		...
		mod_timer(&synopGMAC_cable_unplug_timer, jiffies + (HZ / 2));
	}
}
...
#ifdef CONFIG_POWER_SAVING_MODE
#define REG_STBC_CTRL_PAGE2_BASE_ADDR	(0xF1B0)
#define REG_STBC2_STBC_MAILBOX_3	(REG_STBC_CTRL_PAGE2_BASE_ADDR + 0x05)
#define IOT_ETH_WOL_MAILBOX		(REG_STBC2_STBC_MAILBOX_3)
#define IOT_ETH_WOL_EVENT		(56)
typedef bool (*pfn_micom)(u16, u8);
...
{
	bool ret;
	static pfn_micom micom = (pfn_micom)0;

	if (!micom) {
		micom = resolve_symbol("DRV_STBC_WriteRegister");
		if (!micom) {
			return;
		}
	}

	ret = micom(IOT_ETH_WOL_MAILBOX, IOT_ETH_WOL_EVENT);
	if (!ret) {
		printk(KERN_ERR "[%s] IOT ethernet WOL send cmd to STBC fails\n", __func__);
	}
}
#else
static void iot_work_fn(struct work_struct *work)
	...
	struct pci_dev *pcidev, u32 no_of_desc, u32 desc_mode)
{
	...
	if (desc_mode == RINGMODE) {
		...
		TR("phyAddr=0x%08X, vir=0x%08X\n",
		   phyAddr, (u32)phys_to_virt(phyAddr));
		TR("first_desc=0x%08X, phy=0x%08X\n",
		   (u32)first_desc, (u32)virt_to_phys(first_desc));
		...
		first_desc =
		(struct dma_desc *)
		sys_plat_alloc_memory(sizeof(struct dma_desc), &phyAddr);
	...
	struct pci_dev *pcidev, u32 desc_mode)
{
	...
	if (desc_mode == RINGMODE) {
		...
		sys_plat_free_memory((u32 *)gmacdev->RxDesc, gmacdev->RxDescDma,
		...
		dma_addr_t first_descdma;

		TR("rx-----------------------------------rx\n");
		first_desc          = gmacdev->RxDesc;
		first_descdma       = gmacdev->RxDescDma;
		for (i = 0; i < gmacdev->RxDescCount; i++) {
			synopGMAC_get_desc_data(
				first_desc, &status,
			...
			/*free descriptors memory*/
			sys_plat_free_memory((u32 *)first_desc, first_descdma,
					     sizeof(struct dma_desc));
	...
	struct pci_dev *pcidev, u32 desc_mode)
{
	...
	if (desc_mode == RINGMODE) {
		...
		/*Free descriptors */
		sys_plat_free_memory((u32 *)gmacdev->TxDesc, gmacdev->TxDescDma,
				     sizeof(struct dma_desc) *
				     TRANSMIT_DESC_SIZE);
		...
		*/
		...
		first_descdma       = gmacdev->TxDescDma;
		for (i = 0; i < gmacdev->TxDescCount; i++) {
			synopGMAC_get_desc_data(
...
#define EW_LB_WAIT_DES (-10)
#define EW_LB_SKB_ERR (-7)
#define TIMING_CHECK_US 5000
 ...
 */
static int synopGMAC_SS_self_test_recv_frames(struct synop_device *gmacdev)
{
	u32 desc_index;
	...
	/**
	 * EW, itr is incoming but descriptor not ready yet.
	 */
	u32 visit = 0;
	int rv = 0;
		...
		if (ret == 0 && data1 != 0) {
			...
			*/
			...
			if (synopGMAC_is_rx_desc_valid(status)) {
				...
				gmacdev->ssLoopbackErrorPkt++;
				pr_crit("[NTKETHMAC_Debug] Please Remove cable, Descriptor is not valid status = %08x\n",
					status);
			}
			dev_kfree_skb_any(skb);
			...
			*/
			...
			if (ret < 0) {
				pr_crit("Cannot set Rx Descriptor for skb %08x\n",
					(u32)skb);
				dev_kfree_skb_any(skb);
			}
		}
			...
			/** EW
			 * first visit and no descriptor
			 * But here is ISR, mean RX is incomming but no data yet.
			 * Need a retry
			 */
			if (!visit)
				rv = EW_LB_WAIT_DES;
...
static void synop_handle_received_data(struct net_device *netdev)
{
	...
	if (gmacdev->ssLoopbackEnable) {
		rv = synopGMAC_SS_self_test_recv_frames(gmacdev);

		if (rv) {
			for (u = 0 ; u < TIMING_CHECK_US ; u++) {
				udelay(1);
				rv = synopGMAC_SS_self_test_recv_frames(gmacdev);
				if(!rv) {
					pr_crit("[NTKETHMAC] DES ready %d\n", u);
					break;
				}
			}
		}
			...
			*/
			...
			if (synopGMAC_is_rx_desc_valid(status)) {
				...
				*/
				...
				/*Now the present skb should be set free*/
				dev_kfree_skb_any(skb);
				pr_crit("\033[1;31m[NTKETHMAC_ERROR] %s: Descriptor is not valid status = %08x , next Desc index=%d\033[0m\n",
...
static irqreturn_t synopGMAC_intr_handler(int irq, void *dev_id)
{
	...
	struct synop_adapter_struct *adapter;
	struct synop_device *gmacdev;
	u32 interrupt, dma_status_reg, lpi_status_reg;
	s8 status;
	u32 desc_index;
	s32 skblen;
	...
	/* Read LPI interrupt status */
	lpi_status_reg =
		synopGMACReadReg((u32 *)gmacdev->MacBase, GmacLPICtrlStatus);

	if (lpi_status_reg) {
		/* Just show LPI interrupt status for debug, then do nothing */
		pr_crit("\n%s: lpi_status_reg:%x\n", __func__, lpi_status_reg);
	}
	...
	*/
	...
	if (dma_status_reg & GmacPmtIntr) {
		...
		if (GMAC_Power_down == 1) {
			...
			if (pwsv_running() && (data & GmacPmtMagicPktReceived))
				schedule_work(&iot_work);
		}
	}
...
s32 synopGMAC_linux_open(struct net_device *netdev)
{
	...
	if (count < 0) {
...
#if DEBUG_READ_PHY_REG
		ioremap_write(0xFD130120, 0x1);

		for (i = 0; i <= 100; i++) {
			if (ioremap_read(0xFD130120) & 0x10) {
				pr_crit("[NTKETHMAC_DEBUG]:PHY reg_120=%08x\n",
					ioremap_read(0xFD130120));
				break;
			}
			mdelay(1);
		}
		...
		mdelay(10);
		pr_crit("[NTKETHMAC_DEBUG]:PHY 12C=%08x, 130=%08x\n",
			ioremap_read(0xFD13012C),
			ioremap_read(0xFD130130));
#endif
...
s32 synopGMAC_linux_close(struct net_device *netdev)
{
	...
	/*
	 * Disable the Mac for both tx and rx.
	 * Enable them in synopGMAC_linux_open -> synopGMAC_mac_init.
	 */
	synopGMAC_tx_disable(gmacdev);
	synopGMAC_rx_disable(gmacdev);
	...
	cancel_work_sync(&iot_work);
	pr_warn("[NTKETHMAC]: synopGMAC_linux_close.\n");
...
s32 synopGMAC_linux_xmit_frames(struct sk_buff *skb, struct net_device *netdev)
{
	...
	*/
	...
	mapping = dma_map_single(p_device, skb->data, skb->len, DMA_TO_DEVICE);

	if (dma_mapping_error(p_device, mapping)) {
		pr_crit("%s TX DMA error\n", __func__);
		spin_unlock_irqrestore(&gmacdev->tx_lock, irq_flags);
		return NETDEV_TX_BUSY;
	}
	...
	/* Force memory writes to complete before DMA */
	dma_wmb();
			     ...
			     struct ifreq *ifr, s32 cmd)
{
	...
	case IOCTL_WRITE_REGISTER:
		if (req->unit == 0) {
			/* Write Mac Register*/
			synopGMACWriteReg((u32 *)gmacdev->MacBase,
					  req->addr, req->data);
		} else if (req->unit == 1) {
			/*Write DMA Register*/
			pr_info("GMAC driver : addr 0x%x, data 0x%x\n",
				req->addr, req->data);

			synopGMACWriteReg((u32 *)gmacdev->DmaBase,
					  req->addr, req->data);
	...
	case IOCTL_SS_SELF_TEST_MODE_ON_OFF:
		...
		if (req->unit == 1) {
			pr_crit("Plug test tool in Ethernet port when Wired Loopback.\n");
			...
			if (!SS_selftest_frame) {
				pr_crit("Alloc SS_selftest_frame is Fail.\n");
				break;
			}
...
#ifdef CONFIG_DEBUG_FS
/**
*	for dump MAC register
*/
static int
synop_gmac_dump(struct synop_device *gmacdev)
{
	u32 offset = 0x0, i;
	u32 data;
	u32 gmac_reg_dump[0x50 / sizeof(u32)];
	u32 dma_reg_dump[0x40 / sizeof(u32)];

	for (i = 0; i < ARRAY_SIZE(gmac_reg_dump); i++) {
		gmac_reg_dump[i] =
			synopGMACReadReg((u32 *)gmacdev->MacBase, offset);
		offset += 4;
	}

	pr_crit("Dump GMAC Address:");
	for (i = 0; i < ARRAY_SIZE(gmac_reg_dump); i += 4) {
		pr_crit("0x%08x: 0x%08x 0x%08x 0x%08x 0x%08x\n",
			(SYNOP_GMAC_BASE_ADDR) + (i * 4),
			gmac_reg_dump[i + 0], gmac_reg_dump[i + 1],
			gmac_reg_dump[i + 2], gmac_reg_dump[i + 3]);
	}
	...
	for (i = 0; i < ARRAY_SIZE(dma_reg_dump); i++) {
		dma_reg_dump[i] =
			synopGMACReadReg((u32 *)gmacdev->DmaBase, offset);
		offset += 4;
	}

	pr_crit("Dump DMA Address:");
	for (i = 0; i < ARRAY_SIZE(dma_reg_dump); i += 4) {
		pr_crit("0x%08x: 0x%08x 0x%08x 0x%08x 0x%08x\n",
			(SYNOP_DMA_BASE_ADDR) + (i * 4),
			dma_reg_dump[i + 0], dma_reg_dump[i + 1],
			dma_reg_dump[i + 2], dma_reg_dump[i + 3]);
	}
...
static int
synop_dbg_regdump_get(void *data, u64 *val)
{
	struct synop_device *gmacdev = data;

	synop_gmac_dump(gmacdev);
	*val = 0x0;
	return 0;
}

DEFINE_SIMPLE_ATTRIBUTE(synop_dbg_regdump_fops,
			synop_dbg_regdump_get, NULL,
			"%lld\n");

/**
*	for dump PHY standard register
*/
static int
nvt_internal_PHY_regdump_get(void *data, u64 *val)
{
	struct synop_device *gmacdev = data;
	u16 tmp_data, addr;

	pr_crit("PHY Register dump\n");

	for (addr = 0; addr <= 0x1F; addr++) {
		synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase,
				       gmacdev->PhyBase,
				       addr, &tmp_data);
		pr_crit("PHY ADD 0x%04X: 0x%04X\n", addr, tmp_data);
	}
...
DEFINE_SIMPLE_ATTRIBUTE(synopgmac_internal_PHY_regdump_fops,
			nvt_internal_PHY_regdump_get, NULL,
			"%lld\n");

/**
*	for control PHY ldo (dummy for EW)
*/

static int
nvt_internal_phy_dbg_ldo_get(void *data, u64 *val)
{
	struct synop_device *gmacdev = data;
	void *addr;

	pr_crit("[#####]nvt_internal_PHY_dbg_ldo_get, return 0xC\n");
	*val = 0xC;
	return 0;
}

static int
nvt_internal_phy_dbg_ldo_set(void *data, u64 val)
{
	struct phy_device *phydev = data;
	void *addr;

	pr_crit("[#####]nvt_internal_PHY_dbg_ldo_set, do nothing\n");
	pr_crit("[#####]nvt_internal_PHY_dbg_ldo_set, %llX\n", val);

	return 0;
}

DEFINE_SIMPLE_ATTRIBUTE(
	synop_internal_PHY_dbg_ldo_fops,
	nvt_internal_phy_dbg_ldo_get, nvt_internal_phy_dbg_ldo_set,
	"0x%08llx\n");

/**
*	for return link status
*/

static ssize_t synop_dbg_phy_status_read(
...
static const struct file_operations synop_dbg_phy_status_fops = {
	.read = synop_dbg_phy_status_read,
	.open = simple_open,
...
static int synop_dbg_anypacket_wol_show(struct seq_file *s, void *data)
{
	pr_crit("Anypacket WOL Setting\n");
	pr_crit("Support IPv4 source address and IPv6 source address packet\n");
	pr_crit("Usage:\n");
	pr_crit("	mode=[auto]\n");
	pr_crit("	src-match=[y|n]\n");
	pr_crit("	src-offset=" UNDERLINE "Offset Number" NONECOLOR "\n");
	pr_crit("	src-length=" UNDERLINE "Length Number" NONECOLOR "\n");
	pr_crit("	src-ipaddr=" UNDERLINE "IPv4 address" NONECOLOR "|"
		UNDERLINE "IPv6 address" NONECOLOR "\n");
	return 0;
}

static int synop_dbg_anypacket_wol_open(struct inode *inode, struct file *file)
{
	return single_open(file, synop_dbg_anypacket_wol_show,
			   inode->i_private);
}

static unsigned short crc16_modbus(unsigned char *data, unsigned char length)
{
	int j;
	unsigned short uiCRC = 0xFFFF;
...
static unsigned short u16_bit_reverse(unsigned short v)
{
	unsigned short r = v;
	int s = 15;
...
u16 cal_wof_crc16(unsigned char *s, u8 cnt)
{
	unsigned short u16CRCrev;
	unsigned short u16CRC;

	u16CRC = crc16_modbus(s, cnt);
	u16CRCrev = u16_bit_reverse(u16CRC);
...
struct anypacket_wol_status {
	int mode_config;
	int src_match_config;
	int src_ip_config;
	int src_offset_config;
	int src_length_config;
	int src_offset;
	int src_length;
	u8 src_ip[16];
};

static ssize_t synop_dbg_anypacket_wol_write(struct file *file,
					     const char __user *buf,
					     size_t count,
					     loff_t *ppos)
{
	u32 reg, wkuppktfilter_reg[8];
	struct synop_device *gmacdev;
	char content[count + 1];
	char *command, *item, *field;
	size_t item_len, field_len;
	int i, filter;
	static struct anypacket_wol_status wol_status = {0};

	gmacdev = (struct synop_device *)synopGMACadapter->synopGMACdev;

	strncpy(content, buf, count);
	content[count] = '\0';
	if (content[count - 1] == '\n') {
		content[count - 1] = '\0';
	}
	...
	while (item) {
		...
		if (strncasecmp("mode", item, item_len) == 0) {
			if (strncasecmp("auto", field, field_len) == 0) {
				wol_status.mode_config = 1;
			} else {
				wol_status.mode_config = 0;
			}
		} else if (strncasecmp("src-match", item, item_len) == 0) {
			if (strncasecmp("y", field, field_len) == 0) {
				wol_status.src_match_config = 1;
			} else if (strncasecmp("n", field, field_len) == 0) {
				wol_status.src_match_config = 0;
			} else {
				wol_status.src_match_config = 0;
				pr_crit("[NTKETHMAC] Undefinied src-match.\n");
				goto undefined;
			}
		} else if (strncasecmp("src-ipaddr", item, item_len) == 0) {
			int dot_count = 0;
			int colon = 0, i;
			char *ret_str;
			const char *end;

			ret_str = strchr(field, '.');
			while (ret_str) {
				dot_count++;
				ret_str = strchr(ret_str + 1, '.');
			}
			ret_str = strchr(field, ':');
			if (ret_str)
				colon = 1;

			/* Check if IP format is IPv4 */
			if (dot_count == 3 &&
			    in4_pton(field, -1, wol_status.src_ip, -1,
				     &end) > 0) {
				wol_status.src_ip_config = IP_VER_4;
			/* Check if IP format is IPv6 */
			} else if (colon > 0 &&
				   in6_pton(field, -1, wol_status.src_ip, -1,
					    &end) > 0) {
				wol_status.src_ip_config = IP_VER_6;
			} else {
				pr_crit("[NTKETHMAC] Undefinied IP format.\n");
				wol_status.src_ip_config = 0;
				goto undefined;
			}
		} else if (strncasecmp("src-offset", item, item_len) == 0) {
			if (!kstrtoint(field, 10, &wol_status.src_offset)) {
				/* Offset of IPv4 is 12 */
				if (wol_status.src_offset == 12) {
					wol_status.src_offset_config = IP_VER_4;
				/* Offset of IPv6 is 8 */
				} else if (wol_status.src_offset == 8) {
					wol_status.src_offset_config = IP_VER_6;
				} else {
					pr_crit("[NTKETHMAC] Undefined \
						src-offset.\n");
					wol_status.src_offset_config = 0;
					goto undefined;
				}
			}
		} else if (strncasecmp("src-length", item, item_len) == 0) {
			if (!kstrtoint(field, 10, &wol_status.src_length)) {
				/* IPv4 length is 4 */
				if (wol_status.src_length == 4) {
					wol_status.src_length_config = IP_VER_4;
				/* IPv6 length is 16 */
				} else if (wol_status.src_length == 16) {
					wol_status.src_length_config = IP_VER_6;
				} else {
					pr_crit("[NTKETHMAC] Undefined \
						src-length.\n");
					wol_status.src_length_config = 0;
					goto undefined;
				}
			}
		} else {
			pr_crit("[NTKETHMAC] Undefined command.\n");
			goto undefined;
		}
	...
	/* Check if every config is set successfully. */
	if (wol_status.mode_config &&
	    wol_status.src_match_config &&
	    wol_status.src_ip_config &&
	    wol_status.src_offset_config &&
	    wol_status.src_length_config) {
		u16 crc;

		/* Check if format is IPv4 */
		if (wol_status.src_ip_config == IP_VER_4 &&
		    wol_status.src_offset_config == IP_VER_4 &&
		    wol_status.src_length_config == IP_VER_4) {
			pr_crit("[NTKETHMAC] Enable IPv4 packet wake-up.\n");
			pr_crit("[NTKETHMAC] IP: %pI4\n", wol_status.src_ip);
			pr_crit("[NTKETHMAC] Offset: %d, Length: %d.\n",
				wol_status.src_offset, wol_status.src_length);
		/* Check if formate is IPv6 */
		} else if (wol_status.src_ip_config == IP_VER_6 &&
			   wol_status.src_offset_config == IP_VER_6 &&
			   wol_status.src_length_config == IP_VER_6) {
			pr_crit("[NTKETHMAC] Enable IPv6 packet wake-up.\n");
			pr_crit("[NTKETHMAC] IP: %pI6\n", wol_status.src_ip);
			pr_crit("[NTKETHMAC] Offset: %d, Length: %d.\n",
				wol_status.src_offset, wol_status.src_length);
		} else {
			pr_crit("[NTKETHMAC] IP addr, offset and length \
				are not matched.");
			goto undefined;
		}

		/* Reset wkuppktfilter_reg */
		for (i = 0; i < 8; i++) {
			wkuppktfilter_reg[i] = 0;
		}

		/* Set wkuppktfilter_reg */
		filter = 0;
		for (i = 0; i < wol_status.src_length; i++) {
			wkuppktfilter_reg[filter] |= (0x1 << i);
		}

		if (!filter) {
			wkuppktfilter_reg[4] |= (0x1 << (filter * 8));
		} else {
			wkuppktfilter_reg[4] |= (0x3 << (filter * 8));
		}

		wkuppktfilter_reg[5] |=
		((ETHERNET_HEADER + wol_status.src_offset) << (filter * 8));

		crc = cal_wof_crc16(wol_status.src_ip, wol_status.src_length);

		wkuppktfilter_reg[6 + filter / 2] = (crc <<
						     (filter % 2) * 16);

		/* If remote wake-up FIFO point is not zero,
		 * wake-up frame filter register shoule be reset
		 */
		reg = synopGMACReadReg((u32 *)gmacdev->MacBase,
				       GmacPmtCtrlStatus);
		if (((reg >> 24) & 0x1f))
			synopGMACSetBits((u32 *)gmacdev->MacBase,
					 GmacPmtCtrlStatus, 0x80000000);

		/* Set wake-up frame filter register in sequence */
		for (i = 0 ; i < 8 ; i++)
			synopGMACWriteReg((u32 *)gmacdev->MacBase,
					  GmacWakeupAddr, wkuppktfilter_reg[i]);
	}

	return count;

undefined:
	pr_crit("[NTKETHMAC] Please input defined command.\n");

	return count;
}

static const struct file_operations synop_dbg_anypacket_wol_fops = {
	.open = synop_dbg_anypacket_wol_open,
	.read = seq_read,
	.write = synop_dbg_anypacket_wol_write,
	.llseek = seq_lseek,
...
/* create debugfs node! */
static void
synopgmac_add_debugfs(struct net_device *netdev)
{
	struct synop_adapter_struct *adapter;
	struct synop_device *gmacdev;
	struct dentry *root;
	...
	adapter = netdev_priv(netdev);
	if (!adapter) {
		pr_crit("Unknown Device\n");
		return;
	}

	gmacdev = adapter->synopGMACdev;
	if (!gmacdev) {
		pr_crit("GMAC device structure is missing\n");
		return;
	}
	...
	gmacdev->debugfs_root = root;

	if (!debugfs_create_file("mdio", S_IRUSR, root, gmacdev,
				 &synopgmac_internal_PHY_regdump_fops))
		goto err_node;

	if (!debugfs_create_file("ldo", S_IRUSR, root, gmacdev,
				 &synop_internal_PHY_dbg_ldo_fops))
		goto err_node;

	if (!debugfs_create_file("regdump", S_IRUSR, root, gmacdev,
				 &synop_dbg_regdump_fops))
		goto err_node;

	if (!debugfs_create_file("phy_status", S_IRUSR, root, netdev,
				 &synop_dbg_phy_status_fops))
		goto err_node;

	if (!debugfs_create_file("anypacket_wol", S_IRUSR, root, netdev,
				 &synop_dbg_anypacket_wol_fops))
		goto err_node;
...
err_node:
	...
	gmacdev->debugfs_root = NULL;
err_root:
	pr_crit("[NTKETHMAC]: failed to initialize debugfs\n");
}

static void
synopgmac_remove_debugfs(struct net_device *netdev)
{
	struct synop_adapter_struct *adapter;
	struct synop_device *gmacdev;
	...
	debugfs_remove_recursive(gmacdev->debugfs_root);
}
#endif
...
static int synopGMAC_suspend(struct device *pdev)
{
	...
	synopGMAC_read_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase,
			       PHY_CONTROL_REG, &saved_bmcr);
	pr_crit("%s: Save BMCR %04x\n", __func__, saved_bmcr);
...
static int synopGMAC_resume(struct device *pdev)
{
	...
	u32 desc_index;
	s32 skblen;
	s32 status_phy = -ESYNOPGMACNOERR;
	...
	synopGMAC_setup_tx_desc_queue(gmacdev,
				      NULL,
				      TRANSMIT_DESC_SIZE, RINGMODE);

	synopGMAC_setup_rx_desc_queue(gmacdev,
				      NULL,
				      RECEIVE_DESC_SIZE, RINGMODE);

	/* Setup the Tx and Rx descriptor base address. */
	synopGMAC_init_tx_desc_base(gmacdev);
	synopGMAC_init_rx_desc_base(gmacdev);

	do {
		skb = alloc_skb(netdev->mtu +
					ETHERNET_HEADER + ETHERNET_CRC,
					GFP_ATOMIC);

		if (!skb) {
			pr_crit("[NTKETHMAC] Alloc skb fail.\n");
			synopGMAC_giveup_rx_desc_queue(gmacdev,
							NULL, RINGMODE);
			synopGMAC_giveup_tx_desc_queue(gmacdev,
							NULL, RINGMODE);
			break;
		}
		...
		skblen = skb_tailroom(skb);
		if (skblen > 0) {
			status = synopGMAC_set_rx_qptr(gmacdev,
						virt_to_phys(skb->data),
						(u32)skblen,
						(u32)skb, 0, 0, 0,
						&desc_index);
		} else {
			pr_crit("[NTKETHMAC] Set Rx descriptor fail.\n");
			status = -1;
		}
	...
	if (!(data & Mii_Link)) {
		pr_warn("[NTKETHMAC] Resume Link is Down.\n");
		SysPlat_Init_Ethernet_Phy(gmacdev, 1);
	} else {
		ioremap_write(0xFD130284, 0x9);
		pr_crit("[NTKETHMAC]: 0xFD130288: 0x%x\n",
			ioremap_read(0xFD130288));
		pr_crit("[NTKETHMAC]: 0xFD13028C: 0x%x\n",
			ioremap_read(0xFD13028C));
		pr_crit("[NTKETHMAC]: 0xFD130284: 0x%x\n",
			ioremap_read(0xFD130284));
	}
	...
	if (gmacdev->ssLoopbackEnable) {
		...
		synopGMAC_write_phy_reg((u32 *)gmacdev->MacBase,
					...
					saved_bmcr);
		pr_crit("%s: Restore BMCR %04x\n", __func__, saved_bmcr);

		/* Restore loopback mode setting */
		synopGMAC_frame_filter_disable(gmacdev);
		synopGMAC_rx_tcpip_chksum_drop_disable(gmacdev);
	}

	pr_warn("[NTKETHMAC] Resume completed.\n");
	return 0;
}
...
s32 __init synopGMAC_init_network_interface(void)
{
	...
	*/
	...
	if (!synopGMACadapter->synopGMACdev) {
		free_netdev(netdev);
		pr_crit("[NTKETHMAC]: Alloc synopGMACdev Fail.\n");
		return -ESYNOPGMACNOMEM;
	}
	...
	*/
#ifdef USE_DEVICE_TREE
	synopGMAC_attach(gmacdev,
			0, 0,
			DEFAULT_PHY_BASE);
#else
	synopGMAC_attach(gmacdevv,
	...
	Read_trim_Wq = create_singlethread_workqueue("Eth_readTrim");
	...
	/*init tx spin_lock*/
	spin_lock_init(&gmacdev->tx_lock);
...
void __exit synopGMAC_exit_network_interface(void)
{
...
#ifdef CONFIG_DEBUG_FS
	synopgmac_remove_debugfs(synopGMACadapter->synopGMACnetdev);
#endif
	...
	flush_workqueue(Read_trim_Wq);
	destroy_workqueue(Read_trim_Wq);
