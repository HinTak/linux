...
{
	return dma_alloc_coherent(p_device, size, phyAddr, GFP_KERNEL);
}

/*  memory free for non-cache */
void sys_plat_free_memory(u32 *addr, size_t len)
{
	dma_free_coherent(p_device, len, addr, virt_to_phys(addr));
}
	...
	struct pci_dev *pcidev, u32 no_of_desc, u32 desc_mode)
{
	...
	if (desc_mode == RINGMODE) {
		...
		gmacdev->TxDescDma   = virt_to_phys((u32 *)dma_addr);
	...
	struct pci_dev *pcidev, u32 no_of_desc, u32 desc_mode)
{
	...
	if (desc_mode == RINGMODE) {
		...
		/*save the dma address*/
		gmacdev->RxDescDma   = virt_to_phys((u32 *)dma_addr);
