...
static int star_hw_clk_init(struct net_device *netdev)
{
	int ret;
...
static int star_hw_clk_deinit(struct net_device *netdev)
{
	star_private *star_prv = netdev_priv(netdev);
...
static int  star_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (deepidle == 1) {
		ret = star_hw_clk_init(netdev);
		if (ret) {
			STAR_PR_ERR("star_hw_clk_init fail.\n");
			return ret;
		}
	}
...
struct fb_notifier_data {
	struct notifier_block fb_notifier;
	struct net_device *netdev;
};

static int _mt_eth_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	struct fb_event *evdata = data;
	int blank, err;
	struct fb_notifier_data *eth_fb_notifier_data =
		container_of(self, struct fb_notifier_data, fb_notifier);
	struct net_device *netdev = eth_fb_notifier_data->netdev;
	...
	pr_debug("@%s: blank = %d, event = %lu\n", __func__, blank, event);

	switch (blank) {
	/* LCM ON */
	case FB_BLANK_UNBLANK:
		if (deepidle == 1) {
			err = star_hw_clk_init(netdev);
			if (err) {
				STAR_PR_ERR("star_hw_clk_init fail.\n");
				return err;
			}
...
static struct notifier_block _mt_eth_fb_notifier = {
	.notifier_call = _mt_eth_fb_notifier_callback,
};

static struct fb_notifier_data _mt_eth_fb_notifier_data;
...
static int star_probe(struct platform_device *pdev)
{
	...
	_mt_eth_fb_notifier_data.netdev = netdev;
	_mt_eth_fb_notifier_data.fb_notifier = _mt_eth_fb_notifier;

	if (fb_register_client(&_mt_eth_fb_notifier_data.fb_notifier)) {
		STAR_PR_ERR("@%s: register FB client failed!\n", __func__);
	}
