...
GAMC_DRV_MAKE_VER	("v2.00", "20160817 - change code for 64bit & fix dma_map_single warning")
...
static int sdp_mac_i2c_read_nvt_ephy(struct mii_bus *bus, int phy_id, int regnum){
	...
	ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_14, 0x10, 0x0294);
	...
	if(ephy_priv->selftest_enable)
		...
		ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_14, 0x00, 0x0294);	//[2]1->0 read end
...
static int sdp_mac_i2c_write_nvt_ephy(struct mii_bus *bus, int phy_id, int regnum, u16 val){
	...
	ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_14, 0x14, 0x0c34);	//[2]0->1write start, [5]write mode : 0, [4]i_full_man : 1
	udelay(500);
	if(ephy_priv->selftest_enable)
		ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_14, 0x34, 0x0294);	//[2]1->0 write end
	else
		ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_14, 0x00, 0x0294);	//[2]1->0 write end

	return 0;

}

int musem_tztv_sys_is_oc_support(void)
{
...
static int sdp_mac_mdio_read(struct mii_bus *bus, int phy_id, int regnum)
{
	...
	if(musem_tztv_sys_is_oc_support() == 1){
...
sdpGmac_abnormal_intr_status (const u32 intrStatus, struct net_device* const pNetDev)
{
	...
	if(intrStatus & B_RX_BUF_UNAVAIL) {
		dev_printk(KERN_NOTICE, pGmacDev->pDev, "Rx buffer unavailable Error: int 0x%08x, rx head %lu, rx tail %lu, DMACur %zu\n", 
...
sdp_mac_selftest_do(struct net_device *pNetDev, enum sdp_mac_selftest_num test)
{
	...
	if((phydev->phy_id&0xFFFFFFFF) == 0x00000001){  //in case of oc_board 
		...
		if(eth_loopback == NULL){
			printk("sdp-gmac sdp_mac_selftest_do no symbol %s\n", "sdp_ocl_set_eth_loopback");
			return	-1;
		}
		while(!(phy_read(phydev, MII_BMSR) & BMSR_LSTATUS)){
			if(cnt > 10){
				printk("sdp-gmac forced_link_up was failed\n");
				return -ETIMEDOUT;
			}
			printk("sdp-gmac try forced_link_up for NVT\n");
			eth_loopback(true);
			mdelay(500);
			cnt++;
		}
	...
	if(test != SDP_MAC_LB) {
		...
		while(!(phydev->link)) {
			if(time_before(jiffies, timeout)) {
				...
				sdp_phy_reset(pNetDev, 0);
				phy_write(phydev, MII_BMCR, bmcr);
				
				ret = genphy_read_status(phydev);
				mdelay(50);
				if((ret == 0) && (phydev->link == 1)) {
					dev_err(pGmacDev->pDev, "%s link waiting. schedule delayed... %lums\n",
