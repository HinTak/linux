...
sdp_ring_clear(SDP_GMAC_DEV_T* pGmacDev, struct sdp_mac_desc_ring *ring)
{
	...
	while(CIRC_CNT(head, tail, ring->size) >= 1) {
		...
		if((length) && (desc->data2)){
			DPRINTK_GMAC("%s ring %lu clear length2 %x, data1 %lx\n",
...
sdpGmac_netDev_rx(struct net_device *pNetDev, int budget)
{
	...
	struct rtnl_link_stats64 *pNetStats64 = &pGmacDev->stats64;
	struct sk_buff * pSkb;
	DMA_DESC_T	rxDesc, *rxDesc_dump;
	
	struct sdp_mac_desc_ring *rx_ring_desc;
	u32 checkStatus;
	u32 len, index;
	int descIndex;
	long descIndex_dump;
	...
	while(!sdp_ring_is_empty(pGmacDev, &pGmacDev->rx_ring)){
		...
		descIndex_dump = (uintptr_t)&rxDesc;
...
if(false) {
	...
	print_hex_dump(KERN_DEBUG, "Rx buffer: ", DUMP_PREFIX_ADDRESS, 16, 1,
		...
		if(!checkStatus && (rxDesc.status & RDES0_LS) && (rxDesc.status & RDES0_FS))
		{
			...
			if(pSkb->data_len){
				rx_ring_desc = &pGmacDev->rx_ring;
				index = rx_ring_desc->tail;
				desc_base_addr = (uintptr_t *)&rx_ring_desc->pdesc[0];
				skb_addr = (uintptr_t *)pSkb;

				pr_err("Rx Desc %d for gmac_skb.... sk_buff:0x%lx, pSkb:0x%lx, data_len:0x%08x, rxDesc addr:0x%lx, rxDesc addr dump:0x%lx \n", \
					descIndex, (uintptr_t)rxDesc.data1, (uintptr_t)pSkb, pSkb->data_len, (uintptr_t)&rxDesc, descIndex_dump);

				pr_err("----------------------------------SDP_GMAC_SKB_DUMP------------------------------------\n");
				for(index = 0; index < 0x6C; index+=(sizeof(uintptr_t)*4)){
					pr_err("0x%lx+0x%02x : 0x%lx, 0x%lx, 0x%lx, 0x%lx\n", \
						(uintptr_t)skb_addr, index, *(skb_addr+index/4), *(skb_addr+index/4+1), *(skb_addr+index/4+2), *(skb_addr+index/4+3));
				}
				pr_err("---------------------------------------SDP_GMAC_DESCRIPTO_DUMP------------------------------------------\n");
				
#ifndef CONFIG_PHYS_ADDR_T_64BIT
				pr_err("desc index	  status	 length 	 buffer1	  buffer2	   data1	  data2 	 reserved1		reserve2\n");
				for(index = 0; index < rx_ring_desc->tail; index++){
					rxDesc_dump = &rx_ring_desc->pdesc[index];
					pr_err("0x%lx+0x%x : 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%lx, 0x%lx, 0x%08x, 0x%08x\n", \
					(uintptr_t)desc_base_addr, index*16, rxDesc_dump->status, rxDesc_dump->length, rxDesc_dump->sw_buffer1, rxDesc_dump->sw_buffer2,\
					(uintptr_t)rxDesc_dump->data1, (uintptr_t)rxDesc_dump->data2, rxDesc_dump->reserved1, rxDesc_dump->reserved2);
				}
				pr_err("0x%lx : 0x%x, 0x%x, 0x%x, 0x%x, 0x%lx, 0x%lx, 0x%x, 0x%x\n", \
					(uintptr_t)&rxDesc, rxDesc.status, rxDesc.length, rxDesc.sw_buffer1, rxDesc.sw_buffer2, (uintptr_t)rxDesc.data1, (uintptr_t)rxDesc.data2, rxDesc.reserved1, rxDesc.reserved2);
#else
				pr_err("desc index    status     length      buffer1      buffer2      data1      data2\n");
				for(index = 0; index < rx_ring_desc->tail-1; index++){
					rxDesc_dump = &rx_ring_desc->pdesc[index];
					pr_err("0x%lx+0x%x : 0x%08x, 0x%08x, 0x%llx, 0x%llx, 0x%lx, 0x%lx\n", \
					(uintptr_t)desc_base_addr, index*16, rxDesc_dump->status, rxDesc_dump->length, rxDesc_dump->sw_buffer1, rxDesc_dump->sw_buffer2, (uintptr_t)rxDesc_dump->data1, (uintptr_t)rxDesc_dump->data2);
				}	
				pr_err("0x%lx : 0x%x, 0x%x, 0x%llx, 0x%llx, 0x%lx, 0x%lx\n", \
					(uintptr_t)&rxDesc, rxDesc.status, rxDesc.length, rxDesc.sw_buffer1, rxDesc.sw_buffer2, (uintptr_t)rxDesc.data1, (uintptr_t)rxDesc.data2);
#endif
...
sdp_phy_reset(struct net_device *pNetDev, u32 *data)
{
	...
	printk("sdp-gmac phy reset start\n");
			       ...
			       size_t count, loff_t *ppos)
{
	...
	if (strtobool(buf, &bv) == 0) {
		if(bv == true) {
			...
			phy_start(phydev);
			dev_info(pGmacDev->pDev, "DBG: Done.\n");
		} else {
