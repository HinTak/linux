/***************************************************************************
 *
 *	drivers/net/ethernet/sdp/sdp_mac.c
 ...
 * 2015.03.05,drain.lee: fix prevent warning v1.25
 * 2015.07.29,drain.lee: add ethtool WOL(for PHY) v1.26
 * 2015.08.27,drain.lee: add AXI Bus Setting for Jazz-M v1.27
 * 2015.09.18,drain.lee: add statistics debugfs v1.28
 * 2015.10.19,drain.lee: add PHY start/stop debugfs v1.29
 * 2015.10.26,drain.lee: add priv private statistics v1.30
 * 2015.11.18,drain.lee: add frame filter dev attribute v1.31
 * 2015.12.14,drain.lee: fix selftest timeout check code v1.32
 * 2015.01.12,drain.lee: fix selftest increase timeout value v1.33
 * 2015.02.15,drain.lee: add mac fifo flush timeout v1.34
 * 2015.02.23,drain.lee: increase budget when rx buffer is unavailable v1.35
 * 2016.03.03,drain.lee: new start for linux4.1(2017YR) v2.00
 * 2016.08.17,jk97.son: change code for 64bit & fix dma_map_single warning
 * 2016.12.26,jk97.son: restart autoneg in 10Mbps/Half - flow control off
 * 2017.12.20,jk97.son: print NVT_rx_ephy_BMCR for sdp1701(kantm2) before ethtool_selft test
 * 2019.05.15,jk97.son: reduce debug log of rx over flow
 * 2019.09.04,jk97.son: support 1g path for nikem
 * 2019.11.04,jk97.son: change resume&suspend state && the number of descriptors(from pagesize*12 to pagesize*16)
 */

#define GAMC_DRV_MAKE_VER(ver, desc) \
static const char const * GMAC_DRV_VERSION=ver;	\
static const char const * GMAC_DRV_VERSION_DESC=desc;

GAMC_DRV_MAKE_VER	("v2.00", "2019.09.04,jk97.son: support 1g path for nikem")
...
static inline u32 sdp_mac_phyreadl(phys_addr_t phyaddr)
{
	volatile void __iomem *addr;
	u32 value;

	addr = ioremap_nocache(phyaddr, sizeof(phys_addr_t));
	value = sdp_mac_readl(addr);
	iounmap(addr);
	return value;
}

static inline unsigned long long sdp_mac_get_nsec(void)
{
#ifdef CONFIG_USE_HW_CLOCK_FOR_TRACE
...
static void sdp_mac_gmac_dump(SDP_GMAC_DEV_T *pGmacDev)
{
	...
	resource_size_t phy_base = 0x0;
	struct resource *res;
	u32 i;

	res = platform_get_resource(to_platform_device(pGmacDev->pDev), IORESOURCE_MEM, 0);
	if(!res) {
		dev_err(pGmacDev->pDev, "%s: platform_get_resource return NULL\n", __FUNCTION__);
		return;
	}
	...
	dev_err(pGmacDev->pDev, "===== SDP-MAC GMAC DUMP (VA: 0x%p, PA: 0x%08llx) =====\n",
		pGmacBase, (u64)phy_base);
	for(i = 0; i < ARRAY_SIZE(gmac_reg_dump); i+=4) {
		dev_err(pGmacDev->pDev, "0x%08llx: 0x%08x 0x%08x 0x%08x 0x%08x\n",
...
static void sdp_mac_dma_dump(SDP_GMAC_DEV_T *pGmacDev)
{
	...
	phy_base = res->start + 0x1000;//mac base address + dma offset
	...
	dev_err(pGmacDev->pDev, "===== SDP-MAC DMA DUMP (VA: 0x%p, PA: 0x%08llx) =====\n",
...
static void sdp_mac_ephy_config_dump(SDP_GMAC_DEV_T *pGmacDev)
{
	int i = 0;
	u32 value;
	const void *dump_info[][3] = {
		/* desc, phy address, bit num */
#if defined(CONFIG_ARCH_SDP1406)/* Hawk-M All */
		{"EPHY_PLL_ENABLE_MC", (void *)0x00800328, (void *)28},
		{"EPHY_ENABLE_CPU   ", (void *)0x005C00E0, (void *)23},
		{"EPHY_ENABLE_CPU   ", (void *)0x00140000, (void *)12},
#elif defined(CONFIG_ARCH_SDP1501)/* Jazz-M All */
		{"EPHY_PLL_RESET_MC", (void *)0x007C0328, (void *)29},
		{"EPHY_RESET_MC    ", (void *)0x007C0328, (void *)28},
		{"EPHY_ENABLE_MC   ", (void *)0x007C0328, (void *)30},
		{"EPHY_RESET_CPU   ", (void *)0x005C07B8, (void *)24},
		{"EPHY_ENABLE_CPU  ", (void *)0x00140000, (void *)12},
#endif
	};

	/* EPHY Reset State dump for checking valid MAC input clock */

	for(i = 0; i < ARRAY_SIZE(dump_info); i++) {
		value = sdp_mac_phyreadl((phys_addr_t)((size_t)dump_info[i][1]));
		dev_err(pGmacDev->pDev, "%s: 0x%lu[%lu] == %u(raw 0x%x)\n",
			(const char *)dump_info[i][0], (uintptr_t)dump_info[i][1], (uintptr_t)dump_info[i][2],
			(value >> (uintptr_t)dump_info[i][2]) & 0x1, value);
	}
}
...
void sdp_mac_descriptor_dump(SDP_GMAC_DEV_T *pGmacDev, struct sdp_mac_desc_ring *ring){
	int err;
	unsigned long head;
	unsigned long tail;
	unsigned long flags;
	...
	dev_err(pGmacDev->pDev, "---------------------------------------SDP_GMAC_DESCRIPTO_DUMP START(%s)------------------------------------------\n",\
		ring == &pGmacDev->tx_ring ? "TX DMA" : "RX_DMA");
	dev_err(pGmacDev->pDev, "head : %ld, tail : %ld\n", head, tail);
	...
	while(CIRC_CNT(head, tail, ring->size) >= 1){
		...
		dev_err(pGmacDev->pDev, "desc index  status      length      buffer1     buffer2     data1       data2       reserved1   reserve2\n");
		dev_err(pGmacDev->pDev, "[%02ld]  0x%08x  0x%08x  0x%08x  0x%08x  0x%08lx  0x%08lx  0x%08x  0x%08x\n", \
				tail, desc->status, desc->length, desc->sw_buffer1, desc->sw_buffer2,\
				(uintptr_t)desc->data1, (uintptr_t)desc->data2, desc->reserved1, desc->reserved2);
#else
		dev_err(pGmacDev->pDev, "desc index  status      length      buffer1     buffer2     data1       data2\n");
		dev_err(pGmacDev->pDev, "[%02ld]  0x%08x  0x%08x  0x%08llx  0x%08llx  0x%08lx  0x%08lx\n", \
				tail, desc->status, desc->length, desc->sw_buffer1, desc->sw_buffer2, (uintptr_t)desc->data1, (uintptr_t)desc->data2);
#endif
	...
	dev_err(pGmacDev->pDev, "---------------------------------------SDP_GMAC_DESCRIPTO_DUMP END------------------------------------------\n");
	...
	while(true) {
		...
		if(busy_count >= 100000) {
			dev_err(pGmacDev->pDev, "MDIO busy wait timeout!!"
				"(busy_count %d * %uus,  mdio_ctrl 0x%08x, priv->addr 0x%lx, priv->data 0x%lx)\n",
				busy_count, udelay_time, mdio_ctrl, (uintptr_t)priv->addr, (uintptr_t)priv->data);
...
static int sdp_mac_i2c_read_nvt_VDIP(SDP_GMAC_DEV_T *pGmacDev, u32 reg){
	u32 SLAVEADR_TX_VDIP = 0x54;
	int (*ocl_i2c_read)(u32, u32, u32 *)=NULL;
	u32 buf = 0;

	ocl_i2c_read = symbol_request(SDP_OCL_I2C_ReadWord);
	if(ocl_i2c_read == NULL){
		printk("sdp-gmac sdp_mac_i2c_read_nvt_VDIP no symbol %s\n", "SDP_OCL_I2C_ReadWord");
		return	-0x10000;
	}

	ocl_i2c_read(SLAVEADR_TX_VDIP, reg, &buf);
	dev_err(pGmacDev->pDev, "VDIP 0x14: 0x%x!!\n", buf);

	return buf;

}

int is_oc_support(void){
#if defined(CONFIG_ARCH_SDP1803)
	if(ephy_serdes_path == NULL){
		ephy_serdes_path=ioremap(EMAC_CTRL_0, sizeof(u32));
	}

	/*refer to sdp_ocl_dev.c -> sdp_rx_init_eth_path()*/
	...
	if(emac_base_addr == NULL){
		emac_base_addr = ioremap(EMAC_CTRL_0, sizeof(u32)*8);
	}

	/* serdes lock check */
	if(readl(emac_base_addr+4) & (1 << SERDES_LOCKED_BIT)){
		return 1;
	}
	else{
		return 0;
...
int is_rgmii_support(void)
{
#if defined(CONFIG_ARCH_SDP_NIKE)
	if(readl(emac_base_addr+4) & (1 << SERDES_RGMII_BIT)){
		//RGMII
		return 1;
	}
	else{
...
static int sdp_mac_i2c_read_nvt_ephy(struct mii_bus *bus, int phy_id, int regnum){
	u32 SLAVEADR_TX_VDIP = 0x54;
	u32 u32SubAddr_1E = 0x1E;
	u32 u32SubAddr_14 = 0x14;
	u32 u32SubAddr_B1 = 0xB1;

	int (*ocl_i2c_write)(u32, u32, u32, u32)=NULL;
	int (*ocl_i2c_read)(u32, u32, u32 *)=NULL;
	u32 buf = 0;
	struct sdp_mac_mdiobus_priv *priv = bus->priv;
	SDP_GMAC_DEV_T *pGmacDev = priv->pGmacDev;
	struct sdp_mac_phy_priv *ephy_priv = pGmacDev->phydev->priv;

	ocl_i2c_write = symbol_request(SDP_OCL_I2C_MaskWriteWord);
	if(ocl_i2c_write == NULL){
		printk("sdp-gmac nvt_mdio_path_disable no symbol %s\n", "SDP_OCL_I2C_MaskWriteWord");
		return	-0x10000;
	}
	...
	ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_14, 0x30, 0x0034);
	ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_1E, (regnum<<8) | phy_id, 0x1F1F);
	ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_14, 0x34, 0x0034);	//[2]1->0 read start, [5]read mode : 1, [4]i_full_man : 1
	udelay(500);
	ocl_i2c_read(SLAVEADR_TX_VDIP, u32SubAddr_B1, &buf);
	if(ephy_priv->selftest_enable)
		ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_14, 0x34, 0x0294);	//[2]1->0 write end
	else
		ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_14, 0x30, 0x0034);	//[2]1->0 read end

	return buf&0xFFFF;

}

static int sdp_mac_i2c_write_nvt_ephy(struct mii_bus *bus, int phy_id, int regnum, u16 val){
	u32 SLAVEADR_TX_VDIP = 0x54;
	u32 u32SubAddr_1E = 0x1E;
	u32 u32SubAddr_14 = 0x14;
	u32 u32SubAddr_B1 = 0xB1;
	u32 u32SubAddr_1F = 0x1F;
	...
	ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_14, 0x10, 0x0034);
	ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_1E, (regnum<<8) | phy_id, 0x1F1F);
	ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_1F, val, 0xFFFF);
	ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_14, 0x14, 0x0034);	//[2]0->1write start, [5]write mode : 0, [4]i_full_man : 1
	udelay(500);
	if(ephy_priv->selftest_enable)
		ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_14, 0x34, 0x0294);	//[2]1->0 write end
	else
		ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_14, 0x30, 0x0034);	//[2]1->0 write end
...
static int sdp_mac_mdio_read(struct mii_bus *bus, int phy_id, int regnum)
{
	...
	if(is_oc_support() == 1){
		return sdp_mac_i2c_read_nvt_ephy(bus, phy_id, regnum);
	}
	else{
...
static int sdp_mac_mdio_write(struct mii_bus *bus, int phy_id, int regnum, u16 val)
{
	...
	if(is_oc_support() == 1){
		return sdp_mac_i2c_write_nvt_ephy(bus, phy_id, regnum, val);
	}
	else{
		ret = sdp_mac_mdio_wait(bus);
		if(ret < 0) {
		...
		if(pGmacDev->forced_phylb && !(val&BMCR_LOOPBACK)) {
			dev_info(pGmacDev->pDev, "MDIO write forced phy loopback enable!!"
				"(ret %d, phyid %d, regnum %d, val %u)\n",
				ret, phy_id, regnum, val);
			val |= BMCR_LOOPBACK;
		}
...
sdp_ring_clear(SDP_GMAC_DEV_T* pGmacDev, struct sdp_mac_desc_ring *ring)
{
	...
	while(CIRC_CNT(head, tail, ring->size) >= 1) {
		...
		if((length) && (desc->data1)){
			DPRINTK_GMAC("%s ring %lu clear length1 %x, data1 %lx\n",
				ring->name, tail, length, (uintptr_t)desc->data1);
			dma_unmap_single(pGmacDev->pDev, desc->sw_buffer1,
					length, DMA_FROM_DEVICE);
			dev_kfree_skb_any((struct sk_buff*) desc->data1);
		}
		
		length = DESC_GET_SIZE2(desc->length);
		if((length) && (desc->data2)){
			DPRINTK_GMAC("%s ring %lu clear length2 %x, data2 %lx\n",
				ring->name, tail, length, (uintptr_t)desc->data2);
			dma_unmap_single(pGmacDev->pDev, desc->sw_buffer2,
					length, DMA_FROM_DEVICE);
			dev_kfree_skb_any((struct sk_buff*) desc->data2);
		}
...
sdp_mac_push_rxdesc(SDP_GMAC_DEV_T *pGmacDev, sdp_mac_desc_info_t *info, DMA_DESC_T *set_desc/*IN*/) {
	...
	if(sdpGmac_rxDescChained(set_desc)){
		...
		info->desc->sw_buffer1 = set_desc->sw_buffer1;
		info->desc->sw_buffer2 = set_desc->sw_buffer2;
...
sdpGmac_skbAlign(SDP_GMAC_DEV_T* pGmacDev, struct sk_buff *pSkb) {
	u32 offset = (uintptr_t)pSkb->data & (uintptr_t)(~pGmacDev->bus_mask);
	u32 reserve_size = 0;
...
sdpGmac_netDev_tx(struct net_device *pNetDev, int budget)
{
	...
	struct rtnl_link_stats64 *pNetStats64 = &pGmacDev->stats64;
	DMA_DESC_T	txDesc;
	...
	while(!sdp_ring_is_empty(pGmacDev, &pGmacDev->tx_ring)) {
		...
		DPRINTK_GMAC_FLOW("Tx Desc %d for skb 0x%lx whose status is %08x\n",
					descIndex, (uintptr_t)txDesc.data1, txDesc.status);

		pSkb = (struct sk_buff*)txDesc.data1;

		dma_unmap_single(pGmacDev->pDev, txDesc.sw_buffer1,	pSkb->len, DMA_TO_DEVICE);

		if(eth_hdr(pSkb)) {
			if(is_unicast_ether_addr(eth_hdr(pSkb)->h_dest)) pGmacDev->priv_stats64.tx_unicast_packet++;
			else if(is_broadcast_ether_addr(eth_hdr(pSkb)->h_dest)) pGmacDev->priv_stats64.tx_broadcast_packet++;
			else if(is_multicast_ether_addr(eth_hdr(pSkb)->h_dest)) {
				pNetStats64->multicast++;
				pGmacDev->priv_stats64.tx_multicast_packet++;
			}
		}
		...
		if(!checkStatus) {
			pNetStats64->tx_bytes += txDesc.length & DESC_SIZE_MASK;
			pNetStats64->tx_packets++;

		}
		else {	// ERROR
			pNetStats64->tx_errors++;

			if(txDesc.status & (TDES0_LCO | TDES0_EC))
				pNetStats64->tx_aborted_errors++ ;
			if(txDesc.status & (TDES0_LC | TDES0_NC))
				pNetStats64->tx_carrier_errors++;
		}

		pNetStats64->collisions += TDES0_CC_GET(txDesc.status);
...
sdpGmac_netDev_rx(struct net_device *pNetDev, int budget)
{
	...
	while(!sdp_ring_is_empty(pGmacDev, &pGmacDev->rx_ring)){
		...
		DPRINTK_GMAC("Rx Desc %d for skb 0x%lx whose status is 0x%08x\n",
				descIndex, (uintptr_t)rxDesc.data1, rxDesc.status);


		dma_unmap_single(pGmacDev->pDev, rxDesc.sw_buffer1, DESC_GET_SIZE1(rxDesc.length), DMA_FROM_DEVICE);
...
if(false) {
	...
	print_hex_dump(KERN_DEBUG, "Rx buffer: ", DUMP_PREFIX_ADDRESS, 16, 1,
		...
		if(!checkStatus && (rxDesc.status & RDES0_LS) && (rxDesc.status & RDES0_FS))
		{
			...
			if(pSkb->data_len){
				pr_err("Rx Desc %d for gmac_skb.... sk_buff:0x%lx, pSkb:0x%lx, data_len:0x%08x, len:0x%08x, rxDesc addr:0x%lx\n", \
					descIndex, (uintptr_t)rxDesc.data1, (uintptr_t)pSkb, pSkb->data_len, pSkb->len, (uintptr_t)&rxDesc);
				pSkb->data_len = 0;
			}
			...
			/* stat */
			if(eth_hdr(pSkb)) {
				if(is_unicast_ether_addr(eth_hdr(pSkb)->h_dest)) pGmacDev->priv_stats64.rx_unicast_packet++;
				else if(is_broadcast_ether_addr(eth_hdr(pSkb)->h_dest)) pGmacDev->priv_stats64.rx_broadcast_packet++;
				else if(is_multicast_ether_addr(eth_hdr(pSkb)->h_dest)) {
					pNetStats64->multicast++;
					pGmacDev->priv_stats64.rx_multicast_packet++;
				}
			}
			...
			pNetStats64->rx_packets++;
			pNetStats64->rx_bytes += len;


		} 
		else {	// ERROR
			pNetStats64->rx_errors++;

			if(rxDesc.status & RDES0_OE) pNetStats64->rx_over_errors++;
			if(rxDesc.status & RDES0_LC) pNetStats64->collisions++;
			if(rxDesc.status & RDES0_RWT) pNetStats64->rx_frame_errors++;
			if(rxDesc.status & RDES0_CE) pNetStats64->rx_crc_errors++ ;
			if(rxDesc.status & RDES0_DBE) pNetStats64->rx_frame_errors++ ;
			if(rxDesc.status & RDES0_LE) pNetStats64->rx_length_errors++;
...
__set_rx_qptr:
		...
		if (dma_mapping_error(pGmacDev->pDev, rxDmaAddr)) {
            DPRINTK_GMAC_ERROR("dma_map_single failed \n");
			break;
	    }
	...
	if(pGmacDev->is_rx_stop) {
		dev_printk(KERN_DEBUG, pGmacDev->pDev, "restart receive frame!: new free desc %d, rx head %lu, rx tail %lu, DMACur %zu\n", 
...
sdpGmac_abnormal_intr_status (const u32 intrStatus, struct net_device* const pNetDev)
{
	...
	if(intrStatus & B_RX_BUF_UNAVAIL) {
		dev_printk(KERN_ERR, pGmacDev->pDev, "Rx buffer unavailable Error: int 0x%08x, rx head %lu, rx tail %lu, DMACur %zu\n", 
			intrStatus, pGmacDev->rx_ring.head, pGmacDev->rx_ring.tail, (pDmaReg->curHostRxDesc - pDmaReg->rxDescListAddr)/sizeof(DMA_DESC_T));
		pGmacDev->stats64.rx_dropped++;

		/* stop rx */
		if(pGmacDev->is_rx_stop == false) {
			pGmacDev->is_rx_stop = true;
			pGmacReg->configuration &= ~B_RX_ENABLE;
			dev_printk(KERN_ERR, pGmacDev->pDev, "stop receive frame!\n");
		}
	...
	if(intrStatus & B_RX_OVERFLOW) {
		pGmacDev->stats64.rx_over_errors++;
		DPRINTK_GMAC_ERROR("Rx overflow Error(intrStatus:0x%08x, rx_over_error:%lld)\n", intrStatus, pGmacDev->stats64.rx_over_errors);
		if(time_after(jiffies, prev_intr_time + msecs_to_jiffies(intr_timeout_ms))){
			sdp_mac_gmac_dump(pGmacDev);
			sdp_mac_dma_dump(pGmacDev);
...
sdp_mac_polling_callback(struct hrtimer *timer) {
	...
	hrtimer_start(&pGmacDev->polling_timer, ns_to_ktime((u64)pGmacDev->polling_interval_us*1000ULL), HRTIMER_MODE_REL);
...
static int sdp_mac_regset(struct sdp_mac_reg_set *regset) {
	...
	iomem = ioremap_nocache((phys_addr_t)regset->addr, sizeof(phys_addr_t));
	if(iomem) {
...
// DMA Reset
	...
	for(i = 1; pDmaReg->busMode & B_SW_RESET; i++) {
		if(i > 10000) {
			...
			sdp_mac_ephy_config_dump(pGmacDev);
			return -SDP_GMAC_ERR;
		}
		udelay(100);
...
// all interrupt disable
	...
	pGmacReg->interrupt = 0x60F; // all clear status register
...
#ifdef SDP_GMAC_USE_AXI_BUSMASTER
	pDmaReg->axiBusMode = B_AXI_BLEN4 | B_AXI_BLEN8 | B_AXI_BLEN16 |
						B_AXI_RD_OSR_LMT(0xF) | B_AXI_WR_OSR_LMT(0xF);
#endif
...
sdpGmac_netDev_getStats64(struct net_device *pNetDev, struct rtnl_link_stats64 *storage)
{
	SDP_GMAC_DEV_T * pGmacDev = netdev_priv(pNetDev);

	DPRINTK_GMAC_FLOW("called\n");

	*storage = pGmacDev->stats64;
...
sdpGmac_netDev_hardStartXmit(struct sk_buff *pSkb, struct net_device *pNetDev)
{
	...
	if (dma_mapping_error(pGmacDev->pDev, txDmaAddr)) {
		DPRINTK_GMAC_ERROR("dma_map_single failed \n");
		goto __xmit_out;
    }
	...
	while(sdp_ring_push(pGmacDev, &pGmacDev->tx_ring, &txDesc) < 0) {
		...
		pGmacDev->stats64.tx_errors++;
		pGmacDev->stats64.tx_dropped++;
		dev_kfree_skb(pSkb);
		goto __xmit_out;
	}
...
static int mac_config_setting(int speed){
	u32 SLAVEADR_TX_VDIP = 0x54;
	u32 u32SubAddr_92 = 0x92;
	int (*ocl_i2c_write)(u32, u32, u32, u32)=NULL;
	bool serdes_state = readl(emac_base_addr + 0x4) & (0x1 << SERDES_LOCKED_BIT);
	bool rgmii_state = readl(emac_base_addr + 0x4) & (0x1 << SERDES_RGMII_BIT);
	
	if(!rgmii_state && !serdes_state){
		/* sdp_ephy */		
		if(speed == SPEED_10){
			writel(0x00000000, emac_base_addr + 0x14);
			writel(0x00000000, emac_base_addr + 0x18);
		}
		else if(speed == SPEED_100){
			writel(0x00010001, emac_base_addr + 0x14);
			writel(0x00000000, emac_base_addr + 0x18);
		}
	}
	else if(!rgmii_state && serdes_state){
		/* oc nvt_ephy */
		if(speed == SPEED_10){
			writel(0x00000000, emac_base_addr + 0x14);
			writel(0x00000000, emac_base_addr + 0x18);
			writel(readl(emac_base_addr + 0x4) | 0x5, emac_base_addr + 0x4);
		}
		else if(speed == SPEED_100){
			writel(0x00010001, emac_base_addr + 0x14);
			writel(0x00000000, emac_base_addr + 0x18);
		...
		if(speed == SPEED_10){
			ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_92, 0x0, 0x3);
			writel(0x00000100, serdes_rx_vdip_addr);
			writel(0x00000000, serdes_eth_base_addr+0x98);
			writel(0x01000000, serdes_eth_base_addr+0xa0);
			writel(0x000a0002, emac_base_addr + 0x14);
			writel(0x00000001, emac_base_addr + 0x18);
			writel(readl(emac_base_addr + 0x4) | 0xa, emac_base_addr + 0x4);
		}
		else if(speed == SPEED_100){
			ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_92, 0x0, 0x3);
			writel(0x00000100, serdes_rx_vdip_addr);
			writel(0x00000001, serdes_eth_base_addr+0x98);
			writel(0x01000001, serdes_eth_base_addr+0xa0);
			writel(0x000b0003, emac_base_addr + 0x14);
			writel(0x00010001, emac_base_addr + 0x18);
			writel(readl(emac_base_addr + 0x4) | 0xa, emac_base_addr + 0x4);
		}
		else if(speed == SPEED_1000){
			ocl_i2c_write(SLAVEADR_TX_VDIP, u32SubAddr_92, 0x2, 0x3);
			writel(0x00000102, serdes_rx_vdip_addr);
			writel(0x00000002, serdes_eth_base_addr+0x98);
			writel(0x01000002, serdes_eth_base_addr+0xa0);
			writel(0x000c0004, emac_base_addr + 0x14);
			writel(0x00020001, emac_base_addr + 0x18);
			writel(readl(emac_base_addr + 0x4) | 0xa, emac_base_addr + 0x4);
		}
	}
	else{
		/* built-in external ephy */
		if(speed == SPEED_10){
			writel(0x000d0004, emac_base_addr+0x14);
			writel(0x0f020001, emac_base_addr+0x18);
			writel(0x00000110, emac_base_addr+0x8);
			writel(0x00000000, emac_base_addr+0xc);
			sdp_set_clkrst_mux(EMAC_PLL_CONFIG, 0xff, 0x63);
			writel(readl(emac_base_addr + 0x4) | 0xf, emac_base_addr + 0x4);
		}
		else if(speed == SPEED_100){
			writel(0x000d0004, emac_base_addr+0x14);
			writel(0x0f020001, emac_base_addr+0x18);
			writel(0x00010110, emac_base_addr+0x8);
			writel(0x00010000, emac_base_addr+0xc);
			sdp_set_clkrst_mux(EMAC_PLL_CONFIG, 0xff, 0x9);
			writel(readl(emac_base_addr + 0x4) | 0xf, emac_base_addr + 0x4);
			...
			writel(0x000d0004, emac_base_addr+0x14);
			writel(0x0f020001, emac_base_addr+0x18);
			writel(0x00020110, emac_base_addr+0x8);
			writel(0x00020000, emac_base_addr+0xc);
			sdp_set_clkrst_mux(EMAC_PLL_CONFIG, 0xff, 0x1);
			writel(readl(emac_base_addr + 0x4) | 0xf, emac_base_addr + 0x4);
		}
	}
...
sdp_mac_adjust_link(struct net_device *dev)
{
	...
	if (netif_running(dev) && phydev->link) {
		...
		if (!pGmacDev->oldlink) {
			...
			if(dev->irq <= 0) {
				hrtimer_start(&pGmacDev->polling_timer, ns_to_ktime((u64)pGmacDev->polling_interval_us*1000UL), HRTIMER_MODE_REL);
			}
		}
...
//			wmb();
		...
		unsigned long long timeout_ns = 0, now_ns;
		u32 opmode = 0;
...
//		wmb();
		...
		timeout_ns = sdp_mac_get_nsec() + 1000000000;/* 1sec timeout */
		opmode = readl(&pDmaReg->operationMode);
		while((opmode = readl(&pDmaReg->operationMode)) & (1<<20)) {
			now_ns = sdp_mac_get_nsec();
			if(timeout_ns <= now_ns) {
				/* timeout! one more check */
				if((opmode = readl(&pDmaReg->operationMode)) & (1<<20)) {
					/* real timeout */
					dev_err(pGmacDev->pDev, "fifo flush timeout(1s)! wait %lluns, OPMode[20] == %u(raw 0x%08x)\n",
						now_ns - (timeout_ns-1000000000), (opmode>>20)&0x1, opmode);
					sdp_mac_ephy_config_dump(pGmacDev);
				} else {
					dev_err(pGmacDev->pDev, "fifo flush delayed(1s)! wait %lluns, OPMode[20] == %u(raw 0x%08x)\n",
						now_ns - (timeout_ns-1000000000), (opmode>>20)&0x1, opmode);
				}
	...
	spin_unlock_bh(&pGmacDev->lock);
/*
	if(!(phydev->duplex) && (phydev->speed == SPEED_10) && (phydev->pause == 0x0) && (phydev->link == 0x1) && (phy_failed_count < 3) ){
		dev_warn(&dev->dev, "restart aneg\n");
		genphy_restart_aneg(phydev);
		while(!((phy_read(phydev, 31) >> 12) & 0x1) && (count < 20)){
			mdelay(100);
			count++;
			dev_warn(&dev->dev, "wait restart aneg done %d\n", count);
		}
		phy_failed_count++;
	}
...
*/	
	...
	if (netif_running(dev) && new_state && netif_msg_link(pGmacDev)){
		trace_early_message("link was changed(on/off)");
...
// VD network team request to delete code by below from nike chip
#if defined(CONFIG_ARCH_SDP1501) || defined(CONFIG_ARCH_SDP1406) || defined(CONFIG_ARCH_SDP1601) || defined(CONFIG_ARCH_SDP1412)
	...
	// all interrupt disable
	pDmaReg->intrEnable = 0;
	pDmaReg->status = B_INTR_ALL;	// Clear interrupt pending register
...
sdpGmac_netDev_ioctl (struct net_device *pNetDev, struct ifreq *pRq, int cmd)
{
	...
	if(!netif_running(pNetDev)) {
		switch(cmd) {
			...
			case SIOCSMIIREG:
#if 0
				struct mii_ioctl_data *mii_data = if_mii(pRq);
				/* check BMCR_LOOPBACK bit */
				if(mii_data->reg_num == MII_BMCR) {
					if(mii_data->val_in & BMCR_LOOPBACK) {
						pGmacDev->forced_phylb = true;
					} else {
						pGmacDev->forced_phylb = false;
					}
				}
#endif
				break;
...
static int sdpGmac_netDev_poll(struct napi_struct *napi, int budget)
{
	...
	if(pGmacDev->is_rx_stop) {
		modified_rx_budget = NUM_RX_DESC;
	} else {
		modified_rx_budget = budget;

	}
	work_done = sdpGmac_netDev_rx(pGmacDev->pNetDev, modified_rx_budget);
...
sdpGmac_ethtool_get_stats(struct net_device *pNetDev, struct ethtool_stats *stats, u64 *data)
{
	...
	data[0] = pNetStats64->rx_packets;
	data[1] = pNetStats64->tx_packets;
	data[2] = pNetStats64->rx_bytes;
	data[3] = pNetStats64->tx_bytes;
	data[4] = pNetStats64->rx_errors;
	data[5] = pNetStats64->tx_errors;
	data[6] = pNetStats64->rx_dropped;
	data[7] = pNetStats64->tx_dropped;
	data[8] = pNetStats64->multicast;
	data[9] = pNetStats64->collisions;
	data[10] = pNetStats64->rx_length_errors;
	data[11] = pNetStats64->rx_over_errors;
	data[12] = pNetStats64->rx_crc_errors;
	data[13] = pNetStats64->rx_frame_errors;
	data[14] = pNetStats64->rx_fifo_errors;
	data[15] = pNetStats64->rx_missed_errors;
	data[16] = pNetStats64->tx_aborted_errors;
	data[17] = pNetStats64->tx_carrier_errors;
	data[18] = pNetStats64->tx_fifo_errors;
	data[19] = pNetStats64->tx_heartbeat_errors;
	data[20] = pNetStats64->tx_window_errors;
	data[21] = pNetStats64->rx_compressed;
	data[22] = pNetStats64->tx_compressed;
...
sdp_mac_selftest_setup_desc(SDP_GMAC_DEV_T *pGmacDev)
{
	...
	dmaAddr = dma_map_single(pGmacDev->pDev, pSkb->data, pSkb->len, DMA_TO_DEVICE);
	if (dma_mapping_error(pGmacDev->pDev, dmaAddr)) {
            DPRINTK_GMAC_ERROR("dma_map_single failed \n");
			return -1;
	}
	...
	dmaAddr = dma_map_single(pGmacDev->pDev, pSkb->data, ETH_FRAME_LEN, DMA_FROM_DEVICE);
	if (dma_mapping_error(pGmacDev->pDev, dmaAddr)) {
        DPRINTK_GMAC_ERROR("dma_map_single failed \n");
		return -1;
	}
...
int v_readl(u32 p_addr){
	volatile void __iomem *iomem;
	int temp = 0;

	iomem = ioremap_nocache((phys_addr_t)p_addr, sizeof(int));
	temp = readl(iomem);
	iounmap(iomem);
...
void print_serdes_packet_crc(SDP_GMAC_DEV_T *pGmacDev){
	dev_err(pGmacDev->pDev, "CRC0_PACKET_COUNTER: 0x%x,	CRC0_CRC_COUNTER: 0x%x\n", v_readl(CRC0_PACKET_COUNTER), v_readl(CRC0_CRC_COUNTER));
	dev_err(pGmacDev->pDev, "CRC1_PACKET_COUNTER: 0x%x,	CRC1_CRC_COUNTER: 0x%x\n", v_readl(CRC1_PACKET_COUNTER), v_readl(CRC1_CRC_COUNTER));
	dev_err(pGmacDev->pDev, "CRC2_PACKET_COUNTER: 0x%x,	CRC2_CRC_COUNTER: 0x%x\n", v_readl(CRC2_PACKET_COUNTER), v_readl(CRC2_CRC_COUNTER));
	dev_err(pGmacDev->pDev, "CRC3_PACKET_COUNTER: 0x%x,	CRC3_CRC_COUNTER: 0x%x\n", v_readl(CRC3_PACKET_COUNTER), v_readl(CRC3_CRC_COUNTER));
}
#endif
...
sdp_mac_selftest_compare_frame(SDP_GMAC_DEV_T *pGmacDev, const char *header)
{
	...
	dma_unmap_single(pGmacDev->pDev, dmaDesc_tx.sw_buffer1, pSkb_tx->len, DMA_TO_DEVICE);
	...
	skb_put(pSkb_rx, RDES0_FL_GET(dmaDesc_rx.status) - ETH_FCS_LEN);

	dma_unmap_single(pGmacDev->pDev, dmaDesc_rx.sw_buffer1, ETH_FRAME_LEN, DMA_FROM_DEVICE);
	...
	if(err) {
		...
		dev_err(pGmacDev->pDev,
			"tx desc status %#x, rx desc status %#x, txlen %u rxlen %u\n",
		...
		pr_err("\n");
#if defined(CONFIG_ARCH_SDP1803) || defined(CONFIG_ARCH_SDP_NIKE)
		print_serdes_packet_crc(pGmacDev);
#endif
		err = -1000;
	}
	dev_err(pGmacDev->pDev, "phy addr txbuf: 0x%lx, rxbuf: 0x%lx\n",
		(uintptr_t)dmaDesc_tx.sw_buffer1 , (uintptr_t)dmaDesc_rx.sw_buffer1);
	pr_err("\n");
...
sdp_mac_selftest_mac_xfer(SDP_GMAC_DEV_T *pGmacDev, const char *header)
{
	...
	pDmaReg->status = B_INTR_ALL;//all clear
	...
	while(!(readl(&pDmaReg->status)&B_TX_INTR)) {
		if(time_is_before_jiffies(timeout)) {

			dev_err(pGmacDev->pDev, "dump reg status before tx retry!\n");
			sdp_mac_gmac_dump(pGmacDev);
			sdp_mac_dma_dump(pGmacDev);
			dev_err(pGmacDev->pDev,
				"%s tx desc status %#x, rx desc status %#x\n",
				header, pGmacDev->pTxDesc[pGmacDev->tx_ring.tail].status,
				pGmacDev->pRxDesc[pGmacDev->rx_ring.tail].status);

			/* one more check */
			if(readl(&pDmaReg->status)&B_TX_INTR) {
				dev_info(pGmacDev->pDev, "%s Tx waiting. schedule delayed... %lums\n",
					header, jiffies_to_msecs(jiffies-timeout)+timeout_ms);
				break;
			}

			dev_err(pGmacDev->pDev, "%s Tx timeout! %lums, goto chack rx done\n", header, timeout_ms);
			//err = -1010;
			goto rx_check;
		}
	}
...
rx_check:
	timeout = jiffies + msecs_to_jiffies(timeout_ms);
	while(!(readl(&pDmaReg->status)&B_RX_INTR)) {
		if(time_is_before_jiffies(timeout)) {

			dev_err(pGmacDev->pDev, "dump reg status before rx retry!\n");
			sdp_mac_gmac_dump(pGmacDev);
			sdp_mac_dma_dump(pGmacDev);
			dev_err(pGmacDev->pDev,
				"%s tx desc status %#x, rx desc status %#x\n",
			...
			/* one more check */
			if(readl(&pDmaReg->status)&B_RX_INTR) {
				dev_info(pGmacDev->pDev, "%s Rx waiting. schedule delayed... %lums\n",
					header, jiffies_to_msecs(jiffies-timeout)+timeout_ms);
				break;
			}
...
static int 
sdp_phy_reset(struct net_device *pNetDev, u32 *data)
{
	SDP_GMAC_DEV_T *pGmacDev = netdev_priv(pNetDev);
	struct phy_device *phydev = pGmacDev->phydev;

	if((phydev->phy_id&0xFFFFFFFF) == EPHY_PHYID){
		dev_err(pGmacDev->pDev, "\n phy reset start\n");
		ephy_hw_reset();
		mdelay(2000);
...
sdp_mac_selftest_do(struct net_device *pNetDev, enum sdp_mac_selftest_num test)
{
	...
	int err = 0, ret = 0, cnt = 0;
	int time;
	int SLAVEADR_TX_AHB= 0x78;
	int (*eth_loopback)(int)=NULL;
...
#if defined(CONFIG_ARCH_SDP1803) || defined(CONFIG_ARCH_SDP_NIKE)
	if((phydev->phy_id&0xFFFFFFFF) == 0x00000001){  //in case of oc_board 
		eth_loopback = symbol_request(sdp_ocl_set_eth_loopback);		
		if(eth_loopback == NULL){
			dev_err(pGmacDev->pDev, "sdp_mac_selftest_do no symbol %s\n", "sdp_ocl_set_eth_loopback");
			return	-1;
		}
		for(cnt =0; cnt <10 ; cnt++){
			...
			if(bmsr & BMSR_LSTATUS){
				dev_err(pGmacDev->pDev, "[NVT]forced_link_up done bmsr : 0x%x\n", bmsr);
				break;
			}
			dev_err(pGmacDev->pDev, "[NVT]forced_link_up was failed(%d) bmsr : 0x%x\n", cnt, bmsr);
		}
	}
#endif
	...
	if(test == SDP_MAC_LB) 
		pGmacReg->configuration |= B_LOOP_BACK_EN;
	...
	if(test != SDP_MAC_LB) {
		...
		while(genphy_read_status(phydev) >= 0) {
			...
			if(time_is_before_jiffies(timeout)) {
				/* one more check */
				if((genphy_read_status(phydev) >= 0) && (phydev->link == 1)) {
					dev_info(pGmacDev->pDev, "%s link waiting. schedule delayed... %lums\n",
						sdp_mac_ethtool_selftest_string[test], jiffies_to_msecs(jiffies-timeout)+timeout_ms);
					break;
				}
				dev_err(pGmacDev->pDev, "%s link timeout! %lums, link : %d, phy_link : 0x%x\n", sdp_mac_ethtool_selftest_string[test], timeout_ms, phydev->link, phy_read(phydev, MII_BMSR));
				err = -ETIMEDOUT;
				break;
			}
		}
	...
	if(err == 0) {
		...
		if (phydev->speed == SPEED_1000)
			...
			if (phydev->speed == SPEED_100){
					ctrl |= B_SPEED_100M;
#if defined(CONFIG_ARCH_SDP_NIKE)
					mac_config_setting(SPEED_100);
#endif
			}
			else{
					ctrl &= ~B_SPEED_100M;
#if defined(CONFIG_ARCH_SDP_NIKE)
					mac_config_setting(SPEED_10);
#endif
			}
		}
...
#if defined(CONFIG_ARCH_SDP1803) || defined(CONFIG_ARCH_SDP_NIKE)
		if((phydev->phy_id&0xFFFFFFFF) == 0x00000001){
			sdp_mac_i2c_read_nvt_VDIP(pGmacDev, 0x14);
		}
#endif
		ret = err;
	} else {
...
int sdp1701_ocl_rx_link_state(SDP_GMAC_DEV_T *pGmacDev){

	u32 rdBuf_lsb = 0, rdBuf_msb = 0, ocl_rx_phy_link_status = 0;
	u32 SLAVEADR_RX_AHB = 0x70;
	u32 SLAVEADR_TX_AHB = 0x80;
	int cnt = 20;
	int err = -1;
	int (*ocl_i2c_write)(u32, u32, u32, u32)=NULL;
	int (*ocl_i2c_read)(u32, u32, u32 *)=NULL;
	int (*platform_info)(void) = NULL;
	platform_info = symbol_request(tztv_sys_get_platform_info);
	if(platform_info == NULL){
		dev_err(pGmacDev->pDev, "no symbol platform_info %p\n", ocl_i2c_write);
		goto no_symbol;
	}
	...
	if(platform_info() == 3){
		ocl_i2c_write = symbol_request(SDP_OCL_I2C_MaskWriteWord);
		if(ocl_i2c_write == NULL){
			dev_err(pGmacDev->pDev, "no symbol ocl_i2c_write %p\n", ocl_i2c_write);
			goto no_symbol;
		}
		ocl_i2c_read = symbol_request(SDP_OCL_I2C_ReadWord);
		if(ocl_i2c_read == NULL){
			dev_err(pGmacDev->pDev, "no symbol ocl_i2c_read %p\n", ocl_i2c_read);
			goto no_symbol;
		}
		
		ocl_i2c_write(SLAVEADR_RX_AHB, 0xFD130030, 0x05, 0x05);

		while(1){
			err = ocl_i2c_read(SLAVEADR_RX_AHB, 0xFD130038, &rdBuf_lsb);//default : 0x0d
			if(err < 0){			
				dev_err(pGmacDev->pDev, "i2c lsb read error\n");
				goto error;
			}
			err = ocl_i2c_read(SLAVEADR_RX_AHB, 0xFD13003c, &rdBuf_msb);//default : 0x78
			if(err < 0){			
				dev_err(pGmacDev->pDev, "i2c msb read error\n");
				goto error;
			}
			ocl_rx_phy_link_status = (rdBuf_lsb & 1<<2);
			dev_info(pGmacDev->pDev, "waiting ocl rx link up 0x%02x, 0x%02x\n", rdBuf_msb, rdBuf_lsb);
			if(ocl_rx_phy_link_status || cnt == 0)
				break;
			mdelay(10);
			cnt--;
...
#if defined(CONFIG_ARCH_SDP1601)
	if( get_sdp_board_type() == SDP_BOARD_MAIN){
		err = sdp1701_ocl_rx_link_state(pGmacDev);
		if(err < 0){
			dev_err(pGmacDev->pDev, "ocl link state get fail %d\n", err);
		}
	}	
#endif
...
/* delete internal loopback because checking for error for NVT on KANTM-OCL 
		if(err <= 0) {
			//MAC loopback test
...
void
sdp_mac_ethtool_get_wol(struct net_device *pNetDev, struct ethtool_wolinfo *wol) {
	SDP_GMAC_DEV_T *pGmacDev = netdev_priv(pNetDev);
	struct phy_device *phydev = pGmacDev->phydev;

	return phy_ethtool_get_wol(phydev, wol);
}


int
sdp_mac_ethtool_set_wol(struct net_device *pNetDev, struct ethtool_wolinfo *wol) {
	SDP_GMAC_DEV_T *pGmacDev = netdev_priv(pNetDev);
	struct phy_device *phydev = pGmacDev->phydev;
...
static int sdpGmac_drv_suspend(struct device *dev)
{
	...
	/* backup mac address */
	memcpy(pPower->gmac_macAddr, (void *)&pGmacReg->macAddr_00_High, sizeof(pPower->gmac_macAddr));
...
sdpGmac_drv_resume(struct device *dev)
{
	...
	pPower->gmac_frameFilter = 0;

	/* restore mac address */
	memcpy((void *)&pGmacReg->macAddr_00_High, pPower->gmac_macAddr, sizeof(pPower->gmac_macAddr));

	value=(phy_read(phydev, MII_BMSR) & BMSR_LSTATUS) >> 2;
	dev_printk(KERN_CRIT, pGmacDev->pDev, "link-state : %d\n", value);
...
static const struct ethtool_ops sdpGmac_ethtool_ops = {
	...
	.get_wol		= sdp_mac_ethtool_get_wol,
	.set_wol		= sdp_mac_ethtool_set_wol,
	.reset			= sdp_phy_reset,
};
...
static const struct net_device_ops sdpGmac_netdev_ops = {
	...
	.ndo_get_stats64        = sdpGmac_netDev_getStats64,
...
static int sdp_mac_dbg_mdio_show(struct seq_file *s, void *data)
{
	SDP_GMAC_DEV_T *pGmacDev = s->private;
	struct phy_device *phydev = pGmacDev->phydev;
	int addr = 0;

	if(!phydev) {
		dev_err(pGmacDev->pDev, "[reg_dump_show] phydev is NULL!\n");
		return -EFAULT;
	}

	seq_printf(s, "SDP MAC PHY Register dump\n");
...
static int sdp_mac_dbg_mdio_open(struct inode *inode, struct file *file)
{
	return single_open(file, sdp_mac_dbg_mdio_show, inode->i_private);
}

static ssize_t sdp_mac_dbg_mdio_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if(!phydev) {
		dev_err(pGmacDev->pDev, "[reg_dump_write] phydev is NULL!\n");
		return -EFAULT;
	}
	...
	if(addr > 0x1F) {
		dev_err(pGmacDev->pDev, "invalid address(%u)\n", addr);
		return -EINVAL;
	}
	if(val > 0xFFFF) {
		dev_err(pGmacDev->pDev, "invalid value(0x%x)\n", val);
		return -EINVAL;
	}
...
static const struct file_operations sdp_mac_dbg_mdio_fops = {
	.open		= sdp_mac_dbg_mdio_open,
	.read		= seq_read,
	.write		= sdp_mac_dbg_mdio_write,
	.llseek		= seq_lseek,
...
static ssize_t sdp_mac_dbg_forced_phylb_read(struct file *file, char __user *user_buf,
			      size_t count, loff_t *ppos)
{
	char buf[3];
	SDP_GMAC_DEV_T *pGmacDev = file->private_data;
	
	if (pGmacDev->forced_phylb)
		buf[0] = 'Y';
	else
		buf[0] = 'N';
	buf[1] = '\n';
...
static ssize_t sdp_mac_dbg_forced_phylb_write(struct file *file, const char __user *user_buf,
			       size_t count, loff_t *ppos)
{
	char buf[32];
	size_t buf_size;
	...
	if (strtobool(buf, &bv) == 0) {
		if(pGmacDev->forced_phylb != bv) {
			struct phy_device *phydev = pGmacDev->phydev;

			pGmacDev->forced_phylb = bv;

			if(phydev) {
				if(pGmacDev->forced_phylb) {
					phy_write(phydev, MII_BMCR, phy_read(phydev, MII_BMCR) | BMCR_LOOPBACK);
				} else {
					phy_write(phydev, MII_BMCR, phy_read(phydev, MII_BMCR) & ~BMCR_LOOPBACK);
				}
			}
...
static const struct file_operations sdp_mac_dbg_forced_phylb_fops = {
	.read =		sdp_mac_dbg_forced_phylb_read,
	.write =	sdp_mac_dbg_forced_phylb_write,
	.open =		simple_open,
	.llseek =	default_llseek,
};


static ssize_t sdp_mac_dbg_phy_start_read(struct file *file, char __user *user_buf,
			      size_t count, loff_t *ppos)
{
	char buf[3];
	SDP_GMAC_DEV_T *pGmacDev = file->private_data;
...
static ssize_t sdp_mac_dbg_phy_start_write(struct file *file, const char __user *user_buf,
			       size_t count, loff_t *ppos)
{
	char buf[32];
	size_t buf_size;
	...
	if (strtobool(buf, &bv) == 0) {
		if(bv == true) {
			dev_info(pGmacDev->pDev, "DBG: do PHY Start.\n");
			phy_start(phydev);
			ephy_priv->selftest_enable = 0;
			dev_info(pGmacDev->pDev, "DBG: Done selftest status : %d.\n", ephy_priv->selftest_enable);
		} else {
			dev_info(pGmacDev->pDev, "DBG: do PHY Stop.\n");
			ephy_priv->selftest_enable = 1;
			phy_stop(phydev);
			dev_info(pGmacDev->pDev, "DBG: Done selftest status : %d.\n", ephy_priv->selftest_enable);
		}
	}
...
static const struct file_operations sdp_mac_dbg_phy_start_fops = {
	.read =		sdp_mac_dbg_phy_start_read,
	.write =	sdp_mac_dbg_phy_start_write,
	.open =		simple_open,
	.llseek =	default_llseek,
};
...
#if defined(CONFIG_ARCH_SDP1601)
static int
sdp1710_ocl_rx_link_state_get(void *data, u64 *val)
{
	SDP_GMAC_DEV_T *pGmacDev = data;
	int err = 0;
	...
	err = sdp1701_ocl_rx_link_state(pGmacDev);
	if(err < 0){
		dev_err(pGmacDev->pDev, "ocl link state get fail\n");
		return -1;
	}
...
DEFINE_SIMPLE_ATTRIBUTE(sdp1710_ocl_rx_link_state_fops, sdp1710_ocl_rx_link_state_get, NULL,
	"%lld\n");
#endif
...
sdp_mac_add_debugfs(struct net_device *pNetDev)
{
	...
	if (!debugfs_create_file("regdump", S_IRUGO|S_IWUGO, root, pGmacDev, &sdp_mac_dbg_regdump_fops))
		goto err_node;

	if (!debugfs_create_file("selftest", S_IRUGO|S_IWUGO, root, pGmacDev, &sdp_mac_dbg_selftest_fops))
		goto err_node;

	if (!debugfs_create_file("mdio", S_IRUGO|S_IWUGO, root, pGmacDev, &sdp_mac_dbg_mdio_fops))
		goto err_node;

	if(!debugfs_create_file("forced_phylb", S_IRUGO|S_IWUGO, root, pGmacDev, &sdp_mac_dbg_forced_phylb_fops))
		goto err_node;

	if(!debugfs_create_file("phy_start", S_IRUGO|S_IWUGO, root, pGmacDev, &sdp_mac_dbg_phy_start_fops))
		goto err_node;
...
/* need to OCL RX stop
- OCL RX stop
  echo 1 > /sys/kernel/debug/ethernet/ocl_rx_loopback;
- OCL RX start
  echo 0 > /sys/kernel/debug/ethernet/ocl_rx_loopback;
*/
	if(!debugfs_create_file("sdp1701_nvt_rx_link_state", S_IRUGO|S_IWUGO, root, pGmacDev, &sdp1710_ocl_rx_link_state_fops))
		goto err_node;
#endif
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	struct net_device *pNetDev = platform_get_drvdata(pDev);
	SDP_GMAC_DEV_T *pGmacDev = netdev_priv(pNetDev);
	SDP_GMAC_STATS_T *priv_stats64 = &pGmacDev->priv_stats64;
	struct rtnl_link_stats64 *pNetStats64 = &pGmacDev->stats64;
	ssize_t cur_pos = 0;

	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "Dump sdp-mac NIC statistics\n");
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "rx_packets: %lld\n", pNetStats64->rx_packets);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "tx_packets: %lld\n", pNetStats64->tx_packets);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "rx_bytes: %lld\n", pNetStats64->rx_bytes);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "tx_bytes: %lld\n", pNetStats64->tx_bytes);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "rx_errors: %lld\n", pNetStats64->rx_errors);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "tx_errors: %lld\n", pNetStats64->tx_errors);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "rx_dropped: %lld\n", pNetStats64->rx_dropped);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "tx_dropped: %lld\n", pNetStats64->tx_dropped);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "multicast: %lld\n", pNetStats64->multicast);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "collisions: %lld\n", pNetStats64->collisions);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "rx_length_errors: %lld\n", pNetStats64->rx_length_errors);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "rx_over_errors: %lld\n", pNetStats64->rx_over_errors);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "rx_crc_errors: %lld\n", pNetStats64->rx_crc_errors);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "rx_frame_errors: %lld\n", pNetStats64->rx_frame_errors);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "rx_fifo_errors: %lld\n", pNetStats64->rx_fifo_errors);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "rx_missed_errors: %lld\n", pNetStats64->rx_missed_errors);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "tx_aborted_errors: %lld\n", pNetStats64->tx_aborted_errors);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "tx_carrier_errors: %lld\n", pNetStats64->tx_carrier_errors);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "tx_fifo_errors: %lld\n", pNetStats64->tx_fifo_errors);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "tx_heartbeat_errors: %lld\n", pNetStats64->tx_heartbeat_errors);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "tx_window_errors: %lld\n", pNetStats64->tx_window_errors);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "rx_compressed: %lld\n", pNetStats64->rx_compressed);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "tx_compressed: %lld\n", pNetStats64->tx_compressed);

	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "\nDump sdp-mac private statistics\n");
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "rx_unicast_packet: %lld\n", priv_stats64->rx_unicast_packet);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "rx_broadcast_packet: %lld\n", priv_stats64->rx_broadcast_packet);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "rx_multicast_packet: %lld\n", priv_stats64->rx_multicast_packet);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "tx_unicast_packet: %lld\n", priv_stats64->tx_unicast_packet);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "tx_broadcast_packet: %lld\n", priv_stats64->tx_broadcast_packet);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "tx_multicast_packet: %lld\n", priv_stats64->tx_multicast_packet);

	return cur_pos;
}
static DEVICE_ATTR_RO(statistics);

static ssize_t frame_filter_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct platform_device *pDev = to_platform_device(dev);
	struct net_device *pNetDev = platform_get_drvdata(pDev);
	...
	SDP_GMAC_T *pGmacReg = pGmacDev->pGmacBase;
	ssize_t cur_pos = 0;
	u32 frameFilter = pGmacReg->frameFilter;

	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "filter reg val: 0x%08x\n",
		frameFilter);

	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "B_PROMISCUOUS_EN: %s\n",
		frameFilter&B_PROMISCUOUS_EN?"true":"false");
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "B_HASH_UNICAST: %s\n",
		frameFilter&B_HASH_UNICAST?"true":"false");
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "B_HASH_MULTICAST: %s\n",
		frameFilter&B_HASH_MULTICAST?"true":"false");
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "B_DA_INVERSE_FILTER: %s\n",
		frameFilter&B_DA_INVERSE_FILTER?"true":"false");
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "B_PASS_ALL_MULTICAST: %s\n",
		frameFilter&B_PASS_ALL_MULTICAST?"true":"false");
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "B_PASS_CTRL_FRAMES: %lu\n",
		(frameFilter&B_PASS_CTRL_FRAMES(0x3))>>6);
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "B_BROADCAST_DIS: %s\n",
		frameFilter&B_BROADCAST_DIS?"true":"false");
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "B_SRC_ADDR_FILT_EN: %s\n",
		frameFilter&B_SRC_ADDR_FILT_EN?"true":"false");
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "B_SA_INVERSE_FILTER: %s\n",
		frameFilter&B_SA_INVERSE_FILTER?"true":"false");
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "B_HASH_PERFECT_FILTER: %s\n",
		frameFilter&B_HASH_PERFECT_FILTER?"true":"false");
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "B_RX_ALL: %s\n",
		frameFilter&B_RX_ALL?"true":"false");

	return cur_pos;
}
static DEVICE_ATTR_RO(frame_filter);

static ssize_t filter_broadcast_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct platform_device *pDev = to_platform_device(dev);
	struct net_device *pNetDev = platform_get_drvdata(pDev);
	...
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "%s\n",
		pGmacReg->frameFilter&B_BROADCAST_DIS?"Y":"N");

	return cur_pos;
}
static ssize_t filter_broadcast_store(struct device *dev,
				   ...
				   const char *buf, size_t size)
{
	...
	dev_info(dev, "Frame Filter Broadcast: %s\n", bv?"Y":"N");
	spin_lock_bh(&pGmacDev->lock);
	if (bv) {
		pGmacReg->frameFilter |= B_BROADCAST_DIS;
	} else {
		pGmacReg->frameFilter &= ~B_BROADCAST_DIS;
	}
	spin_unlock_bh(&pGmacDev->lock);
...
static DEVICE_ATTR_RW(filter_broadcast);

static ssize_t promiscuous_mode_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct platform_device *pDev = to_platform_device(dev);
	struct net_device *pNetDev = platform_get_drvdata(pDev);
	...
	cur_pos += scnprintf(buf + cur_pos, PAGE_SIZE - cur_pos, "%s\n",
		pGmacReg->frameFilter&B_PROMISCUOUS_EN?"Y":"N");

	return cur_pos;
}
static ssize_t promiscuous_mode_store(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t size)
{
	struct platform_device *pDev = to_platform_device(dev);
	...
	dev_info(dev, "Promiscuous Mode: %s\n", bv?"Y":"N");
	spin_lock_bh(&pGmacDev->lock);
	if (bv) {
		pGmacReg->frameFilter |= B_PROMISCUOUS_EN;
	} else {
		pGmacReg->frameFilter &= ~B_PROMISCUOUS_EN;
	}
	spin_unlock_bh(&pGmacDev->lock);
...
static DEVICE_ATTR_RW(promiscuous_mode);
/************** Dev Attr End ***************/
...
//	const u8 defaultMacAddr[N_MAC_ADDR] = DEFAULT_MAC_ADDR;
...
#ifdef SDP_GMAC_USE_AXI_BUSMASTER
	const int is_axi_bus = true;
#else
	const int is_axi_bus = false;
#endif
	...
	mdiobus_priv->addr = (void *)&pGmacDev->pGmacBase->gmiiAddr;
	mdiobus_priv->data = (void *)&pGmacDev->pGmacBase->gmiiData;
	...
	if (IS_ERR_OR_NULL(phydev)) {
		if(pGmacDev->plat->fixed_phy_added) {
			...
			snprintf(bus_id, MII_BUS_ID_SIZE + 3, PHY_ID_FMT, "fixed-0", pGmacDev->plat->fixed_phy_id);
...
// net stats init
	pGmacDev->stats64.rx_errors 	    = 0;
	pGmacDev->stats64.collisions       = 0;
	pGmacDev->stats64.rx_crc_errors    = 0;
	pGmacDev->stats64.rx_frame_errors  = 0;
	pGmacDev->stats64.rx_length_errors = 0;
...
// register initalize
	...
	dev_info(pGmacDev->pDev, "Clk %luMHz, IRQ is %d%s, use %s mode, %dbit BUS(%s)\n", pGmacDev->plat->clk_csr/1000000, pNetDev->irq,
		pNetDev->irq?"":"(polling)", retVal==PHY_INTERFACE_MODE_RGMII?"RGMII":(retVal==PHY_INTERFACE_MODE_RMII?"RMII":"NA!!"),
		pGmacDev->plat->bus_width, is_axi_bus?"AXI":"AHB");
	dev_info(pGmacDev->pDev, "desc ring size is rx%lu/tx%lu * %zubytes\n",
...
#if defined(CONFIG_ARCH_SDP_NIKE)
	emac_base_addr = ioremap(EMAC_CTRL_0, sizeof(u32)*8);
	serdes_rx_vdip_addr = ioremap(SERDES_RX_VDIP, sizeof(u32));
	serdes_eth_base_addr = ioremap(SERDES_ETH_BASE, sizeof(u32)*0x100);
#endif
...
#ifdef CONFIG_64BIT
static u64 sdp_mac_dmamask = DMA_BIT_MASK(64);
#else
...
// save resource about net driver
	...
	retVal = device_create_file(&pDev->dev, &dev_attr_statistics);
	retVal |= device_create_file(&pDev->dev, &dev_attr_frame_filter);
	retVal |= device_create_file(&pDev->dev, &dev_attr_filter_broadcast);
	retVal |= device_create_file(&pDev->dev, &dev_attr_promiscuous_mode);
	if (unlikely(retVal)) {
		dev_err(&pDev->dev, "Failed creating dev attrs\n");
		goto __probe_drv_err;
	}
...
static int sdpGmac_drv_remove (struct platform_device *pDev)
{
	...
	struct sdp_gmac_plat *plat = NULL;
...
#if defined(CONFIG_ARCH_SDP_NIKE)
	iounmap(emac_base_addr);
	iounmap(serdes_rx_vdip_addr);
	iounmap(serdes_eth_base_addr);
#endif
...
static int __init sdpGmac_init(void)
{
	...
	DPRINTK_GMAC_FLOW ("called\n");
	pr_info("%s: registered SDP GMAC network driver. ver %s(%s)\n",
		sdpGmac_device_driver.driver.name, GMAC_DRV_VERSION, GMAC_DRV_VERSION_DESC);
