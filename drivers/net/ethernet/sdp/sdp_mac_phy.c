 ...
 * 20181123(Add write functions for dsp,wol,cusom,bist register / Add iot and IPv6 WoL mode for debug)
 * 20191226(turn off automdix_en and waittime plus 500ms after restart autonego)
 * 20190129(Add debug log for anypacket from requester yunseoph.kim)
 */

#define SDP_MAC_PHY_VER		"20190201(modify an action of automdix_en off for link_up)"
...
#if defined(CONFIG_ARCH_SDP1501) || defined(CONFIG_ARCH_SDP1406) || defined(CONFIG_ARCH_SDP1601) || defined(CONFIG_ARCH_SDP1412) || defined(CONFIG_ARCH_SDP1803) || defined(CONFIG_ARCH_SDP_NIKE)
...
#else
		ephy_wolreg_write(phydev, 0x3, EPHY_WOL_EN | EPHY_WOL_MAGIC_DA_EN | EPHY_WOL_MAGIC_BA_EN | EPHY_WOL_CUSTOM_EN | EPHY_WOL_CUS_SA_MATCH_EN ); 
#endif
...
static int ephy_dsp_register_init(struct phy_device *phydev)
{
	...
	ephy_dspreg_write(phydev, 0x18, 0x000D);
...
#if defined(CONFIG_ARCH_SDP1501) || defined(CONFIG_ARCH_SDP1601) || defined(CONFIG_ARCH_SDP1803) || defined(CONFIG_ARCH_SDP_NIKE)
...
static int ephy_config_init(struct phy_device *phydev)
{
	...
	/* for try auto mdix mode and off */
	phy_write(phydev, 17, mdi_mode|EPHY_MDIO17_AUTO_MDIX_EN);
	mdi_mode = phy_read(phydev, 17)&(~EPHY_MDIO17_AUTO_MDIX_EN);
...
{
		u32 random_ms;
		const u32 min_ms = 2500, max_ms = 3000, setp_ms = 100;
...
int phy_writel_mask(unsigned int uw_addr, unsigned int uw_value, unsigned int uw_mask)
{
	volatile unsigned int * v_addr = NULL;
	unsigned int value = 0;
	v_addr = ioremap_nocache(uw_addr, sizeof(unsigned int));	
	if(v_addr == NULL) {
		printk("ioremap failed!!!, uw_addr 0x%x, uw_value 0x%x\n", uw_addr, uw_value);
		return -1;
	}
...
static int nvt_ephy_read_status(struct phy_device *phydev)
{
	...
	/* phy link is up */
		...
		if(readl(emac_path_sel) & (0x1<<EMAC_SERDES_SET_BIT)){
			phydev->link = 1;
			ephy_dynamic_printk(phydev, "Link is up!!!!!!!!!!!!!!");
			trace_early_message("ConnMan eth0 Link is up!!!!!!!!!!!!!!");
		}
...
static int ephy_read_status(struct phy_device *phydev)
{
...
#if	defined(CONFIG_ARCH_SDP1803) || defined(CONFIG_ARCH_SDP_NIKE)
	if((phydev->phy_id & 0xFFFFFFFF) == 0x0){
		phydev->link = 0;
		return 0;
	}
#endif
	...
	if(old_link && !phydev->link) {
...
#endif
		{
	        ...
	        trace_early_message("ConnMan eth0 Link is up!!!!!!!!!!!!!!");
			//pr_emerg("ConnMan eth0 Link is up!!! %d Mbps, flow control %s\n", phydev->speed, phydev->duplex == DUPLEX_FULL ? "full" : "half");
		}
	}
...
static int ephy_dbg_anypacket_wol_show(struct seq_file *s, void *data)
{
	...
	if(!(wol_control&(EPHY_WOL_EN|EPHY_WOL_CUSTOM_EN))) {
		...
		dev_printk(KERN_CRIT, &phydev->dev, "\nAnypacket WOL is diabled\n");
		return 0;
	}
	...
	if((wol_control&(EPHY_WOL_CUS_SA_MATCH_EN|EPHY_WOL_CUS_DA_MATCH_EN))) {
		...
		if(wol_control & EPHY_WOL_CUS_SA_MATCH_EN) {
			...
			if(!(wol_control&EPHY_WOL_CUS_BYPASS_TYPE_CHK)) {
				seq_printf(s, "\tMatch IPv4 Addr %pI4\n", &match_addr[6]);
				dev_printk(KERN_CRIT, &phydev->dev, "Match IPv4 Addr %pI4\n", &match_addr[6]);
				seq_printf(s, "\tMatch IPv6 Addr %pI6\n", match_addr);
				dev_printk(KERN_CRIT, &phydev->dev, "Match IPv6 Addr %pI6\n", match_addr);
			}
			...
			if(custom_len) {
				...
				for(i = sizeof(match_addr) - custom_len; i < sizeof(match_addr); i++) {
					seq_printf(s, " %02x", ((u8 *)match_addr)[i]);
					dev_printk(KERN_CRIT, &phydev->dev, " %02x", ((u8 *)match_addr)[i]);
				}
				seq_printf(s, "\n");
			}
		}
		...
		if(wol_control & EPHY_WOL_CUS_DA_MATCH_EN) {
			...
			seq_printf(s, "Dst Match On.(custom off %d, len %d)\n",
				custom_offset, custom_len);
...
#elif defined(CONFIG_ARCH_SDP_NIKE)
static int path_selection(unsigned int path_sel){
	int restore = 0;

	restore = readl(emac_path_sel) & 0xFF;
	if(path_sel == 0){
		phy_writel_mask(EMAC_PATH_SEL, 0x00000000, 0xFF);
	}
	else{
		phy_writel_mask(EMAC_PATH_SEL, 0x00000025, 0xFF);
	}
...
#elif defined(CONFIG_ARCH_SDP_NIKE)
	serdes_enable = path_selection(false);
#endif

	ephy_dspreg_write(phydev, 0x17, 0x1A0C);
	ephy_bistreg_write(phydev, 0x1B, 0x0005);

	phy_write(phydev, 17, (u16)(phy_read(phydev, 17) & (~EPHY_MDIO17_AUTO_MDIX_EN)));

	dev_err(&phydev->dev, "Omniphy(REG17) now %s %s mode",
	phy_read(phydev, 17)&EPHY_MDIO17_AUTO_MDIX_EN?"Auto":"Manual",
	phy_read(phydev, 17)&EPHY_MDIO17_MDI_MODE?"MDIX":"MDI");	

#if defined(CONFIG_ARCH_SDP1803)	
	set_serdes_path(serdes_enable ? true : false);	
#elif defined(CONFIG_ARCH_SDP_NIKE)
	path_selection(serdes_enable ? true : false);
#endif
...
static int nvt_phy_resume(struct phy_device *phydev)
{
	...
	ephy_dspreg_write(phydev, 0x17, 0x1A0C);
	ephy_bistreg_write(phydev, 0x1B, 0x0005);
	phy_write(phydev, 17, phy_read(phydev, 17) & (~EPHY_MDIO17_AUTO_MDIX_EN));
...
#endif
	dev_info(&phydev->dev, "EPHY reg17: 0x%x, reg01: 0x%x\n", phy_read(phydev, 17), phy_read(phydev, 1));
...
#if defined(CONFIG_ARCH_SDP1803)
	ephy_serdes_path = ioremap_nocache(EMAC_CTRL_0, sizeof(u32));
#elif defined(CONFIG_ARCH_SDP_NIKE)
	emac_path_sel = ioremap_nocache(EMAC_PATH_SEL, sizeof(u32));
#endif
