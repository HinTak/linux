...
//#define SDP_MAC_PHY_VER		"20160303(linux4.1 compile error fix)"
//#define SDP_MAC_PHY_VER		"20160311(bugfix, Jazz-L rev0 WOL is not work)"
#define SDP_MAC_PHY_VER		"20170804(add sdp_mac_phy.h / add selftest_enable option for test mode )"
...
#if defined(CONFIG_ARCH_SDP1601)
	addr = ioremap_nocache(0x9c0800, sizeof(u32));
	writel((readl(addr) & ~(0x1UL<<4)), addr);
	udelay(1);
	writel((readl(addr) | (0x1UL<<4)), addr);
	iounmap(addr);
