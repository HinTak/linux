...
static int prealloc_skb_max = 200;

#define HUARTPOOL_SKB_MAX	350
#define POOL_NAME		"pam"	/* pre allocated mem */
#define NAME_MAX_LEN		4

#define BUF_SIZE_6K		(1024 * 6)
...
struct sk_buff * prealloc_pool_increase(enum prealloc_pool_index pool);
 ...
 */
static struct prealloc_mem prealloc_buf[POOL_INDEX_MAX][HUARTPOOL_SKB_MAX];
static int pool_init_ok_6k;
			...
			unsigned int size)
{
	...
	if (!prealloc) {
		skb = prealloc_pool_increase(POOL_INDEX_6K);		
		if (NULL == skb)
		{
			if (net_ratelimit())
...
#ifdef SAMSUNG_HYPERUART_PREALLOC_SUPPORT
	if (pool_init_ok_6k && (!strcmp(name, "asix") || (!strcmp(name, "ax88179_178a"))))
...
int usbnet_stop (struct net_device *net)
{
	...
	/*debug print for NETDEV WATCHDOG warning to display submitted and completed tx urbs  at interface "ifname"*/
	if(dev->hdebug)
		printk(KERN_ERR "[SRID]stop:[%d][%s][ifname:%s][sub/comp:%u/%u][txqlen:%d/%d]\n", 
				__LINE__, __func__, dev->net->name, dev->tx_submitted, dev->tx_completed, dev->txq.qlen ,TX_QLEN(dev));	
...
fail_pipe:
			...
			{
				if(dev->hdebug)
					printk(KERN_ERR "[SRID]wake:[%d][%s][ifname:%s][sub/comp:%u/%u][status:%d][txqlen:%d/%d]\n", 
							__LINE__, __func__, dev->net->name, dev->tx_submitted, dev->tx_completed, 
							status, dev->txq.qlen, TX_QLEN(dev));
...
static void tx_complete (struct urb *urb)
{
	...
	if (urb->status == 0) {
		...
		case -EILSEQ:
			...
			if(dev->hdebug)
				printk(KERN_ERR "[SRID]stop:[%d][%s][ifname/endp:%s/%u][sub/comp:%u/%u][urb-status:%d][txqlen:%d/%d]\n", 
						__LINE__, __func__, dev->net->name, urb->ep->desc.bEndpointAddress, dev->tx_submitted, 
						dev->tx_completed, urb->status, dev->txq.qlen, TX_QLEN(dev));
				     ...
				     struct net_device *net)
{
	...
	case -EPIPE:
		if(dev->hdebug)
			printk(KERN_ERR "[SRID]stop:[%d][%s][ifname/endp:%s/%u][sub/comp: %u/%u][retval:%d][txqlen:%d/%d]\n", 
					__LINE__, __func__, dev->net->name, urb->ep->desc.bEndpointAddress, dev->tx_submitted, 
					dev->tx_completed, retval, dev->txq.qlen, TX_QLEN(dev));
		netif_stop_queue (net);
		usbnet_defer_kevent (dev, EVENT_TX_HALT);
		usb_autopm_put_interface_async(dev->intf);
		break;
	...
	case 0:
		...
		{
			if(dev->hdebug)
				printk(KERN_DEBUG "[SRID]stop:[%d][%s][ifname/endp:%s/%u][sub/comp: %u/%u][txqlen:%d/%d]\n", 
						__LINE__, __func__, dev->net->name, urb->ep->desc.bEndpointAddress, 
						dev->tx_submitted, dev->tx_completed, dev->txq.qlen, TX_QLEN(dev));
			netif_stop_queue (net);
		}
	}
	spin_unlock_irqrestore (&dev->txq.lock, flags);
...
struct sk_buff * prealloc_pool_increase(enum prealloc_pool_index pool)
{
	...
	spin_lock_irqsave(&hyperuart_prealloc_lock, lockflags);
	if(prealloc_skb_max >= HUARTPOOL_SKB_MAX)
	{
		spin_unlock_irqrestore(&hyperuart_prealloc_lock, lockflags);
		printk(KERN_EMERG "prealloc_pool_increase: HUARTPOOL_SKB_MAX Limit exceed\n");
		return NULL;
	}
	spin_unlock_irqrestore(&hyperuart_prealloc_lock, lockflags);
	...
	if (NULL == skb) {
		printk(KERN_EMERG "prealloc_pool_increase: dev_alloc_skb Failed\n");
		return NULL;
	}
	...
	printk(KERN_EMERG "prealloc_skb_max = %d \n", prealloc_skb_max);
	return skb;
}
...
static int __init usbnet_init(void)
{
...
#if defined(SAMSUNG_USBNET_HYPERUART_IFNAME ) && defined(SAMSUNG_HYPERUART_PREALLOC_SUPPORT)
	int status;
	struct file *f;
	char buf[HUART_IP_MAX_LEN];
	mm_segment_t fs;
	...
	if(IS_ERR(f) || (f == NULL)) 
	{
		printk(KERN_ERR " SRI-D HyperUART file open err!!\n");
		goto nohuart;
	}
	fs = get_fs();
	set_fs(get_ds());
	...
	if(buf[0] != '0')
	{
		printk(KERN_ERR "[%s] SRI-D HyperUART, buf[0] = %c\n",__func__,buf[0]);
		status = prealloc_pool_init(POOL_INDEX_6K);
		if(status < 0)
			printk(KERN_ERR "Hyper UART for USB2.0/USB3.0 can't be used because of no memory!!\n");
	}
nohuart:
#endif /*SAMSUNG_HYPERUART_PREALLOC_SUPPORT*/
#else
	printk(KERN_ERR "[%s] SRI-D Prealloc Pool is not created due to #define changes..\n",__func__);
#endif /*SAMSUNG_USBNET_HYPERUART_IFNAME*/
	return 0;
}
...
#ifdef SAMSUNG_HYPERUART_PREALLOC_SUPPORT
	if(pool_init_ok_6k)
		prealloc_pool_free(POOL_INDEX_6K);
#endif
}
