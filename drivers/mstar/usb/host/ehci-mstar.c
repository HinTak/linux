...
void Titania3_series_start_ehc(unsigned int UTMI_base, unsigned int USBC_base, unsigned int UHC_base, unsigned int BC_base, unsigned int flag)
{
...
#if defined(CONFIG_MSTAR_PreX14)
	writeb(readb((void*)(UTMI_base+0x09*2-1)) & (u8)(~BIT3), (void*)(UTMI_base+0x09*2-1)); // Disable force_pll_on
	writeb(readb((void*)(UTMI_base+0x08*2)) & (u8)(~BIT7), (void*)(UTMI_base+0x08*2)); // Enable band-gap current
	...
	if(UTMI_base == _MSTAR_UTMI1_BASE)      // UTMI setting only for Port1, couse of Eye diagram
		writeb(0x60, (void*) (UTMI_base+0x2C*2));
	else // other port
		writeb(0x6C, (void*) (UTMI_base+0x2C*2));
	...
	writeb(readb((void *)(BC_base+(0xc*2))) & (u8)(~0x40), (void *)(BC_base+(0xc*2)));  // [6]= reg_into_host_bc_sw_tri
	writeb(readb((void *)(BC_base+(0x3*2-1))) & (u8)(~0x40), (void *)(BC_base+(0x3*2-1)));  // [6]= reg_host_bc_en
		...
		struct usb_hcd **hcd_out, struct platform_device *dev)
{
	...
	if( 0==strcmp(dev->name, "Mstar-ehci-1") )
	{
		...
		Titania3_series_start_ehc(_MSTAR_UTMI0_BASE, _MSTAR_USBC0_BASE, _MSTAR_UHC0_BASE, _MSTAR_BC0_BASE,  flag);
	}
	else if( 0==strcmp(dev->name, "Mstar-ehci-2") )
	{
		printk("Mstar-ehci-2 H.W init\n");
		Titania3_series_start_ehc(_MSTAR_UTMI1_BASE, _MSTAR_USBC1_BASE, _MSTAR_UHC1_BASE, _MSTAR_BC1_BASE,  flag);
	}
#ifdef ENABLE_THIRD_EHC
	else if( 0==strcmp(dev->name, "Mstar-ehci-3") )
	{
		printk("Mstar-ehci-3 H.W init\n");
		Titania3_series_start_ehc(_MSTAR_UTMI2_BASE, _MSTAR_USBC2_BASE, _MSTAR_UHC2_BASE, _MSTAR_BC2_BASE,  0 );
	}
#endif
	...
	else if( 0==strcmp(dev->name, "Mstar-ehci-4") )
	{
		...
		Titania3_series_start_ehc(_MSTAR_UTMI3_BASE, _MSTAR_USBC3_BASE, _MSTAR_UHC3_BASE, _MSTAR_BC3_BASE,  0 );
	}
#endif
...
static int ehci_hcd_mstar_drv_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	printk("ehci_hcd_mstar_platform_suspend...\n");
...
static int ehci_hcd_mstar_drv_resume(struct platform_device *pdev)
{
	...
	printk("ehci_hcd_mstar_platform_resume...\n");
#endif
	if (!hcd) {
		printk("[USB] ehci_hcd_mstar_drv_resume no HCD!\n");
		return -ENODEV;
	}

#if defined(CONFIG_MSTAR_USB_STR_PATCH)
	if (is_resuming())
#endif
	Titania3_series_start_ehc(hcd->utmi_base, hcd->usbc_base, hcd->ehc_base, hcd->bc_base, flag);
	enable_irq(hcd->irq);
