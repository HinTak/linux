...
static int slave_alloc (struct scsi_device *sdev)
{
	 ...
	 */
#if defined(HOTPLUG) && (defined(CONFIG_MSTAR_PreX14) || defined(CONFIG_MSTAR_X14))	//tony if disconnect, update will cause kernel panic
...
static int slave_configure(struct scsi_device *sdev)
{
	...
	if(!us->pusb_dev || !us->pusb_dev->bus->controller )
		return 1;
 
	if (!us->pusb_dev->bus->controller->dma_mask)
		blk_queue_bounce_limit(sdev->request_queue, BLK_BOUNCE_HIGH);

#if defined(CONFIG_HIGHMEM) && (defined(CONFIG_MSTAR_PreX14) || defined(CONFIG_MSTAR_X14))
	blk_queue_bounce_limit(sdev->request_queue, BLK_BOUNCE_HIGH);
#endif
	...
	if(us->serial && sdev->serial)
		sdev->serial = (char *)(us->serial);

	// add for usb device path
	if(us->pusb_dev  && sdev->usbdevpath )
