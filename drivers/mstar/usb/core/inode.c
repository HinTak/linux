...
static int usbfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (!inode) {
		dev_dbg("%s: could not get inode!",__func__);
		return -ENOMEM;
	}
...
//	root = d_alloc_root(inode);
	...
	if (!root) {
		dev_dbg("%s: could not get root dentry!",__func__);
		iput(inode);
		return -ENOMEM;
	}
	sb->s_root = root;
...
static void usbfs_add_bus(struct usb_bus *bus)
{
	...
	snprintf (name,sizeof(name), "%03d", bus->busnum);
...
static void usbfs_add_device(struct usb_device *dev)
{
	...
	snprintf (name,sizeof(name), "%03d", dev->devnum);
