...
//#define GZIP_VER_STR	"20171228(reduce stack usage)"
//#define GZIP_VER_STR	"20180913(add MuseM feature, support input scatter start offset)"
#define GZIP_VER_STR	"20191116(apply 64 byte aligned restriction for Nike)"
...
#if defined(CONFIG_SDP_UNZIP_AUTH_STANDALONE)/* after KantM */
...
#if defined(CONFIG_SDP_UNZIP_FIRST_PAGE_OFFSET)/* after MuseM */
#define R_GZIP_IN_BUF_SIZE_1	0x1E0
#endif
...
#if IS_ENABLED(CONFIG_SDP_UNZIP_64_BYTE_ALIGNED_RESTRICTION)
#define GZIP_ALIGNADDR	64
#define GZIP_ALIGNSIZE	64
#else/*ENABLED(SDP_UNZIP_64_BYTE_ALIGNED_RESTRICTION)*/
#define GZIP_ALIGNADDR	8
#define GZIP_ALIGNSIZE	64
#endif/*ENABLED(SDP_UNZIP_64_BYTE_ALIGNED_RESTRICTION)*/
...
#define GZIP_INPUT_SCATTER_MODE
...
static irqreturn_t sdp_unzip_isr(int irq, void* devId)
{
	...
	//pr_info("sdp-unzip: req#%u irq pend=0x%08x errorcode=0x%08x\n", uzreq->request_idx, pend, readl(sdp_unzip->base + R_GZIP_ERR_CODE));
	...
	if (sdp_unzip->auth && readl(sdp_unzip->base + R_CP_CTRL) & 2) {		//use SHA auth case
		...
		if (sdp_unzip->auth && (sdp_unzip->auth->sha256_result != GZIP_HASH_OK))
		{
			dump++;
			if (!sdp_unzip->quiet) {
				pr_err("sdp-unzip: req#%u sha256_result error : %d\n", uzreq->request_idx, (int)sdp_unzip->auth->sha256_result);
			}
		}
	}
		...
		if(isr_total_time_ns >= 1000000000UL) {
			if (!sdp_unzip->quiet) {
				pr_err("sdp-unzip: req#%u ISR Time over 1s!! total:%lluns, callback:%lluns\n",
					uzreq->request_idx, isr_total_time_ns, isr_callback_time_ns);
			}
		}
...
struct sdp_unzip_desc* sdp_unzip_alloc_descriptor(u32 flags)
{
	...
	uzreq->request_idx = sdp_unzip->req_idx++;
	...
	sdp_unzip_cb_t cb, void *arg)
{
	...
	if(uzreq == NULL) {
		dev_err(sdp_unzip->dev, "current unzip request is NULL!\n");
		return -EINVAL;
	}

	/* for debug */
	for(i = 0; i < nr_ipages; i++) {
		if(ipages[i]&((dma_addr_t)(GZIP_ALIGNADDR-1))) {
			dev_err(sdp_unzip->dev, "req#%u phys input[%02d] [0x%08llx, %d] 0x%xbytes",
				uzreq->request_idx, i, (u64)ipages[i], nr_ipages, input_bytes);
			is_dump = true;
		}
	}
	if(is_dump) {
		...
		dev_err(sdp_unzip->dev, "req#%u input 0x%xbytes, output 0x%xbytes, flags=0x%x",
			uzreq->request_idx, input_bytes, output_bytes, flags);
	...
	dev_info(sdp_unzip->dev, "req#%u page input [0x%08llx, %d] 0x%xbytes, output [0x%08llx, %d] 0x%xbytes",
		uzreq->request_idx, (u64)ipages[0], nr_ipages, input_bytes, opages?(u64)opages[0]:0x0, nr_opages, output_bytes);
#endif

#ifndef GZIP_INPUT_SCATTER_MODE
	if(nr_ipages != 1) {
		dev_err(sdp_unzip->dev, "req#%u Not Supported Input Scatter buffer! input pages# %d", uzreq->request_idx, nr_ipages);
		return -EINVAL;
	}
#endif

	if(!(nr_ipages > 0 && nr_ipages <= GZIP_NR_PAGE)) {
		dev_err(sdp_unzip->dev, "req#%u Invalid Input page number %d", uzreq->request_idx, nr_ipages);
		return -EINVAL;
	}

	if(!(nr_opages > 0 && nr_opages <= GZIP_NR_PAGE)) {
		dev_err(sdp_unzip->dev, "req#%u Invalid Output page number %d", uzreq->request_idx, nr_opages);
		return -EINVAL;
	}

	/* check input align*/
	if(input_bytes&(GZIP_ALIGNSIZE-1)) {
		dev_err(sdp_unzip->dev, "req#%u Invalid Input align bytes 0x%x", uzreq->request_idx, input_bytes);
		return -EINVAL;
	}
	for(i = 0; i < nr_ipages; i++) {
#if defined(CONFIG_SDP_UNZIP_FIRST_PAGE_OFFSET)
		if(ipages[i]&(GZIP_ALIGNADDR-1)) {
#else
		if(ipages[i]&( (nr_ipages==1?GZIP_ALIGNADDR-1:GZIP_PAGESIZE-1) )) {
#endif
			dev_err(sdp_unzip->dev, "req#%u Invalid Input align address[%d] 0x%llx", uzreq->request_idx, i, (u64)ipages[i]);
			return -EINVAL;
		}
	}
	...
	if(output_bytes&(GZIP_ALIGNSIZE-1)) {
		dev_err(sdp_unzip->dev, "req#%u Invalid Output align bytes 0x%x", uzreq->request_idx, output_bytes);
		return -EINVAL;
	}
	for(i = 0; i < nr_opages; i++) {
		if(opages[i]&( (nr_opages==1?GZIP_ALIGNADDR-1:GZIP_PAGESIZE-1) )) {
			dev_err(sdp_unzip->dev, "req#%u Invalid Output align address[%d] 0x%llx", uzreq->request_idx, i, (u64)opages[i]);
			return -EINVAL;
		}
	}
...
#if defined(CONFIG_SDP_UNZIP_FIRST_PAGE_OFFSET)
	if(nr_ipages > 1) {
		writel(GZIP_PAGESIZE - (ipages[0]&(GZIP_PAGESIZE-1)), sdp_unzip->base + R_GZIP_IN_BUF_SIZE);
	} else {
		writel(input_bytes, sdp_unzip->base + R_GZIP_IN_BUF_SIZE);
	}
	writel(GZIP_PAGESIZE, sdp_unzip->base + R_GZIP_IN_BUF_SIZE_1);
#else
	if(nr_ipages > 1) {
...
{
	struct sdp_unzip_req *uzreq = to_sdp_unzip_req(uzdesc);
	void *_page_phys = NULL;
	dma_addr_t *output_page_phys = NULL;
	dma_addr_t *input_page_phys = NULL;
	...
	if(uzreq->inents > GZIP_NR_PAGE || uzreq->inents > GZIP_NR_PAGE) {
		dev_err(sdp_unzip->dev, "req#%u Invalid sg nents!! input %u, output %u", uzreq->request_idx, uzreq->inents, uzreq->onents);
	}

#ifdef SDP_UNZIP_DEBUG
	dev_info(sdp_unzip->dev, "req#%u sg input %u, output %u", uzreq->request_idx, uzreq->inents, uzreq->onents);
#endif
	...
	_page_phys = kmalloc(sizeof(dma_addr_t) * (uzreq->inents + uzreq->onents), GFP_KERNEL);
	if(!_page_phys) {
		dev_err(sdp_unzip->dev, "req#%u alloc page phys failed(%p)\n", uzreq->request_idx, _page_phys);
		goto _err_free;
	}
	input_page_phys = (dma_addr_t *)_page_phys;
	output_page_phys = ((dma_addr_t *)_page_phys) + uzreq->inents;

	/* get phys address */
#if defined(CONFIG_SDP_UNZIP_FIRST_PAGE_OFFSET)
	if(uzreq->isgl->offset & (GZIP_ALIGNSIZE - 1)) {
...
#ifdef SDP_UNZIP_DEBUG
			dev_info(sdp_unzip->dev, "req#%u sg input 0x%llx, 0x%x", uzreq->request_idx, (u64)sg_phys(sg), sg->length);
#endif
		}
#ifdef SDP_UNZIP_DEBUG
		dev_info(sdp_unzip->dev, "req#%u sg input merge 0x%llx, 0x%x", uzreq->request_idx, (u64)input_page_phys[0], uzreq->ilength);
#endif
	} else {/* input scatter offset mode */
...
#else/*defined(CONFIG_SDP_UNZIP_FIRST_PAGE_OFFSET)*/
	if(uzreq->isgl->offset) {
		...
		input_nr_pages = uzreq->inents;
		for_each_sg(uzreq->isgl, sg, uzreq->inents, i) {
			if((sg_phys(sg)&~PAGE_MASK) || ((i < uzreq->inents-1) && (sg->length != GZIP_PAGESIZE))) {
...
#endif/*defined(CONFIG_SDP_UNZIP_FIRST_PAGE_OFFSET)*/
...
#ifdef SDP_UNZIP_DEBUG
	dev_info(sdp_unzip->dev, "req#%u sg output 0x%llx, 0x%x", uzreq->request_idx, (u64)sg_dma_address(sg), sg_dma_len(sg));
#endif
	}
...
_err_free:
		...
		if(_page_phys) kfree(_page_phys);
		up(&sdp_unzip->sema);
	...
	bool may_wait)
{
	...
	if(uzreq->inents > GZIP_NR_PAGE || uzreq->inents > GZIP_NR_PAGE) {
		dev_err(sdp_unzip->dev, "req#%u Invalid sg nents!! input %u", uzreq->request_idx, uzreq->inents);
	}

#ifdef SDP_UNZIP_DEBUG
	dev_info(sdp_unzip->dev, "req#%u hash sg input %u", uzreq->request_idx, uzreq->inents);
#endif
...
static int _sdp_unzip_run_selftest(struct sdp_unzip_t *uz, struct sdp_unzip_test_vector_t *vec)
{
	...
	struct scatterlist in_sgl[(ALIGN(vec->ilen, PAGE_SIZE)/PAGE_SIZE) + 1];
	...
	if(vec->is_input_scatter) {
		int offset = vec->input_start_offset;
		int remain = vec->ilen;
		struct scatterlist *sg = NULL;
		int scatter_nents = 0;

		sg_init_table(in_sgl, ARRAY_SIZE(in_sgl));
		scatter_buf = kmalloc(ARRAY_SIZE(in_sgl) * PAGE_SIZE, GFP_KERNEL);
		if(scatter_buf == NULL) {
			return -ENOMEM;
		}

		i = 0;
		sg_set_page(&in_sgl[0], virt_to_page(scatter_buf + (PAGE_SIZE * (ARRAY_SIZE(in_sgl) - 1))), PAGE_SIZE - offset, offset);
		remain -= PAGE_SIZE - offset;

		for(i = 1; remain > 0 && i < ARRAY_SIZE(in_sgl); i++) {
			sg_set_page(&in_sgl[i], virt_to_page(scatter_buf + (PAGE_SIZE * (ARRAY_SIZE(in_sgl) - i - 1))), MIN(PAGE_SIZE, remain), 0x0);
			remain -= MIN(PAGE_SIZE, remain);
		}

		scatter_nents = i;
		sg_mark_end(&in_sgl[scatter_nents - 1]);

		// for_each_sg(in_sgl, sg, scatter_nents, i) {
		// 	dev_info(sdp_unzip->dev, "before in_sgl#%d %#p, 0x%x", i, sg_virt(sg), sg->length);
		// }

		sg_copy_from_buffer(in_sgl,  scatter_nents, (void *)vec->input, vec->ilen);

		// for_each_sg(in_sgl, sg, scatter_nents, i) {
		// 	dev_info(sdp_unzip->dev, "after in_sgl#%d %#p, 0x%x", i, sg_virt(sg), sg->length);
		// }

		// dev_info(sdp_unzip->dev, "in_sgl make done!\n");
	} else {
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	const u32 bufsize = 0xf000;

	u32 planlen = 0, complen, complen_align;
	int ret, i;
	int result = -1;
	enum { CASE_DECOMP, CASE_DECRYPT_DECOMP, CASE_HASH_DECOMP, CASE_WHOLE, CASE_HASH, CASE_DECRYPT, CASE_HASH_DECRYPT, CASE_DECOMP_SCATTER_OFFSET, CASE_WHOLE_SCATTER_OFFSET, CASE_MAX };
...
#if defined(CONFIG_SDP_UNZIP_FIRST_PAGE_OFFSET)
	testvecs[CASE_DECOMP_SCATTER_OFFSET] = (struct sdp_unzip_test_vector_t){
		.name = "decompress with scatter offset",
		.input = compressed,
		.result = data_buf,
		.ilen = complen_align,
		.rlen = bufsize,
		...
		.sha256_result = NULL,
		.is_zlib = is_zlib,
		.is_input_scatter = true,
		.input_start_offset = GZIP_ALIGNSIZE,
	};

	testvecs[CASE_WHOLE_SCATTER_OFFSET] = (struct sdp_unzip_test_vector_t){
		.name = "hash+decrypt->decompress with scatter offset",
		.input = encrypted,
		.result = data_buf,
		.ilen = complen_align,
		.rlen = bufsize,
...
#define _PRINT_RESULT(b, p, testvecs) \
	if((testvecs).name) {	\
		(p) += scnprintf((b) + (p), PAGE_SIZE - (p), "%-50s:%s(%d)\n", (testvecs).name,	\
	...
	if(result >= 0) {
		...
		for(tc = 0; tc < CASE_MAX; tc++) {
			_PRINT_RESULT(buf, cur_pos, testvecs[tc]);
		}
	}
