...
#define PERIOD_LOGOFF    500   /* 500ms */
#define PERIOD_DUMPSTACK 1000 /* 1000ms */
#define COUNT_LOGOFF     10
#define COUNT_DUMPSTACK  500

static void init_error(struct sdp_runtime_pm_node *node)
{
	node->rpm.err_period = PERIOD_LOGOFF;
	node->rpm.err_count = 0;
}
static void check_error(struct sdp_runtime_pm_node *node)
{
	s64 delta;
	ktime_t curr = ktime_get();

	if (node->rpm.err_count == 0)
		node->rpm.err_time = curr;

	delta = ktime_ms_delta(curr, node->rpm.err_time);
	
	node->rpm.err_count++;
	if (node->rpm.err_period == PERIOD_LOGOFF) {
		pr_warn("[sdp_runtime_pm] %s: call in suspend from %s\n", node->rpm.name, current->comm);
		if (node->rpm.err_count >= COUNT_LOGOFF) {
			if (delta < PERIOD_LOGOFF) {
				pr_err("[sdp_runtime_pm] %s: forced to disable error log.\n", node->rpm.name);
				node->rpm.err_period = PERIOD_DUMPSTACK;
			}
			node->rpm.err_count = 0;
		}
	}
	else {
		if (delta > PERIOD_DUMPSTACK) {
			pr_err("[sdp_runtime_pm] %s: %d calls in suspend from %s.\n", node->rpm.name, node->rpm.err_count, current->comm);
			if (node->rpm.err_count > COUNT_DUMPSTACK)
				dump_stack();
			node->rpm.err_count = 0;
		}
	}
...
int sdp_runtime_pm_put(char *name)
{
	...
	if (node->rpm.usage_count == 0 && node->rpm.dev && node->rpm.suspend) {
		...
		while (node->rpm.busy_count > 0 && timeout-- > 0) {
			msleep(10);
			if (timeout <= 900 && (timeout%100)==0)
				pr_err("[sdp_runtime_pm] %s is still busy...\n", node->rpm.name);
		}
