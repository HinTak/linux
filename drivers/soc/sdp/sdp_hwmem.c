...
sdp_hwmem_get(struct file * file, SDP_GET_HWMEM_T* p_hwmem_info)
{
	...
	struct page *page;
	unsigned long order, addr, vaddr = 0;
	size_t size;
	...
	hwmem_info.phy_addr = 0;
	hwmem_info.vir_addr = 0;
	...
	if (populate)
		mm_populate(vaddr, populate);

	hwmem_info.vir_addr = vaddr;
...
{
#ifdef CONFIG_64BIT
	__dma_map_area (vir_addr, size, DMA_FROM_DEVICE);
#else
...
{
#ifdef CONFIG_64BIT
	__dma_map_area (vir_addr, size, DMA_TO_DEVICE);
#else
...
{
#ifdef CONFIG_64BIT
	__dma_flush_range(vir_addr,(const void *)((u32)vir_addr+size));
#else
...
static int sdp_hwmem_ddr_check(SDP_HWMEM_DDR_CHECK_T* args)
{
	...
	for (i = 0; i < SDP_DDR_MAX; i++) {
		ddr_test_mem_p[i] = (uint8_t*)0;
	}
	...
	if(ddr_check.ddr_type & SDP_DDR_CHECK_0)
	{
		...
		if (ddr_check.test_size > size) {
			PRINT_HWMEM_ERR("ERROR: DDR check test_size cannot be more than ddr side(%x) \n", size);
			ret = -ENOMEM;
			goto exit;
		}
	...
	if(ddr_check.ddr_type & SDP_DDR_CHECK_1)
	{
		...
		ret_val = of_property_read_u32_array(np_mem, "samsung,align1", data, 3);
		...
		if (ret_val)
		{
			PRINT_HWMEM_ERR("failed to parse samsung SDP_DDR_1 memory\n");
			//return ret;
		}
	...
	if(ddr_check.ddr_type & SDP_DDR_CHECK_2)
	{
...
#elif defined(CONFIG_ARCH_SDP1501)
		ret_val = of_property_read_u32_array(np_mem, "samsung,align2", data, 3);		
#else
		PRINT_HWMEM_ERR("ERROR: None of HAWKM_UHD_1.5 or HAWKM_UHD_2.5 SOC chosen for DDR Check test\n");
		ret = -ENOMEM;
		goto exit;
		...
		if (ret_val)
		{
			PRINT_HWMEM_ERR("failed to parse samsung SDP_DDR_2 memory\n");
			//return ret;
		}
	...
	if(ddr_check.ddr_type & SDP_DDR_CHECK_3)
	{
#ifdef CONFIG_ARCH_SDP1501
		ret_val = of_property_read_u32_array(np_mem, "samsung,align4", data, 3);

		if (ret_val)
		{
			PRINT_HWMEM_ERR("failed to parse samsung SDP_DDR_3 memory\n");
			//return ret;
		}
		...
		ddr_test_mem_p[SDP_DDR_3] = ioremap_nocache(address, size);
		if(!ddr_test_mem_p[SDP_DDR_3])
		{
			PRINT_HWMEM_ERR("Failed to get DDR3 memory for DDR check\n");
			ret = -ENOMEM;
			goto exit;
		}
		ddr_check.ddr_return.sdp_ddr3_check_result = 1;		
#endif
	}

	if(ddr_check.ddr_type & SDP_DDR_CHECK_4)
	{
#ifdef CONFIG_ARCH_SDP1501
		ret_val = of_property_read_u32_array(np_mem, "samsung,dp-vt-y", data, 3);

		if (ret_val)
		{
			PRINT_HWMEM_ERR("failed to parse samsung SDP_DDR_4 memory\n");
			//return ret;
		}
		...
		ddr_test_mem_p[SDP_DDR_4] = ioremap_nocache(address, size);
		if(!ddr_test_mem_p[SDP_DDR_4])
		{
			PRINT_HWMEM_ERR("Failed to get DDR3 memory for DDR check\n");
			ret = -ENOMEM;
			goto exit;
		}
		ddr_check.ddr_return.sdp_ddr4_check_result = 1;		
#endif
	}	
	...
	do
	{
		...
		// Write the test pattern in DDR 3
		if(ddr_check.ddr_type & SDP_DDR_CHECK_3)
			sdp_write_ddr_mem(ddr_test_mem_p[SDP_DDR_3],ddr_check.test_size,write_pattern_p,pattern_size,1);
		// Write the test pattern in DDR 4		
		if(ddr_check.ddr_type & SDP_DDR_CHECK_4)
			sdp_write_ddr_mem(ddr_test_mem_p[SDP_DDR_4],ddr_check.test_size,write_pattern_p,pattern_size,1);
		...
		// Read, compare and check the DDR 3
		if(ddr_check.ddr_return.sdp_ddr3_check_result && sdp_check_ddr_mem(ddr_test_mem_p[SDP_DDR_3],ddr_check.test_size,write_pattern_p,pattern_size) != 0)
			ddr_check.ddr_return.sdp_ddr3_check_result = 0;
		
		// Read, compare and check the DDR 4
		if(ddr_check.ddr_return.sdp_ddr4_check_result && sdp_check_ddr_mem(ddr_test_mem_p[SDP_DDR_4],ddr_check.test_size,write_pattern_p,pattern_size) != 0)
			ddr_check.ddr_return.sdp_ddr4_check_result = 0;
...
int ddr_margin(void)
{
	...
	up_down_addr_vir = ((unsigned int) ddr_reg_base) + UPDOWN_ADDR_OFFSET;
...
#ifdef CONFIG_64BIT
	if (file->f_flags & O_SYNC)
		vma->vm_page_prot = __pgprot_modify(vma->vm_page_prot,  PTE_ATTRINDX_MASK, PTE_ATTRINDX(0) | PTE_UXN);
#else
	if (file->f_flags & O_SYNC)
