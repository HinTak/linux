...
struct req_hw {
	...
	u8 *enc_ivec;	// /* bits 0-63: random nonce; 64:127: counter value */
...
int sdp_unzip_hw_hash_fn( unsigned char *buf, size_t buf_len, char *hash ) {
	struct sdp_unzip_desc *uzdesc = NULL;
	struct scatterlist in_sgl[HW_MAX_IBUFF_SG_LEN];
	u32 flags = 0;
	int ret;

	flags |= GZIP_FLAG_ENABLE_AUTH | GZIP_FLAG_AUTH_STANDALONE;
	...
	ret = buf_to_sglist(buf, buf_len, in_sgl, ARRAY_SIZE(in_sgl));
	if(ret <= 0) {
		ret = -EINVAL;
		goto l_free_desc;
	}

	uzdesc->auth.sha256_length = buf_len;
	uzdesc->auth.sha256_digest_out = (u32 *)hash;

	ret = sdp_unzip_standalone_hash_async(uzdesc, in_sgl, NULL, NULL, true);
...
EXPORT_SYMBOL(sdp_unzip_hw_hash_fn);

int sdp_unzip_hw_decrypt_fn(struct page **src, int src_offset,
							struct page **dst, int dst_offset,
							int length, enum hw_iovec_encrypt_type enc_type,
							const u8 *enc_key, const u8 *enc_ivec) {

	struct sg_table in_sgt, out_sgt;
	int src_npages, dst_npages, ret;
	struct sdp_unzip_desc *uzdesc = NULL;
	u32 flags = 0;

	if(enc_type != HW_IOVEC_ENCRYPT_AES128_CTR) {
		pr_err("[%s:%u] invalid encrypt type 0x%x\n", __FUNCTION__, __LINE__, enc_type);
		return -EINVAL;
	}
	...
	src_npages = ((src_offset + length) >> PAGE_SHIFT) + (((src_offset + length)&PAGE_MASK)?1:0);
	dst_npages = ((dst_offset + length) >> PAGE_SHIFT) + (((dst_offset + length)&PAGE_MASK)?1:0);

	ret = sg_alloc_table_from_pages(&in_sgt, src, src_npages, src_offset, length, GFP_KERNEL);
	ret = sg_alloc_table_from_pages(&out_sgt, dst, dst_npages, dst_offset,  ALIGN(length, INPUT_ALIGNSIZE), GFP_KERNEL);

	uzdesc->auth.aes_ctr_iv = (u32 *)enc_ivec;
	uzdesc->auth.aes_user_key = (u32 *)enc_key;

	ret = sdp_unzip_decompress_async(uzdesc, in_sgt.sgl, out_sgt.sgl, NULL, NULL, true);
...
EXPORT_SYMBOL(sdp_unzip_hw_decrypt_fn);
...
static int vdfsapi_selftest_show(struct seq_file *s, void *data)
{
	...
	rq_hw.enc_type = HW_IOVEC_ENCRYPT_AES128_CTR;
	rq_hw.hashdata = hash;
...
static int vdfsapi_hwcap_show(struct seq_file *s, void *data)
{
	...
	seq_printf(s, PREFIX"enc_type       = 0x%02x\n", hwcap.enc_type);
	seq_printf(s, PREFIX"hash_type      = 0x%02x\n", hwcap.hash_type);
	seq_printf(s, PREFIX"min_size       = %d\n", hwcap.min_size);
	seq_printf(s, PREFIX"max_size       = %d\n", hwcap.max_size);
	seq_printf(s, PREFIX"hw_decrypt_fn  = 0x%8p\n", hwcap.hw_decrypt_fn);
	seq_printf(s, PREFIX"hw_hash_fn     = 0x%8p\n", hwcap.hw_hash_fn);
