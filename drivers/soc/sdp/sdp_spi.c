...
static int	sdp_spi_gpio_setup(struct spi_device *spi) {
	return gpio_request_one(spi->cs_gpio, GPIOF_DIR_OUT, "SPI-CS");
}
		...
		for (i = 0; psize >= onesize; psize -= onesize, initregs += onesize, i++) {
			...
			if(iomem) {
				...
				dev_printk(KERN_DEBUG, dev,
					"of initreg addr %p 0x%08x, mask 0x%08x, val 0x%08x\n",
					iomem, addr, mask, val);
				iounmap(iomem);
				udelay(50);
			} else {
				dev_err(dev, "initregs ioremap failed: %08x %08x %08x\n",
						addr, mask, val);
				return -ENOMEM;
			}
		}
...
static int sdp_spi_parse_dt(struct device *dev, struct sdp_spi_info *spi_info)
{
	...
	nr = of_gpio_named_count(np, "gpio-muxed");
	if (nr > 0) {
		int i;
		for (i = 0; i < nr; i++)
			spi_info->gpio_muxed[i] = of_get_named_gpio(np, "gpio-muxed", i);
		spi_info->nr_gpio_muxed = nr;
	}

	return 0;
}

/* preoccupy muxed gpios prior to other potential gpio users */
static int sdp_spi_request_gpios(struct device *dev, struct sdp_spi_info *spi_info)
{
	int i;

	for (i = 0; i < spi_info->nr_gpio_muxed; i++) {
		int gpio = spi_info->gpio_muxed[i];
		char label[16];
		if (!gpio_is_valid(gpio))
			dev_warn(dev, "gpio %d invalid\n", gpio);
		else {
			snprintf(label, 16, "spi%02d", i);
			if (devm_gpio_request(dev, gpio, label) < 0)
...
static int
sdp_spi_dbg_regdump_get(void *data, u64 *val)
{
	...
	spi_bus_lock(master);
	dev_info(&sdpspi->pdev->dev,"SPI Regs Dump\n");
	dev_info(&sdpspi->pdev->dev, "iomem base 0x%8p\n", sdpspi->regs_base);
	dev_info(&sdpspi->pdev->dev, "SSPCR0  0x%04x\n", sdp_spi_read_u16(sdpspi, SSPCR0));
	dev_info(&sdpspi->pdev->dev, "SSPCR1  0x%04x\n", sdp_spi_read_u16(sdpspi, SSPCR1));
	dev_info(&sdpspi->pdev->dev, "SSPDR   0x%04x\n", sdp_spi_read_u16(sdpspi, SSPDR));
	dev_info(&sdpspi->pdev->dev, "SSPSR   0x%04x\n", sdp_spi_read_u16(sdpspi, SSPSR));
	dev_info(&sdpspi->pdev->dev, "SSPCPSR 0x%04x\n", sdp_spi_read_u16(sdpspi, SSPCPSR));
	dev_info(&sdpspi->pdev->dev, "SSPIIR  0x%04x\n", sdp_spi_read_u16(sdpspi, SSPIIR));
	spi_bus_unlock(master);

	*val = 0x0;
	return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(sdp_spi_dbg_regdump_fops, sdp_spi_dbg_regdump_get, NULL,
	"%lld\n");


static int
sdp_spi_dbg_selftest(struct spi_master *master, int mode)
{
	struct sdp_spi *sdpspi;
	static struct spi_device *dummydev = NULL;
	int ret = 0;

	//u16 save_mode;
	//u32 save_max_speed_hz;
	//u8 save_bits_per_word;

	sdpspi = spi_master_get_devdata(master);

	if(!dummydev) {
		dummydev = spi_alloc_device(master);
		if(!dummydev) {
			dev_err(&sdpspi->pdev->dev, "failed to alloc spi dummydev.\n");
			return -ENODEV;
		}
	}

	dummydev->mode = mode;
	dummydev->max_speed_hz = 100000;
	dummydev->bits_per_word = 8;

//	spi_bus_lock(master);
	ret = spi_setup(dummydev);
	if(!ret) {
		unsigned int i;
		u8 txbuf[256] = {0};
		u8 rxbuf[256] = {0};
		...
		for(i = 0; i < ARRAY_SIZE(txbuf); i++) {
			txbuf[i] = i & 0xFF;
			rxbuf[i] = i & 0xFF;
		}

		ret = spi_sync_transfer(dummydev, xfer, ARRAY_SIZE(xfer));
		if(!ret) {
			print_hex_dump(KERN_INFO, "Tx: ", DUMP_PREFIX_ADDRESS, 16, 1,
		       xfer[0].tx_buf, xfer[0].len, true);
			pr_info("\n");
			print_hex_dump(KERN_INFO, "Rx: ", DUMP_PREFIX_ADDRESS, 16, 1,
		       xfer[0].rx_buf, xfer[0].len, true);
			pr_info("\n");
			if(!memcmp(txbuf, rxbuf, ARRAY_SIZE(txbuf))) {
				dev_info(&sdpspi->pdev->dev, "spi loopback test ok!\n");
			} else {
				dev_info(&sdpspi->pdev->dev, "spi loopback test fail!\n");
			}
		}
	}
//	spi_bus_unlock(master);
...
static int
sdp_spi_dbg_internal_lb_get(void *data, u64 *val) {
	struct spi_master *master = data;
	*val = 0x0;

	return sdp_spi_dbg_selftest(master, SPI_MODE_3 | SPI_LOOP);
}
DEFINE_SIMPLE_ATTRIBUTE(sdp_spi_dbg_internal_lb_fops, sdp_spi_dbg_internal_lb_get, NULL,
	"%lld\n");


static int
sdp_spi_dbg_external_lb_get(void *data, u64 *val) {
	struct spi_master *master = data;
	*val = 0x0;

	return sdp_spi_dbg_selftest(master, SPI_MODE_3);
}
DEFINE_SIMPLE_ATTRIBUTE(sdp_spi_dbg_external_lb_fops, sdp_spi_dbg_external_lb_get, NULL,
	"%lld\n");
...
static void
sdp_spi_add_master_debugfs(struct spi_master *master) {
	struct sdp_spi *sdpspi;
	struct dentry *root;

	sdpspi = spi_master_get_devdata(master);

	root = debugfs_create_dir(dev_name(&sdpspi->pdev->dev), NULL);
	...
	sdpspi->debugfs_root = root;

	if (!debugfs_create_file("regdump", S_IRUSR, root, master, &sdp_spi_dbg_regdump_fops))
		goto err_node;

	if (!debugfs_create_file("internal_lb", S_IRUSR, root, master, &sdp_spi_dbg_internal_lb_fops))
		goto err_node;

	if (!debugfs_create_file("external_lb", S_IRUSR, root, master, &sdp_spi_dbg_external_lb_fops))
		goto err_node;
...
err_node:
	...
	sdpspi->debugfs_root = NULL;
err_root:
	dev_err(&sdpspi->pdev->dev, "failed to initialize debugfs\n");
}
#endif
...
sdp_spi_probe(struct platform_device *pdev)
{
	...
	if (!pdev->dev.platform_data)
		pdev->dev.platform_data =
			devm_kzalloc(&pdev->dev, sizeof(struct sdp_spi_info), GFP_KERNEL);
	
	sdp_spi_parse_dt(&pdev->dev, pdev->dev.platform_data);

	sdp_spi_request_gpios(&pdev->dev, pdev->dev.platform_data);
	...
	master->mode_bits = SPI_CPOL | SPI_CPHA | SPI_LOOP | SPI_SETAUTH_MODE;
	master->flags= 0;
#ifdef CONFIG_OF
...
//		goto fail_put_clock;
	...
	snprintf(wq_name, sizeof(wq_name), "sdp_spid%02d", master->bus_num);
	...
	dev_info(&pdev->dev, "SPI Support Slave Clock %ldhz - [Min %ld - Max %ld]\n",
		sdp_spi_clkrate(sdpspi),
...
#ifdef CONFIG_DEBUG_FS
	sdp_spi_add_master_debugfs(master);
#endif
...
sdp_spi_remove(struct platform_device *pdev)
{
	...
	release_mem_region(res->start, resource_size(res));
	if (sdpspi->clk) {
		clk_disable_unprepare(sdpspi->clk);
		clk_put(sdpspi->clk);
	}
	platform_set_drvdata(pdev, NULL);
...
static int sdp_spi_suspend(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct spi_master *master = platform_get_drvdata(pdev);
	struct sdp_spi *sdpspi = spi_master_get_devdata(master);
...
static int sdp_spi_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct spi_master *master = platform_get_drvdata(pdev);
	struct sdp_spi *sdpspi = spi_master_get_devdata(master);

	sdp_spi_of_do_initregs(dev);
...
static const struct dev_pm_ops sdp_spi_pm_ops = {
	.suspend	= sdp_spi_suspend,
	.resume		= sdp_spi_resume,
};
