...
static int sdp1601_chk_master(struct sdp_bus_err_logger_t *bus_err)
{
	return (readl(bus_err->m_base+ERRVLD));
}

static int sdp1601_read_master(struct sdp_bus_err_logger_t *bus_err)
{
	unsigned int log0, log1, log3;
	...
	bus_err->err_type = sdp1601_get_err_type(log0);
	bus_err->err_code = sdp1601_get_err_code(log0);
	bus_err->master_ip = sdp1601_get_master_ip(log1);
	bus_err->target_ip = sdp1601_get_target_ip(log1);
	bus_err->addr = log3;
	return 0;
}

static int sdp1601_chk_master_tzasc(struct sdp_bus_err_logger_t *bus_err)
{
	return (bus_err->err_type == 1 && bus_err->err_code== 1 && bus_err->master_ip == 5 && \
		(bus_err->target_ip == 4 || bus_err->target_ip == 7 || bus_err->target_ip == 10));
}

static int sdp1601_pop_master_tzasc(struct sdp_bus_err_logger_t *bus_err, int print)
{
	if (print) {
		printk(KERN_ERR "SDP TZASC_WARNING : master=%x(%s), addr=%x\n",
			bus_err->master_ip, sdp1601_get_master_ip_str(bus_err->master_ip), bus_err->addr);
	}
...
static int sdp1601_chk_master_prevent(struct sdp_bus_err_logger_t *bus_err)
{
	return (bus_err->addr >= 0xff0000 && bus_err->addr < 0xff1000);
}

static int sdp1601_pop_master_prevent(struct sdp_bus_err_logger_t *bus_err, int print)
{
	if (print) {
		printk(KERN_ERR "SDP PREVENT_WARNING : type=%x(%s), code=%x(%s), master=%x(%s)\n",
			bus_err->err_type, sdp1601_get_err_type_str(bus_err->err_type),
			bus_err->err_code, sdp1601_get_err_code_str(bus_err->err_code),
			bus_err->master_ip, sdp1601_get_master_ip_str(bus_err->master_ip));	
	}
...
static int sdp1601_pop_master(struct sdp_bus_err_logger_t *bus_err)
{
	__draw_line();
	...
	printk(KERN_ERR "MAIN : type=%x(%s), code=%x(%s), master=%x(%s), target=%x(%s), addr=%x\n",
		...
		bus_err->err_code, sdp1601_get_err_code_str(bus_err->err_code),
		bus_err->master_ip, sdp1601_get_master_ip_str(bus_err->master_ip),
		bus_err->target_ip, sdp1601_get_target_ip_str(bus_err->target_ip),
		bus_err->addr);
...
static int sdp1601_chk_slave_sb(struct sdp_bus_err_logger_t *bus_err)
{
	return (readl(bus_err->s_base[0]+ERRVLD));
}

static int sdp1601_pop_slave_sb(struct sdp_bus_err_logger_t *bus_err)
{
	unsigned int log0, log1, log3;
	unsigned int err_type, err_code, slave_ip, addr; 
...
static int sdp1601_chk_slave_nm(struct sdp_bus_err_logger_t *bus_err)
{
	return (readl(bus_err->s_base[1]+ERRVLD));
}

static int sdp1601_pop_slave_nm(struct sdp_bus_err_logger_t *bus_err)
{
	unsigned int log0, log1, log3;
	unsigned int err_type, err_code, slave_ip, addr; 
...
static int sdp1601_bus_err_logger(void)
{
	...
	spin_lock_irqsave(&bus_err->lock, flags);

	printk(KERN_ERR "\nsdp_bus_err_logger start.\n");
	
	while (sdp1601_chk_master(bus_err) && cnt < 16) {

		sdp1601_read_master(bus_err);

		if (sdp1601_chk_master_tzasc(bus_err)) {
			sdp1601_pop_master_tzasc(bus_err, 1);
		}
		else if (sdp1601_chk_master_prevent(bus_err)) {
			sdp1601_pop_master_prevent(bus_err, 1);
		}
		else {
			__draw_line();
			printk(KERN_ERR "SDP BUS ERR LOGGER (CNT=%d)\n", cnt);
			
			sdp1601_pop_master(bus_err);

			if (bus_err->target_ip == 1) { // sub normal
				sdp1601_pop_slave_nm(bus_err);
			}
			else if (bus_err->target_ip == 2) { // sub standby
				sdp1601_pop_slave_sb(bus_err);
			}
		}
		mdelay(1);
		cnt++;
	...
	printk(KERN_ERR "pop all remain sub normal errors.\n");
	cnt = 0;
	while (sdp1601_chk_slave_nm(bus_err) && cnt++ < 16)
		sdp1601_pop_slave_nm(bus_err);

	printk(KERN_ERR "pop all remain sub standby errors.\n");
	cnt = 0;
	while (sdp1601_chk_slave_sb(bus_err) && cnt++ < 16)
		sdp1601_pop_slave_sb(bus_err);

	printk(KERN_ERR "sdp_bus_err_logger end.\n");

	spin_unlock_irqrestore(&bus_err->lock, flags);

	return 0;
}

static void sdp1601_bus_err_work(struct work_struct *work)
{
	struct sdp_bus_err_logger_t *bus_err = &sdp_bus_err;
	unsigned long flags;
	int cnt = 0;
	...
	while (sdp1601_chk_master(bus_err) && cnt < 16) {
		...
		if (sdp1601_chk_master_tzasc(bus_err)) {
			sdp1601_pop_master_tzasc(bus_err, 0);
		}
		else if (sdp1601_chk_master_prevent(bus_err)) {
			sdp1601_pop_master_prevent(bus_err, 0);
		}
		udelay(100);
		cnt++;
	}
	...
	schedule_delayed_work(&bus_err->work, msecs_to_jiffies(1000));
	
	return;
}

int sdp_buserr_socite(unsigned int *type, unsigned int *addr)
{
	struct sdp_bus_err_logger_t *bus_err = &sdp_bus_err;
	*type = bus_err->type;
	*addr = bus_err->addr;
	return bus_err->valid;
}
EXPORT_SYMBOL(sdp_buserr_socite);
...
static u32 __get_flow_ctrl(int i) {

	return readl(sdp_bus_mon_tbl[i].iomem + 0x80) & 0xffff;
}
...
int sdp1601_bus_mon_logger(void)
{
	...
	for (i = 0; i < sizeof(sdp1601_bus_mon_tbl)/sizeof(struct sdp_bus_mon_tbl_t); i++) {
		...
		if (!strncmp(sdp1601_bus_mon_tbl[i].name, "CPU", sizeof(sdp1601_bus_mon_tbl[i].name)))
			continue;
...
static int sdp1601_bus_err_init(void)
{
	...
	spin_lock_init(&bus_err->lock);

	INIT_DELAYED_WORK(&bus_err->work, sdp1601_bus_err_work);
	schedule_delayed_work(&bus_err->work, msecs_to_jiffies(1000));
...
int sdp_bus_flowctrl_get(const char *id)
{
	int i;
	for (i = 0; i < sizeof(sdp1601_bus_mon_tbl)/sizeof(struct sdp_bus_mon_tbl_t); i++) {
		if (!strncmp(sdp1601_bus_mon_tbl[i].name, id, sizeof(sdp1601_bus_mon_tbl[i].name)))
			return __get_flow_ctrl(i);
	}
	return 0;
}
EXPORT_SYMBOL(sdp_bus_flowctrl_get);
