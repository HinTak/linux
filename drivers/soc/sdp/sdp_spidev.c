 ...
 * 20160530 fix module build error
*/
...
#define VERSION_SRTING		"20180915"
...
spidev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SPI_IOC_WR_WHOLE_CONFIG:
	{
		...
		if (retval == 0) {
			...
			if(chip == NULL){
				dev_dbg(&spi->dev, "SPI_IOC_WR_WHOLE_CONFIG: chip data is null.\n");
				break;
			}
...
static const struct file_operations spidev_fops = {
	...
	static int sdp_spi_get_smack64_label(struct device *dev, char* buf, int size){
		snprintf(buf,size, "%s", "*");
		return 0;
	}
#endif

static char * sdp_spi_devnode(struct device *dev, umode_t *mode)
{
	if ( !mode ) {
		printk (KERN_ERR"sdp_spidev : mode is NULL\n" );
		return NULL;
	}
...
#ifdef CONFIG_SECURITY_SMACK_SET_DEV_SMK_LABEL
	spidev_class->devnode = sdp_spi_devnode;
	spidev_class->get_smack64_label = sdp_spi_get_smack64_label;
#endif
