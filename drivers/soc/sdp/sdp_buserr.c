...
static char *nikem_srcid_str[] = {
	"mUSB2_0", "mUSB2_1", "mUSB2_2", "mUSB2_3", "mUSB2_4", "mAMIC", "mARM_A7QR", "mARM_A7QW", 
	"mARM_DAP", "mARM_ETR", "mAUD_ADSPR", "mAUD_ADSPW", "mAUD_AIO", "mAVD", "mCHN", "mCTR0", 
	"mCTR1", "mDMIC", "mDVDE0", "mDVDE1", "mDVDE2", "mDVDE3", "mFRC0R", "mFRC0W", "mFRC1R", 
	"mFRC1W", "mGAR", "mGAW", "mGP0", "mGP1", "mGP2", "mGPUR", "mGPUW", "mHDMI", "mHEN", "mJPEG", 
	"mMFD0", "mMFD1", "mMSP_TSD", "mNPUR", "mNPUW", "mNRCR", "mNRCW", "mPERIn", "mPERIs", "mROTR", 
	"mROTW", "mSCLR0", "mSCLR1", "mSCLR2", "mSCLW0", "mSCLW1", "mSCLW2", "mSNPU_CPU", "mSNPU_DMAR", 
	"mSNPU_DMAW", "mSNPU_FLCR", "mSNPU_FLCW", "mSRP", 
}; 

static char *nikem_dstid_lv1_str[] = {
	"DS_BUS_MAIN", "SVC_BUS_MAIN", "sBUS_SUB", "sEBUS", 
	"sXMIFC0", "sXMIFC1", "sXMIFC2", "sXMIFD0", "sXMIFD1", "sXMIFD2",
};

static char *nikem_dstid_lv2_str[] = {
	"sFRC_MC", "sFRC_ME", "sFRC_MON", "sFRC_POST", "sFRC_SA", "-", "-", "-", "sHDMI_HDCP", 
	"sHDMI_LINK_P0", "sHDMI_LINK_P1", "sHDMI_LINK_P2", "sHDMI_LINK_P3", "-", "-", "-", "sMSP_ATSC", 
	"sMSP_HCAS", "sMSP_MON", "sMSP_SE", "sMSP_TSD", "-", "-", "-", "sAUD_AIO", "sAUD_AIO_MON", 
	"sAUD_SPI", "sAUD_SPI_DMA", "sAMIC0", "sAMIC1", "sAMIC_MON", "-", "sDMIC0", "sDMIC1", 
	"sDMIC_MON", "-", "sXMIFc_MEMS", "sXMIFc_TZC", "sXMIFc_TZCID", "-", "sXMIFd_MEMS", 
	"sXMIFd_TZC", "sXMIFd_TZCID", "-", "sAUD_ADSP", "sAUD_ADSP_MON", "sAVD", "sAVD_MON", "sCHN", 
	"sCHN_MON", "sCYS_MON", "sCYS_PMU", "sDVDE", "sDVDE_MON", "sGP", "sGP_MON", "sGPU_CTRL", 
	"sGPU_MON", "sHDMI_HMON", "sHDMI_MON", "sHEN", "sHEN_MON", "sJPEG", "sJPEG_MON", "sMFD", 
	"sMFD_MON", "sNPU", "sNPU_MON", "sNRC", "sNRC_MON", "sROT", "sROT_MON", "sSCL", "sSCL_MON", 
	"sSERDES_LINK", "sSERDES_PHY", "sSRP", "sSRP_MON", "sTCON", "sTCON_DEMURA", "sTOP_CLKRST", 
	"sTOP_PADCTRL", "sUSB", "sUSB_MON", "sVE", "sVE_GQE",     "SVC_BUS_SUB", "SVC_BUS_SUB_tz", 
	"sAFE", "sAMIC_EDID", "sARM_SNP", "sCTR", "sCYS", "sCYS_GIC", "sCYS_STM", "sDSCD", "sEBUS_REG", 
	"sGA", "sGPU", "sPERIn", "sPERIs", "sPRE", "sSNPU_MON", "sSNPU_SFR", "sSNPU_SRAM", 
	"sTCON_VX1", "sXMIFcu_APB", "sXMIFdu_APB",
};

static struct sdp_buserr_logger nikem_buserr_sub = {
	"nikem-buserr-sub",
	0xf84000, 
	0x40,
	{
		{ "opcode",   0x14,  1,  0xf, common_opcode_str, ARRAY_SIZE(common_opcode_str) },
		{ "errcode",  0x14,  8,  0x7, common_errcode_str, ARRAY_SIZE(common_errcode_str) },
		{ "dst(lv2)", 0x18,  7,  0x7f, nikem_dstid_lv2_str, ARRAY_SIZE(nikem_dstid_lv2_str) },
		{ "addr",	  0x20,  0,  ~0ul, NULL, 0 },
	},
	{ NULL, },
};

static struct sdp_buserr_logger nikem_buserr_main = {
	"nikem-buserr-main",
	0x4fe8000, 
	0x40,
	{
		{ "opcode",   0x14,  1,  0xf, common_opcode_str, ARRAY_SIZE(common_opcode_str) },
		{ "errcode",  0x14,  8,  0x7, common_errcode_str, ARRAY_SIZE(common_errcode_str) },
		{ "src",      0x18, 17, 0x3f, nikem_srcid_str, ARRAY_SIZE(nikem_srcid_str) },
		{ "dst(lv1)", 0x18, 13,  0xf, nikem_dstid_lv1_str, ARRAY_SIZE(nikem_dstid_lv1_str) },
		{ "addr",     0x20,  0, ~0ul, NULL, 0 },
	},
	{ NULL, &nikem_buserr_sub, &nikem_buserr_sub, },
};

static struct sdp_buserr_logger nikel_buserr_sub = {
	"nikel-buserr-sub",
	0xf84000, 
	0x40,
	{
...
static struct sdp_buserr_logger nikel_buserr_main = {
	"nikel-buserr-main",
	0x4fe8000, 
	0x40,
	{
	...
	{ NULL, &nikel_buserr_sub, &nikel_buserr_sub, },
};
...
static int __init sdp_buserr_init(void)
{
	...
	else if (soc_is_nikem()) {
		sdp_buserr_add_logger(&nikem_buserr_main);
		sdp_buserr_add_logger(&nikem_buserr_sub);
	}
	else if (soc_is_nikel()) {
		sdp_buserr_add_logger(&nikel_buserr_main);
		sdp_buserr_add_logger(&nikel_buserr_sub);
	}
