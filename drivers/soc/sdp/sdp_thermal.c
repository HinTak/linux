...
static void write_log(unsigned int throttle)
{
	...
	__vfs_write(fp, tmu_logbuf, len, &fp->f_pos);
...
static ssize_t show_print_on(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, 3, "%d\n", print_on);
}

static ssize_t store_print_on(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	u32 on;
...
static DEVICE_ATTR(print_on, 0644, show_print_on, store_print_on);
...
static void sdp_tmu_handle_throttle(struct sdp_tmu_info *info, int cur_state, u32 cur_temp)
{
	...
	if (cur_temp <= info->throttle[throt_idx].stop_temp) {
		pr_warn("TMU: change state: %s throttle -> %s. %d'C\n",
	...
	/* 2. change to next state */
		...
		pr_warn("TMU: change state: %s throttle -> %s throttle. %d'C\n",
...
static void sdp_handler_tmu_state(struct work_struct *work)
{
	...
	if (info->exec_temp == 0 && local_clock() >= 10000000000ull) //10s
		info->exec_temp = cur_temp;

	if (print_on) {
		pr_warn("TMU: temperature = %d \'C\n", cur_temp);
	}
	...
	case TMU_STATUS_INIT:
		...
		//first_try = true;
		pr_warn("TMU: init state is %d(%d'C)\n", info->tmu_state, cur_temp);
		break;
	...
	sdp_tmu_notifier_call_chain(SDP_TMU_UPDATE_TEMP, (struct throttle_params*)&cur_temp);
...
static void sdpxxxx_init_tmu(struct sdp_tmu_info * info)
{
	u32 chipid[4];
	u32 val, slope, vref;
...
//	pr_err("%s: tmu\n", __FUNCTION__);
	...
	/* tmu defalut setting */
	writel(0x0000c278, (void *)(info->tmu_base + 0x08)); /* default vref, slope sel */
	writel(0x01ec099a, (void *)(info->tmu_base + 0x10)); /* wait cnt 1, 2 */
	writel(0x3232099a, (void *)(info->tmu_base + 0x14)); /* wait cnt 4, 5 */
	writel(0x30016001, (void *)(info->tmu_base + 0x18)); /* wait cnt 6, sample cnt */

	chipid[0] = readl((void*)(info->chipid_base + 0x08));
	chipid[1] = readl((void*)(info->chipid_base + 0x0c));
	chipid[2] = readl((void*)(info->chipid_base + 0x10));
	chipid[3] = readl((void*)(info->chipid_base + 0x14));

	vref = ((chipid[3]&0x3)<<3) | ((chipid[2]>>29)&0x7);
	slope = ((chipid[2]>>25)&0xf);

	printk(KERN_INFO "TMU: slope=0x%x, vref=0x%x\n", slope, vref);

	if (chipid[0]!=0 || chipid[1]!=0 || chipid[2]!=0 || chipid[3]!=0) {
		val = readl((void *)(info->tmu_base + SDP_PAD_CTRL50));
		val &= ~(0xf << 12);
		val &= ~(0x1f << 5);
		val |= (slope << 12);
		val |= (vref << 5);
		writel(val, (void *)(info->tmu_base + SDP_PAD_CTRL50));
	}	
}
...
{
	/* read efuse */
	diff_val = (readl((void*)(info->chipid_base + 0x10)) >> 17) & 0xFF;
	if (diff_val == 0) {
...
static int sdpxxxx_enable_tmu(struct sdp_tmu_info * info)
{
	u32 val;
...
//	pr_err("%s: tmu\n", __FUNCTION__);
	...
	sdpxxxx_init_tmu(info);

	/* Temperature sensor enable */
	val = readl((void *)(info->tmu_base + SDP_PAD_CTRL50)) | TSC_TEM_T_EN;
	writel(val, (void *)(info->tmu_base + SDP_PAD_CTRL50));
...
#define TMU_MAX_TEMP_DIFF	(20)
static u32 sdpxxxx_get_temp(struct sdp_tmu_info *info)
{
	int curr_temp = 0;
	int temp = 0;
	...
	curr_temp = (readl((void *)(info->tmu_base + SDP_PAD_CTRL51)) >> TSC_TEM_T_TS_8BIT_TS) & 0xFF;

	/* calibration */
	curr_temp = curr_temp + diff_val - (TSC_DEGREE_25 - 25);
	...
	/* sanity check */
	if( (abs(curr_temp - prev_temp) <= TMU_MAX_TEMP_DIFF) || first_try ){
		temp = curr_temp;
		sanity = true;
		if(first_try){
			pr_warn("tmu : init temperature(curr=%d, prev=%d)\n", curr_temp, prev_temp);
			first_try = false;
		}
	}
	else{
		if(sanity){
			temp = prev_temp;
			pr_warn("tmu : insane value(curr=%d, prev=%d) => discard\n", curr_temp, prev_temp);				
		}
		else{
			if(curr_temp > prev_temp)
				temp = prev_temp + TMU_MAX_TEMP_DIFF;
			else
				temp = (prev_temp - TMU_MAX_TEMP_DIFF > 0) ? prev_temp - TMU_MAX_TEMP_DIFF	: 0;

			pr_warn("tmu : insane value(curr=%d, prev=%d) => try to chase => tmp=%d\n", curr_temp, prev_temp, temp);
		}
		sanity = false;
	}
...
static int sdpxxxx_cpu_hotplug_down(struct sdp_tmu_info * info)
{
...
static int sdpxxxx_cpu_hotplug_up(struct sdp_tmu_info * info)
{
#ifdef CONFIG_HOTPLUG_CPU
	u32 cpu;
	struct device * dev;
...
/* init */
int sdpxxxx_tmu_init(struct sdp_tmu_info * info)
{

	//pr_err("%s: tmu\n", __FUNCTION__);

	info->enable_tmu = sdpxxxx_enable_tmu;
	info->get_temp = sdpxxxx_get_temp;
	if (info->cpu_hotplug_temp > 0) {
		info->cpu_hotplug_down = sdpxxxx_cpu_hotplug_down;
		info->cpu_hotplug_up = sdpxxxx_cpu_hotplug_up;
	}
	return 0;
}
...
static int sdp_tmu_init_sysfs(struct sdp_tmu_info *info)
{
	...
	ret = device_create_file(info->dev, &dev_attr_print_on);
	if (ret != 0) {
		pr_err("Failed to create throttle on file: %d\n", ret);
		goto err_sysfs_file5;
	}

	ret = device_create_file(info->dev, &dev_attr_boot_temp);
	if (ret != 0) {
		pr_err("Failed to create init temp file: %d\n", ret);
		goto err_sysfs_file6;
	}
	
	ret = device_create_file(info->dev, &dev_attr_exec_temp);
	if (ret != 0) {
		pr_err("Failed to create exec temp file: %d\n", ret);
		goto err_sysfs_file7;
	}

	ret = sysfs_create_link(info->dev->kobj.parent, &(info->dev->kobj), "thermal");
	if(ret < 0) {
		goto err_sysfs_link;
	}
...
err_sysfs_link:	
	device_remove_file(info->dev, &dev_attr_exec_temp);

err_sysfs_file7:
	device_remove_file(info->dev, &dev_attr_boot_temp);
	
err_sysfs_file6:
	device_remove_file(info->dev, &dev_attr_print_on);

err_sysfs_file5:
	device_remove_file(info->dev, &dev_attr_print_temp);
...
int sdp_tmu_get(void)
{
	struct sdp_tmu_info *info = &tmu_info;
	int temp = 0;

	if (info->get_temp)
		temp = (int)info->get_temp(info);

	return temp;
}
EXPORT_SYMBOL(sdp_tmu_get);
...
static int sdp_tmu_probe(struct platform_device *pdev)
{
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "tmu");
	if (!res) {
		pr_err("TMU: ERROR - failed to get tmu memory region resource\n");
		ret = -ENODEV;
		goto err;
	}
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "chipid");
	if (!res) {
		pr_err("TMU: ERROR - failed to get chipid memory region resource\n");
		ret = -ENODEV;
		goto err;
	}

	info->chipid_base = devm_ioremap(&pdev->dev, res->start, (unsigned long)resource_size(res));
	if (!(info->chipid_base)) {
		pr_err("TMU: ERROR - failed ioremap()\n");
		ret = -ENOMEM;
		goto err;
	...
	info->boot_temp = info->get_temp(info);
	pr_err("TMU: boot temperature %u\'C\n", info->boot_temp);
	sdp_tmu_notifier_call_chain(SDP_TMU_UPDATE_TEMP, (struct throttle_params*)&info->boot_temp);
...
err:
	if (info->tmu_base)
		devm_iounmap(&pdev->dev, info->tmu_base);
	if (info->chipid_base)
		devm_iounmap(&pdev->dev, info->chipid_base);
