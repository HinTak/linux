...
static void sdp1601_init_tmu(struct sdp_tmu_info * info)
{
	u32 chipid[4];
	u32 val, slope, vref;
...
static int sdp_tmu_suspend_ext(void *dev)
{
	struct sdp_tmu_info *info = (struct sdp_tmu_info *)dev_get_drvdata(dev);

	pr_info("TMU: suspend_ext\n");
	
	cancel_delayed_work(&info->polling);

	return 0;
}
static int sdp_tmu_resume_ext(void *dev)
{
	struct sdp_tmu_info *info = (struct sdp_tmu_info *)dev_get_drvdata(dev);
	
	pr_info("TMU: resume_ext\n");
...
static int sdp_tmu_probe(struct platform_device *pdev)
{
...
#if 0 //def CONFIG_SDP_DEV_PM
	{
		...
		dev_pm.priority = SDP_DEV_PM_PRI_NORMAL;
		dev_pm.dev = (void *)(info);
		dev_pm.suspend = sdp_tmu_suspend_ext;
		dev_pm.resume = sdp_tmu_resume_ext;
		sdp_dev_pm_register(&dev_pm, "sdp-thermal");
	}
#endif
