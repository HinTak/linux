...
#include <soc/novatek/ndp_hwmem_io.h>

#undef DEBUG_NDP_HWMEM
/*#define DEBUG_NDP_HWMEM 1*/

#ifdef DEBUG_NDP_HWMEM
#define DPRINT_NDP_HWMEM(fmt, args...) printk("[%s]" fmt, __func__, ##args)
#else
#define DPRINT_NDP_HWMEM(fmt, args...)
#endif

#define PRINT_HWMEM_ERR(fmt, args...) printk(KERN_ERR"[%s]" fmt, __func__, ##args)
...
static inline void
ndp_hwmem_inv_range(const void* vir_addr, const unsigned long phy_addr, const size_t size)
{
	dmac_map_area(vir_addr, size, DMA_FROM_DEVICE);
}

static inline void
ndp_hwmem_clean_range(const void *vir_addr, const unsigned long phy_addr, const size_t size)
{
	dmac_map_area(vir_addr, size, DMA_TO_DEVICE);
}

static inline void
ndp_hwmem_flush_range(const void *vir_addr, const unsigned long phy_addr, const size_t size)
{
...
static int
ndp_hwmem_cache(unsigned int cmd, NDP_CACHE_HWMEM_T *p_hwmem_info)
{
	int ret_val = 0;
	NDP_CACHE_HWMEM_T hwmem_info;

	ret_val = (int) copy_from_user(&hwmem_info, p_hwmem_info, sizeof(NDP_CACHE_HWMEM_T));
...
ndp_hwmem_ioctl(struct file *file, unsigned int cmd, unsigned long args)
{
	...
	switch(cmd){
		...
		case (CMD_HWMEM_CLEAN_RANGE): 
		case (CMD_HWMEM_FLUSH_RANGE): 
			ret_val = ndp_hwmem_cache(cmd, (NDP_CACHE_HWMEM_T *) args);
			break;
		default:
			break;
	}
...
static int __init ndp_hwmem_init(void)
{
	...
	if (ret_val) {
		...
		printk(KERN_INFO"[NDP_HWMEM] %s initialized\n", NDP_HWMEM_NAME);
	}
