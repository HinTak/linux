...
static irqreturn_t nvt_unzip_isr(int irq, void *devId)
{
	...
	config = readl(nvt_unzip->base + HAL_GZIP_HW_CONFIG);
	writel((config & ~HAL_GZIP_IFF_MODE_EN), nvt_unzip->base + HAL_GZIP_HW_CONFIG);
	...
	*/
	...
	{
		pr_err("unzip: uzreq NULL pointer\n");
	}
...
struct nvt_unzip_buf *nvt_unzip_alloc(size_t len)
{
		...
		return ERR_PTR(-EINVAL);
	{
		...
		if (i == HW_MAX_SIMUL_THR) {
			...
			dev_err(nvt_unzip->dev, "[Status_DRV_UNZIP] all input buf is not avail!!!\n");
			for (j = 0; j < HW_MAX_SIMUL_THR; j++)
				debug_nvt_unzip_inputbuf(j);
			dev_err(nvt_unzip->dev, "Status %d timestamp   %llx\n",
				i, ts);
