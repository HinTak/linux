...
static int unzip_hal_reset(enum hw_iovec_comp_type, bool may_wait);
...
static int nvt_unzip_probe(struct platform_device *pdev)
{
	...
	dev_err(nvt_unzip->dev, "[DEBUG]register base %x\n", res->start);
	...
	dev_err(nvt_unzip->dev, "[DEBUG] interregister base %x\n", res->start);
