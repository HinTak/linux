...
struct nvt_auth_t
{
    ...
    u32 data_tail_len; 
    u32 req_idx;
	u32 aeswait_state;	     // for debug
	u32 auth_state;
	u32 pre_auth_state;	     // for debug
	volatile u32 Done_req_idx;
    volatile u32 Done_byteCount;
    volatile u32 Done_in_aes_timeout;	    // recovery case
	volatile u32 in_aesisr;
	volatile u32 in_shaisr;
	volatile u32 in_aeswait_timeout;
	volatile u32 in_shawait_timeout;	
    ...
    unsigned long long clock_tmpns[TIMEARRAY_NO];      	
};
...
static unsigned long long nvt_aes_calls=0;
static unsigned long long nvt_aes_nouse_calls=0;           //cy...test
void __iomem *temp_addr =  NULL;                           //cy...test 
...
static void debug_nvt_auth_status(void)
{
    ...
    dev_err(nvt_auth->dev, "in_shaisr 0x%x\n",     
            nvt_auth->in_shaisr);

    dev_err(nvt_auth->dev, "in_shawait_timeout 0x%x\n",     
            nvt_auth->in_shawait_timeout);

    dev_err(nvt_auth->dev, "in_aesisr 0x%x\n",     
            nvt_auth->in_aesisr);

    dev_err(nvt_auth->dev, "in_aeswait_timeout 0x%x\n",     
            nvt_auth->in_aeswait_timeout);

    dev_err(nvt_auth->dev, "aeswait_state 0x%x\n",     
            nvt_auth->aeswait_state);

    dev_err(nvt_auth->dev, "aes auth_state 0x%x\n",     
            nvt_auth->auth_state);

    dev_err(nvt_auth->dev, "pre_aes auth_state 0x%x\n",     
        nvt_auth->pre_auth_state);

    dev_err(nvt_auth->dev, "aes done req idx 0x%x\n",     
            nvt_auth->Done_req_idx);

    dev_err(nvt_auth->dev, "aes done bytecount 0x%x\n",     
            nvt_auth->Done_byteCount);
	
    dev_err(nvt_auth->dev, "aes Done_in_aes_timeout 0x%x\n",     
            nvt_auth->Done_in_aes_timeout);

    // temp here     
    {
	    u8 u8idx=0;
       	dev_err(nvt_auth->dev, "clock_ns=0x%llx\n", nvt_auth->clock_ns);		
		for(u8idx=0; u8idx<TIMEARRAY_NO; u8idx++)
		{
        	dev_err(nvt_auth->dev, "tempns[%x]=0x%llx\n",	 
	        	u8idx, nvt_auth->clock_tmpns[u8idx]);
		}	
    }
...
static irqreturn_t nvt_auth_shaisr(int irq, void* devId)
{
    ...
    u32 u32Bytecnt=0, u32DoneFlag=0;
	...
	if(nvt_auth->in_shawait_timeout)		
	{
	   dev_err(nvt_auth->dev,"req_idx =%x(0x%llx), shaisr_timeout\n", nvt_auth->req_idx, nvt_auth_calls);
	   return IRQ_HANDLED;		
	}	

	nvt_auth->in_shaisr = 1;
...
#if 0   // for test only  	// for CY...debug
			if( (nvt_auth->req_idx%0x500) == 0 )
			{
				//dev_err(nvt_auth->dev, "req_idx... =%x(shaIsrD), ns=0x%llx, sys-ns=0x%llx", nvt_auth->req_idx, nvt_auth_nsecs, (nvt_auth_nsecs+nvt_auth->clock_ns));	
				dev_err(nvt_auth->dev, "req_idx... =%x(shaIsrD), shaisr_call=0x%llx\n", nvt_auth->req_idx, nvt_auth_calls);				
			}			
#endif		

            u32Bytecnt = readl(nvt_auth->shadma.base + ADMA_SRC_BYTE_COUNT);            
            if (nvt_auth_sha_dbg.input_size == u32Bytecnt)
            {
                u32DoneFlag = 1;
            }
			else
			{
				dev_err(nvt_auth->dev, "req_idx~~ =%x(0x%llx), sha_cnt diff:%x-%x\n", nvt_auth->req_idx, nvt_auth_calls, nvt_auth_sha_dbg.input_size, u32Bytecnt);			 
			}

            //complete(&nvt_auth->shadma.wait);      //cy...don't release event here
        }
        val&=~datadone;
    }
    if(val&tomask)
    {
		...
		dev_err(nvt_auth->dev,"req_idx =%x(%x), shaisr_timeout_1\n", nvt_auth->req_idx, u32DoneFlag);					
        nvt_auth_dump();   //debug20191214  
        //complete(&nvt_auth->shadma.wait);             //cy...don't release event here
    }
	...
	nvt_auth->in_shaisr = 0;	
	
    return IRQ_HANDLED;
}
#else   // else of #if	defined(CONFIG_ARCH_NVT72671D)
...
#if 0   // for test only  	// for CY...debug
		if( (nvt_auth->req_idx%0x100) == 0 )
		{
			//dev_err(nvt_auth->dev, "req_idx... =%x(shaIsrD), ns=0x%llx, sys-ns=0x%llx", nvt_auth->req_idx, nvt_auth_nsecs, (nvt_auth_nsecs+nvt_auth->clock_ns));	
			dev_err(nvt_auth->dev, "req_idx... =%x(shaIsrD), ns=0x%llx, sys-ns=0x%llx", nvt_auth->req_idx, nvt_auth_nsecs, (nvt_auth_nsecs+nvt_auth->clock_ns));	
			
			// CY test only..... assume the error flag is set and don't complete the "sahdma.wait"			
			//if(gu8ErrFlag==0)
			//   gu8ErrFlag = 1;			
		}
		//else
...
#endif  // end of #if	defined(CONFIG_ARCH_NVT72671D)
...
//	static unsigned long long nvt_aes_calls=0;
    ...
    static u32 u32Cnt=0, intval=0, bDump=0;
    u32 u32tmp=0, u32tmp1=0, u32Idx=0;     //cy...test        
    u32 u32Flag = 0; 
	...
	nvt_aes_calls++; 
	if(nvt_auth->in_aeswait_timeout)		
	{
	   if((nvt_aes_calls&0x1F)==0x1F)
	   {	
	       dev_err(nvt_auth->dev,"req_idx =%x(0x%llx)(%x), aesisr_timeout\n", nvt_auth->req_idx, nvt_aes_calls, nvt_auth->aeswait_state);
	   }
	   return IRQ_HANDLED;		
	}   

	nvt_auth->in_aesisr = 1;

	if((nvt_auth_aes_dbg.input_size==nvt_auth->Done_byteCount)&&(nvt_auth->Done_req_idx == nvt_auth->req_idx))
	{	
	   // clear isr status again (it should be cleared in "aesctr_wait")	   
	   if((nvt_auth->auth_state>=8)&&(nvt_auth->auth_state<16))
	   {	   
		  if((nvt_aes_calls&0x1F)==0x1F)
		  {
			  dev_err(nvt_auth->dev,"req_idx =%x(0x%llx)(%x), aes_done(%x)\n", nvt_auth->req_idx, nvt_aes_calls, nvt_auth->auth_state, nvt_auth->Done_byteCount);	
		  }
	   ...
	   nvt_auth->in_aesisr = 0;	   
	   return IRQ_HANDLED; 	 	
	}

    nvt_auth->clock_tmpns[0] = nvt_auth_get_nsecs();	
...
#endif
#else
    if(nvt_auth->req_idx==u32Cnt)
    {
       u32tmp =  readl(nvt_auth->aesdma.base+SCE_ADMA_INT_STATUS_AND_CLR);
       //dev_err(nvt_auth->dev,"req_idx =%x(0x%llx), aes interrupt: %x-%x(%x)\n", nvt_auth->req_idx, nvt_aes_calls, val, u32tmp, intval);
	   
	   if((u32tmp==intval))
       {      
		  readl(temp_addr);				   
		  u32tmp = readl(nvt_auth->aesdma.base+SCE_ADMA_SRC_BYTE_COUNT);  
		  readl(temp_addr);				   
		  u32tmp1 = readl(nvt_auth->aesdma.base+SCE_ADMA_DST_BYTE_COUNT);	
		  ...
		  dev_err(nvt_auth->dev,"req_idx =%x(%x), auth_state.. in ISR=%x, count:%x - %x(%x)\n", nvt_auth->req_idx, nvt_auth->Done_req_idx, nvt_auth->auth_state, nvt_auth_aes_dbg.input_size, u32tmp, u32tmp1);		  
		  //if((nvt_auth->auth_state>5)&&(u32tmp==u32tmp1)&&(u32tmp==0))
		  if((nvt_auth->auth_state>5))
		  {
			  dev_err(nvt_auth->dev,"req_idx =%x, close(isr err)\n", nvt_auth->req_idx);		  
		      //aesctr_reset(); 
			  //sha256_reset();
			  nvt_auth->aesdma.ISRErr = 0xEEEEEEEE;    
              nvt_auth->in_aesisr = 0;			  
			  complete(&nvt_auth->aesdma.wait);   		
			  return IRQ_HANDLED;
		  }	
		  else if((nvt_auth_aes_dbg.input_size==u32tmp)&&(u32tmp == u32tmp1)&&(nvt_auth->req_idx == nvt_auth->Done_req_idx)) 
		  {		      
			  dev_err(nvt_auth->dev,"req_idx =%x, close(bytecont done)\n", nvt_auth->req_idx);		  
		      //aesctr_reset(); 
			  //sha256_reset();
			  nvt_auth->aesdma.ISRErr = 0xAAAAAAAA;    
              nvt_auth->in_aesisr = 0;			  
			  complete(&nvt_auth->aesdma.wait);  			  
			  return IRQ_HANDLED;
		  }
	...
	{
	   u32Cnt = nvt_auth->req_idx;	
	   bDump = 0;	   
	}   

	if(nvt_auth->auth_state>5)
	{
		dev_err(nvt_auth->dev,"req_idx =%x, auth_state in ISR=%x\n", nvt_auth->req_idx, nvt_auth->auth_state);			
	}
    ...
    if(val&0x1)
    {
		...
		writel(0, nvt_auth->aesdma.base+SCE_INT);
        ...
        if(val&donemask)
        {
            ...
            if(nvt_auth->aesdma.ISRStat == donemask)
            {
                ...
                nvt_auth->clock_tmpns[1] = nvt_auth_get_nsecs();	

#if 0   // for test only  	// for CY...debug
				if( (nvt_auth->req_idx%0x500) == 0 )
				{
					dev_err(nvt_auth->dev, "req_idx...=%x(AESIsr)(%x), aesisr_call=0x%llx (0x%llx) ", nvt_auth->req_idx, nvt_auth->auth_state, nvt_aes_calls, nvt_aes_nouse_calls);
				}				
#endif			

				if(((nvt_auth_aes_dbg.input_size==nvt_auth->Done_byteCount)&&(nvt_auth->Done_req_idx == nvt_auth->req_idx))||(nvt_auth->Done_in_aes_timeout == 1))
				{				    
					 u32Flag = 5;
				}
				else
				{				      
					//compare DST byte count			
					readl(temp_addr);					
					u32tmp = readl(nvt_auth->aesdma.base+SCE_ADMA_SRC_BYTE_COUNT);	
					readl(temp_addr);					
					u32tmp1 = readl(nvt_auth->aesdma.base+SCE_ADMA_DST_BYTE_COUNT); 							
					if((nvt_auth_aes_dbg.input_size==u32tmp)&&(u32tmp == u32tmp1))
					{ 			
						u32Flag = 1; 				   	
						nvt_auth->Done_req_idx = nvt_auth->req_idx;
						nvt_auth->Done_byteCount= u32tmp;					
					}
					else
					{
						//dev_err(nvt_auth->dev,"req_idx=%x, AES bytecount diff SRC:%x, DTS=%x\n", nvt_auth->req_idx, u32tmp, u32tmp1);

						// debug test, try to read again					
						u32tmp = readl(nvt_auth->aesdma.base+SCE_ADMA_SRC_BYTE_COUNT);	
						u32tmp1 = readl(nvt_auth->aesdma.base+SCE_ADMA_DST_BYTE_COUNT);
					    if((nvt_auth_aes_dbg.input_size==u32tmp)&&(u32tmp == u32tmp1))
						{ 			
						...
						dev_err(nvt_auth->dev,"req_idx=%x(%x), AES.. bytecount diff SRC:%x, DTS=%x, Flag=%x\n", nvt_auth->req_idx, nvt_auth->auth_state, u32tmp, u32tmp1, u32Flag);					
						// debug test end								
					}
				}

				nvt_auth->clock_tmpns[2] = nvt_auth_get_nsecs(); 
				if((nvt_auth->clock_tmpns[2] - nvt_auth->clock_tmpns[1]) > 10000000) 
				{     
					dev_err(nvt_auth->dev, "req_idx(AESIsr_21)=%x(%x), aesisr_call=0x%llx, ns(0x%llx-0x%llx) ", nvt_auth->req_idx, nvt_auth->auth_state, nvt_aes_calls, nvt_auth->clock_tmpns[2], nvt_auth->clock_tmpns[1]);
				}					
                //nvt_auth->aesdma.ISRStat=0;
                //complete(&nvt_auth->aesdma.wait);    //cy...don't release event here...
            }
            val&=~datadone;
        }
        if(val&tomask)
        {
			...
			//cy...test			
			u32tmp = readl(nvt_auth->aesdma.base+SCE_ADMA_INT_STATUS_AND_CLR); 	
			if((u32tmp&tomask)==tomask)
			{
				dev_err(nvt_auth->dev,"aesisr.1 no clear?: %x\n", u32tmp);
			}
			//test end
			...
			dev_err(nvt_auth->dev,"req_idx =%x(%x), aesisr_timeout_1\n", nvt_auth->req_idx, u32Flag);
		...
		if(nvt_auth->Done_in_aes_timeout == 1)     // done in timeout process
		{
	        if(val)
	        {
	            //ignore other status
				...
				if((u32tmp&val)==val)
				{
					dev_err(nvt_auth->dev,"aesisr.2 no clear?: %x\n", u32tmp);
				}
				//test end
	        }
			...
			//cy...test 		
			readl(temp_addr);			
			u32tmp = readl(nvt_auth->aesdma.base+SCE_INTSTS);	
			if((u32tmp&1)==1)
			{
				dev_err(nvt_auth->dev,"aesisr.3 no clear?: %x\n", u32tmp);
			}
			//test end
	...
	{
     	nvt_aes_nouse_calls++;
		dev_err(nvt_auth->dev,"aes nouse interrupt: %x\n", val);		
	}

	nvt_auth->clock_tmpns[3] = nvt_auth_get_nsecs(); 	
	if((nvt_auth->clock_tmpns[3] - nvt_auth->clock_tmpns[0]) > 10000000) 
	{     
		dev_err(nvt_auth->dev, "req_idx(AESIsr_30)=%x(%x), aesisr_call=0x%llx, ns(0x%llx-0x%llx) ", nvt_auth->req_idx, nvt_auth->auth_state, nvt_aes_calls, nvt_auth->clock_tmpns[3], nvt_auth->clock_tmpns[0]);
	}
	#if 0
	else   // temp test, remove later
	{
		if( (nvt_auth->req_idx%0x500) == 0 )
		{
			dev_err(nvt_auth->dev, "req_idx(AESIsr..)=%x(%x), aesisr_call=0x%llx, ns(0x%llx-0x%llx-0x%llx-0x%llx) ", nvt_auth->req_idx, nvt_auth->auth_state, nvt_aes_calls, nvt_auth->clock_tmpns[3], nvt_auth->clock_tmpns[2], nvt_auth->clock_tmpns[1], nvt_auth->clock_tmpns[0]);
		}					
	}
	#endif
			
    nvt_auth_aes_dbg.last_intr_status = val;
	nvt_auth->Done_in_aes_timeout = 0;     // event finished
    nvt_auth->in_aesisr = 0;	
    return IRQ_HANDLED;
}
...
static void aesctr_reset(void)
{
    TRACE_ENTRY();   
	//0xD00:
	//Bit6: Clear destination byte count	
	//Bit5: Clear source byte count	
	//Bit4: HW reset 		
...
static void aesctr_init(u32* key, u32* iv)
{
    ...
    nvt_auth->bypass=0;
	nvt_auth->in_aeswait_timeout = 0;			
	nvt_auth->in_aesisr=0;
	nvt_auth->Done_in_aes_timeout = 0;
...
////////////////
    ...
    u32tmp = readl(nvt_auth->aesdma.base+SCE_ADMA_HW_CONFIGURE);
	u32cnt = 0;
    while(1)
    {
	    if(u32tmp!=0x1009005)		
	    {	    
			dev_err(nvt_auth->dev,"req_idx =%x(%x), SCE_ADMA_HW_CONFIGURE: %x\n", nvt_auth->req_idx, u32tmp,u32cnt);    			
	    }
		...
		if(u32cnt>0x500)
		{
		   dev_err(nvt_auth->dev,"req_idx =%x(%x), SCE_ADMA_HW_CONFIGURE fail\n", nvt_auth->req_idx,u32cnt);
		   break;
		}
...
#define AESWAIT_CNT_MAX    200
static int aesctr_wait(void)
{
	unsigned int src_byte_cnt, val;	
	unsigned int wait_cnt = 0, status_cnt=0;

    TRACE_ENTRY();   
    //wait_for_completion(&nvt_auth->aesdma.wait);
	while (0 == wait_for_completion_timeout(&nvt_auth->aesdma.wait,
	   msecs_to_jiffies(5)))
	 {
        if (wait_cnt >= AESWAIT_CNT_MAX)
        {
            break;
        }	
		wait_cnt ++;

		nvt_auth->aeswait_state = 1;
		
		if(nvt_auth->in_aesisr==0)   //no aesisr process
		{
		    nvt_auth->aeswait_state = 2;
		
		    nvt_auth->in_aeswait_timeout = 1;
	        val = readl(nvt_auth->aesdma.base + SCE_ADMA_SRC_BYTE_COUNT);
	        src_byte_cnt = readl(nvt_auth->aesdma.base + SCE_ADMA_DST_BYTE_COUNT);
		    //nvt_auth->in_aeswait_timeout = 0;			
			if(nvt_auth_aes_dbg.input_size==src_byte_cnt)
			{
		        nvt_auth->aeswait_state = 3;
				
				nvt_auth->Done_req_idx = nvt_auth->req_idx;
				nvt_auth->Done_byteCount= src_byte_cnt;	
                // 				
				writel(-1, nvt_auth->aesdma.base+SCE_ADMA_INT_STATUS_AND_CLR);	  
				writel(0, nvt_auth->aesdma.base+SCE_ADMA_INT_ENABLE);				
				writel(0x3, 		   nvt_auth->aesdma.base+SCE_INTSTS);	
		        ...
		        nvt_auth->aeswait_state = 4;				
			
				//dev_err(nvt_auth->dev, "req idx=%x(%x)(%x), aeswait-break : 0x%x-0x%x\n",
				//	nvt_auth->req_idx, wait_cnt, status_cnt, src_byte_cnt, val);
				if(val!=src_byte_cnt)
				{
		            nvt_auth->aeswait_state = 5;
					
					dev_err(nvt_auth->dev, "req idx=%x(%x)(%x), src-dst bytecnt not equal? 0x%x-0x%x\n",
						nvt_auth->req_idx, wait_cnt, status_cnt, src_byte_cnt, val);				    
				}				
				...
				if(nvt_auth->fifoenable)
				{
					if(((val&SCE_ADMA_FIFO_INT_DATA_DONE)!=SCE_ADMA_FIFO_INT_DATA_DONE) && ((nvt_auth->shadma.ISRStat&SCE_ADMA_FIFO_INT_DATA_DONE)!=SCE_ADMA_FIFO_INT_DATA_DONE))
					{
					   dev_err(nvt_auth->dev, "req idx=%x, AES ISR Flag-Stat : 0x%x-0x%x-0x%x-0x%x\n",
						  nvt_auth->req_idx, nvt_auth->aesdma.ISRStat, val, readl(nvt_auth->aesdma.base+SCE_ADMA_INT_STATUS_AND_CLR), readl(nvt_auth->aesdma.base+SCE_INTSTS));
					}
				}
				else
				{			
					if(((val&SCE_ADMA_INT_DATA_DONE)!=SCE_ADMA_INT_DATA_DONE) && ((nvt_auth->shadma.ISRStat&SCE_ADMA_INT_DATA_DONE)!=SCE_ADMA_INT_DATA_DONE))
					{
					   dev_err(nvt_auth->dev, "req idx~=%x, AES ISR Flag-Stat : 0x%x-0x%x-0x%x-0x%x\n",
						  nvt_auth->req_idx, nvt_auth->aesdma.ISRStat, val, readl(nvt_auth->aesdma.base+SCE_ADMA_INT_STATUS_AND_CLR), readl(nvt_auth->aesdma.base+SCE_INTSTS));
					}
		        ...
		        nvt_auth->aeswait_state = 6;
				
	            break;		 
			}
			else
			{	
		        nvt_auth->aeswait_state = 7;
				
			    nvt_auth->in_aeswait_timeout = 0;			
			    status_cnt += 0x10000;
				dev_err(nvt_auth->dev, "req idx=%x(%x)(%x), wait_bytecnt : 0x%x - 0x%x,\n",
					nvt_auth->req_idx, wait_cnt, status_cnt, src_byte_cnt, nvt_auth_aes_dbg.input_size);

		        nvt_auth->aeswait_state = 8;
			}
		}
		else   // in isr process
		{
			nvt_auth->aeswait_state = 9;
		
		    if((nvt_auth_aes_dbg.input_size==nvt_auth->Done_byteCount)&&(nvt_auth->Done_req_idx == nvt_auth->req_idx))
		    {
			    nvt_auth->aeswait_state = 10; 
				
				dev_err(nvt_auth->dev, "req idx=%x(%x)(%x), aeswait-break-1 : 0x%x\n",
					nvt_auth->req_idx, wait_cnt, status_cnt, nvt_auth->Done_byteCount);
		    
		        //break;
		    }
			else
			{
			    nvt_auth->aeswait_state = 11;
				
				dev_err(nvt_auth->dev, "req idx=%x(%x)(%x)(0x%llx), wait_bytecnt-1 : 0x%x - 0x%x - 0x%x\n",
					nvt_auth->req_idx, wait_cnt, status_cnt, nvt_aes_calls, nvt_auth->Done_byteCount, nvt_auth_aes_dbg.input_size, nvt_auth->Done_req_idx);
			
			    status_cnt++;
				// ISR no response? check if byte cnt is OK
				// It's a critical condition. Maybe reading the same APB slave register in ISR now!!!!!
				if((status_cnt&0x1F)==0x1F)
				{
			        src_byte_cnt = readl(nvt_auth->aesdma.base + SCE_ADMA_DST_BYTE_COUNT);
					dev_err(nvt_auth->dev, "req idx=%x(%x)(%x), aesisr noresp? high risk timing!!! : 0x%x - 0x%x\n",
						nvt_auth->req_idx, wait_cnt, status_cnt, nvt_auth_aes_dbg.input_size, src_byte_cnt);					
					if(nvt_auth_aes_dbg.input_size==src_byte_cnt)
					{
						nvt_auth->Done_req_idx = nvt_auth->req_idx;
						nvt_auth->Done_byteCount= src_byte_cnt; 	
						nvt_auth->Done_in_aes_timeout = 1;

						#if 0   // don't clear ISR status here. 
						writel(-1, nvt_auth->aesdma.base+SCE_ADMA_INT_STATUS_AND_CLR);	  
				        writel(0, nvt_auth->aesdma.base+SCE_ADMA_INT_ENABLE);				
				        writel(0x3, 		   nvt_auth->aesdma.base+SCE_INTSTS);	
				        writel(0, nvt_auth->aesdma.base+SCE_INT);	
	...
	nvt_auth->aeswait_state = 12;

	if(wait_cnt > 1)	
	{
        dev_err(nvt_auth->dev, "req idx=%x, aeswait_cnt 0x%x(0x%x)\n",nvt_auth->req_idx, wait_cnt, status_cnt);
	}
	
	if (wait_cnt >= AESWAIT_CNT_MAX)
	{
        dev_err(nvt_auth->dev, "req idx=%x, aes time out 0x%x\n",nvt_auth->req_idx, wait_cnt);	
//        complete(&nvt_auth->aesdma.wait);											
		nvt_auth_dump();
//		nvt_unzip_dump_reg();

	    nvt_auth->aesdma.ISRErr = 0xBBBBBBBB;
	}
    ...
    if(nvt_auth->aesdma.ISRErr)
    {
        ...
        dev_err(nvt_auth->dev, "req idx=%x, AES ISRStat %x, ISRErr %x\n", 
                nvt_auth->req_idx, nvt_auth->aesdma.ISRStat, nvt_auth->aesdma.ISRErr);
        up(&nvt_auth->sema);
        return (-EINVAL);
    }
...
#define WAIT_CNT_MAX    500
static noinline int sha256_wait(void)
{    
    ...
    unsigned int wait_cnt = 0;
     ...
     while (0 == wait_for_completion_timeout(&nvt_auth->shadma.wait,
		   msecs_to_jiffies(5)))
     {
			...
			if(nvt_auth->in_shaisr==0)	 //no shaisr process
			{			
				nvt_auth->in_shawait_timeout = 1;
	            src_byte_cnt = readl(nvt_auth->shadma.base + ADMA_SRC_BYTE_COUNT);
	            //dev_err(nvt_auth->dev, "src_byte_cnt 0x%x\n", src_byte_cnt);
	            
	            if (nvt_auth_sha_dbg.input_size == src_byte_cnt)
	            {
					writel(-1, nvt_auth->shadma.base+ADMA_INT_STATUS_AND_CLR);	  
					writel(0, nvt_auth->shadma.base+ADMA_INT_ENABLE);			   
					nvt_auth->in_shawait_timeout = 0;
				    ...
				    #if 0
	                //dev_err(nvt_auth->dev, "same, src_byte_cnt 0x%x\n",src_byte_cnt);
	                val = readl(nvt_auth->shadma.base+ADMA_INT_STATUS_AND_CLR);
	            #ifdef CONFIG_NVT_UNZIP_FIFO
				    if(((val&ADMA_FIFO_INT_DATA_DONE)!=ADMA_FIFO_INT_DATA_DONE) && ((nvt_auth->shadma.ISRStat&ADMA_FIFO_INT_DATA_DONE)!=ADMA_FIFO_INT_DATA_DONE))			
	            #else
	                if(((val&ADMA_INT_DATA_DONE)!=ADMA_INT_DATA_DONE) && ((nvt_auth->shadma.ISRStat&ADMA_INT_DATA_DONE)!=ADMA_INT_DATA_DONE))
	            #endif					
	                {
					    dev_err(nvt_auth->dev, "req idx=%x, SHA ISR Flag-Stat : 0x%x - 0x%x, 0x%x\n",
							nvt_auth->req_idx, nvt_auth->shadma.ISRStat, val, readl(nvt_auth->shadma.base+ADMA_INT_STATUS_AND_CLR));
	                }
				    #endif
	            ...
	            {	            
	                if((wait_cnt&0x0F)==0x0F)
	                {
	                   dev_err(nvt_auth->dev, "req idx=%x, wait, src_byte_cnt 0x%x, size %x\n",nvt_auth->req_idx, src_byte_cnt, nvt_auth_sha_dbg.input_size);
	                }
					nvt_auth->in_shawait_timeout = 0;	                
	                continue;
	            }
			}
			else  // in sha isr process
			{
				dev_err(nvt_auth->dev, "req idx=%x(%x) in shaisr process\n",nvt_auth->req_idx, wait_cnt);			
			}            
     }

    //if(wait_cnt != 0)
	if(wait_cnt > 1)	
         dev_err(nvt_auth->dev, "req idx=%x, wait_cnt 0x%x\n",nvt_auth->req_idx, wait_cnt);

    //while (0 == wait_for_completion_timeout(&nvt_auth->shadma.wait,
    //		   msecs_to_jiffies(3000)))
    if (wait_cnt >= WAIT_CNT_MAX)		   
    {		
		dev_err(nvt_auth->dev, "gu8ErrFlag wait, req_idx =%x, sys-ns=0x%llx", nvt_auth->req_idx, nvt_auth_get_nsecs());	
        complete(&nvt_auth->shadma.wait);									
		{
			dev_err(nvt_auth->dev, "auth timeout\n");
			dev_err(nvt_auth->dev, "SHA ISRStat %x, ISRErr %x\n",
					nvt_auth->shadma.ISRStat, nvt_auth->shadma.ISRErr);
			nvt_auth_dump();
			nvt_unzip_dump_reg();
			nvt_auth->shadma.ISRErr = 0xBBBBBBBB;
		}					
		dev_err(nvt_auth->dev, "No gu8ErrFlag wait, req_idx =%x, sys-ns=0x%llx", nvt_auth->req_idx, nvt_auth_get_nsecs());						
		//BUG();
    }
    ...
    if(nvt_auth->shadma.ISRErr)
    {
        ...
        dev_err(nvt_auth->dev, "req_idx =%x, SHA ISRStat %x, ISRErr %x\n", 
                nvt_auth->req_idx, nvt_auth->shadma.ISRStat, nvt_auth->shadma.ISRErr);
        up(&nvt_auth->sema);
        return (-EINVAL);
    }
...
#else   // else #if	defined(CONFIG_ARCH_NVT72671D)
        ...
        while (0 == wait_for_completion_timeout(&nvt_auth->shadma.wait,
		   msecs_to_jiffies(1)))
         {
            ...
            if (nvt_auth_sha_dbg.input_size == src_byte_cnt)
            {
                //dev_err(nvt_auth->dev, "same, src_byte_cnt 0x%x\n",src_byte_cnt);
                val = readl(nvt_auth->shadma.base+ADMA_INT_STATUS_AND_CLR);
                if((val!=0x80000003) && (nvt_auth->shadma.ISRStat!=0x80000003))
                {
				    dev_err(nvt_auth->dev, "req idx=%x, SHA ISR Flag-Stat : 0x%x - 0x%x, 0x%x\n",
						nvt_auth->req_idx, nvt_auth->shadma.ISRStat, val, readl(nvt_auth->shadma.base+ADMA_INT_STATUS_AND_CLR));
                }
...
int hw_auth_start(dma_addr_t *ipages, int nr_ipages, unsigned int input_bytes, struct nvt_unzip_auth_t *auth, u32 offset)
{
    ...
    nvt_auth->pre_auth_state = nvt_auth->auth_state;     // record the final state of the previous request
		
	nvt_auth->auth_state = 9;
    ...
    nvt_auth->auth_state = 16;

    //down(&nvt_auth->sema);     
	ret = down_timeout((&nvt_auth->sema), msecs_to_jiffies(100));
	if(ret)
	{
		dev_err(nvt_auth->dev, "auth down_timeout fail\n");
    ...
    if(nvt_auth->fifoenable)
    {
        ...
        nvt_auth->auth_state = 20;
    }
#endif     
    
    nvt_auth->req_idx++;

	nvt_auth->auth_state = 1;
...
int hw_auth_wait(void)
{
	...
	nvt_auth->auth_state = 3;

    sha256_wait();

	nvt_auth->auth_state = 4;
	...
	nvt_auth->auth_state = 7;

    sha256_reset(); 

	nvt_auth->auth_state = 8;	

	if( (nvt_auth->req_idx%0x500) == 0 )
	{
		dev_err(nvt_auth->dev, "req_idx...=%x((0x%llx))", nvt_auth->req_idx, nvt_aes_calls);
	}				
...
static int nvt_auth_probe(struct platform_device *pdev)
{
...
#if 1
#if	defined(CONFIG_ARCH_NVT72671D)
	// *** read another APB slave register
	if(temp_addr == NULL)
	{
		//temp_addr = ioremap_nocache(0xfd0d0000, 0x4);					
		temp_addr = ioremap(0xfd0d0000, 0x4);
		if (temp_addr == NULL) 
		{
			dev_err(nvt_auth->dev,"AUTH fail to ioremap gpio 0xfd0d0000\n");
		}
		else
		{
			dev_err(nvt_auth->dev, "~get gpio base-val: 0x%x, 0x%x\n",
					(unsigned int)temp_addr, readl(temp_addr)); 					
		}
	}	
	// cy...test end
#endif	
	...
	if (of_get_property(dev->of_node, "enable_fifo_mode", NULL)) {
		...
		dev_err(dev,"hw auth support fifo mode, 1090217\n");
	}
#endif
