...
static int vdtime_read_proc(struct seq_file *m, void *v)
{
    ...
    #ifdef VD_VERBOSE
	pr_err("read time offset = %llu msec\n",
		 vdtime_dev.vdt.vd_msec_offset);
	pr_err("read tz offset = %d min\n",
		 vdtime_dev.vdt.vd_min_tz);
	pr_err("read dst offset = %d min\n",
		 vdtime_dev.vdt.vd_min_dst);
    #endif
			...
			size_t count, loff_t *ppos)
{
	...
	if (count > VD_MAX_PROC_SIZE) {
		pr_err("Error writing to /proc/vd_time:\n");
		pr_err("too long line, %d-byte length is permited\n",
			VD_MAX_PROC_SIZE);
		return -EINVAL;
	}
    ...
    /* Parse user data */
	num = sscanf(vdtime_proc_data, "%lld %d %d %d",
...
#ifdef VD_VERBOSE
	pr_err("write time offset = %lld msec\n",
		 user_vdt.vd_msec_offset);
	pr_err("write tz offset = %d min\n",
		 user_vdt.vd_min_tz);
	pr_err("write dst offset = %d min\n",
		 user_vdt.vd_min_dst);
#endif
...
static const struct file_operations vdtime_proc_fops = {
	.owner =  THIS_MODULE,
...
static int __init vdtime_init(void)
{
    ...
    /* Create a proc entry */
	vdtime_proc_entry = proc_create("vd_time", 0666, NULL,
					&vdtime_proc_fops);
	if (!vdtime_proc_entry) {
		pr_err("Error creating vd_time proc entry");
		return -ENOMEM;
	}
