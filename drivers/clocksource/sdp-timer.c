 ...
 * This is for clock source, clock event device, and local timer support.
 */
...
static int clkevt_broadcast_rating = 400;
static int clkevt_local_rating =  400;
...
#if 0
	/* we had used fixed shift value(32) */
...
static void __cpuinit _sdp_clockevent_init(struct sdp_clock_event_dev *sdp_evt)
{
	...
	pr_info("%s: clockevent device using timer%d, irq%d. rating %d\n",
			sdp_evt->name, sdp_evt->timer_id, sdp_evt->irq, sdp_evt->evt->rating);
...
static void __init sdp_clockevent_init(struct device_node *np)
{
	...
	evt->rating = clkevt_broadcast_rating;
	evt->shift = 32;
...
static int __cpuinit sdp_localtimer_setup(struct clock_event_device *evt)
{
	...
	evt->rating = clkevt_local_rating;
	evt->shift = 32;
...
static void __init sdp_timer_of_init(struct device_node *np)
{
	...
	if (!of_property_read_u32(np, "clkevt_broadcast_rating", &tmp))
		clkevt_broadcast_rating = tmp;
	if (!of_property_read_u32(np, "clkevt_local_rating", &tmp))
		clkevt_local_rating = tmp;

	pr_info("%d %d %d\n", clksource_rating, clkevt_broadcast_rating, clkevt_local_rating);
