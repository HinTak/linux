/* drivers/cpufreq/sdp1501-cpufreq.c
 *
 * Copyright (c) 2014 Samsung Electronics Co., Ltd.
 *		http://www.samsung.com
 *
 * SDP1501 Jazz-M CPU frequency scaling support
 *
 */
...
static struct cpufreq_frequency_table sdp1501_freq_table[] = {
	{ L0, 1300*1000},
	{ L1, 1200*1000},	
	{ L2, 1100*1000},	
	{ L3, 1000*1000},
...
static unsigned int sdp1501_volt_table[CPUFREQ_LEVEL_END];

static const unsigned int sdp1501_asv_voltage[CPUFREQ_LEVEL_END][CPUFREQ_ASV_COUNT] = {
...
#define REG_SDP1501_PLL_CPU_PMS		(SFR_VA + 0x005C0000 - PHY_REG_BASE)
#define REG_SDP1501_PWM_CLK_CON		(SFR_VA + 0x00790008 - PHY_REG_BASE)
#define USE_DVFS_CLOCKS			(1 << 16)
#define SEL_DVFSHALF			12
#define SEL_ARM_VS_DVFS			(1 << 8)
#define REG_SDP1501_PLL_CPU_CTRL	(SFR_VA + 0x005C000C - PHY_REG_BASE)
#define PLL_CPU_LOCK_EN			(1 << 0)
#define REG_SDP1501_PLL_RESET_N_CTRL	(0x005C0230) /* must use lock, physical address */
#define PLL_CPU_RESET_N			(1 << 20)
#define REG_SDP1501_PLL_CPU_LOCK	(SFR_VA + 0x005C08D8 - PHY_REG_BASE)
#define CPU_PLL_LOCK			(1 << 0)
#define CPUFREQ_TEMP_FREQ		(250000);

static unsigned int sdp1501_get_speed(unsigned int cpu)
{
	unsigned int ret;
	unsigned int pms;
	unsigned int infreq;
	...
	pms = readl((void *)REG_SDP1501_PLL_CPU_PMS);
...
static void set_volt_table(int result)
{
	...
	freq = sdp1501_get_speed(0);
	for (i = L0; i < CPUFREQ_LEVEL_END; i++) {
		if ((sdp1501_freq_table[i].frequency*1000) == freq)
			break;
	}
	...
	pr_info("DVFS: cur CPU clk = %dMHz, max support freq is %dMHz",
				freq/1000000, sdp1501_freq_table[i].frequency/1000);
	
	for (i = L0; i < max_real_idx; i++)
		sdp1501_freq_table[i].frequency = CPUFREQ_ENTRY_INVALID;

	pr_info("DVFS: CPU voltage table is setted with asv group %d\n", result);

	if (result < CPUFREQ_ASV_COUNT)
		for (i = L0; i < CPUFREQ_LEVEL_END; i++) {
			sdp1501_volt_table[i] = sdp1501_asv_voltage[i][result];
		}
	else
...
static void sdp1501_set_clkdiv(unsigned int old_index, unsigned int new_index)
{
	u32 val;
	u8 tmp_div;
	...
	if (old_index > new_index) {
		if (sdp1501_freq_table[new_index].frequency > 1000000)
			tmp_div = 1;
		else if (sdp1501_freq_table[new_index].frequency > 500000)
			tmp_div = 2;
		else
			tmp_div = 3;
	} else {
		 ...
		 */
		tmp_div = (int)sdp1501_freq_table[old_index].frequency / CPUFREQ_TEMP_FREQ;
	...
	val = readl((void *)REG_SDP1501_PWM_CLK_CON) | USE_DVFS_CLOCKS;
	writel(val, (void *)REG_SDP1501_PWM_CLK_CON);

	/* set the mux to selected ams clock(sel_ams_half) */
	val = readl((void *)REG_SDP1501_PWM_CLK_CON) & (~(0x3 << SEL_DVFSHALF));
	val |= (u32)tmp_div << SEL_DVFSHALF; /* AMS clk div */
	writel(val, (void *)REG_SDP1501_PWM_CLK_CON);

	/* change CPU clock source to Temp clock(sel_arm_ams) */
	val = readl((void *)REG_SDP1501_PWM_CLK_CON);
	val &= ~SEL_ARM_VS_DVFS;
	writel(val, (void *)REG_SDP1501_PWM_CLK_CON);

	/* pll lock enable(pll_cpu_locken) */
	val = readl((void *)REG_SDP1501_PLL_CPU_CTRL) | PLL_CPU_LOCK_EN;
	writel(val, (void *)REG_SDP1501_PLL_CPU_CTRL);

	/* PWD off */
	sdp_set_clockgating(REG_SDP1501_PLL_RESET_N_CTRL, PLL_CPU_RESET_N, 0);

	/* change CPU pll value */
	writel(clkdiv_cpu[new_index], (void *)(REG_SDP1501_PLL_CPU_PMS));

	/* PWD on */
	sdp_set_clockgating(REG_SDP1501_PLL_RESET_N_CTRL, PLL_CPU_RESET_N, PLL_CPU_RESET_N);

	/* wait PLL lock */
	while (!(readl((void *)REG_SDP1501_PLL_CPU_LOCK) & CPU_PLL_LOCK));
...
static void sdp1501_set_frequency(unsigned int cpu, unsigned int old_index,
				unsigned int new_index, unsigned int mux)
{
	...
	if (sdp_cpufreq_print_on)
		printk("@$%u\n", sdp1501_freq_table[new_index].frequency/10000);


	spin_lock_irqsave(&freq_lock, flags);

	/* change cpu frequnecy */
	sdp1501_set_clkdiv(old_index, new_index);

	spin_unlock_irqrestore(&freq_lock, flags);
}

int sdp1501_cpufreq_init(struct sdp_dvfs_info *info)
{
	u32 val;
	...
	info->volt_table = sdp1501_volt_table;
	info->freq_table = sdp1501_freq_table;
	info->set_freq = sdp1501_set_frequency;
	info->update_volt_table = update_volt_table;
	info->get_speed = sdp1501_get_speed;
	
	return 0;	
}
EXPORT_SYMBOL(sdp1501_cpufreq_init);
