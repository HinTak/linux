...
static void sdp1307_set_frequency(unsigned int old_index, unsigned int new_index)
{
	unsigned long flags;
...
static unsigned int sdp1307_get_speed(void)
{
	unsigned int ret;
	unsigned int val;
...
static void set_volt_table(void)
{
	...
	freq = sdp1307_get_speed() / 1000000;
	...
	if (i < CPUFREQ_LEVEL_END) {
		...
		pr_info("DVFS: current CPU clk = %dMHz, max support freq is %dMHz",
				freq, sdp1307_freq_table[i].frequency/1000);
	} else {
		max_support_idx = L2;
		pr_info("DVFS: current CPU clk = %dMHz, max support freq is %dMHz",
	...
	if (sdp_result_of_asv < CPUFREQ_ASV_COUNT) { 
		for (i = 0; i < CPUFREQ_LEVEL_END; i++) {
			sdp1307_volt_table[i] = 
				sdp1307_asv_voltage[i][sdp_result_of_asv];
		}
	} else {
