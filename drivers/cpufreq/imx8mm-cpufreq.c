...
static int imx8mm_set_target(struct cpufreq_policy *policy, unsigned int index)
{
...
static void imx8mm_cpufreq_ready(struct cpufreq_policy *policy)
{
...
static int imx8mm_cpufreq_init(struct cpufreq_policy *policy)
{
	int ret;
	...
	if (ret) {
		dev_err(cpu_dev, "imx8mm cpufreq init failed!\n");
		return ret;
	}
...
static struct cpufreq_driver imx8mm_cpufreq_driver = {
	.flags = CPUFREQ_NEED_INITIAL_FREQ_CHECK,
	.verify = cpufreq_generic_frequency_table_verify,
	.target_index = imx8mm_set_target,
	.get = cpufreq_generic_get,
	.init = imx8mm_cpufreq_init,
	.name = "imx8mm-cpufreq",
	.ready = imx8mm_cpufreq_ready,
...
static int imx8mm_cpufreq_probe(struct platform_device *pdev)
{
	...
	ret = cpufreq_register_driver(&imx8mm_cpufreq_driver);
	if (ret) {
	...
	of_node_put(np);
	dev_info(cpu_dev, "registered imx8mm-cpufreq\n");
...
static int imx8mm_cpufreq_remove(struct platform_device *pdev)
{
	cpufreq_cooling_unregister(cdev);
	cpufreq_unregister_driver(&imx8mm_cpufreq_driver);
	dev_pm_opp_free_cpufreq_table(cpu_dev, &freq_table);
	if (free_opp)
		of_free_opp_table(cpu_dev);
	clk_put(a53_clk);
...
static struct platform_driver imx8mm_cpufreq_platdrv = {
	.driver = {
		.name	= "imx8mm-cpufreq",
	},
	.probe		= imx8mm_cpufreq_probe,
	.remove		= imx8mm_cpufreq_remove,
};
module_platform_driver(imx8mm_cpufreq_platdrv);

MODULE_AUTHOR("Anson Huang <Anson.Huang@nxp.com>");
MODULE_DESCRIPTION("Freescale i.MX8MM cpufreq driver");
MODULE_LICENSE("GPL");
