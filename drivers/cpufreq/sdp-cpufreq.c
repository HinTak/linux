...
static bool sdp_cpufreq_volt_on = false;
...
static inline bool is_regulator_detected(void)
{
	return ((cpu_regulator) && (!IS_ERR(cpu_regulator)));
}
				...
				unsigned int target_freq)
{
	...
	if (old_index == CPUFREQ_ENTRY_INVALID) {
		ret = old_index;
		printk("%s: error old_index=%x\n", __FUNCTION__, old_index);
		goto out;
	}

	new_index = sdp_cpufreq_get_index(target_freq);
	if (new_index == CPUFREQ_ENTRY_INVALID) {
		ret = new_index;
		printk("%s: error new_index=%x\n", __FUNCTION__, new_index);
		goto out;
	}
	...
	if (freqs.new > freqs.old && is_regulator_detected() && sdp_cpufreq_volt_on) {
...
int sdp_cpufreq_freq_fix(unsigned int freq, bool on)
{
...
{
	if (nId >= DVFS_LOCK_ID_END)
		return -EPERM;
	
	return (int)sdp_cpufreq_limit_level[nId];
}
EXPORT_SYMBOL(sdp_cpufreq_limit_get);
...
static ssize_t sdp_cpufreq_volt_on_show(struct kobject *kobj,
				struct attribute *attr, char *buf)
{
	ssize_t ret;
...
static ssize_t sdp_cpufreq_volt_on_store(struct kobject *a, struct attribute *b,
				const char *buf, size_t count)
{
	unsigned int on;
	int ret;
	...
	if (on == 1)
		sdp_cpufreq_volt_on = true;
	else
		sdp_cpufreq_volt_on = false;

	if (is_regulator_detected() && sdp_cpufreq_volt_on == false)
		regulator_set_voltage(cpu_regulator, sdp_info->volt_table[0], sdp_info->volt_table[0]);

	mutex_unlock(&cpufreq_lock);
			
	printk(KERN_DEBUG "cpufreq volt on = %u\n", on);

	return (ssize_t)count;
}
static struct global_attr volt_on = __ATTR(volt_on, 0644, sdp_cpufreq_volt_on_show, sdp_cpufreq_volt_on_store);
...
static int sdp_cpufreq_dev_register(void)
{
	...
	err = sysfs_create_file(cpufreq_global_kobject, &volt_on.attr);
	if (err < 0)
		goto out;
...
#if defined(CONFIG_ARM_SDP1803_CPUFREQ)
int sdp1803_cpufreq_tmu_notifier(struct notifier_block *notifier, unsigned long event, void *v);
#endif

static int get_level_ceiling(unsigned int freq, unsigned int *level)
{
	struct cpufreq_frequency_table *table;
	unsigned int i;
	...
	for (i = sdp_info->max_real_idx;
		...
		else if (table[i].frequency < freq && i > sdp_info->max_real_idx) {
			*level = i - 1;
			return 0;
		}
	}
			  	     ...
			  	     unsigned long event, void *v)
{
	...
	case SDP_TMU_FREQ_LIMIT:
		...
		ret = get_level_ceiling(param->cpu_limit_freq[0], &level);
		if (ret < 0) {
...
#if defined(CONFIG_ARM_SDP1803_CPUFREQ)
	sdp1803_cpufreq_tmu_notifier(notifier, event, v);
#endif
...
static int sdp_cpufreq_cpu_init(struct cpufreq_policy *policy)
{
	...
	policy->freq_table = sdp_info->freq_table;
...
#elif defined(CONFIG_ARM_SDP1501_CPUFREQ)
extern int sdp1501_cpufreq_init(struct sdp_dvfs_info *info);
#elif defined(CONFIG_ARM_SDP1601_CPUFREQ)
extern int sdp1601_cpufreq_init(struct sdp_dvfs_info *info);
#elif defined(CONFIG_ARM_SDP1803_CPUFREQ)
extern int sdp1803_cpufreq_init(struct sdp_dvfs_info *info);
#elif defined(CONFIG_ARM_SDP_NIKE_CPUFREQ)
extern int sdp_nike_cpufreq_init(struct sdp_dvfs_info *info);
#endif
...
static int __init sdp_cpufreq_init(void)
{
...
#elif defined(CONFIG_ARM_SDP1501_CPUFREQ)
	ret = sdp1501_cpufreq_init(sdp_info);
#elif defined(CONFIG_ARM_SDP1601_CPUFREQ)
	ret = sdp1601_cpufreq_init(sdp_info);
#elif defined(CONFIG_ARM_SDP1803_CPUFREQ)
	ret = sdp1803_cpufreq_init(sdp_info);
#elif defined(CONFIG_ARM_SDP_NIKE_CPUFREQ)
	ret = sdp_nike_cpufreq_init(sdp_info);
#endif
	if (ret)
		goto err_vdd_arm;
	...
	/* get cpu regulator */
	cpu_regulator = regulator_get_optional(NULL, "CPU_PW");

	register_pm_notifier(&sdp_cpufreq_pm_nb);

#ifdef CONFIG_SDP_AVS
	sdp_info->cur_group = sdp_avs_get_step("cpu_avs");
#endif

	if (sdp_info->update_volt_table) {
		sdp_info->update_volt_table(sdp_info->cur_group);
	}
