...
static unsigned int sdp1304_get_speed(void)
{
	unsigned int ret;
	unsigned int pms;
	...
	if (sdp_result_of_asv < CPUFREQ_ASV_COUNT) { 
		for (i = 0; i < CPUFREQ_LEVEL_END; i++) {
			if (board_table_sel >= CPUFREQ_BOARD_SBB &&
				...
				sdp1304_volt_table[i] = sdp1304_asv_voltage_sbb[i][sdp_result_of_asv];
			else
				sdp1304_volt_table[i] = sdp1304_asv_voltage_maintv[i][sdp_result_of_asv];
		}
	} else {
...
static void sdp1304_set_clkdiv(unsigned int old_index, unsigned int new_index)
{
	...
	if (old_index > new_index) {
		 ...
		 */
		if (new_index <= L5) /* target freq is higher than or equl to 1GHz */
	...
	/* freq down direction */
		 ...
		 */
		tmp_div = sdp1304_freq_table[old_index].frequency / CPUFREQ_TEMP_FREQ;
	...
	val |= SEL_FTEST_AMS | (tmp_div << SEL_AMSHALF); /* AMS clk div */
...
static void sdp1304_set_frequency(unsigned int old_index, unsigned int new_index)
{
	unsigned long flags;
...
early_param("jackpackTV", sdp1304_cpufreq_board_jack);
