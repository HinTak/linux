...
#define MMC_STRENGTH	"mmc_strength"
...
/* Descrtiptor List Base address */
	...
	int	successed_rdqs;			//save rdqs value
...
typedef struct pad_drive_strength {
	u32 strength_reg;
	u32 strength_mask;
	u32 strength_value;
} s_mmc_strength;
...
//	int (*get_bus_wd)(u32 slot_id);
	...
	int numStrengthProperty;
	s_mmc_strength *strength;
};
...
int sdp_read_clkrst_mux(u32 phy_addr);
...
struct board_settings_head {
	u64 chipid;
	u32 checksum;
	u32 runtime_flags;
};
struct board_settings_museo_ddr {
	u32 soc_vref[4];
	u32 mr14[4];		/* bit [7:0] */
};
struct board_settings_museo {
	struct board_settings_head	head;
	struct board_settings_museo_ddr museo_ddr;
};

#define USER_PARTITION	0
#define BOOT_PARTITION1 1
#define BOOT_PARTITION2 2
#define RPMB_PARTITION2 3

#if 0
#define PLL_CLK_COUNTER_BASE	0x00F50CB0
static void *v_pll_clk_cnt_base;
#endif
