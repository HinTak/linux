...
static int sdp_mmch_find_boot_part_size(struct mmc_card *card)
{
	int i, size;

	for (i = 0; i < MMC_NUM_PHY_PARTITION; i++) {
		if (card->part[i].area_type == MMC_BLK_DATA_AREA_BOOT) {
			size = card->part[i].size;
			return size;
		}
	}
...
static int sdp_mmch_dbg_write_boot1_user_pattern(void *data, u64 val)
{
	...
	u32 status = 0;
	u32 error_flag = 0;
	u32 boot_part_size = 0;
	u32 sector_size = card->ext_csd.data_sector_size;
	...
	boot_part_size = sdp_mmch_find_boot_part_size(card);
	if (boot_part_size == 0) {
		dev_err(&sdpmmch->pdev->dev, "%s boot part size searching fail!\n", __FUNCTION__);
		return -1;
	}
	...
	for(i = 0; i < 1; i++) {
		...
		// arg -> 32 bit sector (512 B) address (SPEC.)
		cmd.opcode = MMC_WRITE_BLOCK;
		cmd.arg = boot_part_size / sector_size - 1;	// Last 512bytes of boot partition
...
DEFINE_SIMPLE_ATTRIBUTE(sdp_mmch_dbg_write_boot1_user_pattern_fops, NULL, sdp_mmch_dbg_write_boot1_user_pattern,
	"0x%08llx\n");

static int sdp_mmch_dbg_write_boot1_fixed_pattern
	(struct mmc_host *host, struct mmc_card *card, struct seq_file *s)
{
	SDP_MMCH_T *sdpmmch = mmc_priv(host);
	...
	dev_info(mmc_dev(host), "info write boot partition 0x%X\n", val);
...
static int mmc_write_boot1_show(struct seq_file *s, void *data)
{
	struct mmc_host *host = s->private;
	SDP_MMCH_T *sdpmmch = mmc_priv(host);
	...
	sdp_mmch_dbg_write_boot1_fixed_pattern(host, host->card, s);
...
static int mmc_write_boot1_open(struct inode *inode, struct file *file)
{
	return single_open(file, mmc_write_boot1_show, inode->i_private);
}

static const struct file_operations sdp_mmch_dbg_write_boot1_fixed_pattern_fops = {
	.open		= mmc_write_boot1_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static int sdp_mmch_dbg_read_boot1_pattern(void *data, u64 *val)
{
	struct mmc_host *host = data;
	struct mmc_card *card = host->card;
	SDP_MMCH_T *sdpmmch = mmc_priv(host);
...
DEFINE_SIMPLE_ATTRIBUTE(sdp_mmch_dbg_read_boot1_pattern_fops, sdp_mmch_dbg_read_boot1_pattern, NULL,
	"0x%08llx\n");

int get_shift_bit(s_mmc_strength *s_strength){

	u32 shift_bit = 0;
	u32 i = 0;
	while(!(s_strength->strength_mask >> shift_bit & 0x1)){
		shift_bit++;
	}
...
static int mmch_ew_dbg_strength_get(void *data, u64 *val)
{
	...
	*val = (mmch_readl(addr)&0x70000000UL)>>28;
	iounmap(addr);
#elif defined(CONFIG_ARCH_SDP1803) || defined(CONFIG_ARCH_SDP_NIKE)
	for (i = 0; i < platdata->numStrengthProperty; i++) {
		shift_bit = get_shift_bit(&platdata->strength[i]);
		*val = (sdp_read_clkrst_mux(platdata->strength[i].strength_reg) & platdata->strength[i].strength_mask) >> shift_bit;
...
static int mmch_ew_dbg_strength_set(void *data, u64 val)
{
	...
	u32 temp;
	u32 shift_bit = 0;
	u32 *strength_value = (u32 *)kmalloc(sizeof(platdata->strength->strength_value) * platdata->numStrengthProperty, GFP_KERNEL);

#if defined(CONFIG_ARCH_SDP1404) || defined(CONFIG_ARCH_SDP1501)
	if(val >= 0 && val <= 3) {
		strength_value[0] = (u32)(val << 28);
	} else {
	...
	if(val >= 0 && val <= 5) {
		...
		for (i = 0; i < platdata->numStrengthProperty; i++) {
			shift_bit = get_shift_bit(&platdata->strength[i]);
			strength_value[i] = (u32)(val << shift_bit);
		}
#endif
	}
	else {
	...
	temp &= ~0x30000000;
	mmch_writel(temp|strength_value[0], addr);
...
#elif defined(CONFIG_ARCH_SDP1803) || defined(CONFIG_ARCH_SDP_NIKE)
	for (i = 0; i < platdata->numStrengthProperty; i++) {
		sdp_set_clkrst_mux(platdata->strength[i].strength_reg, platdata->strength[i].strength_mask, strength_value[i]);
		dev_info(&sdpmmch->pdev->dev, "write strength 0x%08X\n", strength_value[i]);
	}
#endif

	kfree(strength_value);
...
{
	int numStrengthMember, val, buswidth = 0;
	const char *clkname;
	...
	numStrengthMember = sizeof(s_mmc_strength) / sizeof(plat->strength->strength_reg);

	plat->numStrengthProperty = of_property_count_u32_elems(dev->of_node, MMC_STRENGTH) / numStrengthMember;
	if (plat->numStrengthProperty < 0){
		dev_info(dev, "strength property not found, please check dts file\n");
		plat->numStrengthProperty = 1;
	}

	plat->strength = (s_mmc_strength *)kmalloc(sizeof(s_mmc_strength) * plat->numStrengthProperty, GFP_KERNEL);
	if(of_property_read_u32_array(dev->of_node, MMC_STRENGTH, &plat->strength[0].strength_reg, plat->numStrengthProperty * numStrengthMember))
	{
		dev_info(dev, "strength property not found, please check dts file\n");
	}
#endif
...
int mmc_set_card_wp(struct mmc_host *host, int wp)
{
	u8 csd_be128[16];
	int i, ret;
	struct mmc_card *card = host->card;
	...
	//csd_be128[15] = crc7_be(0, csd_be128, 15) | 0x1;
	...
	if (!ret) {
		...
		if (wp) {
			...
			card->csd.temp_wp = 1;
			dev_err(&sdpmmch->pdev->dev, "updated  temp_wp[%d]\n", card->csd.temp_wp);
		} 
		else {
			card->raw_csd[3] &= ~BIT(12);
			card->csd.temp_wp = 0;
		...
		dev_err(&sdpmmch->pdev->dev, "update was failed raw_csd[%d]: 0x%08x\n", 3, card->raw_csd[3]);
	}

	return ret;
}

static int mmc_temp_wp_set(void *data, u64 val){
	struct mmc_host *host = data;
	SDP_MMCH_T *sdpmmch = mmc_priv(host);
	...
	if(val == 0){
		mmc_set_card_wp(host, 0);
		dev_err(&sdpmmch->pdev->dev, "mmc_temp_wp_set 0[%lld]\n", val);
	}
	else{
		mmc_set_card_wp(host, 1);
		dev_err(&sdpmmch->pdev->dev, "mmc_temp_wp_set 1[%lld]\n", val);
	}
	
	return 0;
}

DEFINE_SIMPLE_ATTRIBUTE(mmc_temp_wp_fops, NULL, mmc_temp_wp_set, "0x%08llx\n");


static int mmc_csd_dump_get(void *data, u64 *val){
	struct mmc_host *host = data;
	struct mmc_card *card = host->card;
	SDP_MMCH_T *sdpmmch = mmc_priv(host);
	...
	for(i=0; i<4; i++){
		snprintf(type, 20, "before raw_csd[%d]: 0x%08x\n", i, card->raw_csd[i]);
		dev_err(&sdpmmch->pdev->dev, "before raw_csd[%d]: 0x%08x\n", i, card->raw_csd[i]);
	}
/*
	mmc_claim_host(host);
	
	ret = mmc_send_csd(card, card->raw_csd);
	if(ret)
		return ret;
	...
	for(i=0; i<4; i++){
		snprintf(type, 20, "raw_csd[%d]: 0x%08x\n", i, card->raw_csd[i]);
		dev_err(&sdpmmch->pdev->dev, "raw_csd[%d]: 0x%08x\n", i, card->raw_csd[i]);
	}
*/
	return 0;
}

DEFINE_SIMPLE_ATTRIBUTE(mmc_csd_dump_fops, mmc_csd_dump_get, NULL, "0x%08llx\n");
...
static void mmch_add_host_debugfs(struct mmc_host *host)
{
	...
	if (!debugfs_create_file("error", S_IRUSR, root, host, &mmc_error_fops))
 		return;
	//For ASB Board Test
	if (!debugfs_create_file("writeboot", S_IRUSR, root, host, &sdp_mmch_dbg_write_boot1_fixed_pattern_fops))
	{
		dev_err(&sdpmmch->pdev->dev, "failed to create writeboot file in debugfs.\n");
		return;
	}
	if (!debugfs_create_file("writepattern", S_IRUSR, root, host, &sdp_mmch_dbg_write_boot1_user_pattern_fops))
	{
		dev_err(&sdpmmch->pdev->dev, "failed to create writepattern file in debugfs.\n");
		return;
	}
	if (!debugfs_create_file("readpattern", S_IRUSR, root, host, &sdp_mmch_dbg_read_boot1_pattern_fops))
	{
	...
	if (!debugfs_create_file("csd_dump", S_IRUSR, root, host, &mmc_csd_dump_fops))
	{
		dev_err(&sdpmmch->pdev->dev, "failed to create read file in debugfs.\n");
		return;
	}

	if (!debugfs_create_file("temp_wp", S_IRUGO|S_IWUGO, root, host, &mmc_temp_wp_fops))
	{
		dev_err(&sdpmmch->pdev->dev, "failed to create read file in debugfs.\n");
		return;
	}
...
#if defined(CONFIG_ARCH_SDP1803) || defined(CONFIG_ARCH_SDP_NIKE)
	for (i = 0; i < platdata->numStrengthProperty; i++)
		sdp_set_clkrst_mux(platdata->strength[i].strength_reg, platdata->strength[i].strength_mask, platdata->strength[i].strength_value);
#endif
...
#if defined(CONFIG_ARCH_SDP1803) || defined(CONFIG_ARCH_SDP_NIKE)
	for (i = 0; i < plat->numStrengthProperty; i++)
		sdp_set_clkrst_mux(plat->strength[i].strength_reg, plat->strength[i].strength_mask, plat->strength[i].strength_value);
#endif
