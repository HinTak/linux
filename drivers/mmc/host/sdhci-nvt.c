...
static int nvt_mmc_init(struct nvt_mmc_hwplat *hwplat)
{
	...
	if (!ioremap_done) {
		...
		hwplat->adll_bg = ioremap_nocache(NVT_ADLL_BIAS, 4);

		if ((!hwplat->adll_bg)) {
			info_log("adll_bg(%p) remap failed", hwplat->adll_bg);
			return -1;
		}
#endif
	...
	SYS_CLK_SetMpll(CLK_PAGE_B, EN_MPLL_OF_EMMC, 208000>>2);
	udelay(1);
	...
	SYS_CLK_SetMpll(CLK_PAGE_B, EN_MPLL_OF_EMMC, (hwplat->src_clk/1000)>>2);
	udelay(1);
...
static void nvt_mmc_set_adll_init(void __iomem *fcr)
{
	...
	reg_val = ADLL_CFG_SET_ADLL_EN(reg_val, 0x1);
	...
	// toggle reset bit
	writel_or_bits(ADLL_CFG_ADS_RESET, fcr+REG_FCR_ADLL_CFG_REG);
	udelay(50);
...
#define KHZ (1000)
			...
			this circuit is run x4 clk that others*/
				SYS_CLK_SetMpll(CLK_PAGE_B, EN_MPLL_OF_EMMC,
					(FASTEST_CLK_ENABLE_THRESHOLD/KHZ));
				udelay(1);
			}
		...
		/* turn on ADLL BAIS for EMMC */
		writel((readl(hwplat->adll_bg) | (NVT_ADLL_BIAS_EMMC_PDN)),
				hwplat->adll_bg);
...
static int mmc_execute_hs400_do_rdqs_tuning(struct mmc_card *card, struct seq_file *s)
{
	...
	pr_info("RDQS Tuning start! range %d to %d, inc %d, current rdqs %lld\n",
	...
	if (median < 0) {
		...
		rdqs_err("RDQS Tuning fail! error %d\n", median);
		ret = median;
	} else {
		...
		if (s != NULL)
			seq_printf(s, "%d / %s / %d\n", MAX_RDQS_COUNT, str_rdqs, median);
		ret = median;
	}
	return ret;
...
static int nvt_mmc_rdqs_tuning_set(void *data, u64 val)
{
	...
	if (val > (RDQS_MASK >> RDQS_BIT_SHIFT)) {
		...
		struct mmc_card *card)
{
	...
	ret = mmc_execute_hs400_do_rdqs_tuning(card, NULL);
...
static int mmc_driver_strength_get(void *data, u64 *val)
{
	u32 reg_val = 0;
...
static int mmc_driver_strength_set(void *data, u64 val)
{
	int i;
...
DEFINE_SIMPLE_ATTRIBUTE(mmc_driver_strength_fops,
	mmc_driver_strength_get, mmc_driver_strength_set, "%llu\n");
...
static int mmc_delaystep_show(struct seq_file *s, void *data)
{
	struct mmc_host *host = s->private;
	/*Tref=1/180MHz=5.556ns ,1 step =3.125%*5.556ns~173.6ps*/
	if (((_hwplat->src_clk/KHZ)>>2) == 180000)
		seq_printf(s, "173.6 \nps/step\n");
	else
		seq_printf(s, "check the emmc clk!\n");
	return 0;
}
static int mmc_delaystep_open(struct inode *inode, struct file *file)
{
	return single_open(file, mmc_delaystep_show, inode->i_private);
}
static const struct file_operations mmc_delaystep_fops = {
	.open		= mmc_delaystep_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	...
	for (i = 0; i < 1; i++) {
		...
		if (!cmd.error) {
			do {
				...
				if (ret) {
					pr_err("tuning %2d send status fail!(%d)", i, ret);
					break;
				}
				udelay(10);
...
static int mmc_rdqs_show(struct seq_file *s, void *data)
{
	...
	mmc_execute_hs400_do_rdqs_tuning(card, s);
...
static const struct file_operations mmc_rdqs_tune_fops = {
	.open		= mmc_rdqs_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
...
#ifdef MMC_HS400_IO_DRIVING
	if (!debugfs_create_file("rdqs", S_IRUSR | S_IWUSR, host->debugfs_root,
			host, &mmc_rdqs_fops))
		goto err;

#ifdef CONFIG_MMC_SDHCI_NVT_HS400_RDQS_TUNING
	if (!debugfs_create_file("rdqs_tune", S_IRUSR, host->debugfs_root, host,
			&mmc_rdqs_tune_fops))
		goto err;
#endif

	if (!debugfs_create_file("delaystep", S_IRUSR, host->debugfs_root, host,
			&mmc_delaystep_fops))
		goto err;

	if (!debugfs_create_file("clk_phase", S_IRUSR | S_IWUSR,
			host->debugfs_root, host, &mmc_write_phse_fops))
		goto err;
#endif

	if (!debugfs_create_file("strength", S_IRUSR | S_IWUSR,
			host->debugfs_root, host, &mmc_driver_strength_fops))
		goto err;

	if (!debugfs_create_file("readpattern", S_IRUSR, host->debugfs_root,
			host, &mmc_read_pattern_fops))
		goto err;

	if (!debugfs_create_file("writepattern", S_IRUSR | S_IWUSR,
			host->debugfs_root, host, &mmc_write_pattern_fops))
		goto err;

	if (!debugfs_create_file("error", S_IRUSR, host->debugfs_root, host,
...
err:
	dev_err(&host->class_dev, "host debugfs is not initialized.\n");

}
#endif
#endif

static void sdhci_nvt_hw_reset(struct sdhci_host *host)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct nvt_mmc_hwplat *hwplat = pltfm_host->priv;
	...
	if (host->mmc->card){
		...
		if (retry_cnt > 5) {
			pr_err("*** %s retry over 5 times, please check environment!!!***\n", __func__);
			BUG_ON(1);
		}

		if (nvt_mmc_init(hwplat) != 0)
			pr_err("*** nvt mmc hw reset failed\n");
		...
		host->pwr = 0;  //re-setting power
		host->clock = 0; //re-setting clk

		retry_cnt++;
	}
	else{
		pr_info("Ignore hw reset when card not ready!\n");
	}
			...
			struct platform_device *plt_dev)
{
	...
	if (node) {
		...
		if (of_get_property(node, "full-pwr-cycle", NULL))
			host->mmc->caps2 |= MMC_CAP2_FULL_PWR_CYCLE;

		if (of_get_property(node, "packed-cmd", NULL))
			host->mmc->caps2 |= MMC_CAP2_PACKED_CMD;

		if (of_get_property(node, "wait-while-busy", NULL))
			host->mmc->caps |= MMC_CAP_WAIT_WHILE_BUSY;

		if (of_get_property(node, "hw-reset", NULL))
			host->mmc->caps |= MMC_CAP_HW_RESET;

		if (of_get_property(node, "no-prescan-powerup", NULL))
			host->mmc->caps2 |= MMC_CAP2_NO_PRESCAN_POWERUP;

		if (!of_property_read_u32(dev->of_node, "irq-affinity",
			&affinity))
			if (num_online_cpus() > affinity) {
				irq_set_affinity(host->irq,
				cpumask_of(affinity));
			}
#ifdef CONFIG_MMC_SDHCI_NVT_HS400
...
#ifdef CONFIG_NVT_MMC_CLKGATE
	nvt_mmc_clk_ungating(nvt_host, mrq->cmd);
#endif
...
static int __init nvt_mmc_host_init(void)
{
	pr_info("registered NVT MMC Host driver.\n");
	return platform_driver_register(&sdhci_nvt_driver);
}
static void __exit nvt_mmc_host_exit(void)
{
	platform_driver_unregister(&sdhci_nvt_driver);
}
subsys_initcall(nvt_mmc_host_init);
module_exit(nvt_mmc_host_exit);
