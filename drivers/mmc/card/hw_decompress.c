...
static inline enum hw_iovec_hash_type convert_hw_hashtype(enum req_hash_type type)
{
	switch (type) {
	case REQ_HASH_TYPE_UNDEF:
		return HW_IOVEC_HASH_NONE;
	case REQ_HASH_TYPE_SHA1:
		return HW_IOVEC_HASH_SHA1;
	case REQ_HASH_TYPE_SHA256:
		return HW_IOVEC_HASH_SHA256;
	case REQ_HASH_TYPE_MD5:
...
int calculate_hash(unsigned long vstart,
		unsigned long pstart, unsigned long length,
		 struct req_hash *rq_hash, int *sw_hash_handled)
{
	int retval = 0;
	...
	if (!DEC_FORCE_TO_USE_SW_HASH && 
			hw.hash_type == convert_hw_hashtype(rq_hash->hash_type)) {
		/*
...
static inline int do_decompression(struct hw_req *hw_req, int req_cnt)
{
	...
	if (hw_req->rq_hash) {
		...
		if (calculate_hash(vstart, pstart,
					length, hw_req->rq_hash, &sw_hash_handled))
		{
...
unzip_wait:
	...
	if (sw_hash_handled || (hw_req->err && hw_req->preinited) || !hw_req->rq_hash)
		unzip_decompress_wait(hw_req->buff, hw_req->out_cnt,
				hw_req->out_pages, NULL);
	else
		unzip_decompress_wait(hw_req->buff, hw_req->out_cnt,
				hw_req->out_pages, hw_req->rq_hash->data);
out:
...
err:
	...
	hw_req->comp_type = comp_type;
	hw_req->buff->size =  hw_req->compr_len; //TOCHECK !!!!!!!!!!
	hw_req->rq_hash = rq_hash;
	/* Decompress */
