...
/*
	DEC_USE_CACHED_UNZIP_BUFF : value 1 -> Using cached memory for un
		zip buffer
	DEC_FORCE_TO_USE_SW_HASH  : value 1 -> Force to use S/W hash.
 */

#ifdef CONFIG_NVT_UNZIP
#define DEC_USE_CACHED_UNZIP_BUFF 0
#define DEC_FORCE_TO_USE_SW_HASH  0
#endif

#ifdef CONFIG_SDP_UNZIP
#define DEC_USE_CACHED_UNZIP_BUFF 1
#define DEC_FORCE_TO_USE_SW_HASH  0
#endif
...
int arg_to_hw_req_fast(void *arg)
{
	struct hw_req *hw_req = (struct hw_req*)arg;
...
static int buf_to_sglist(void *vaddr, unsigned int length, struct scatterlist *sg, int max_nents)
{
	...
	if(nents == 1) {
		...
		sg_set_buf(sg, base + offset, PAGE_SIZE - offset);
		for(i = 1; i < nents-1; i++) {
			sg_set_buf(sg + i, base + (PAGE_SIZE * i), PAGE_SIZE);
		}
		sg_set_buf(sg + i, base + (PAGE_SIZE * i),
		...
		unsigned int len, unsigned long long phys_off)
{
	...
	req->bio->bi_iter.bi_sector = phys_off >> 9;
...
static atomic_t unzip_buff = ATOMIC_INIT(0);
#endif
		...
		unsigned int out_cnt)
{
	...
	hw_req->buff = unzip_alloc(HW_MAX_IBUFF_SZ);
...
#ifdef CONFIG_NVT_UNZIP_FIFO
	atomic_inc(&unzip_buff);
#endif

	hw_req->reqs = kmalloc(vec_cnt * sizeof(struct request *), GFP_NOWAIT);
	if (!hw_req->reqs) {
		unzip_free(hw_req->buff);
#ifdef CONFIG_NVT_UNZIP_FIFO
		atomic_dec(&unzip_buff);
#endif
		kfree(hw_req);
		return ERR_PTR(-ENOMEM);
	}
...
static void rq_end_io(struct request *rq, int err)
{
	...
	if (atomic_dec_and_test(&hw_req->req_cnt)) {
		...
		if (hw_req->preinited) {
			BUG_ON(!hw_req->fast);
			unzip_update_endpointer(hw_req->private);
		} 
...
int calculate_hash(unsigned long vstart,
		unsigned long pstart, unsigned long length,
		 struct req_hw *rq_hw, int *sw_hash_handled)
{
	int retval = 0;
	...
	if (!DEC_FORCE_TO_USE_SW_HASH && 
			hw.hash_type == rq_hw->hash_type) {
		/*
		   - We don't need to care cache. It's H/W Hash!
		   - H/W hash IP provide only 1 hash_type, so we don't call
			 hash function with specific hash type.
		 */
		calculate_hw_hash(vstart, pstart, length);

		return retval;
	}

	/*
		If using cached unzip buffer, you need to invalidate cache
		before calculating hash with the buffer.
	 */
	if (DEC_USE_CACHED_UNZIP_BUFF) {
#if defined(CONFIG_ARM64)
		// ToDo : How to implement??
#elif defined(CONFIG_ARM)
		outer_inv_range(pstart, pstart + length);
		dmac_unmap_area((void *)vstart, length, DMA_FROM_DEVICE);
#endif
	}

	switch (rq_hw->hash_type) {
	case HW_IOVEC_HASH_MD5:
		retval = calculate_sw_hash_md5((unsigned char *)vstart,
				length,	rq_hw->hashdata);
		break;
	case HW_IOVEC_HASH_SHA1:
		retval = calculate_sw_hash_sha1((unsigned char *)vstart,
				length, rq_hw->hashdata);
		break;
	case HW_IOVEC_HASH_SHA256:
		retval = calculate_sw_hash_sha256((unsigned char *)vstart,
				length, rq_hw->hashdata);
		break;
	default:
		retval = -EINVAL;
	...
	/* set hash is handled by S/W (Do not wait H/W hash completion)*/
	*sw_hash_handled = 1;
...
repeat:
	...
	/* Setup decompressor */
	ret = unzip_decompress_async(
			hw_req->sg, hw_req->compr_len,
			hw_req->out_pages, hw_req->out_cnt,
			&hw_req->private, unzip_done,
			hw_req, may_wait,
			hw_req->comp_type, hw_req->rq_hw);
...
static inline int do_decompression(struct hw_req *hw_req, int req_cnt)
{
	...
	if (hw_req->fast){
		...
		hw_req->result = unzip_decompress_async(
				hw_req->sg, hw_req->compr_len,
				hw_req->out_pages, hw_req->out_cnt,
				&hw_req->private, unzip_done,
				hw_req, true,
	...
	if (hw_req->rq_hw && hw_req->rq_hw->hash_type != HW_IOVEC_HASH_NONE) {
		unsigned long vstart = (unsigned long)hw_req->buff->vaddr +
								hw_req->off;
		unsigned long pstart = (unsigned long)hw_req->buff->paddr +
								hw_req->off;
		unsigned long length = hw_req->compr_len;

		kref_get(&hw_req->kref);

		/* Place hash function here for parallel hash working */
		if (calculate_hash(vstart, pstart,
					length, hw_req->rq_hw, &sw_hash_handled))
		{
			/* H/W hash always return true */
			pr_err("S/W Hash generation error\n");
			hw_req_put(hw_req);
			hw_req->result = -EINVAL;
			goto out;
		}
...
unzip_wait:
	...
	unzip_decompress_wait(hw_req->private);
out:
 ...
 * @rq_hw:    hash type and hash pointer(allocated from upper layer)
...
err:
	...
	buf_to_sglist(hw_req->buff->vaddr + hw_req->off, hw_req->compr_len,
			hw_req->sg, HW_MAX_IBUFF_SG_LEN);

	hw_req->comp_type = rq_hw->compr_type;
	hw_req->buff->size =  hw_req->compr_len; //TOCHECK !!!!!!!!!!
	hw_req->rq_hw = rq_hw;
	/* Decompress */
