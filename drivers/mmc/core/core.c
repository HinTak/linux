...
static void end_special_req(struct mmc_request *mrq)
{
	...
	/* For special request on UNZIP FIFO read */
	if (req->special) {
		unsigned long stop_time = jiffies + (3 * HZ);

		/* Wait for unzip finish */
		if (atomic_read(&unzip_busy) > 1) {
			pr_err("__UNZIP_ERROR__: unzip_busy:%d\n", atomic_read(&unzip_busy));
			pr_err("__UNZIP_ERROR__: unzip_busy:%d\n", atomic_read(&unzip_busy));
			pr_err("__UNZIP_ERROR__: unzip_busy:%d\n", atomic_read(&unzip_busy));
			pr_err("__UNZIP_ERROR__: unzip_busy:%d\n", atomic_read(&unzip_busy));
		}

		while (atomic_read(&unzip_busy) && (!nvt_unzip_dma_done())) {
			if (time_after(jiffies, stop_time)) {
				pr_err("__REQ_TIMEOUT__: %d\n", atomic_read(&unzip_busy));
				nvt_unzip_dump();
				fifo_regdump();
		...
		/* Disable FIFO for next read */
		fifo_atf_done();
	}
}
#endif /* CONFIG_NVT_UNZIP_FIFO */
...
extern unsigned int mmc_get_part_curr(struct mmc_card *card);
static inline void mmc_check_write_gpt_area(struct mmc_host *host,
					    struct mmc_request *mrq)
{
	unsigned int part_curr;

	/* before mmc is attached, card is null */
	if (unlikely(!host->card))
		return;

	/* filter only write commands */
	 ...
	 * part_cur
	 *	0x0 : User Area
	 *	0x1 : EXT_CSD_PART_CONFIG_ACC_BOOT0
	 *	0x3 : EXT_CSD_PART_CONFIG_ACC_RPMB
	 */
	part_curr = mmc_get_part_curr(host->card);
	if (unlikely(part_curr != 0x00))
		return;

	/* check lba address is over GPT Area(64KiB) */
	if (likely(mrq->cmd->arg & ~0x7F))
		return;

	printk(KERN_ERR "[WARN] Protect GPT Area from writing!!\n");
	BUG();
}
#else
static inline void mmc_check_write_gpt_area(struct mmc_host *host,
...
static int mmc_start_request(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	mmc_check_write_gpt_area(host, mrq);	/* FlashFS : protect gpt area */
				  ...
				  struct mmc_request *mrq)
{
	...
	while (1) {
		...
		cmd->error = 0;
		__mmc_start_request(host, mrq);
#endif /* end of CONFIG_MMC_TIMEOUT */
	}
	mmc_retune_release(host);
}
