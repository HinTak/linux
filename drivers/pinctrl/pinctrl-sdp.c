...
static int sdp_gpio_get(struct gpio_chip *chip, unsigned offset)
{
	...
	if (test_bit(offset, &gpio_bank->output))
		...
		data = readl(reg + SDP_GPIO_PRDATA);
	data >>= offset;
	data &= 1;
		...
		{	
			...
			{	
				...
				if(pad_save[i][j+k*3+1]>>8)
				{
					...
					if((sdp_pinctrl[i]->start_offset+k*0xC) == 0x2E0){
						pad_save[i][j+k*3] = pad_save[i][j+k*3]&0x0000FFFF;
						writel(pad_save[i][j+k*3],reg);
					}
					else if((sdp_pinctrl[i]->start_offset+k*0xC) == 0x2EC){
						pad_save[i][j+k*3] = pad_save[i][j+k*3]&0xFFFFFF00;
						writel(pad_save[i][j+k*3],reg);
					}
					else
#endif
