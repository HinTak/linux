...
struct dvb_atsc30dmx {
	...
	struct module* owner;
	int (*start)(struct dvb_atsc30dmx *p);	
	int (*stop)(struct dvb_atsc30dmx *p);
	int (*set_source)(struct dvb_atsc30dmx *p, const dmx_source_t *src);
	...
	int (*atsc30dmx_cb) (struct dvb_atsc30dmx *p, const u8 *buffer1, size_t buffer1_len, const u8 *buffer2, size_t buffer2_len);

	/* for atsc3.0 lmt packet data */
	struct dvb_ringbuffer lmt_buffer;
	int (*atsc30lmt_cb) (struct dvb_atsc30dmx *p, const u8 *buffer1, size_t buffer1_len, const u8 *buffer2, size_t buffer2_len);

	/* for atsc3.0 alp get status */
	int (*get_status) (struct dvb_atsc30dmx *p, struct dmx_status *status);

	/* for atsc3.0 set mPLP mode */	
	int (*set_plp_mode)(struct dvb_atsc30dmx *p, const dmx_plp_mode_t *plp_mode);
