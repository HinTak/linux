...
#define SN1202033_CHNAGE_DELAY	200
...
static int sn1202033_pmic_probe(struct platform_device *pdev)
{
	struct sn1202033_info *info;
	...
	config.dev = &pdev->dev;
	config.init_data = pmic_plat_data->init_data;
	config.driver_data = sn1202033;

	rdev = regulator_register(&sn1202033->desc, &config);
	if (IS_ERR(rdev)) {
		dev_err(&pdev->dev,
...
static int sn1202033_pmic_remove(struct platform_device *pdev)
{
