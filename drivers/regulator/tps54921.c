...
static int tps54921_pmic_probe(struct platform_device *pdev)
{
	...
	rdev = regulator_register(&pmic->desc, &config);
	if (IS_ERR(rdev)) {
		dev_err(&pdev->dev,
...
static int tps54921_pmic_remove(struct platform_device *pdev)
{
