...
static int callback_thread(void *data)
{
    ...
    complete_and_exit(&p_ibiza->ib_thread.thread_complete, retval);
}
...
static int btpcm_ibiza_start(struct btpcm *p_btpcm, int nb_pcm_frames, int nb_pcm_packets, int synchronization)
{
    ...
    init_completion(&p_ibiza->ib_thread.thread_complete);
#ifndef BTPCM_IBIZA_TONE
...
static int btpcm_ibiza_stop(struct btpcm *p_btpcm)
{
    ...
    if (!atomic_read(&p_ibiza->started))
    {
   	...
   	BTPCM_DBG("Queue clear resource:0x%p\n", p_btpcm);
...
static void btpcm_clear_resource(struct work_struct *work)
{
	...
	/* Wait for thread to complete befrore we exit*/
	wait_for_completion(&p_ibiza->ib_thread.thread_complete);
...
static int btpcm_ibiza_pcm_available(struct btpcm_ibiza *p_ibiza)
{
    ...
    pcm_buf_size = p_ibiza->ibiza_pcm_buf_size;//added by Moon

	if (left_write_offset > p_ibiza->left_read_offset)
	{
		BTPCM_DBG("1 returns=%d\n", (left_write_offset - p_ibiza->left_read_offset) / BTPCM_HW_PCM_SAMPLE_SIZE);
        if((left_write_offset - p_ibiza->left_read_offset) > (pcm_buf_size/4))//added by Moon
        {
           BTPCM_ERR("###/4###########1#PCM DROP(%d)\n", left_write_offset - p_ibiza->left_read_offset);
           p_ibiza->left_read_offset = left_write_offset;
           return 0;
        }
	...
	else
	{
        ...
        if((left_write_offset - p_ibiza->left_read_offset + pcm_buf_size) > (pcm_buf_size/4))//added by Moon
        {
            BTPCM_ERR("###/4###########2#PCM DROP(%d)\n", left_write_offset - p_ibiza->left_read_offset + pcm_buf_size);
            p_ibiza->left_read_offset = left_write_offset;
            return 0;
        }
