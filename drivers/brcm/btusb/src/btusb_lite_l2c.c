...
int btusb_lite_l2c_add(struct btusb *p_dev, tL2C_STREAM_INFO *p_l2c_stream)
{
    ...
    setup_timer(&p_l2c_ccb->monitor, monitorcb, (unsigned long)p_dev);
...
int btusb_lite_l2c_remove(struct btusb *p_dev, UINT16 local_cid)
{
    ...
    del_timer_sync(&p_l2c_ccb->monitor);
...
int btusb_lite_l2c_send(struct btusb *p_dev, BT_HDR *p_msg, UINT16 local_cid)
{
    ...
    if (!GKI_queue_is_empty(&p_dev->back_queue) || (p_l2c_ccb->tx_pending >= p_l2c_ccb->link_xmit_quota) || (atomic_read(&lock) == 255))
    {
		...
		{
			BTUSB_ERR("Tx Quota reached(%d out of %d) for L2CAP channel (lcid=0x%x). Backup in buffer; Len:%d\n",
			...
			{
				BTUSB_ERR("Backup Full; Drop Buffer\n");
...
clean:		p_msg_dup = (BT_HDR *)GKI_dequeue(&p_dev->back_queue);
    ...
    atomic_set(&timeout, 1);
    if(p_l2c_ccb->monitor.function)
    	mod_timer(&p_l2c_ccb->monitor, jiffies + msecs_to_jiffies(4500));
    else
    	BTUSB_ERR("Timer function released\n");
    return 0;
}
...
static void monitorcb(unsigned long data)
{
	struct btusb *p_dev = (struct btusb*) data;
	UINT8 msg[1] = {0}; /* dummy message */
