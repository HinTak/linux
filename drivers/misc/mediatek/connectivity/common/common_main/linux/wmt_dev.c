...
#define WMT_IOCTL_SET_COUNTRY_CODE _IOW(WMT_IOC_MAGIC, 31, char*)
...
LONG WMT_unlocked_ioctl(struct file *filp, UINT32 cmd, ULONG arg)
{
	...
	case WMT_IOCTL_SET_LAUNCHER_KILL:
		...
		break;
	case WMT_IOCTL_SET_COUNTRY_CODE:
		if (copy_from_user(gCountryCode, (void *)arg, 16)) {
			iRet = -EFAULT;
			break;
		}

		gCountryCode[16] = '\0';
		WMT_INFO_FUNC("wmt set country code %s.\n", gCountryCode);
		break;
