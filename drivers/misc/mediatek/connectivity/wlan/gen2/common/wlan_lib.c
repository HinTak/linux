...
//extern INT32 wmt_dbg_assert_test(INT32 par1, INT32 par2, INT32 par3);
...
/*----------------------------------------------------------------------------*/
// by ssu : 20181212
// For reading & writing mac address from/to eeprom
#define SS_VD_ENABLE_READ_MAC_EEPROM	1
#define SS_VD_ENABLE_WRITE_MAC_EEPROM	1
#define EEPROM_START_ADDR_WIFI_MAC	0x0020
...
#if SS_VD_ENABLE_READ_MAC_EEPROM
	UINT_8 eeprom_wifi_mac_address[] = NULL_MAC_ADDR;
	const UINT_8 invalid_wifi_mac_address[] = BC_MAC_ADDR;
	ssize_t mac_read_result = 0;
	const struct kernel_symbol *sym_read = NULL;
	ssize_t (*eeprom_dev_read)(char *pbuf, size_t count, unsigned long addr);
	P_REG_INFO_T prRegInfo = NULL;
#endif
 
#if SS_VD_ENABLE_WRITE_MAC_EEPROM
	ssize_t mac_write_result = 0;
	const struct kernel_symbol *sym_write = NULL;
...
#if SS_VD_ENABLE_READ_MAC_EEPROM
			// find symbol(interface) for reading Wi-Fi mac address from eeprom
			sym_read = find_symbol("eeprom_dev_read", NULL, NULL, 1, true);	
			if (sym_read)
			{
				eeprom_dev_read = (void*)sym_read->value;
		                mac_read_result = eeprom_dev_read((char *)eeprom_wifi_mac_address, PARAM_MAC_ADDR_LEN, EEPROM_START_ADDR_WIFI_MAC);
				if (mac_read_result >= 0) // success
				{
					if (kalMemCmp(eeprom_wifi_mac_address, invalid_wifi_mac_address, PARAM_MAC_ADDR_LEN) == 0)
					{
						DBGLOG(INIT, ERROR, "EEPROM Wi-Fi MAC address is all 0xFF\n");
						mac_read_result = -1;
					}
					else
					{
						// mac address assignment from eeprom
						kalMemCopy(&rMacAddr[0], &eeprom_wifi_mac_address[0], PARAM_MAC_ADDR_LEN);
						DBGLOG(INIT, ERROR, "Copied Wi-Fi MAC address from EEPROM - %02X:%02X:%02X:%02X:%02X:%02X\n",
							rMacAddr[0], rMacAddr[1], rMacAddr[2], rMacAddr[3], rMacAddr[4], rMacAddr[5]);
					}
				}
			}
			else
			{
				DBGLOG(INIT, ERROR, "Failed to find eeprom interface for reading Wi-Fi MAC address\n");
				mac_read_result = -1;
			}
 
			// cases of failure : reading failure from eeprom or bad mac address(All 0xFF)
			// generating mac address from timetick
...
// Write mac address to eeprom with random generated with timetick() value.
#if SS_VD_ENABLE_WRITE_MAC_EEPROM
				// find symbol(interface) for writing Wi-Fi mac address from eeprom
				sym_write = find_symbol("eeprom_dev_write", NULL, NULL, 1, true);
				if (sym_write)
				{
					eeprom_dev_write = (void*)sym_write->value;
			                mac_write_result = eeprom_dev_write((char *)rMacAddr, 6, EEPROM_START_ADDR_WIFI_MAC);
					if (mac_write_result >= 0) // success
					{
						// mac address assignment to eeprom with time tick
						DBGLOG(INIT, ERROR, "Written Wi-Fi MAC address to EEPROM - %02X:%02X:%02X:%02X:%02X:%02X\n",
							rMacAddr[0], rMacAddr[1], rMacAddr[2], rMacAddr[3], rMacAddr[4], rMacAddr[5]);
					}						
				}

				if (!sym_write || mac_write_result < 0)
				{
					DBGLOG(INIT, ERROR, "EEPROM MAC Writing is failed\n");
				}
#endif
			}

			// by ssu : 20190108
			// update reginfo mac address
			prRegInfo = kalGetConfiguration(prAdapter->prGlueInfo);
			kalMemCopy(&prRegInfo->aucMacAddr[0], &rMacAddr[0], PARAM_MAC_ADDR_LEN);
				
#else // org part(MTK mac generation from timetick())
#if CFG_SHOW_MACADDR_SOURCE
		DBGLOG(INIT, TRACE, "Using dynamically generated MAC address");
#endif
		/* dynamic generate */
		...
		kalMemCopy(&rMacAddr[3], &u4SysTime, 3);
#endif // SS_VD_ENABLE_EEPROM_INTERFACE
