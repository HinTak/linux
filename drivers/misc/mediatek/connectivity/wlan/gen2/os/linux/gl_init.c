...
#if 0 // for using gCountryCode, otherwise it is using NVRAM data (SS uses gCountryCode coming from wmt_launcher -r option setting
		/* load country code */
...
#else
		prRegInfo->au2CountryCode[0] = (UINT_16) gCountryCode[0];
		prRegInfo->au2CountryCode[1] = (UINT_16) gCountryCode[1];
#endif		
...
bailout:
			...
			if (i4Status < 0) {
				...
				for (u4FwCnt = 0; u4FwCnt < 16; u4FwCnt++)
					DBGLOG(INIT, WARN, "0x%08x \n", MCU_REG_READL(HifInfo, CONN_MCU_CPUPCR));
