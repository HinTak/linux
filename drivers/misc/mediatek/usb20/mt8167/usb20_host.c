...
static void musb_vbus_detect_work(struct work_struct *data)
{
	...
	if (val) {
		pr_err("mt_usb_vbus_detect_int has happenned since gpio61: %i, IRQF_TRIGGER_HIGH\n", val);
...
void mt_usb_vbusdet_init(struct musb *musb) 
{
#ifdef CONFIG_USB_VBUSDET_IN_GPIO
...
#ifndef CONFIG_USB_TYPE_A_SUPPORT_ADB
...
#ifdef CONFIG_USB_TYPE_A_SUPPORT_ADB
static void musb_id_pin_work(struct work_struct *data)
{
	DBG(0, "+%s:%d finish\n", __func__, __LINE__);
	if (gpio_get_value(iddig_pin)) {
		DBG(0, "VIZIO MCU switch to device mode\n");
		if (mtk_musb->is_host) {
		...
		DBG(0, "VIZIO MCU switch to host mode\n");
...
#if !defined(CONFIG_MTK_MUSB_SW_WITCH_MODE) || defined(CONFIG_USB_TYPE_A_SUPPORT_ADB)
	if (!mtk_musb->is_ready) {
		/* dealy 5 sec if usb function is not ready */
...
#endif
	DBG(0, "disable iddig irq @lin %d\n", __LINE__);
	DBG(0, "ext id pin interrupt assert\n");
	return IRQ_HANDLED;
}
