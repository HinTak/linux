...
struct kdbus_conn_reply {
	...
	char service_name[KDBUS_NAME_MAX_LEN];
#endif
};
...
static DEFINE_RATELIMIT_STATE(kdbus_err_all_connections, 60 * HZ, 1);
					 ...
					 struct kdbus_conn_reply **reply_wait)
{
	...
	if (atomic_read(&conn_src->reply_count) >
		...
		if(___ratelimit(&kdbus_err_state, __func__))
			printk(KERN_ALERT "[kdbus]kdbus_conn_max_request_pending err, %s(%d) --> %s(%d), bus : %s, cookie : %llu, reply_count : %d \n", 
				conn_src->proc_name, conn_src->pid, conn_dst->proc_name, conn_dst->pid, 
				conn_src->bus->name, msg->cookie, atomic_read(&conn_src->reply_count));
		return -EMLINK;
	}
...
#ifdef CONFIG_SMART_DEADLOCK_PROFILE_MODE
			if (reply_wait) {
				reply_wait->timestamp = jiffies;
				if (msg->dst_id == KDBUS_DST_ID_NAME)
					strncpy(reply_wait->service_name,
							kmsg->dst_name, strlen(kmsg->dst_name) + 1);
				else
					reply_wait->service_name[0] = '\0';
			}
#endif
		}
		else
		   ...
		   struct kdbus_conn **c)
{
	...
	/* privileged processes can impersonate somebody else */
	if (creds || seclabel) {
		ret = kdbus_meta_new(&conn->meta);
		if (ret < 0)
			goto exit_release_names;

		if (creds) {
			ret = kdbus_meta_append_data(conn->meta,
					KDBUS_ITEM_CREDS,
	...
	conn->pid = current->group_leader->pid;
	memset(conn->proc_name, '\0', 16);
	strncpy(conn->proc_name, current->group_leader->comm, 15);
	/* lock order: domain -> bus -> ep -> names -> conn */
	mutex_lock(&bus->lock);
	mutex_lock(&ep->lock);
	...
	if (!capable(CAP_IPC_OWNER) &&
		...
		if(___ratelimit(&kdbus_err_all_connections, __func__)) {
			int i, count = 0;
			struct kdbus_conn *conn_iter;

			hash_for_each(bus->conn_hash, i, conn_iter, hentry)
				printk (KERN_ALERT "[%04d] %4d (%s)\n", ++count, conn_iter->pid, conn_iter->proc_name);
		}
...
#define CONVERT_MS_INTO_SEC(time)  (time / 1000) , (time % 1000)
static void kdbus_conn_dump_all_pending_reply()
{
	struct kdbus_bus *bus = NULL;
	if (kdbus_domain_init != NULL) {
		mutex_lock(&kdbus_domain_init->lock);
		list_for_each_entry(bus, &kdbus_domain_init->bus_list, domain_entry) {
			...
			hash_for_each(bus->conn_hash, i, conn, hentry) {
				...
				list_for_each_entry_safe(r, r_tmp,
						&conn->reply_list,
						entry) {
					write_dbus_info("|%16s(%-5d) |%16s(%-5d) |%32s\t|   %7llu   |  %2lu.%3lu  | %-16s |\n",
							r->conn->proc_name, r->conn->pid, conn->proc_name,
							conn->pid, r->service_name[0] != '\0' ? r->service_name : "--------------------",
							r->cookie, CONVERT_MS_INTO_SEC(jiffies_to_msecs(jiffies - r->timestamp)), bus->name);
				}
				mutex_unlock(&conn->lock);
			}
			mutex_unlock(&bus->lock);
		}
		mutex_unlock(&kdbus_domain_init->lock);
	}
}
