		 ...
		 bool policy, struct kdbus_ep **ep)
{
	...
	printk("kdbus: register a device\n");
	...
	if (ret < 0) {
		...
		printk("kdbus: Successfully registered\n");
	...
	if (bus_path) {
		...
		if (!ret) {
			...
			printk("kdbus: Start to set security of device %s\n", bus_path);
			ret = security_inode_setsecurity(inode, xattr_name,
					xattr_value, strlen(xattr_value), 0);
			printk("kdbus: Successfully set security\n");
			path_put(&path);
		}
		kfree(bus_path);
	}
