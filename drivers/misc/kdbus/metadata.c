 ...
 */
void kdbus_meta_free(struct kref *kref)
{
...
void kdbus_meta_unref(struct kdbus_meta *meta)
{
	if (!meta)
		return;

	kref_put(&meta->kref, kdbus_meta_free);
}
...
static int kdbus_meta_append_cred(struct kdbus_meta *meta)
{
	...
	creds.pid = (__u64)task_tgid_vnr(current);
	creds.tid = (__u64)task_pid_vnr(current);
