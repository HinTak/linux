...
static atomic_t memnotify_last_threshold = ATOMIC_INIT(THRESHOLD_HIGH);
...
{
	return totalram_pages - si_mem_available();
}
...
void memnotify_threshold(gfp_t gfp_mask)
{
	...
	if ((threshold == THRESHOLD_CRITICAL)) {
		if (smdl_lmf_status() == false) 
			hook_smart_deadlock_exception_case(SMART_DEADLOCK_LMF_START);
		if (dump_tasks_once_lv2 == false) {
			dump_tasks_once_lv2 = true;
			pr_err("[LMF] dump_tasks was called by resourced. Memory available : %lu MB\n",
				available);
			dump_tasks(NULL, NULL, NULL);
		}
	} else if (smdl_lmf_status() == true && threshold <= THRESHOLD_NORMAL)
		hook_smart_deadlock_exception_case(SMART_DEADLOCK_LMF_STOP);
	/* return quickly when normal case */
	if (likely(threshold == last_threshold))
		return;
	...
	if (threshold < last_threshold) {
		...
		if (available < leave_threshold)
			return;
		if (available < memnotify_leave_thresholds[THRESHOLD_NORMAL])
		...
		struct class_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret == 0) {
		...
		/*
		 * this is dummy enter threshold as we donot add new 
		 * enter threshold but only add new leave threshold 
		 */
		memnotify_enter_thresholds[THRESHOLD_NORMAL] = val;
...
static ssize_t threshold_high_store(struct class *class,
		struct class_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = -EINVAL;
	long val;
	...
	if (ret == 0) {
		ret = count;
		memnotify_leave_thresholds[THRESHOLD_NORMAL] = val;
		pr_info("[LMF] high threshold set : %ld MB\n", val);
	}
	return ret;
}
...
extern bool only_one_time_call;
bool panic_ratelimit_interval_flag;
bool panic_ratelimit_burst_flag;
		 ...
		 struct class_attribute *attr, const char *buf, size_t count)
{
	...
	if(!only_one_time_call || panic_ratelimit_interval_flag){
		pr_err("oom_panic_burst_interval entry set only once\n");
		return ret;
	}
	...
	if (ret == 0) {
		ret = count;
		panic_ratelimit_interval_flag = true;
		 ...
		 struct class_attribute *attr, const char *buf, size_t count)
{
	...
	if(!only_one_time_call || panic_ratelimit_burst_flag){
		pr_err("oom_panic_burst_number entry set only once\n");
		return ret;
	}
	...
	if (ret == 0) {
		ret = count;
		panic_ratelimit_burst_flag = true;
...
static CLASS_ATTR(threshold_high, S_IWUSR, NULL, threshold_high_store);
...
static int __init lowmemnotify_init(void)
{
	...
	err = class_create_file(memnotify_class, &class_attr_threshold_high);
	if (err) {
		pr_err("[LMF] %s: couldn't create threshold high.\n",
			__func__);
		goto error_create_threshold_high_class_file;
	}
 ...
 error_create_dump_tasks:
	class_remove_file(memnotify_class, &class_attr_threshold_high);
 error_create_threshold_high_class_file:
