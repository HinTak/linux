...
#ifdef CONFIG_ARCH_NVT72668
#define CONFIG_EEPROM_I2C_PORT 1
#define CONFIG_EEPROM_I2C_SLV_ADDR 0xA0
#define GPIO_MAIN_EEPROM_WP 22

struct i2c_client client_temp = {
		.addr = ((CONFIG_EEPROM_I2C_SLV_ADDR&0xFF)>>1), // slaveaddress : 7bit
		.adapter = 0,
	};
...
int eep_i2c_write(const unsigned char *buf, int len)
{
	...
	if (eep_client) {
		...
		if ((ret) > 0 && eeprom_wc) {
			addr = (buf[0] << 8) | buf[1];
			end_addr = addr + len;
			while ((addr < eeprom_size) && (addr < end_addr))
				eeprom_wc[addr++]++;
		}
	} else {
...
#ifdef CONFIG_ARCH_NVT72668
	int (*nt_gpio_request)(struct gpio_chip *, unsigned ) = 0;
	void (*nt_gpio_free)(struct gpio_chip *, unsigned ) = 0;
	int (*nt_gpio_direction_out)(struct gpio_chip *, unsigned , int ) = 0;
	void (*nt_gpio_set)(struct gpio_chip *, unsigned , int ) = 0;
#endif
...
#endif
#ifdef CONFIG_ARCH_NVT72668
		gpio = GPIO_MAIN_EEPROM_WP;

		nt_gpio_request = (int (*)(void))resolve_symbol("nt726xx_gpio_request");
		nt_gpio_free = (void (*)(void))resolve_symbol("nt726xx_gpio_free");;
		nt_gpio_direction_out = (int (*)(void))resolve_symbol("nt726xx_gpio_direction_out");;
		nt_gpio_set = (void (*)(void))resolve_symbol("nt726xx_gpio_set");;

		if(nt_gpio_request && nt_gpio_free && nt_gpio_direction_out && nt_gpio_set)
		{
			ret = nt_gpio_request(NULL, gpio);
			ret = nt_gpio_direction_out(NULL, gpio, 0); // output
			nt_gpio_set(NULL, gpio, protect); //protect value
			nt_gpio_free(NULL, gpio);
		}
		else
		{
			eep_err(KERN_ERR "[eep_set_wp] find_symbol error\n");
		}
...
long eep_ioctl(struct file *fp, unsigned int cmd, unsigned long args)
{
	...
	case EEPROM_READ_DATA:
		...
		break;
	case EEPROM_SET_WRITE_COUNT:
		if (eeprom_wc) {
			if (copy_from_user(eeprom_wc, (unsigned int *)args, eeprom_size * sizeof(unsigned int))) {
				eep_err(KERN_ERR
					"[eep_ioctl:EEPROM_SET_WRITE_COUNT] failed copy_from_user.\n");
				ret = -EFAULT;
			} else
				eep_err(KERN_ERR "set eeprom write count.");
		}
		break;
	case EEPROM_GET_WRITE_COUNT:
		if (eeprom_wc) {
			if (copy_to_user((unsigned int *)args, eeprom_wc, eeprom_size * sizeof(unsigned int))) {
				eep_err(KERN_ERR
					"[eep_ioctl:EEPROM_GET_WRITE_COUNT] failed copy_to_user.\n");
				ret = -EFAULT;
			} else
				eep_err(KERN_ERR "get eeprom write count.");
		}
		break;
			...
			struct device_attribute *attr, char *buf)
{
	...
	write_count = vmalloc(eeprom_size * sizeof(unsigned int));
	idx = vmalloc(eeprom_size * sizeof(unsigned int));
	if (write_count && idx) {
		z = 0;
		for (i = 0; i < eeprom_size; i++) {
			if (eeprom_wc[i] > 0) {
				idx[z] = i;
				write_count[z] = eeprom_wc[i];
				z++;
			}
		}
		count = z;
		...
		for (i = 0; i < count - 1; i++)
			for (z = i + 1; z < count; z++)
				if (write_count[z] > write_count[i]) {
					temp = write_count[i];
					write_count[i] = write_count[z];
					write_count[z] = temp;
		...
		printk("=== eeprom write count top 10 ===\n");
		printk("=== total written address count : %d\n", count);
		for (i = 0; i < 10; i++)
			if (write_count[i] > 0)
				printk("[%d][addr=0x%X][count=%u]\n", i, idx[i], write_count[i]);
		printk("=== end of list ===\n");
	}
			...
			const struct i2c_device_id *id)
{
	int ret = 0;
#ifdef CONFIG_ARCH_NVT72668
	struct i2c_adapter *a = i2c_get_adapter(CONFIG_EEPROM_I2C_PORT); // i2c channel
	printk(KERN_CRIT "[eep_probe] Called.\n");

	client_temp.adapter = a;
	eep_client = &client_temp;
#else //original
 ...
 */
#ifdef CONFIG_ARCH_NVT72668
static struct platform_driver eep_i2c_driver = {
	.driver = {
	.name = "s24c512c",
...
static int __init eep_init(void)
{
...
#ifdef CONFIG_ARCH_NVT72668
	res = platform_driver_register(&eep_i2c_driver);
#else //original
	...
	eeprom_wc = vmalloc(eeprom_size * sizeof(unsigned int));
	if (eeprom_wc)
		memset(eeprom_wc, 0, eeprom_size * sizeof(unsigned int));
	else
		eep_log(KERN_INFO "memory not allocated for eeprom write count.\n");
...
static void __exit eep_exit(void)
{
	...
	class_destroy(eep_class);
#ifdef CONFIG_ARCH_NVT72668
	platform_driver_unregister(&eep_i2c_driver);
#else //original
