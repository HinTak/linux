...
static unsigned char micom_dbg_print_count = 3;
...
static int micom_msg_open(struct inode *inode, struct file *filp)
{
	dev_dbg(micom_dev, "MSG device is opened.\n");
	return 0;
}
...
static int micom_msg_release(struct inode *inode, struct file *filp)
{
	dev_dbg(micom_dev,"MSG device is closed.\n");
	return 0;
}
...
static void wt61p807_msg_cb(struct sdp_micom_msg *msg, void *dev_id)
{
	...
	if (ack == msg->msg[0]) {
		...
		send_i_am_ok = 1;
	} else if( msg->msg[0] == SDP_MICOM_ACK_ALIVE_MSG ) {
		if( micom_dbg_print_count > 0 ) {
			dev_err( micom_dev, "[Micom State][PwrOnMode:%d][ErrState:%X]\n", 
				msg->msg[1], 
				(msg->msg[2]<<24)|(msg->msg[3]<<16)|(msg->msg[4]<<8)|(msg->msg[5]<<0)  );
			micom_dbg_print_count--;
		}
	}
}
...
{
	platform_driver_register(&wt61p807_msg_driver);
}

void wt61p807_msg_exit(void)
{
	platform_driver_unregister(&wt61p807_msg_driver);
}
