...
//drivers/tty/serial/sdp_serial-regs.h
...
#if defined(CONFIG_SERIAL_SDP2)
static irqreturn_t sdp_micom_handle_tx_irq(int irq, void *id)
{
				    ...
				    struct platform_device *pdev)
{
	...
	port->regs = devm_ioremap_resource(port->dev, res);
	if (!port->regs) {
		dev_err(port->dev, "ioremap_failed\n");
		return -ENODEV;
	}
	...
	if (ret < 0) {
		dev_err(port->dev, "failed to find irq resource for TX uart\n");
		return -EINVAL;
	}
	...
	if (ret < 0) {
		dev_err(port->dev, "failed to find irq resource for RX uart\n");
		return -EINVAL;
	}
...
#if defined (CONFIG_SERIAL_SDP2)
	ret = devm_request_threaded_irq(port->dev, port->tx_irq, NULL,
			sdp_micom_handle_tx_irq, IRQF_ONESHOT,
			dev_name(port->dev), port);
	if (ret) {
		dev_err(port->dev, "cannot get tx irq %d\n", port->tx_irq);
		return ret;
	}
#endif
...
static int sdp_micom_probe(struct platform_device *pdev)
{
	...
	port->long_isp_resp = 0;
#if defined (CONFIG_SERIAL_SDP2)
	dev_info(port->dev, "MICOM multi function device, tx irq[%d], rx irq[%d]\n", port->tx_irq, port->irq);
#else
...
static int sdp_micom_resume(struct device *dev)
{
	...
	if (!port)
	{
		dev_err(port->dev, "Failed Micom Uart Resume. Wrong Param(NULL port)\n");
		mutex_unlock(&micom_lock);
		return 0;
	}
