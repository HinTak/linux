...
#include <mach/map.h>
//#include <drivers/tty/serial/sdp_serial-regs.h>

#include "tztv-micom.h"
...
static unsigned char _trace_enable = 0;
...
static void sdp_micom_print_cpu2micom_cmd( const char *s, unsigned int count)
{
	...
	//Get Task Name.
	task = pid_task(find_get_pid(current->pid),PIDTYPE_PID);
	if( task == NULL ) {
		dev_info( micom_port->dev, "task is null.\n");
		task_name = default_str;
	} else {
		task_name = task->comm;
	}

	if( micom_port == NULL ) {
		dev_err(micom_port->dev, "micom_port is null\n");
		return;
	}
	if( count > 2 ) {
		if( s[0] == 0xFF && s[1] == 0xFF ) {
			switch( s[2] ) {
			case 0x25:	//CTM_SET_BOOT_MODE(suspend mode)
			case 0xD3:	//CTM_CNTL_ENTER_SUSPEND_MODE
				dev_err(micom_port->dev, "[%03d,%s] Suspend(cmd:0x%02X)\n", current->pid, task_name, s[2]);
				break;
			case 0x12:	//CTM_POWER_OFF
				dev_err(micom_port->dev,  "[%03d,%s] Power OFF(cmd:0x%02X)\n", current->pid, task_name , s[2]);
				break;
			case 0x1D:	//CTM_SYSTEM_RESTART
			case 0x56:	//CTM_SYSTEM_REBOOT
			case 0x66:	//CTM_MICOM_REBOOT
			case 0x8F:	//CTM_CPU_RESET_COMMAND
			case 0xC8:	//CTM_SET_SUSPEND_RESTART
			case 0x8E:	//CTM_STANDBY_REBOOT
				dev_err(micom_port->dev, "[%03d,%s] Reboot(cmd:0x%02X)\n", current->pid, task_name , s[2]);
				break;
			case 0x33:	//CTM_SCREEN_ON
				dev_err(micom_port->dev,  "[%03d,%s] Screen ON(cmd:0x%02X)\n", current->pid, task_name , s[2]);
				break;
			case 0x54:	//CTM_SCREEN_OFF
				dev_err(micom_port->dev,  "[%03d,%s] Screen OFF(cmd:0x%02X)\n", current->pid, task_name , s[2]);
				break;
			case 0x35:	//CTM_BSP_DUAL_PART_CTRL
				dev_err(micom_port->dev,   "[%03d,%s] Dual Part Ctrl(%d,%d)\n", current->pid, task_name,  s[3], s[4]);
				break;
			case 0x34:	//CTM_SET_ROLLBACK_FLAG
				dev_err(micom_port->dev,   "[%03d,%s] Set Rollback\n", current->pid, task_name );
				break;
			default:
				break;
			}
...
static void sdp_micom_print_micom2cpu_cmd( struct sdp_micom_msg const *msg)
{
	static char previous_key = 0xFF, previous_cmd = 0xFF;
	if( previous_cmd == msg->msg[0] && previous_key  == msg->msg[1] ) {
		return;
	}
	if( micom_port == NULL ) {
		return;
	...
	switch( msg->msg[0] ) {
	case 0x10:		//SDP_MICOM_EVT_KEYPRESS
		switch(msg->msg[1]) {
		case 0x00:	//DISCRET_ALL_POWER_OFF
		case 0x01:	//DISCRET_ALL_POWER_ON
		case 0x99:	//DISCRET_POWER_ON
		case 0x98:	//DISCRET_POWER_OFF
		case 0x02:	//IR_KEY_POWER
		case 0xF5:	//IR_KEY_PRE_POWER
			dev_err(micom_port->dev,   "(Ack)IrK(Pre:0x%02X,Cur:0x%02X)\n", previous_key, msg->msg[1]);
			break;
		default:
			break;
		}
		previous_cmd = msg->msg[0];
		previous_key = msg->msg[1];
		break;
	case 0x11:		//SDP_MICOM_EVT_PANEL_KEYPRESS
		dev_err(micom_port->dev,   "(Ack)PanelK (Pre:0x%02X, Cur:0x%02X)\n", previous_key, msg->msg[1] );
		previous_cmd = msg->msg[0];
		previous_key = msg->msg[1];
		break;
	case 0x20:		//SDP_MICOM_ACK_POWERON_MODE
		dev_err(micom_port->dev,   "(Ack)Reason(%d)\n", msg->msg[1] );
		break;
	case 0x1E:		//SDP_MICOM_EVT_KEYRELEASE
		dev_err(micom_port->dev,   "(Ack)RelK(Pre:0x%02X)\n", previous_key);
		break;
	default:
		break;
	}
			...
			const char *s, unsigned int count)
{
	...
	if( _trace_enable ) {
		if( !(_trace_enable == 0x1 && (s[2] == 0xA0 || s[2] == 0x18) )) {
			struct task_struct *task;
			char *task_name = NULL;
			char default_str[] = "null";
			...
			task = pid_task(find_get_pid(current->pid),PIDTYPE_PID);
			if( task == NULL ) {
				dev_err( micom_port->dev, "task is null.\n");
				task_name = default_str;
			} else {
				task_name = task->comm;
			}
			dev_err(micom_port->dev,  "[%03d,%s] [SendD](len:%d)[0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X]n" ,
				 current->pid, task_name, count, s[0],s[1],s[2],s[3],s[4],s[5],s[6],s[7],s[8]);
		}
	}
	do {
		sentByte = sdp_messagebox_write_direct((unsigned char*)s, count);
	} while( (retry > 0) && (sentByte != count));
	if( _trace_enable ) {
		if( !(_trace_enable == 0x1 && (s[2] == 0xA0 || s[2] == 0x18) )) {
		dev_err(micom_port->dev,  "[SendD][Check](retry:%d)(SentByte:%d)\n", retry, sentByte);
	}
	}
	return sentByte;
}
			...
			const char *s, unsigned int count)
{
	...
	//Print Special Command for debugging.
	sdp_micom_print_cpu2micom_cmd( s, count);
	...
	if( _trace_enable ) {
		if( !(_trace_enable == 0x1 && (s[2] == 0xA0 || s[2] == 0x18) )) {
			...
			dev_err(micom_port->dev,  "[%03d,%s] [Send](len:%d)[0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X]\n", 
				 current->pid, task_name, count, s[0],s[1],s[2],s[3],s[4],s[5],s[6],s[7],s[8]);
		}
	}
	do {
	...
	if( _trace_enable ) {
		if( !(_trace_enable == 0x1 && (s[2] == 0xA0 || s[2] == 0x18) )) {
		dev_err(micom_port->dev,  "[Send][Check](retry:%d)(SentByte:%d)\n", retry, sentByte);
	}
	}
		...
		while(retry--) {
			...
			if (!ret) {
				...
				if (-ERESTARTSYS == ret) {
					...
					if (!retry) {
						dev_err(port->dev, "sdp-micom: Timeout error - command 0x%x, (ret:%d)\n", cmd, ret);
					...
					struct sdp_micom_msg *msg, int id)
{
	...
	if ((id == SDP_MICOM_DEV_ISP) && (port->block_flag == BLOCK)) {
		...
		//Print Special Ack Command.
		sdp_micom_print_micom2cpu_cmd( msg);
...
void sdp_micom_messagebox_handle_irq( unsigned char *pBuffer, unsigned int size)
{
	...
	if( _trace_enable ) {
		if( !(_trace_enable == 0x1 && (pBuffer[2] == 0xA0 || pBuffer[2] == 0x29) )) {
		dev_err(micom_port->dev,  "[Ack](len:%d)[0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X][0x%02X]\n", 
			size, pBuffer[0],pBuffer[1],pBuffer[2],pBuffer[3],pBuffer[4],pBuffer[5],pBuffer[6],pBuffer[7],pBuffer[8]);
	}
	}
...
static ssize_t sdp_micom_trace_enable( struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	if (strncmp(buf, "1", 1) == 0) {
		//Enable Trace except 0xA0
		_trace_enable = 1;
	} else if (strncmp(buf, "3", 1) == 0 ) {
		//Disable Trace with 0xA0
		_trace_enable = 3;
	} else {
		_trace_enable = 0;
	}
...
static DEVICE_ATTR(trace_enable, S_IWUSR, NULL, sdp_micom_trace_enable);

static struct attribute *sdp_micom_attributes[] = {
	&dev_attr_trace_enable.attr,
	NULL
};

static const struct attribute_group sdp_micom_attribute_group = {
	.attrs = sdp_micom_attributes,
};
...
static int sdp_micom_probe(struct platform_device *pdev)
{
	...
	if (!port) {
		printk(KERN_ERR "micom(messagebox):Failed to Alloc Port mem\n");
		return -ENOMEM;
	}
	...
	if (!(port->buf)) {
		printk(KERN_ERR "micom(messagebox):Failed to Alloc Buf\n");
		return -ENOMEM;
	}
	...
	if( sysfs_create_group(&pdev->dev.kobj, &sdp_micom_attribute_group) ) {
...
static int sdp_micom_remove(struct platform_device *pdev)
{
	...
	mfd_remove_devices(port->dev);
	sysfs_remove_group(&pdev->dev.kobj, &sdp_micom_attribute_group);
	return 0;
}
...
static int sdp_micom_suspend(struct device *dev)
{
	...
	if (!port)
	{
		printk(KERN_ERR "Failed Micom Messagebox Suspend. Wrong Param(NULL port)\n");
		return 0;
	}
...
static int sdp_micom_resume(struct device *dev)
{
	...
	if (!port)
	{
		printk(KERN_ERR "Failed Micom Messagebox Resume. Wrong Param(NULL port)\n");
		mutex_unlock(&micom_lock);
		return 0;
	}
	port->count = 0;
...
static const struct dev_pm_ops sdp_micom_pm_ops = {
	...
	.resume_early = sdp_micom_resume,
};

static const struct of_device_id sdp_micom_dt_match[] = {
	{ .compatible = "samsung,tztv-micom" },
	{},
};
...
{
	platform_driver_register(&sdp_micom_driver);
	/*
	wt61p807_rc_map_init();
	wt61p807_map_panel_init();
	wt61p807_ar_init();
	wt61p807_bs_init();
	wt61p807_cec_init();
	wt61p807_isp_init();
	wt61p807_ontimer_init();
	wt61p807_rc_init();
	wt61p807_panel_init();
	wt61p807_rtc_init();
	wt61p807_system_grp_init();
	*/
	wt61p807_msg_init();
	return 0;
}
...
static void __exit sdp_micom_exit(void)
{
	...
	/*
	wt61p807_rc_map_exit();
	wt61p807_map_panel_exit();
	wt61p807_ar_exit();
	wt61p807_bs_exit();
	wt61p807_cec_exit();
	wt61p807_isp_exit();
	wt61p807_ontimer_exit();
	wt61p807_panel_exit();
	wt61p807_rc_exit();
	wt61p807_rtc_exit();
	wt61p807_system_grp_exit();
	*/
	wt61p807_msg_exit();
}
