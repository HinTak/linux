/**
 *   @mainpage   Flex Sector Remapper : LinuStoreIII_1.2.0_b040-FSR_1.2.1p1_b146_RTM
 *
 *   @section Intro Intro
...
#endif
		/* New check deals with 2K & 4K page sizes separately */
        /* Check # of blocks of Reservoir validity - 2K page size case */
        if ( ((pstLLDSpec->nSctsPerPG == 4) &&
				 (nBlksInRsv > (BML_NUM_OF_BMS_2KB_PG * BML_BMFS_PER_BMS)) ) ||
				 (nBlksInRsv == 0))
        {
            FSR_DBZ_RTLMOUT(FSR_DBZ_ERROR,  (TEXT("[BIF:ERR]   %s(nBlksInRsv(%d) for 2K device is out of bound) / %d line\r\n"),
                                            __FSR_FUNC__, nBlksInRsv, __LINE__));
            bRe = FALSE32;
            break;
        }
        /* Check # of blocks of Reservoir validity - 4K page size case */
        if ( ((pstLLDSpec->nSctsPerPG == 8) &&
				 (nBlksInRsv > (BML_NUM_OF_BMS_4KB_PG * BML_BMFS_PER_BMS)) ) ||
				 (nBlksInRsv == 0))
        {
            FSR_DBZ_RTLMOUT(FSR_DBZ_ERROR,  (TEXT("[BIF:ERR]   %s(nBlksInRsv(%d) for 4K device is out of bound) / %d line\r\n"),
                                            __FSR_FUNC__, nBlksInRsv, __LINE__));
            bRe = FALSE32;
            break;
        }
