...
//TODO cache op
#ifndef __KERNEL__
#define CACHE_INV(buf,len)	
#define CACHE_WB(buf,len)	
#else
#define CACHE_INV(buf,len) 	dma_map_single(0,(void*)buf,len,DMA_FROM_DEVICE);
#define CACHE_WB(buf,len) 	dma_map_single(0,(void*)buf,len,DMA_TO_DEVICE);
#endif
...
LLD_FUNC UINT32 NFC_GetECCBits(UINT32 subpage_cnt) ;
...
LLD_FUNC ER NFC_Init(unsigned char ucCE);
LLD_FUNC ER NFC_ReadID(unsigned char ucCE, NANDIDData* pstNANDID);
LLD_FUNC ER NFC_SetType(unsigned char ucCE, NANDIDData* pNandID);
LLD_FUNC ER NFC_ReadPage_WholePage(unsigned char ucCE, UINT32 dwPageAddr, UINT8 *dwBuffer, UINT8 *dwExtraBuffer, UINT8 ucECCType);
LLD_FUNC ER NFC_WritePage_WholePage(unsigned char ucCE, ULONG dwPageAddr, UINT8 *dwBuffer, UINT8 *dwExtraBuffer, UINT8 ucECCType);
LLD_FUNC ER NFC_EraseBlock(unsigned char ucCE, ULONG dwPageAddr);
LLD_FUNC ER NFC_Reset(unsigned char ucCE);
...
INT32   NTK_NFCON_ReadPage(UINT32 nDev, UINT32 nRowAddress, UINT32 nColAddress)
{
	...
	if(gEnableECC==1)
		nLLDRe = NFC_ReadPage_WholePage(ucChipEn, nRowAddress, __pa(pMBuf), __pa(pSBuf), NAND_ECC_TYPE_REEDSOLOMON);
	else
		nLLDRe = NFC_ReadPage_WholePage(ucChipEn, nRowAddress, __pa(pMBuf), __pa(pSBuf), NAND_ECC_TYPE_NONE_16);
...
#ifdef RDUMP
	{
		...
		nTESTRe = NFC_ReadPage_WholePage(ucChipEn, nRowAddress, pP, pS, NAND_ECC_TYPE_REEDSOLOMON);
...
INT32   NTK_NFCON_ProgramPage(UINT32 nDev, UINT32 nRowAddress, UINT32 nColAddress)
{
	...
	if(gEnableECC==1)
	{
		nLLDRe = NFC_WritePage_WholePage(ucChipEn, nRowAddress, __pa(pMBuf), __pa(pSBuf), NAND_ECC_TYPE_REEDSOLOMON);
	}
	else
	{
		nLLDRe = NFC_WritePage_WholePage(ucChipEn, nRowAddress, __pa(pMBuf), __pa(pSBuf), NAND_ECC_TYPE_NONE_16);
	}
...
INT32 NTK_NFCON_ERASE(UINT32 nDev, UINT32 nRowAddress, UINT32 nColAddress)
{
	...
	nLLDRe = NFC_EraseBlock(ucChipEn, nRowAddress);
...
#define RD_REG ntk_readl
#define WR_REG ntk_writel
		...
		UINT8 *dwBuffer, UINT8 *dwExtraBuffer)
{
    ...
    erRet = NFC_ReadPage_2K(0, (unsigned long)dwBuffer, (unsigned long)dwExtraBuffer, dwPageAddr);

    if(erRet != NAND_ERR_OK)
    {
		FSR_OAM_DbgMsg("Extra:0x%x", *(unsigned long*)dwExtraBuffer);
	    FSR_OAM_DbgMsg("   0x%x", *(((unsigned long*)dwExtraBuffer) +1));
	    FSR_OAM_DbgMsg("   0x%x", *(((unsigned long*)dwExtraBuffer) +2));
	    FSR_OAM_DbgMsg("   0x%x\r\n", *(((unsigned long*)dwExtraBuffer) +3));

		FSR_OAM_DbgMsg("   0x%x", *(((unsigned long*)dwExtraBuffer) +4));
		FSR_OAM_DbgMsg("   0x%x", *(((unsigned long*)dwExtraBuffer) +5));
		FSR_OAM_DbgMsg("   0x%x", *(((unsigned long*)dwExtraBuffer) +6));
		FSR_OAM_DbgMsg("   0x%x\r\n", *(((unsigned long*)dwExtraBuffer) +7));

		FSR_OAM_DbgMsg("   0x%x", *(((unsigned long*)dwExtraBuffer) +8));
		FSR_OAM_DbgMsg("   0x%x", *(((unsigned long*)dwExtraBuffer) +9));
		FSR_OAM_DbgMsg("   0x%x", *(((unsigned long*)dwExtraBuffer) +10));
		FSR_OAM_DbgMsg("   0x%x\r\n", *(((unsigned long*)dwExtraBuffer) +11));

		FSR_OAM_DbgMsg("   0x%x", *(((unsigned long*)dwExtraBuffer) +12));
		FSR_OAM_DbgMsg("   0x%x", *(((unsigned long*)dwExtraBuffer) +13));
		FSR_OAM_DbgMsg("   0x%x", *(((unsigned long*)dwExtraBuffer) +14));
		FSR_OAM_DbgMsg("   0x%x\r\n", *(((unsigned long*)dwExtraBuffer) +15));

		FSR_OAM_DbgMsg("Err:0x%x", erRet);
		if(erRet == NAND_ERR_UNCORECTABLE)
		{
		...
		UINT8 *dwBuffer, UINT8 *dwExtraBuffer)
{
	...
	FSR_OAM_DbgMsg("*****************NFC_WritePage_FreeRun Page:%d\n", dwPageAddr);
    ...
    erRet = NFC_WritePage_2K(0, (unsigned long)dwBuffer, (unsigned long*) dwExtraBuffer, dwPageAddr);

	if(erRet == NAND_ERR_SYSTEM)
	{
		FSR_OAM_DbgMsg("NFC_WritePage_FreeRun Fail !!\n");
		return FSR_LLD_PREV_READ_ERROR;
	}

	if(erRet == NAND_ERR_PROGRAM)
	{
		return FSR_LLD_PREV_READ_ERROR;
	}
