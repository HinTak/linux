...
#if defined(CONFIG_ARCH_SDP1202)|| defined(CONFIG_ARCH_SDP1302)|| defined(CONFIG_ARCH_SDP1304)
...
#elif  defined(CONFIG_ARCH_SDP1307) /* PreGolf-V */
static u32 g_sdp_ccepfb_membase;
#endif

static struct fb_info * sdp_fbinfo[LAST_LAYER];

#ifdef CONFIG_ARCH_SDP1106 /* EchoS */
#define SDP_CCEPFB_MEMBASE (0xA0000000 + (436 << 20))
#elif defined(CONFIG_ARCH_SDP1202)||defined(CONFIG_ARCH_SDP1302)||defined(CONFIG_ARCH_SDP1304)
u32 SDP_CCEPFB_MEMBASE; 
#elif defined(CONFIG_ARCH_SDP1207) // FoxB
u32 SDP_CCEPFB_MEMBASE[LAST_LAYER]; 
#elif defined(CONFIG_ARCH_SDP1307)  /* GolfV */
u32 SDP_CCEPFB_MEMBASE; 
#else
...
#if defined(CONFIG_ARCH_SDP1202)||(CONFIG_ARCH_SDP1302)||(CONFIG_ARCH_SDP1304)
...
// only support 1:1 scale
   ...
   if(GfxFlipMode == GFX_V_FLIP || GfxFlipMode == GFX_HV_FLIP ) //Gfx Flip Case
    ...
    CCEPFB_WRITEL(phy_membase+g_crop_offset, (unsigned int *)(p_ccepfb->regbase + O_BASE_ADDR));	// base address 

#if defined(CONFIG_ARCH_SDP1207)||(CONFIG_ARCH_SDP1307)//FoxB or GolfV
{
	unsigned int val;
	val = CCEPFB_READL((unsigned int *)(p_ccepfb->regbase + O_BASE_ADDR));	// base address     
	CCEPFB_WRITEL(0x1, (unsigned int *)(p_ccepfb->regbase + O_OSD_UPDATE));	// base address 
	g_phy_membase = phy_membase;
	g_regbase = p_ccepfb->regbase;
}
#endif
...
unsigned int setGpuCropInfo(u32 crop_offset)
{

#if defined(CONFIG_ARCH_SDP1207)||(CONFIG_ARCH_SDP1307)//FoxB or GolfV

	if(g_crop_offset != crop_offset)
	{
		CCEPFB_WRITEL(g_phy_membase+crop_offset, (g_regbase + O_BASE_ADDR));	// base address 
		unsigned int val;
		val = CCEPFB_READL((g_regbase + O_BASE_ADDR));	// base address     

		CCEPFB_WRITEL(0x1, (g_regbase + O_OSD_UPDATE));	// base address 
	}
#endif
    g_crop_offset =  crop_offset;
    return 0;
}
EXPORT_SYMBOL(setGpuCropInfo);
...
static int __init sdp_ccepfb_var_init(const SDP_CCEPFB_INFO_T* p_ccepfb_info, struct fb_var_screeninfo *p_varinfo)
{
    ...
    SDP_CCEPFB_MEMBASE = (0x40000000 + (( (sdp_sys_mem0_size/0x100000) +100 +1 ) << 20));
#else
...
static int __init sdp_ccepfb_probe(struct platform_device *pdev)
{
	...
	{
#ifdef CONFIG_FRAMEBUFFER_CONSOLE
		p_ccepfb_info = (ccepfb_initcon_disable) ? (const SDP_CCEPFB_INFO_T *)sdp_ccepfb_info : (const SDP_CCEPFB_INFO_T *)sdp_ccepfb_info_con;
#else
		p_ccepfb_info = (const SDP_CCEPFB_INFO_T *)sdp_ccepfb_info;
#endif
	}
...
#ifdef CONFIG_ARCH_SDP1207	
		p_ccepfb->regbase = (u32)base[layer];
		p_ccepfb->osd_regbase = (u32)base[LAYER_OSD];
#else
...
//		retval = fb_find_mode(&p_fbinfo->var, p_fbinfo, "640x540@60", NULL, 0, NULL, 8);	// ????
    	...
    	INFO_CCEPFB("%s framebuffer at physical: 0x%08x, regbase: 0x%08x\n", 
			p_fbinfo->fix.id, (unsigned int)p_fbinfo->fix.smem_start, p_ccepfb->regbase);
