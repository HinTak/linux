				     ...
				     unsigned long ufstat)
{
	if(ourport->enhanced)
	{
		if (ufstat & SDP_UFSTAT2_RXFIFO_FULL)
			return (int) ourport->port.fifosize;
		return ufstat & SDP_UFSTAT2_RXFIFO_MASK;
	}
	else
	{
...
static void sdp_serial_rx_chars(struct sdp_uart_port *ourport)
{
 ...
 ignore_char:
	...
	/* reset RX FIFO */	
	if (need_rx_reset) {
		wr_regl(port, SDP_UFCON, SDP_UFCON_DEFAULT | SDP_UFCON_RESETRX);
	}
}
...
//		spin_unlock_irqrestore(&port->lock, flags);
	...
	spin_lock_irqsave(&ourport->tx_lock, flags2);
	while (!uart_circ_empty(xmit) && count > 0) {
		char output = xmit->buf[xmit->tail];
		if(ourport->enhanced)	{
			if (rd_regl(port, SDP_UFSTAT) & SDP_UFSTAT2_TXFIFO_FULL)
				break;
		}
		else	{
	...
	spin_unlock_irqrestore(&ourport->tx_lock, flags2);
...
static irqreturn_t sdp_serial_handle_tx_irq(int irq, void *id)
{
	...
	if (pend & SDP_UTRSTAT_ERRI) {
//		sdp_serial_tx_chars(ourport);
		wr_regl(port, SDP_UTRSTAT, SDP_UTRSTAT_ERRI);
	}

	return IRQ_HANDLED;
}

static irqreturn_t sdp_serial_handle_rx_irq(int irq, void *id)
{
	struct sdp_uart_port *ourport = id;
	struct uart_port *port = &ourport->port;
	unsigned int pend = rd_regl(port, SDP_UTRSTAT);
...
static int sdp_serial_startup(struct uart_port *port)
{
	...
	ret = request_irq(ourport->tx_irq, sdp_serial_handle_tx_irq, IRQF_SHARED,
			  sdp_serial_portname(port), ourport);
//	printk("sdp_serial_handle_tx_irq : irq %d registed!!!\n", ourport->tx_irq);
	if (ret) {
		dev_err(port->dev, "cannot get irq %d\n", ourport->tx_irq);
		return ret;
	}
	ret = request_irq(ourport->rx_irq, sdp_serial_handle_rx_irq, IRQF_SHARED,
			  sdp_serial_portname(port), ourport);
//	printk("sdp_serial_handle_rx_irq : irq %d registed!!!\n", ourport->rx_irq);
	if (ret) {
		dev_err(port->dev, "cannot get irq %d\n", ourport->rx_irq);
		return ret;
	}
	...
	if(!cpumask_empty(&ourport->irq_affinity_mask)) {
		irq_set_affinity(ourport->tx_irq, &ourport->irq_affinity_mask);
		irq_set_affinity(ourport->rx_irq, &ourport->irq_affinity_mask);
	} else if(cpu_online((u32) ourport->irq_affinity)) {
		irq_set_affinity(ourport->tx_irq, cpumask_of((u32) ourport->irq_affinity));
		irq_set_affinity(ourport->rx_irq, cpumask_of((u32) ourport->irq_affinity));
	}
...
static int sdp_serial_console_txrdy(struct uart_port *port)
{
	...
	ufstat = rd_regl(port, SDP_UFSTAT);
	if(ourport->enhanced)
		return (ufstat & SDP_UFSTAT2_TXFIFO_FULL) ? 0 : 1;
	else
...
static void sdp_serial_console_putchar(struct uart_port *port, int ch)
{
	...
	wr_regl(cons_uart, SDP_UTXH, (u8) ch);
}
			...
			const char *s, unsigned int count)
{
	...
	spin_lock_irqsave(&ourport->tx_lock, flags);
	uart_console_write(cons_uart, s, count, sdp_serial_console_putchar);
	spin_unlock_irqrestore(&ourport->tx_lock, flags);
}
...
static void sdp_serial_resetport(struct uart_port *port)
{
	...
	/* reset both fifos */
	if(ourport->enhanced)	{
		wr_regl(port, SDP_UMCON, SDP_UMCON2_DEFAULT);
		wr_regl(port, SDP_UFCON, SDP_UFCON2_DEFAULT | SDP_UFCON_RESETBOTH);
		wr_regl(port, SDP_UFCON, SDP_UFCON2_DEFAULT);
	}
	else	{
				    ...
				    struct platform_device *platdev)
{
	...
	spin_lock_init(&ourport->tx_lock);
	...
	if (ret < 0) {
		dev_err(dev, "failed to find irq resource for uart TX\n");
		return -EINVAL;
	}

	ourport->tx_irq = (u32) ret;

	ret = platform_get_irq(platdev, 1);
	if (ret < 0) {
		dev_err(dev, "failed to find irq resource for uart RX\n");
		return -EINVAL;
	}

	ourport->rx_irq = (u32) ret;
...
static struct platform_driver sdp_serial_driver = {
	...
	.driver		= {
		...
		.of_match_table	= of_match_ptr(sdp_serial_dt_match),
	},
};
