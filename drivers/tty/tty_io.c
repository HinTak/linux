...
static void tty_vhangup_session(struct tty_struct *tty)
{
	...
	printk(KERN_ERR "%s vhangup session...\n", tty_name(tty, buf));
#endif
	__tty_hangup(tty, 1);
}
...
#ifdef TTY_DEBUG_HANGUP
		printk(KERN_ERR "error attempted to write to tty [0x%p]"
		       " = NULL", tty);
#endif
	}
...
#ifdef TTY_DEBUG_HANGUP
	printk(KERN_ERR "[%s]: %s(pid %d,tgid %d) release %s at %lu(ms), tty count=%d\n", __func__,current->comm,current->pid,current->tgid, tty_name(tty, buf),(unsigned long)(current->se.exec_start>>20), tty->count);
#endif
...
#ifdef TTY_DEBUG_HANGUP
	printk(KERN_ERR "%s: %s: final close\n", __func__, tty_name(tty, buf));
#endif
	/*
...
#ifdef TTY_DEBUG_HANGUP
	printk(KERN_ERR "%s: %s: freeing structure...\n", __func__, tty_name(tty, buf));
#endif
	/*
...
retry_open:
	...
	if (tty->driver->type == TTY_DRIVER_TYPE_PTY &&
		...
		noctty = 1;
#ifdef TTY_DEBUG_HANGUP
	printk(KERN_ERR "[%s]: %s(pid %d,tgid %d) opening %s at %lu(ms)\n", __func__,current->comm,current->pid,current->tgid,tty->name,(unsigned long)(current->se.exec_start >> 20));
#endif
	if (tty->ops->open)
	...
	if (retval) {
#ifdef TTY_DEBUG_HANGUP
		printk(KERN_ERR "[%s]: %s(pid %d,tgid %d) error %d in opening %s at %lu(ms)\n", __func__,current->comm,current->pid,current->tgid, retval, tty->name,(unsigned long)(current->se.exec_start >> 20));
#endif
