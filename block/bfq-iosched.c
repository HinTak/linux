...
static int R_fast[2] = {17415, 34791*2};
/*
...
static void bfq_arm_slice_timer(struct bfq_data *bfqd)
{
	...
	/* for non rotational media, do not wait */
...
static int bfq_init_queue(struct request_queue *q, struct elevator_type *e)
{
	...
	/* for non rotational media keep idle slice 0 */
	if (blk_queue_nonrot(bfqd->queue))
		bfqd->bfq_slice_idle = 0;
	else
...
{
	return snprintf(page, sizeof(int)*8, "%d\n", var);
}
...
static ssize_t bfq_weights_show(struct elevator_queue *e, char *page)
{
	...
	num_char += snprintf(page + num_char,
				strnlen("Tot reqs queued \n\n", 100) +
				sizeof(int)*8,
				"Tot reqs queued %d\n\n", bfqd->queued);

	spin_lock_irq(bfqd->queue->queue_lock);

	num_char += snprintf(page + num_char, strnlen("Active:\n", 100),
				"Active:\n");
	list_for_each_entry(bfqq, &bfqd->active_list, bfqq_list) {
		num_char += snprintf(page + num_char,
			strnlen("pid: weight , nr_queued  , dur \n", 100)
			+ sizeof(int)*8*5,
	...
	num_char += snprintf(page + num_char, strnlen("Idle:\n", 100),
				"Idle:\n");
	list_for_each_entry(bfqq, &bfqd->idle_list, bfqq_list) {
			num_char += snprintf(page + num_char,
				strnlen("pid: weight , dur \n", 100) +
				sizeof(int)*8*3,
...
static int __init bfq_init(void)
{
	...
	T_fast[1] = msecs_to_jiffies(2000*2);
